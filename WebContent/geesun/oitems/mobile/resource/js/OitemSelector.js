define([ "dojo/_base/declare", "dojox/mobile/_ItemBase", "dojo/_base/lang", 
         ...
         "dojo/on", "dojo/request", "dojo/touch", "./OitemSelectorListItem","mui/i18n/i18n!geesun-oitems:geesunOitemsShoppingTrolley","mui/i18n/i18n!sys-mobile:mui.category"], 
	...
	return declare("km.oitems.mobile.js.OitemSelector", [ ItemBase ], {
		...
		catDataUrl: 'geesun/oitems/geesun_oitems_manage/geesunOitemsManage.do?method=cateList&categoryId={categoryId}&getTemplate=0&modelName=com.landray.kmss.geesun.oitems.model.GeesunOitemsManage&authType=00',
		oitemDataUrl: 'geesun/oitems/geesun_oitems_listing/geesunOitemsListing.do?method=checkOitemsList&deptBudger=true&categoryId={categoryId}&fdApplicationId={fdApplicationId}',
		checkUrl: "geesun/oitems/geesun_oitems_warehousing_record_joinlist/geesunOitemsWarehousingRecordJoinList.do?method=checkNum&geesunOitemsListingId={geesunOitemsListingId}",
		addUrl: "geesun/oitems/geesun_oitems_shopping_trolley/geesunOitemsShoppingTrolley.do?method=addOitems&fdOitemsId={fdOitemsId}&fdChecked={fdChecked}&fdApplicationId={fdApplicationId}&warehousingRecordId={warehousingRecordId}&fdPrice={fdPrice}&fdCurNum={fdCurNum}&fdIsCancle={fdIsCancle}",
		listRecordUrl : 'geesun/oitems/geesun_oitems_warehousing_record_joinlist/geesunOitemsWarehousingRecordJoinList.do?method=listRecord&fdListingId={fdListingId}&fdApplicationId={fdApplicationId}',
		getContentUrl : "geesun/oitems/geesun_oitems_shopping_trolley/geesunOitemsShoppingTrolley.do?method=loadTrolley&orderby=fdNo&fdApplicationId={fdApplicationId}",
		
		searchDataUrl: 'geesun/oitems/geesun_oitems_listing/geesunOitemsListing.do?method=checkOitemsList&deptBudger=true&fdApplicationId={fdApplicationId}&keywords={keywords}',
		...
		initialize: function(){
			...
			ctx.selectedOitems = {};
			topic.publish('geesun/oitems/selectedoitem/get', function(selectedOitems) {
		...
		bindEvents: function(){
			...
			on(ctx.listCon, on.selector('.muiCateClick', "click"), function(e){
				...
				//解决问题单 #109747：在500毫秒内请求多次，只取第一次
				if(lock) return;
		        lock = true;
						...
						array.forEach(ctx.oitemList, function(d){
							...
							if(d.fdId == id){
								...
								request.post(config.baseUrl + lang.replace(ctx.checkUrl, {
									geesunOitemsListingId : d.fdId
								}), {
									handleAs: 'json'
								}).then(function(res){
									if(res['moreThanOne'] == "false"){
										var recordJoinListId = res['geesunOitemsWarehousingRecordJoinListId'];
										var fdPrice = res['fdPrice'];
										var curNum = res['curNum']; 
										var geesunOitemsListingId = res['geesunOitemsListingId'];
										var promise = ctx.addTrolley(true,recordJoinListId, fdPrice, curNum, geesunOitemsListingId);
										promise.then(function(res){
											if(ctx.isMul) {
												...
												if(domClass.contains(node, 'muiCateSeled')) {
													...
													topic.publish('/geesun/oitems/cancelSelected', geesunOitemsListingId);
								...
								ctx.addTrolley(true,d.fdId, d.fdPrice, d.curNum, d.oitemId).then(function(res){
									if(ctx.isMul) {
										...
										if(domClass.contains(node, 'muiCateSeled')) {
											...
											topic.publish('/geesun/oitems/cancelSelected', d.oitemId);
			...
			topic.subscribe('/geesun/oitems/cancelSelected', function(oitemId) {
			...
			topic.subscribe('/mui/category/submit', function(_ctx, items) {
				...
				request.post(config.baseUrl + lang.replace(ctx.getContentUrl, {
					...
					items = res;
					topic.publish('geesun/oitems/oitemselector/result', items);
				});
			});
			
		},
		
		addTrolley: function(fdChecked,recordJoinListId,fdPrice,curNum,geesunOitemsListingId){
			var ctx = this;
			...
			request.post(config.baseUrl + lang.replace(ctx.listRecordUrl, {
				...
				array.forEach(res.datas, function(d){
					...
					t.push({
						...
						text: _t.title+" ("+Msg['geesunOitemsShoppingTrolley.fdAmount']+":"+_t.fdCurNumber+", "+Msg['geesunOitemsShoppingTrolley.fdReferencePrice']+":"+_t.fdPrice+")",
						record: _t
					});
				...
				request.post(config.baseUrl + lang.replace(ctx.oitemDataUrl, {
					...
					array.forEach(res.datas, function(d){
						...
						t.push({
							type: ctx.TYPE_OITEM,
							fdId: _t.fdId,
							text: _t.title+" ("+Msg['geesunOitemsShoppingTrolley.fdAmount']+":"+_t.fdAmount+", "+Msg['geesunOitemsShoppingTrolley.fdReferencePrice']+":"+_t.fdReferencePrice+")",
							oitem: _t
						});
						ctx.scrollableViewContainer.style.transform="translate3d(0,0,0)";
					});
