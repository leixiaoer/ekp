...
<script type="text/javascript">
	...
	seajs.use(['lui/dialog'], function(dialog){
	 ...
	 function clear_data(){
			...
			document.getElementById("listIframe").src = '<c:url value="/geesun/oitems/geesun_oitems_listing/geesunOitemsListing.do?method=inCountData" />';
		 }
	 function isNullDate(){
	...
	function getDateByFormat(str){
	 ...
	 function checkDate(){
		...
		function loadList(){
			...
			document.getElementById("listIframe").src = '<c:url value="/geesun/oitems/geesun_oitems_listing/geesunOitemsListing.do?method=inCountData" />&fdStartTime='+fdStartTime+'&fdEndTime='+fdEndTime
	...
	<ui:content id="geesunOitemsInCountContent" title="${ lfn:message('geesun-oitems:geesunOitems.tree.reporting1') }">
		<div id="contentDiv" style="width:95%;min-height:500px;padding: 20px">
			<div style="float:right;padding-right:30px">
					<ui:button text="${lfn:message('button.export') }" order="2" onclick="Com_SubmitNoEnabled(document.geesunOitemsListingForm, 'inCountExport');">
					</ui:button>
			</div>
			<center>
