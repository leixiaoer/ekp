...
<template:include ref="default.list">
	...
	<template:replace name="path">			
		...
		<c:if test="${ empty param.pKey or param.pKey eq 'EHR' }">
			<ui:menu-item text="EHR组织架构同步"></ui:menu-item>
		...
		<c:if test="${ param.pKey eq 'RECORD' }">
			<ui:menu-item text="EHR组织架构同步"></ui:menu-item>
			<ui:menu-item text="${ lfn:message('geesun-org:menus.title.record.middle') }" >
			</ui:menu-item>
		</c:if>
		</ui:menu>
	</template:replace>
		...
		<div class="lui_list_nav_frame">
			 <ui:accordionpanel>				 
				...
				<ul class='lui_list_nav_list'>
					<li><a href="javascript:void(0)" onclick="setUrl('EHR','','');">${ lfn:message('geesun-org:menus.title.flona.sap') }</a></li>
					...
					<%-- <li><a href="javascript:void(0)" onclick="setUrl('RECORD','','');">${ lfn:message('geesun-org:menus.title.record.middle') }</a></li> --%>
				</ul>
				</ui:content>
				<!-- 其他操作 -->
	...
	<template:replace name="content">
		...
		<c:if test="${param.pKey eq 'ORGAN' }">
			<%@ include file="/geesun/org/geesun_org_organ/criteria.jsp"%>
		</c:if>
		<c:if test="${param.pKey eq 'PERSON' }">
			<%@ include file="/geesun/org/geesun_org_person/criteria.jsp"%>
		</c:if>
		<c:if test="${param.pKey eq 'POST' }">
			<%@ include file="/geesun/org/geesun_org_post/criteria.jsp"%>
		</c:if>
		<c:if test="${param.pKey eq 'RECORD' }">
			<%@ include file="/geesun/org/geesun_org_record/criteria.jsp"%>
		</c:if>
	</template:replace>
</template:include>
