	...
	window.buildNode = function(data) {
		...
		if (data.fdId != "more") {
			if (data.pic && data.pic != "") {
				$item.append('<div class="item-img"><img src="' + data.pic + '"></img></div>');
			} else {
			...
			node.on('click', function() {
				if (auth == "true") {
					...
					url = data.url + "&type=" + window.type + "&sysVerId=" + data.versionId + "&version=" + window.version + "&createUrl=" + encodeURIComponent(createUrl + thirdMallCreateParam);
