	...
	if(avaliableDepts!=null){
		...
		for(int i=0;i<avaliableDepts.size();i++){
			...
			if(dingDeptMap.keySet().contains(dept.getString("name"))){
				...
				if(relationMap.keySet().contains(dept.getString("id"))){
					out.println("<td><b>已关联<b></td>");
				}else{
					out.println("<td>执行关联..........</td>");
					OmsRelationModel model = new OmsRelationModel();
					model.setFdEkpId(dept.getString("id"));
					model.setFdAppPkId(dingDeptMap.get(dept.getString("name")));
	...
	if(avaliablePersons!=null){
		...
		for(int i=0;i<avaliablePersons.size();i++){
			...
			if(dingPersonList.contains(person.getString("loginName"))){
				...
				if(relationMap.containsValue(person.getString("loginName"))){
					out.println("<td><b>已关联<b></td>");
				}else{
					out.println("<td>执行关联..........</td>");
					OmsRelationModel model = new OmsRelationModel();
					model.setFdEkpId(person.getString("id"));
					model.setFdAppPkId(person.getString("loginName"));
					model.setFdAppKey("default");
					omsRelationService.add(model);
				}
				out.print("</tr>");
