define(["dojo/_base/declare",
	...
	var Service = declare("third.ding.xform.builtin.batchLeave.mobile.service", null , {
		...
		init_ff: function(){// 初始化方法
        	...
        	window.check_time_now=this.check_time_now;
        },
		doInit : function(){
			this.getUrlInfo();
			this.init_ff();
		...
		initInEdit : function(status){		
			...
			validatorUtil.addBatchCheckTimeValidation('validateBatchCheckTime',"请选择当月时间补卡",this.check_time_now);
		...
		addRow_ObjValidate : function(obj){
		    window.mySetTimeout(function(){
                 ...
                 fdReplacementTimeWidget._set("validate", "required validateBatchCheckTime  validateBatchReplace validateBatchCheckNotCanSupply");
                 fdReplacementTimeWidget._set("_validate", "required validateBatchCheckTime  validateBatchReplace validateBatchCheckNotCanSupply");
		...
		listenBatchReplaceValueChange : function(){
			...
			topic.subscribe(this.DATE_VALUE_CHANGE, function(triggerWidget){
                 ...
                 if(new Date(value).getMonth() == new Date().getMonth()&&new Date(value).getFullYear() == new Date().getFullYear()){
					 console.log(new Date(value).getFullYear()+"-"+new Date(value).getMonth());
        ...
        check_time_now : function(divId,value){
			if(new Date(value).getMonth() == new Date().getMonth()&&new Date(value).getFullYear() == new Date().getFullYear()){
				$("#"+divId).siblings("[name*='fd_replacement_ul']").find("[name*='nowData']")[0].setAttribute("validatorresult","pass");
			}else{
				$("#"+divId).siblings("[name*='fd_replacement_ul']").find("[name*='nowData']")[0].setAttribute("validatorresult","nopass");
			}
			var idEle= registry.byId(divId);
			validatorUtil.getValidation().validateElement(idEle);
        },
