...
/*#155133-钉钉套件加班问题-开始*/
function addBatchOvertimeHoursValidation (validatorName, errorMessage) {
	let validation = $KMSSValidation();
	validation.addValidator(validatorName, errorMessage, function (v, e, o) {
		let result = true;
		var tr = $(e).parents("xformflag").parents(".xform_inputText").parent("td").parent("tr");//当前行对象
		//获取加班时长
		var fd_duration = $(tr).find("[name*='fd_duration']").val();
		//获取加班开始时间
		var fd_work_start_time = $(tr).find("[name*='fd_work_start_time']").val();
		//获取加班结束时间
		var fd_work_end_time = $(tr).find("[name*='fd_work_end_time']").val();
		var interfacevalidator = this.getValidator("overtimeHours");
		if(fd_work_start_time && fd_work_end_time){
			//转换格式
			fd_work_start_time = fd_work_start_time.replace(/-/g, "/");
			fd_work_end_time = fd_work_end_time.replace(/-/g, "/");
			...
			//获取相差小时数
			var hour = ms / 1000 / 60 / 60;
			if((fd_work_start_time.getDate() != fd_work_end_time.getDate() && hour <= '24') && fd_duration == '0'){
				interfacevalidator.error = "加班时长异常，可能是由于跨天加班所致，请重新填写加班时间";
				$(tr).find("[name*='fd_duration']").attr("readOnly",true);//不可编辑
				result = false;
			}else if(fd_duration == '0'){
				interfacevalidator.error = "加班时长异常，请重新填写加班时间";
				$(tr).find("[name*='fd_duration']").attr("readOnly",true);//不可编辑
				result = false;
			}
		}
