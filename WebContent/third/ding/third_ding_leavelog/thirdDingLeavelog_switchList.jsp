    ...
    <template:replace name="content">
        <div style="margin:5px 10px;">
            ...
            <list:listview id="listview">
                <ui:source type="AjaxJson">
                    {url:appendQueryParameter('/third/ding/third_ding_leavelog/thirdDingLeavelog.do?method=switchData')}
                </ui:source>
                <!-- 列表视图 -->
                <list:colTable rowHref="/third/ding/third_ding_leavelog/thirdDingLeavelog.do?method=switchView&fdId=!{fdId}" isDefault="false" name="columntable">
                    <list:col-checkbox />
                    <list:col-serial/>
                    <list:col-auto props="docSubject;docCreator.name;fdUserid;docAlterTime" url="" />
                    ...
                    <list:col-html style="width:60px;" title="${ lfn:message('third-ding:thirdDingLeavelog.optitle') }">		
						if(row['fdIstrue'] == '0'){
							{$<a href="#" onclick="resyncCheckToDing('{%row.fdId%}');" class="com_btn_link"><bean:message bundle="third-ding" key="thirdDingLeavelog.synctip"/></a>$}
						}
					</list:col-html>
                </list:colTable>
            </list:listview>
        ...
        <script>
            ...
            seajs.use([ 'lui/jquery', 'lui/dialog' ], function($, dialog) {
            	 // 重新同步到钉钉
                window.resyncCheckToDing=function(fdId){
                	var msg = '${lfn:message("third-ding:thirdDingLeavelog.confirmSync")}';
	            	var url = '<c:url value="/third/ding/third_ding_leavelog/thirdDingLeavelog.do?method=resyncSwitch&fdId=" />'+fdId;
