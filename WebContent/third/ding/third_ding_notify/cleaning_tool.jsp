						...
						success : function(data) {
							if(data.success){
								...
								if(data.notSendArray){
									...
									$("#notSend").empty();
									$("#notSend").append('<tr><td colspan="6" style="text-align:right;"><div style="margin-right:20px"><font style="color:red">提醒: 待办重推必须选择待办任务接口才能使用。原来使用工作流接口推送失败的待办，也将改用待办任务接口推送。</font><input type="button" value="批量推送"  onclick="reSend()"/></div></td></tr>');
								...
								if(data.checkErrorArray){
									...
									$("#checkError").empty();
									$("#checkError").append('<tr style="font-weight: bold;"><td>序号</td><td>待办标题</td><td>待办人员</td><td>接口方式</td><td>URL</td></tr>');
									//角标，出现两个id一样的，待处理
									...
									for(var i=0;i<data.checkErrorArray.length;i++){
										$("#checkError").append('<tr><td>'+(i+1)+'</td><td>'+data.checkErrorArray[i].title+'</td><td>'+data.checkErrorArray[i].user+'</td><td>'+data.checkErrorArray[i].type+'</td><td>'+data.checkErrorArray[i].url+'</td></tr>');
									}
								}
							}else{
                     		...
                     		$.ajax({
        						...
        						success : function(data) {
        							if(data.success){
        								...
        								dialog.alert("处理失败："+data.message);
        							}
        							d.hide();
        							queryNotify();
        						} ,
