	...
	function initIframe(){
		...
		contentBody.bind({
			"keyup input focus" : function() {
				target.checkCommentContent(this);
				target.checkWordsNum(this, iframeType, "${param.iframeId}");
			},
			// 兼容IE右键粘贴字数限制
			"paste cut" : function() {
				var _this = this;
				setTimeout(function(){
					target.checkCommentContent(_this);
					target.filterOuterImg(_this);
					target.checkWordsNum(_this,iframeType, "${param.iframeId}");
				}, 2);
			}
		});	
	}
...
<body contentEditable="true" hideFocus="" onload="initIframe();"></body>
</html>