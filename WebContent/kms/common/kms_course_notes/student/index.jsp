...
<template:include ref="expand.list" rwd="true" spa="true" >
	...
	<template:replace name="content">
		<ui:tabpanel id="tabpanel" layout="sys.ui.tabpanel.list" cfg-router="true">
	  		 <%--我安排的 --%>
		 	<ui:content  
		 		 title="${lfn:message('kms-common:module.kms.allNotes')}" >
				...
				<list:listview channel="notes_index">
					...
					<list:colTable layout="sys.ui.listview.columntable" name="columntable" 
						rowHref="/kms/common/kms_notes/kmsCourseNotes.do?method=view&fdId=!{fdId}">
							...
							<list:col-html title="${ lfn:message('kms-common:kmsCourseNotes.docSubject')}" headerStyle="max-width: 500px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" style="text-align:left;padding:0 8px">
								{$
									<div class="com_content" title="{%row['fdNotesContent']%}">{%row['fdNotesContent']%}</div>
								$}
							</list:col-html>
