	...
	function jump(){
		$.ajax({
			...
			data : {
				method : 'moduleIndex',
				fdId : '${JsParam.id}',
				keyword : '${JsParam.keyword}',
				pageNo : $('#${JsParam.id}_pageno').val(),
				rowSize : $('#${JsParam.id}_rowsize').val()
			},
			cache : false,
			error : function(){
			},
...
$}
		...
		<td>{%itemList[i].kmsKnowledgeCategory%}</td>
		<td>{%itemList[i].fdTotalCount%}</td>
...
<div class="page c" id="${JsParam.id}-page">
<p class="jump">每页<input type="text" value="{%data[0].pageWrapper.page.rowsize%}" id="${JsParam.id}_rowsize" class="i_a m_l20"/>条<input type="text" value="{%data[0].pageWrapper.page.pageno%}" class="i_a m_l20" id="${JsParam.id}_pageno">/共{%data[0].pageWrapper.page.totalPage%}页<span class="btn_b kmservice_btn_b"><a href="javascript:jump();" title="跳转到"><span>Go</span></a></span></p>
