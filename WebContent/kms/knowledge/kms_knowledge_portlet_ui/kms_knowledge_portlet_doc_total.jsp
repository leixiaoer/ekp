...
<%!
	// 模块过滤
	public boolean moduleExist(String path){
		Map<String, Boolean> modules = new ConcurrentHashMap<String, Boolean>();
...
<script>
    ...
    function renderMonthPublishHtml(num){
    	if(num){
    		seajs.use(['lui/jquery'], function($) {
            	var text="${lfn:message('kms-knowledge:kmsKnowledge.portlet.knowledge.add.month')}";
                //'本月新增了<em>'+num+'</em>份知识';
            	var html=text.replace("%num%","<em>"+num+"</em>")
    ...
    function renderRichHtml() {
        ...
        seajs.use(['lui/jquery','lui/dialog'], function($,dialog) {
        	...
        	if(scoreView){
       			numtype="score";
				scoreHtml="${lfn:message('kms-knowledge:kmsKnowledge.portlet.knowledge.score')}：";
            }else if(richView){
        		numtype="rick";
				scoreHtml="${lfn:message('kms-knowledge:kmsKnowledge.portlet.knowledge.rich')}：";
            }
            // 异步获取积分或者财富数据
            $.ajax({
