...
<ui:ajaxtext>
    ...
    <ui:dataview>
		<ui:source type="AjaxJson">
		      {"url":"/kms/knowledge/kms_knowledge_portlet/kmsKnowledgePortlet.do?method=getKnowledge&rowsize=${param.rowsize}&type=${param.type}&scope=${param.scope}&categoryId=${param.categoryId}&showCreator=${param.showCreator}&showCreated=${param.showCreated}&sumSize=!{param.sumSize}&dataType=col&orderby=${fdOrderBy}&ordertype=down"}
		</ui:source>
											...
											<div class="lui_fl">
										       <span class="lui_info_item">
												...
												<c:if test="${param.showCreated!=''}">
												   <span class="lui_info_item">
											          <bean:message bundle="kms-knowledge" key="kmsKnowledge.portlet.read" />：{%data[i]['read_c_t']%}
											       </span>
												</c:if>
											</div>
