...
function checkValue() {
	...
	if (!$("input[name='personIds']").val()) {
		...
		// 没选择任何通知人...
		if (!result) {
			alert('<kmss:message bundle="kms-knowledge" key="kmsKnowledge.listener.timer.checkPerson" />');
	...
	/* if (!$("input[name='titleRegulation']").val()) {
		alert('<kmss:message bundle="kms-knowledge" key="kmsKnowledge.listener.timer.titleRegulation.check" />');
...
function genTitleRegByFormula(fieldId, fieldName){
	Formula_Dialog(fieldId,fieldName,Formula_GetVarInfoByModelName("com.landray.kmss.kms.wiki.model.KmsWikiMain"), "String");
}
...
<table id="List_ViewTable" class="tb_normal" width="98%">
	<%-- <tr height="21">
		...
		<td class="td_normal_title" width="20%">
			<bean:message bundle="kms-knowledge" key="kmsKnowledge.titleRegulation" />
		</td>
		<td>
			...
			<a href="#" onclick="genTitleRegByFormula('titleRegulation','titleRegulationName')"><bean:message bundle="kms-knowledge" key="kmsKnowledge.formula" /></a>
			<br/> 
				<bean:message bundle="kms-knowledge" key="kmsKnowledge.listener.timer.titleRegulation.tip"/>
		</td>
	</tr> --%>
	<tr height="21">
		<td class="td_normal_title" width="20%">
	...
	<%-- <tr>
		<td class="td_normal_title" width="20%"><kmss:message key="kmsKnowledge.listener.timer.startRule" bundle="kms-knowledge" /></td>
		<td>
			<label><input type="checkbox" name="skipDraftNode" value="true"><kmss:message key="kmsKnowledge.listener.timer.startRuleSkipDraftNode" bundle="kms-knowledge" /></label>
		</td>
	</tr> --%>
	<tr>
		<td class="td_normal_title" width="20%" rowspan="2">
			<bean:message bundle="kms-knowledge" key="kmsKnowledge.listener.timer.createScope"/>
		</td>
		<td>
