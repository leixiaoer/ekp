		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
				{
					url:'/kms/knowledge/kms_knowledge_base_doc/kmsKnowledgeBaseDocIndex.do?method=listChildren&orderby=docPublishTime&ordertype=down&docStatus=30&rowsize=12&authType=2'
				}
			</ui:source>
				<!-- 列表视图 -->
				<list:colTable isDefault="false"
					id="subjectCol"
					onRowClick="selectLink('!{fdId}','!{docSubject}','!{fdKnowledgeType}','!{fdDescription1}','!{docPublishTime}','!{docAuthorName}','!{docCategoryName}')"
				   name="columntable">				
			   <list:col-checkbox headerStyle="width:1%"/>
				<list:col-serial headerStyle="width:1%"/>
				<list:col-html
					...
					if(!mulSelect){
						...
						LUI.$('#listview [name="List_Tongle"]').bind('click', function() {
							...
							if (this.checked) { 
								...
								for (var i = 0; i < datas.length; i ++) {
										selectLink(datas[i].fdId, datas[i].docSubject, datas[i].fdKnowledgeType, datas[i].fdDescription, datas[i].docPublishTime, datas[i].docAuthorName, datas[i].docCategoryName);
								}
							} else {
						...
						LUI.$('#listview [name="List_Selected"]').bind('click', function() { //点击列表某一项的checkbox
							...
							if (!this.checked) {
								...
								if (val != null)
									selectLink(val.fdId, val.docSubject, val.fdKnowledgeType, val.fdDescription, val.docPublishTime, val.docAuthorName, val.docCategoryName);
							}
						});
	 	...
	 	seajs.use(['lui/jquery','lui/dialog'], function($, dialog){
	 		...
	 		window.refreshCheckbox = function() {
					...
					if(box_len>7){
	 				...
	 				window.onSubmit =  function() {
						...
						var mulSelect = ${param.mulSelect } ;
	 					...
	 					for(var i = 0; i < values.length; i++) {
		 					var data = {fdId:"",docSubject:"",fdKnowledgeType:"",fdDescription:"",docPublishTime:"",docAuthorName:"",docCategoryName:""}; //不能放在括号外面，因为这样会造成每次data新值都会修改rtn里面的data
	 						data.fdId = values[i].id;
	 						data.docSubject  = values[i].name;
	 						data.fdKnowledgeType = values[i].type;
	 						data.fdDescription = values[i].fdDescription ? values[i].fdDescription : "";
	 						data.docPublishTime = values[i].docPublishTime;
	 						data.docAuthorName = values[i].docAuthorName;
	 						data.docCategoryName = values[i].docCategoryName;
	 						rtn.push(data);
	 					}
	 				...
	 				<%-- 设置选中 --%>
	 				window.selectLink = function(id, docSubject, fdKnowledgeType, fdDescription, docPublishTime, docAuthorName, docCategoryName) { 
	 					var data = {
	 							"id":id,
	 							"name":htmlDecodeByRegExp(docSubject),
	 							"type":fdKnowledgeType,
	 							"fdDescription":fdDescription ? fdDescription : "" ,
