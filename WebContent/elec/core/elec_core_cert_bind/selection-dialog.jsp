				...
				seajs.use(['lui/topic'],function(topic){
					...
					topic.subscribe("list.changed",function(data){
						if(data.datas.length==0){
							$(".selection_dialog_btn_ok",parent.document).hide();
						}else{
							$(".selection_dialog_btn_ok",parent.document).show();
						}
					})
				});
			}
			...
			function _getSelectedData(emptyMsg){
				...
				if(rowContents.length == 0){
					seajs.use(['lui/dialog'],function(dialog){
						dialog.alert(emptyMsg?emptyMsg:"${lfn:message('sys-ui:ui.dialog.template.message')}");
					});
					return null;
				}
