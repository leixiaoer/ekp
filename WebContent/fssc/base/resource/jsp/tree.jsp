...
<html:form action="${modelUrl }" method="post" enctype="multipart/form-data">
	<div id="optBarDiv">
		<input type="button" value="${lfn:message('fssc-base:button.showList')}"
				onclick="showList();" />
		...
		<kmss:authShow roles="ROLE_FSSCBASE_IMPORT" extendOrgElements="${staffManagerList}">
			<input type="button" value="<bean:message key="fssc-base:button.exportTemplate"/>"
	...
	<script>
		...
		function generateTree() {
			...
			n1.authType = "01";
			n2 = n1.AppendBeanData("fsscBaseTreeService&modelName=${param.modelName}&parentId=!{value}&fdCompanyId="
							+ fdCompanyId);
			LKSTree.Show();
		}
		...
		function exportData(){
			window.open(Com_Parameter.ContextPath+"fssc/base/fssc_base_business/fsscBaseBusiness.do?method=exportData&modelName=${param.modelName}&fdCompanyId=${param.companyId}");
		}
		//导入
		window.importData = function(){
			seajs.use(['lui/dialog','lang!fssc-base'],function(dialog,lang){
				dialog.iframe(
						'/fssc/base/resource/jsp/fsscBaseImport.jsp?modelName=${param.modelName}',
		...
		window.downloadTemplate = function(){
			window.open(Com_Parameter.ContextPath+'fssc/base/fssc_base_business/fsscBaseBusiness.do?method=downloadTemplate&modelName=${param.modelName}');
		}
