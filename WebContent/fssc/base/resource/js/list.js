...
//      if(LUI('del')){ LUI('toolbar').removeButton(LUI('del'));}
	...
	window.switchTree = function(){
		//除公司下会计科目 、预算科目，其他档案可进行新增、删除、复制等操作
		var canUse={};
		if(typeof(fdIsOpenIssued)!="undefined"&&fdIsOpenIssued){//启用了下发，公司下会计科目 、预算科目不能新增、删除、赋值
		...
		if(typeof(fdIsOpenIssued)!="undefined"&&fdIsOpenIssued){//启用了下发，公司下会计科目 、预算科目不能新增、删除、赋值
				url=url.replace("&unuse=true","");//替换&unuse=true
				url+="&can=false";
		}
