seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/dialog_common','lang!fssc-base','lui/util/env'], function($, dialog , topic, dialogCommon,lang,env){
	...
	//导出
	window.exportSupplier = function(){
		var fdCompanyId = Com_GetUrlParameter(window.location.href,'fdCompanyId');
		fdCompanyId = fdCompanyId?fdCompanyId:'';
		var url=env.fn.formatUrl(listOption.basePath+'?method=exportSupplier&fdCompanyId='+fdCompanyId);
		window.open(url);
	}
	//导入
	window.importSupplier = function(){
		...
		window.dia = dialog.iframe(
			'/fssc/base/fssc_base_supplier/fsscBaseSupplierImport.jsp?modelName='+listOption.modelName+"&tempate="+formName,
