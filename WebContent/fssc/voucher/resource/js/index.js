seajs.use(['lui/framework/module','lui/jquery','lui/dialog','lui/topic','lui/spa/const','lui/framework/router/router-utils','lui/toolbar','lang!fssc-voucher','lang!'],
		function(Module, jquery, dialog, topic,spaConst,routerUtils ,toolbar,lang,comlang){
	
	var module = Module.find('fsscVoucher');
		...
		$router.define({
				...
				routes : [{
					...
					action : {
						type : 'tabpanel',
						options : {
							panelId : 'fsscVoucherPanel',
							contents : {
								'fsscVoucherContent' : { title : comlang['list.alldoc'], route:{ path: '/listAll' }, cri :{'mydoc':'all','j_path':'/listAll'} , selected : true }
							}
						}
					}
				}]
