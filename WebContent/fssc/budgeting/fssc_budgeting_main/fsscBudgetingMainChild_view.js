seajs.use(['lui/jquery','lui/dialog','lui/dialog_common','lui/util/str','lui/util/env','lang!fssc-budgeting','lang!'], function($, dialog, dialogCommon,strutil,env,lang,comlang){
	...
	window.initData=function(){
		...
		//增加页面宽度
		$(".tempTB").attr("style","width: 100%; min-width: 980px; max-width: 1200px; margin: 0px auto;");
	...
	window.editDoc=function(fdId,fdStatus,fdSchemeId){
		...
		if(fdStatus=='1'){ //草稿
			url=env.fn.formatUrl('/fssc/budgeting/fssc_budgeting_main/fsscBudgetingMain.do?method=budgetingEdit&fdId='+fdId+"&fdSchemeId="+fdSchemeId+"&orgType="+fdOrgType);
		}else if(fdStatus=='3'){
