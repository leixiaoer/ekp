...
<template:include ref="default.view">
    ...
    <template:replace name="toolbar">
        ...
        <ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="4">
        	<kmss:auth requestURL="/fssc/budgeting/fssc_budgeting_approval_auth/fsscBudgetingApprovalAuth.do?method=edit&fdId=${param.fdId}">
		        <ui:button text="${lfn:message('button.edit')}" onclick="Com_OpenWindow('fsscBudgetingApprovalAuth.do?method=edit&fdId=${param.fdId}','_self');" order="2" />
		    </kmss:auth>
		    <kmss:auth requestURL="/fssc/budgeting/fssc_budgeting_approval_auth/fsscBudgetingApprovalAuth.do?method=delete&fdId=${param.fdId}">
		        <ui:button text="${lfn:message('button.delete')}" onclick="if(!confirmDelete())return;Com_OpenWindow('fsscBudgetingApprovalAuth.do?method=delete&fdId=${param.fdId}','_self');" />
		    </kmss:auth>
    ...
    <template:replace name="path">
        <ui:menu layout="sys.ui.menu.nav">
            ...
            <ui:menu-item text="${ lfn:message('fssc-budgetng:table.fsscBudgetingApprovalAuth') }" href="/fssc/budget/fssc_budgeting_auth/" target="_self" />
        </ui:menu>
    </template:replace>
		            ...
		            <tr>
		                ...
		                <td colspan="3" width="85.0%">
		                    ...
		                    <div id="_xform_fdOrgListIds" _xform_type="address">
		                        <xform:address propertyId="fdOrgListIds" propertyName="fdOrgListNames" mulSelect="true" orgType="ORG_TYPE_ORGORDEPT" showStatus="view" textarea="true" style="width:95%;" />
		                    </div>
		                </td>
		            </tr>
		             <tr>
