...
<script>
    ...
    seajs.use(['lui/dialog','lang!fssc-expense'],function(dialog,lang){
    	window.FSSC_SelectInvoiceType = function(index){
    		...
    		dialogSelect(false,'fssc_base_expense_item_fdParent','fdDetailList_Form[*].fdExpenseItemId','fdDetailList_Form[*].fdExpenseItemName',null,{fdCompanyId:fdCompanyId,fdNotId:'fdNotId'});
    	}
    	window.FSSC_SelectInvoiceCompany = function(index){
    		index = DocListFunc_GetParentByTagName('TR').rowIndex-1;
    		window.curIndex = index;
    		dialogSelect(false,'fssc_base_company_fdCompany','fdDetailList_Form[*].fdCompanyId','fdDetailList_Form[*].fdCompanyName',null,null,null);
    	}
    	window.FSSC_SelectCostCenter = function(index){
    		...
    		dialogSelect(false,'fssc_base_cost_center_selectCostCenter','fdDetailList_Form[*].fdCostCenterId','fdDetailList_Form[*].fdCostCenterName',null,{fdCompanyId:fdCompanyId,fdNotId:'fdNotId'});
    	}
    		...
    		$.post(
    			'${LUI_ContextPath}/fssc/expense/fssc_expense_main/fsscExpenseMainData.do?method=getExpenseDetail',
  				...
  				function(data){
  					...
  					if($("[name=fdExpenseMainId]").val()!='${fsscExpenseShareMainForm.fdExpenseMainId}'){
  						FSSC_AddInitDetail();
  					}
  				}
    		);
    	...
    	window.FSSC_ChangeMoney = function(v,e,index){
    		...
    		var fdStandardMoney = (fdApplyMoney*fdExchangeRate).toFixed(2);
    	...
    	window.FSSC_SelectCurrency = function(index){
    		...
    		dialogSelect(false,'fssc_base_currency_fdCurrency','fdDetailList_Form[*].fdCurrencyId','fdDetailList_Form[*].fdCurrencyName',null,null,function(rtn){
    	...
    	window.FSSC_SelectProject = function(){
    		...
    		dialogSelect(false,'fssc_base_project_project','fdDetailList_Form[*].fdProjectId','fdDetailList_Form[*].fdProjectName',null,{fdCompanyId:fdCompanyId,fdNotId:'fdNotId','fdProjectType':'1'});
    	}
    	//提交时校验金额是否相等
    	...
    	$(function(){
    		FSSC_AfterExpenseMainSelected([{fdId:'${fsscExpenseShareMainForm.fdExpenseMainId}'}]);
    	})
    })
</script>
<br/>