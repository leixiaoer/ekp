 ...
 */
function tableFreezeStarter2(tableId, defaultFreezeTitle, defaultFreezeCol, multihead, showIndex, method) {
	//IE不支持冻结，屏蔽按钮及样式
    ...
    if(!divHeight){
    	divHeight=div[0].style.maxHeight;
    }
    ...
    // 当默认行数为0时，第一次添加行之后，由于td的宽度发生改变导致left:xxxPx 有误差，所以这里模仿一下点击冻结列方法
        ...
        if(tableDivInfoArray[tableId].isFreezeCol) {
            ...
            table.find(trSelector).each(function (tdIndex, tdObj) {
                ...
                $(tdObj).hover(function () {
                    freeze.show();
                    table.children("tbody").children("tr").find("td:lt(" + (freezeLastTdIndex + 1) + ")").addClass("freeze_preview");
                }, function () {
                    freeze.hide();
                    table.children("tbody").children("tr").find("td:lt(" + (freezeLastTdIndex + 1) + ")").removeClass("freeze_preview");
                });
            });
        }
