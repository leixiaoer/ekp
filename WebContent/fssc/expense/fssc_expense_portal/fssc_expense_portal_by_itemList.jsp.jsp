...
<link rel="stylesheet" type="text/css" href="${KMSS_Parameter_ContextPath}fssc/expense/resource/css/main1.css" /> 
<script>
...
<script>
		function initSelect(){
				...
				data.UseCache = false;
				var rtn = data.AddBeanData("fsscBaseCompanyService&authCurrent=true").GetHashMapArray();
				if(rtn&&rtn.length > 0){
					var optionstring="";
 ...
 <div style="padding-top:10px;padding-left: 20px;" hidden="true">
...
function FS_loadEvidenceExpense(){
	 var data1 = new KMSSData();
		...
		var rtn1 = data1.AddBeanData("fsscExpensePortalService&authCurrent=true&fdType=checkSubjectList&fdCompanyId="+fdCompanyId).GetHashMapArray();
		if(rtn1.length>0){
...
<script id="test" type="text/html"> 
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab-table3" >
						<thead>
            <tr>
              <th><span class="table-th-span">排名<span></th>
              <th class="table-th-name"><span>费用预算<span></th>
