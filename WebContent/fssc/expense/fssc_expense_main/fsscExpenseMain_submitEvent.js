				...
				success:function(rtn){
					...
					for(var i=0;i<len;i++){
						...
						if(fdBudgetShowType=='1'){
							...
							if(showBudget){
								$("#buget_status_"+i).html(lang['py.money.total']+showBudget.fdTotalAmount+"<br>"+lang['py.money.using']+showBudget.fdOccupyAmount+"<br>"+lang['py.money.used']+showBudget.fdAlreadyUsedAmount+"<br>"+lang['py.money.usable']+"<span class='budget_money_"+i+"'>"+showBudget.fdCanUseAmount+"</span>");
		...
		$.ajax({
			...
			success:function(rtn){
				...
				for(var i=0;i<rtn.data.length;i++){
					...
					$("#standard_status_"+rtn.data[i].index).attr("class","budget_container");
					$("#standard_status_"+rtn.data[i].index).addClass("budget_status_"+rtn.data[i].status);
					$("#standard_status_"+rtn.data[i].index).attr("title",lang['py.standard.'+rtn.data[i].status]);
				}
			}
		});
		return pass;
