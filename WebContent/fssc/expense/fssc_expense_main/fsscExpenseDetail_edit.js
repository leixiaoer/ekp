...
seajs.use(['lui/dialog','lang!fssc-expense','lang!','lui/util/env'],function(dialog,lang,comlang,env){
	...
	window.FSSC_SelectPlace = function(id,name){
		...
		var fdCompanyId = $("[name='fdCompanyId']").val();
		dialog.iframe(
			'/fssc/expense/fssc_expense_detail/fsscExpenseDetail_selectCity.jsp?fdCompanyId='+fdCompanyId,
			lang['tips.selectCity'],
			function(data){
				if(!data)return;
				$("[name='"+id+"']").val(data.id);
...
//			return false;
//		}
//		dialogSelect(false,'fssc_base_berth_fdBerth','fdTravelList_Form[*].fdBerthId','fdTravelList_Form[*].fdBerthName',null,{fdVehicleId:fdVehicleId});
//	}
	...
	window.FSSC_SelectProjectDetail = function(){
		var index = DocListFunc_GetParentByTagName('TR').rowIndex-1;
		dialogSelect(false,'fssc_base_project_project','fdDetailList_Form[*].fdProjectId','fdDetailList_Form[*].fdProjectName',null,{'fdCompanyId':$('[name=fdCompanyId]').val(),'fdProjectType':1},function(rtn){
			if(rtn){
	...
	window.FSSC_AddExpenseDetail = function(){
		...
		if(index>0){
			...
			var data = new KMSSData();
			data = data.AddBeanData("fsscBaseInputTaxService&authCurrent=true&fdExpenseItemId="+$("[name='fdDetailList_Form["+index+"].fdExpenseItemId']").val());
			data = data.GetHashMapArray();
	...
	//费用明细选择税率
  	window.FSSC_SelectInputTaxRate=function(){
  		...
  		dialogSelect(false,'fssc_base_input_tax_getInputTax','fdDetailList_Form['+index+'].fdInputTaxRateId','fdDetailList_Form['+index+'].fdInputTaxRate',null,{fdCompanyId:fdCompanyId,fdExpenseItemId:fdExpenseItemId},function(rtn){
  			if(!rtn){
  				return;
  			}
	...
	window.FSSC_SelectCostCenter = function(){
		...
		dialogSelect(false,'fssc_base_cost_center_selectCostCenter','fdDetailList_Form[*].fdCostCenterId','fdDetailList_Form[*].fdCostCenterName',null,{fdCompanyId:fdCompanyId,fdNotId:'fdNotId'},function(rtn){
	...
	window.FSSC_SelectExpenseItem = function(){
		...
		dialogSelect(false,'fssc_base_expense_item_selectExpenseItem','fdDetailList_Form[*].fdExpenseItemId','fdDetailList_Form[*].fdExpenseItemName',null,{fdCompanyId:fdCompanyId,fdNotId:'fdNotId',fdCategoryId:fdCategoryId},function(){
	...
	window.FSSC_SelectWbs = function(){
		...
		dialogSelect(false,'fssc_base_expense_wbs_selectWbs','fdDetailList_Form[*].fdWbsId','fdDetailList_Form[*].fdWbsName',null,{fdCompanyId:fdCompanyId,fdNotId:'fdNotId'},function(rtn){
			if(rtn){
				FSSC_MatchProvsion(index);
			}
		});
	...
	window.FSSC_SelectWbsIsProject = function(){
		...
		dialogSelect(false,'fssc_base_expense_wbs_selectWbs','fdDetailList_Form[*].fdWbsId','fdDetailList_Form[*].fdWbsName',null,{fdCompanyId:fdCompanyId,fdProjectId:fdProjectId,fdNotId:'fdNotId'},function(rtn){
			if(rtn){
				FSSC_MatchProvsion(index);
			}
		});
	...
	window.FSSC_SelectInnerOrder = function(){
		...
		dialogSelect(false,'fssc_base_expense_order_selectInnerOrder','fdDetailList_Form[*].fdInnerOrderId','fdDetailList_Form[*].fdInnerOrderName',null,{fdCompanyId:fdCompanyId,fdNotId:'fdNotId'},function(rtn){
			if(rtn&&rtn.length>0){
	...
	window.FSSC_SelectCurrency = function(){
		...
		var fdCompanyId = $("[name='fdDetailList_Form["+index+"].fdCompanyId']").val();
		dialogSelect(false,'fssc_base_currency_fdCurrency','fdDetailList_Form[*].fdCurrencyId','fdDetailList_Form[*].fdCurrencyName',null,{existCurrency:existCurrency.join(';'),fdCompanyId:fdCompanyId},function(rtn){
			if(!rtn||rtn.length==0){
			...
			var fdCurrencyId = rtn[0].fdId,
    			data = new KMSSData();
			$("[name='fdDetailList_Form["+index+"].fdExchangeRate']").val("");
    		data = data.AddBeanData('fsscBaseExchangeRateService&authCurrent=true&type=getRateByCurrency&fdCurrencyId='+fdCurrencyId+'&fdCompanyId='+fdCompanyId).GetHashMapArray();
			if(data.length>0){
	...
	window.FSSC_ChangeMoney = function(v,e,index){
		...
		if(fdExchangeRate){
			fdStandardMoney = (fdApplyMoney*fdExchangeRate).toFixed(2);
		}else{
	...
	window.FSSC_GetTotalMoney = function(){
		...
		$("[name$=fdApplyMoney]").each(function(){
			var tr = DocListFunc_GetParentByTagName("TR",this);
			var rate = $(tr).find("[name$=fdExchangeRate]").val();
			totalStandardMoney += this.value*rate;
		});
		if(isNaN(totalStandardMoney)){
			return;
		}
		$("[name=fdTotalStandaryMoney]").val(totalStandardMoney.toFixed(2));
		FSSC_XFormDispatch(totalStandardMoney.toFixed(2),$("[name=fdTotalStandaryMoney]")[0]);
		$("[name=fdTotalApprovedMoney]").val(totalStandardMoney.toFixed(2));
		FSSC_XFormDispatch(totalStandardMoney.toFixed(2),$("[name=fdTotalApprovedMoney]")[0]);
		//设置摊销总金额，重置摊销明细
	...
	window.FSSC_ChangeExchangeRate = function(v,e,i){
		...
		var fdCompanyId = $("[name=fdCompanyId]").val();
		var data = new KMSSData();
		data.AddBeanData("fsscBaseCompanyService&authCurrent=true&type=isCurrencySame&fdCompanyId="+fdCompanyId);
		data = data.GetHashMapArray();
	...
	window.FSSC_ShowDetailBudgetInfo = function(index){
		...
		if(fdBudgetShowType=='1'){//显示图标
			...
			$("#buget_status_"+index).html(lang['py.money.total']+showBudget.fdTotalAmount+"<br>"+lang['py.money.using']+showBudget.fdOccupyAmount+"<br>"+lang['py.money.used']+showBudget.fdAlreadyUsedAmount+"<br>"+lang['py.money.usable']+"<span class='budget_money_"+i+"'>"+showBudget.fdCanUseAmount+"</span>");
			$(".budget_money_"+index).css("color",fdBudgetStatus=='2'?"red":"#333");
		}
	}
	...
	window.FSSC_SelectNote = function(){
		...
		dialogSelect(true,'fssc_expense_detail_selectNote','fdNoteIds','fdNoteNames',null,{},function(rtn){
			if(!rtn||rtn.length==0){
				return;
			}
	...
	window.FSSC_SelectPayCurrency = function(){
		...
		dialogSelect(false,'fssc_base_currency_fdCurrency','fdAccountsList_Form[*].fdCurrencyId','fdAccountsList_Form[*].fdCurrencyName',null,{existCurrency:existCurrency.join(';'),fdCompanyId:fdCompanyId,fdPayCurrencyType:fdPayCurrencyType},function(rtn){
			if(!rtn||rtn.length==0){
				return;
			}
			var fdCurrencyId = rtn[0].fdId,
			...
			if(rate.length>0){
				...
				data = data.AddBeanData('fsscBaseExchangeRateService&type=getRateByCurrency&authCurrent=true&fdCurrencyId='+fdCurrencyId+'&fdCompanyId='+fdCompanyId).GetHashMapArray();
				if(data.length>0){
	...
	window.FSSC_SelectAccount = function(){
		var index = DocListFunc_GetParentByTagName('TR').rowIndex-1;
		dialogSelect(false,'fssc_base_account_fdAccount','fdAccountsList_Form[*].fdAccountId','fdAccountsList_Form[*].fdAccountName',null,null,function(rtn){
			if(rtn){
	...
	window.FSSC_SelectPayWay = function(){
		...
		dialogSelect(false,'fssc_base_pay_way_getPayWay','fdAccountsList_Form[*].fdPayWayId','fdAccountsList_Form[*].fdPayWayName',null,{fdCompanyId:fdCompanyId},function(rtn){
			if(rtn){
	...
	window.FSSC_SelectPayBank = function(){
		...
		dialogSelect(false,'fssc_base_pay_bank_fdBank','fdAccountsList_Form[*].fdBankId','fdAccountsList_Form[*].fdPayBankName',null,{fdCompanyId:fdCompanyId},function(data){
			if(data){
	...
	window.FSSC_SetDefaultCurrency = function(){
		...
		var fdCompanyId = $("[name=fdCompanyId]").val();
		var data = new KMSSData();
		data.AddBeanData("fsscBaseCompanyService&authCurrent=true&type=getStandardCurrencyInfo&fdCompanyId="+fdCompanyId);
		data = data.GetHashMapArray();
		if(data.length>0){
				...
				function(data) {
					if(!data){
						...
						for(var key in detailInfo){
							...
							$("[name='fdDetailList_Form["+index+"].fdBudgetMoney']").val(detail.fdInvoiceMoney);
							$("[name='fdDetailList_Form["+index+"].fdStandardMoney']").val(detail.fdInvoiceMoney);
							$("[name='fdDetailList_Form["+index+"].fdApprovedStandardMoney']").val(detail.fdInvoiceMoney);
							$("[name='fdDetailList_Form["+index+"].fdApprovedApplyMoney']").val(detail.fdInvoiceMoney);
							...
							if(fdInvoiceType=='10100'){//专票
								...
								data1 = data1.AddBeanData("fsscBaseInputTaxService&authCurrent=true&multi=true&fdExpenseItemId="+$("[name='fdDetailList_Form["+index+"].fdExpenseItemId']").val());
...
//					}
	 ...
	 window.clearInvoiceDetail=function(){
		...
		if(rtnData&&rtnData.length>0){
			...
			for(var i=0;i<len;i++){
				DocList_DeleteRow($("#TABLE_DocList_fdInvoiceList_Form").find("tr").not(".tr_normal_title")[i]);
			}
		}
	 }
	 //删除费用明细，级联清除发票明细发票
	...
	window.FSSC_CheckDetailImportData = function(datas,cols,firstIndex){
		...
		if(fdExpenseType=='2'&&fdIsTravelAlone=='false'){
			...
			if(scityArray.length>0){
				$.post(
					Com_Parameter.ContextPath+'fssc/base/fssc_base_area/fsscBaseArea.do?method=checkCityExists',
		...
		if(fdExtendField.indexOf(';2;')>-1){
			...
			if(rateArray.length>0){
				$.post(
					Com_Parameter.ContextPath+'fssc/base/fssc_base_input_tax/fsscBaseInputTax.do?method=checkRateExists',
