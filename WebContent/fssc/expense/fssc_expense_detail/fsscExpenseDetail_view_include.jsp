...
<table class="tb_normal" id="TABLE_DocList_fdDetailList_Form" align="center">
        ...
        <tr KMSS_IsContentRow="1">
            ...
            <td align="center">
                <div id="_xform_fdDetailList_Form[${vstatus.index}].fdCostCenterId" _xform_type="dialog">
                    ...
                    style="width:90%;">
                        dialogSelect(false,'fssc_base_cost_center_selectCostCenter','fdDetailList_Form[*].fdCostCenterId','fdDetailList_Form[*].fdCostCenterName');
                    </xform:dialog>
                </div>
            </td>
            </c:if>
...
<script>
    ...
    seajs.use(['lui/dialog','lang!fssc-expense'],function(dialog,lang){
			...
			$("#fee_status_"+index).attr("title",lang['py.fee.'+fdFeeStatus]);
 			...
 			for(var i=0;i<info.length;i++){
	   		...
	   		if(fdBudgetShowType=='1'){//显示图标
	   			...
	   			$("#buget_status_"+index).html(lang['py.money.total']+showBudget.fdTotalAmount+"<br>"+lang['py.money.using']+showBudget.fdOccupyAmount+"<br>"+lang['py.money.used']+showBudget.fdAlreadyUsedAmount+"<br>"+lang['py.money.usable']+"<span class='budget_money_"+index+"'>"+showBudget.fdCanUseAmount+"</span>");
    	...
    	window.FSSC_ChangeMoney=function(v,e,i){
    		...
    		var fdStandardMoney = numMulti(fdExchangeRate,fdApplyMoney);
    		...
    		$("[name='fdDetailList_Form["+index+"].fdStandardMoney']").val(fdStandardMoney.toFixed(2));
    		$("[name='fdDetailList_Form["+index+"].fdApprovedStandardMoney']").val(fdStandardMoney.toFixed(2));
    		var fdBudgetMoney = numMulti(fdBudgetRate,fdApplyMoney);
    		...
    		$("[id='_xform_fdDetailList_Form["+index+"].fdStandardMoney']").html(fdStandardMoney.toFixed(2))
    		var fdTotalApprovedMoney = 0;
    		$("[name$=fdApprovedApplyMoney]").each(function(i){
    			var rate = $("[name='fdDetailList_Form["+i+"].fdExchangeRate']").val()
    			fdTotalApprovedMoney = numAdd(fdTotalApprovedMoney,numMulti(this.value*1,rate));
    		})
    		$("[name=fdTotalApprovedMoney]").val(fdTotalApprovedMoney.toFixed(2));
    		$("#fdTotalApprovedMoney").html(fdTotalApprovedMoney.toFixed(2));
