...
function Designer_Control_Cost_Center_Load(rtn){
	...
	data.AddBeanData("fsscFeeDataService&type=getDefaultCostCenter&fdCompanyId="+rtn[0].fdId);
	...
	if(!info){
		...
		if(fdBudgetShowType=='1'){//显示图标
			...
			seajs.use(['lang!fssc-fee'],function(lang){
				...
				if(props.fdShowMoney.indexOf("4")>-1){
					html.push(lang['py.money.usable']+"<span class='",mon,"'>",showBudget.fdCanUseAmount,"</span>");
				}
			})
			...
			if(tb){
				...
				for(var i=0;i<len;i++){
					...
					if(fdBudgetShowType=='1'){
						...
						showBudget = showBudget?showBudget:{fdTotalAmount:0,fdOccupyAmount:0,fdAlreadyUsedAmount:0,fdCanUseAmount:0};
						if(showBudget){
							var html = [];
							if(props.fdShowMoney.indexOf("1")>-1){
								html.push(lang['py.money.total'],showBudget.fdTotalAmount,"<br>");
							...
							if(props.fdShowMoney.indexOf("4")>-1){
								html.push(lang['py.money.usable']+"<span class='budget_money_",i,"'>",showBudget.fdCanUseAmount,"</span>");
							}
				...
				if(!pass){
					if(fdBudgetShowType=='1'){
						...
						if(showBudget){
							...
							if(props.fdShowMoney.indexOf("4")>-1){
								html.push(lang['py.money.usable']+"<span class='budget_money_",0,"'>",showBudget.fdCanUseAmount,"</span>");
							}
...
Com_Parameter.event.submit.push(function(){
	...
	for(var i=0;i<len;i++){
		...
		params.push({
			'fdCompanyId':$("[name='extendDataFormInfo.value("+props.fdCompanyId+")']").val(),
			'fdExpenseItemId':$("[name*='."+i+"."+props.fdExpenseItemId.replace(/.+\.(.+)/,"$1")+")']").val(),
			'fdSpecialId':$("[name*='."+i+"."+props.fdSpecialId.replace(/.+\.(.+)/,"$1")+")']").val(),
			'fdPersonId':$("[name*='."+i+"."+props.fdPersonId.replace(/.+\.(.+)/,"$1")+".id)']").val(),
			'fdMoney':$("[name*='."+i+"."+props.fdMoneyId.replace(/.+\.(.+)/,"$1")+")']").val()||0,
			'fdCurrencyId':$("[name*='."+i+"."+props.fdCurrencyId.replace(/.+\.(.+)/,"$1")+")']").val(),
			'fdTravelDays':$("[name*='."+i+"."+props.fdTravelDays.replace(/.+\.(.+)/,"$1")+")']").val(),
			'fdVehicleId':$("[name*='."+i+"."+props.fdVehicleId.replace(/.+\.(.+)/,"$1")+"_vehicle_id)']").val(),
			'fdBerthId':$("[name*='."+i+"."+props.fdVehicleId.replace(/.+\.(.+)/,"$1")+"_berth_id)']").val(),
			'fdAreaId':$("[name*='."+i+"."+props.fdAreaId.replace(/.+\.(.+)/,"$1")+")']").val(),
			'fdPersonNumber':$("[name*='."+i+"."+props.fdPersonNumberId.replace(/.+\.(.+)/,"$1")+")']").val()||1,
			'index':i
		})
	}
	var pass = true;
	...
	if(props.fdMatchType=='1'){
		...
		//显示金额
			...
			seajs.use(['lang!fssc-fee'],function(lang){
				...
				if(props.fdShowMoney.indexOf("4")>-1){
					html.push(lang['py.money.usable']+"<span class='budget_money'>",fdBudgetInfo.fdCanUseAmount||0,"</span>");
				}
			})
		...
		$("input").filter(function(){return /\S+\.\d+\.\S+_budget_status/.test(this.name)}).each(function(i){
			...
			//显示金额
				...
				seajs.use(['lang!fssc-fee'],function(lang){
					...
					if(props.fdShowMoney.indexOf("4")>-1){
						html.push(lang['py.money.usable']+"<span class='budget_money_",i,"'>",fdBudgetInfo.fdCanUseAmount||0,"</span>");
					}
				})
...
function Designer_SelectArea(fdCompanyId,id){
	...
	if(window.DocListFunc_GetParentByTagName){
		index = DocListFunc_GetParentByTagName('TR').rowIndex-1;
	}
	seajs.use(['lui/dialog','lang!fssc-fee'],function(dialog,lang){
	dialog.iframe(
		'/fssc/fee/fssc_fee_xform/jsp/selectCity.jsp?fdCompanyId='+$("[name*="+fdCompanyId+"]").val(),
		...
		function(data){
			...
			if(index===''){
				$("[name='extendDataFormInfo.value("+id+")']").val(data.id);
				$("[name='extendDataFormInfo.value("+id+"_name)']").val(data.name).focus();
			}else{
				$("[name$='."+index+"."+id+")']").val(data.id);
				$("[name$='."+index+"."+id+"_name)']").val(data.name).focus();
			}
			$("[name*='"+id+"']").trigger("change");
		},{width:800,height:600,close:true}
	);
	});
}
//改变币种后更新汇率
function Designer_Control_Currency_Evnet_Changed(data,id,params){
	...
	kms.AddBeanData("fsscBaseExchangeRateService&authCurrent=true&type=getRateByCurrency&fdCurrencyId="+data[0].fdId+"&fdCompanyId="+fdCompanyId);
