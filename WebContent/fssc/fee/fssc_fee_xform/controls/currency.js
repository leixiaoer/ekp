if(Designer_Lang.checkVersion){
	Designer_Config.operations['currency']={
			imgIndex : 87,
			title:Designer_Lang.controlCurrency_attr_title,
			titleTip:Designer_Lang.controlNew_AddressTitleTip,
			run : function (designer) {
				designer.toolBar.selectButton('currency');
			},
			type : 'cmd',
			order: 1,
			select: true,
	...
	Designer_Config.controls.currency = {
			type : "currency",
			storeType : 'field',
			inherit    : 'base',
			onDraw : _Designer_Control_Currency_OnDraw,
			drawXML : _Designer_Control_Currency_DrawXML,
			...
			attrs : {
				...
				company : {
					text :Designer_Lang.controlCurrency_attr_company,
					value : '',
					required: true,
					type : 'self',
					draw:_Designer_Control_Attr_Currency_Self_Company_Draw,
					show : true,
					checkout: function(msg, name, attr, value, values, control){
						if(!values.companyId){
							msg.push(Designer_Lang.controlCurrency_attr_checkout_companyNull)
							return false;
						}
						return true;
					}
				...
				defaultValue:{
					...
					required: true,
					type : 'self',
					draw:_Designer_Control_New_Currency_Self_Draw,
					show:true
				},
				width : {
			...
			onAttrLoad:_Designer_Control_Attr_Currency_OnAttrLoad,
			info : {
				name: Designer_Lang.controlCurrency_attr_title
			},
			resizeMode : 'onlyWidth'
	}
	Designer_Config.buttons.control.push("currency");
	//把控件增加到右击菜单区
	Designer_Menus.add.menu['currency'] = Designer_Config.operations['currency'];
}
function _Designer_Control_Currency_OnDraw(parentNode, childNode){
	// 必须要做ID设置
	...
	var htmlCode = _Designer_Control_Currency_DrawByType(this.parent, this.attrs, this.options.values, this);
	domElement.innerHTML = htmlCode;	
}

function _Designer_Control_Currency_DrawByType(parent,attrs,values,target){
	...
	var defaultValue = {'null':"",0:"",1:Designer_Lang.controlCurrency_attr_default_value};
...
function _Designer_Control_Currency_DrawXML() {	
	...
	//核算汇率
	buf.push('<extendSimpleProperty ');
	buf.push('name="', values.id, '_cost_rate" ');
	buf.push('label="', values.label, '" ');
	buf.push('type="String" ');
	if (values.required == "true") {
		buf.push('notNull="true" ');
	...
	//预算汇率
	buf.push('<extendSimpleProperty ');
	buf.push('name="', values.id, '_budget_rate" ');
	buf.push('label="', values.label, '" ');
	buf.push('type="String" ');
	if (values.required == "true") {
		buf.push('notNull="true" ');
...
function _Designer_Control_Attr_Currency_OnAttrLoad(form,control){
	
}
//显示默认值的属性面板
function _Designer_Control_New_Currency_Self_Draw(name, attr, value, form, attrs, values){
	var display = "none",selected="";
	if(values.defaultValue=='1'){
		selected = "selected";
	}
	var htmlCode = "<select name='defaultValue' class='attr_td_select' style='width:95%' onchange='_Show_Currency_DefaultValue(this)'>";
	htmlCode+='<option value="">'+Designer_Lang.controlAddressAttrDefaultValueNull+'</option>';
	htmlCode+='<option value="1" '+selected+'>'+Designer_Lang.controlCurrency_attr_default_value+'</option>';
	htmlCode+="</select><div id='defaultCurrency' style='display:"+display+";'><input type='text' style='width:80%' readonly name='defaultCurrencyName'></div>";
	return Designer_AttrPanel.wrapTitle(name, attr, value, htmlCode);
}
function _Show_Currency_DefaultValue(obj){
	if(obj.value=='2'){
		document.getElementById('defaultCurrency').style.display='';
	}else{
		document.getElementById('defaultCurrency').style.display='none';
	}
}

function _Designer_Control_Attr_Currency_Self_Company_Draw(name, attr,
...
function Designer_Currency_DefaultValue_Validator(){
	return true;
}

function Designer_Currency_DefaultValue_Checkout(){
	return true;
}
