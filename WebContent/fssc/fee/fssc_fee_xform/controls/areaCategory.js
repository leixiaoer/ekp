Designer_Config.operations['areaCategory']={
		imgIndex : 80,
		title:Designer_Lang.controlAreaCategory_attr_title,
		titleTip:Designer_Lang.controlNew_AddressTitleTip,
		run : function (designer) {
			designer.toolBar.selectButton('areaCategory');
		},
		type : 'cmd',
		order: 1,
		select: true,
...
Designer_Config.controls.areaCategory = {
		type : "areaCategory",
		storeType : 'field',
		inherit    : 'base',
		onDraw : _Designer_Control_AreaCategory_OnDraw,
		drawXML : _Designer_Control_AreaCategory_DrawXML,
		...
		attrs : {
			...
			company : {
				...
				required: true,
				type : 'self',
				draw:_Designer_Control_Attr_AreaCategory_Self_Company_Draw,
				show : true,
				checkout: function(msg, name, attr, value, values, control){
					if(!values.companyId){
						msg.push(Designer_Lang.controlAreaCategory_attr_checkout_companyNull)
						return false;
					}
					return true;
				}
		...
		onAttrLoad:_Designer_Control_Attr_AreaCategory_OnAttrLoad,
		info : {
			name: Designer_Lang.controlAreaCategory_attr_title
		},
		resizeMode : 'onlyWidth'
}
Designer_Config.buttons.control.push("areaCategory");
//把控件增加到右击菜单区
Designer_Menus.add.menu['areaCategory'] = Designer_Config.operations['areaCategory'];
function _Designer_Control_AreaCategory_OnDraw(parentNode, childNode){
	// 必须要做ID设置
	...
	var htmlCode = _Designer_Control_AreaCategory_DrawByType(this.parent, this.attrs, this.options.values, this);
	domElement.innerHTML = htmlCode;	
}

function _Designer_Control_AreaCategory_DrawByType(parent,attrs,values,target){
...
function _Designer_Control_AreaCategory_DrawXML() {
...
function _Designer_Control_Attr_AreaCategory_OnAttrLoad(form,control){
	
}

function _Designer_Control_Attr_AreaCategory_Self_Company_Draw(name, attr,
