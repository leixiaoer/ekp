Designer_Config.operations['expenseItem']={
		imgIndex : 85,
		title:Designer_Lang.controlExpenseItem_attr_title,
		titleTip:Designer_Lang.controlNew_AddressTitleTip,
		run : function (designer) {
			designer.toolBar.selectButton('expenseItem');
		},
		type : 'cmd',
		order: 1,
		select: true,
...
Designer_Config.controls.expenseItem = {
		type : "expenseItem",
		storeType : 'field',
		inherit    : 'base',
		onDraw : _Designer_Expense_Item_OnDraw,
		drawXML : _Designer_Control_Expense_Item_DrawXML,
		...
		attrs : {
			...
			company : {
				...
				required: true,
				type : 'self',
				draw:_Designer_Control_Attr_Expense_Item_Self_Company_Draw,
				show : true,
				checkout: function(msg, name, attr, value, values, control){
					if(!values.companyId){
						msg.push(Designer_Lang.controlExpenseItem_attr_checkout_companyNull)
						return false;
					}
					return true;
				}
		...
		onAttrLoad:_Designer_Control_Attr_Expense_Item_OnAttrLoad,
		info : {
			name: Designer_Lang.controlExpenseItem_attr_title
		},
		resizeMode : 'onlyWidth'
}
Designer_Config.buttons.control.push("expenseItem");
//把控件增加到右击菜单区
Designer_Menus.add.menu['expenseItem'] = Designer_Config.operations['expenseItem'];
function _Designer_Expense_Item_OnDraw(parentNode, childNode){
	// 必须要做ID设置
	...
	var htmlCode = _Designer_Control_Expense_Item_DrawByType(this.parent, this.attrs, this.options.values, this);
	domElement.innerHTML = htmlCode;	
}

function _Designer_Control_Expense_Item_DrawByType(parent,attrs,values,target){
...
function _Designer_Control_Expense_Item_DrawXML() {	
...
function _Designer_Control_Attr_Expense_Item_OnAttrLoad(form,control){
	
}

function _Designer_Control_Attr_Expense_Item_Self_Company_Draw(name, attr,
