if(Designer_Lang.SAPEnabled){
	Designer_Config.operations['internalOrder']={
			imgIndex : 3,
			title:Designer_Lang.controlInternalOrder_attr_title,
			titleTip:Designer_Lang.controlInternalOrder_attr_title,
			run : function (designer) {
				designer.toolBar.selectButton('internalOrder');
			},
			type : 'cmd',
			order: 1,
			select: true,
	...
	Designer_Config.controls.internalOrder = {
			type : "internalOrder",
			storeType : 'field',
			inherit    : 'base',
			onDraw : _Designer_Control_InternalOrder_OnDraw,
			drawXML : _Designer_Control_InternalOrder_DrawXML,
			...
			attrs : {
				...
				company : {
					...
					required: true,
					type : 'self',
					draw:_Designer_Control_Attr_InternalOrder_Self_Company_Draw,
					show : true,
					checkout: function(msg, name, attr, value, values, control){
						if(!values.companyId){
							msg.push(Designer_Lang.controlInternalOrder_attr_checkout_companyNull)
							return false;
						}
						return true;
					}
			...
			onAttrLoad:_Designer_Control_Attr_InternalOrder_OnAttrLoad,
			info : {
				name: Designer_Lang.controlInternalOrder_attr_title
			},
			resizeMode : 'onlyWidth'
	}
	Designer_Config.buttons.control.push("internalOrder");
	//把控件增加到右击菜单区
	Designer_Menus.add.menu['internalOrder'] = Designer_Config.operations['internalOrder'];
}
function _Designer_Control_InternalOrder_OnDraw(parentNode, childNode){
	// 必须要做ID设置
	...
	var htmlCode = _Designer_Control_InternalOrder_DrawByType(this.parent, this.attrs, this.options.values, this);
	domElement.innerHTML = htmlCode;	
}

function _Designer_Control_InternalOrder_DrawByType(parent,attrs,values,target){
...
function _Designer_Control_InternalOrder_DrawXML() {
...
function _Designer_Control_Attr_InternalOrder_OnAttrLoad(form,control){
	
}

function _Designer_Control_Attr_InternalOrder_Self_Company_Draw(name, attr,
