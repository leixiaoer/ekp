...
window.selectCurrency = function(){
    ...
    if(fdPaymentType=='1'){
    	 dialogSelect(false,'fssc_base_currency_fdCurrency','fdDetail_Form[*].fdBaseCurrencyId','fdDetail_Form[*].fdBaseCurrencyName',null,{'fdCompanyId':fdCompanyId,'fdModelCurrencyIds':$('[name=fdModelCurrencyIds]').val()},function(rtn){
    		  if(rtn){   
    	...
    	dialogSelect(false,'fssc_base_currency_fdCurrency','fdDetail_Form[*].fdBaseCurrencyId','fdDetail_Form[*].fdBaseCurrencyName',null,{'fdCompanyId':fdCompanyId,'fdPaymentCurrencyId':$('[name=fdModelCurrencyIds]').val()},function(rtn){
	        if(rtn){
    		var fdCurrencyId = rtn[0].fdId, data = new KMSSData();
	        $("input[name='fdDetail_Form["+index+"].fdRate']").val("");
	        data = data.AddBeanData('fsscBaseExchangeRateService&type=getRateByCurrency&fdCurrencyId='+fdCurrencyId+'&fdCompanyId='+fdCompanyId).GetHashMapArray();
...
window.selectPayway = function(){
	...
	dialogSelect(false,'fssc_base_pay_way_fdPayWay','fdDetail_Form[*].fdBasePayWayId','fdDetail_Form[*].fdBasePayWayName',null,{'fdCompanyId':fdCompanyId},function(rtn){
		if(rtn){
...
window.selectPayBank=function(){
	var index = DocListFunc_GetParentByTagName('TR').rowIndex-1;
	  ...
	  dialogSelect(false,'fssc_base_pay_bank_fdBank','fdDetail_Form['+index+'].fdBasePayBankId','fdDetail_Form['+index+'].fdBasePayBankName', null, {fdCompanyId:fdCompanyId},function(rtn){
			if(rtn){
...
window.FSSC_SelectPayeeCity = function(){
	var index = DocListFunc_GetParentByTagName('TR').rowIndex-1;
	 dialogSelect(false,'fssc_cmb_city_code','fdDetail_Form[*].fdPayeeCityCode','fdDetail_Form[*].fdPayeeCity',null,null,function(rtn){
		 if(rtn){
			$("[name='fdDetail_Form["+index+"].fdPayeeCityCode']").val(rtn[0]['fdProvincial']);
