define(function(require, exports, module) {
		...
		var dialogObj = dialog.build({
			config:{
				...
				content : {
					...
					scroll : false,
					type : "iframe",
					url : '/fssc/common/resource/jsp/common-template.jsp?_key=template_select',
					params:null
				}
			}
		});
	...
	var dialogSelect = function(modelName, mulSelect, source, params, idField, nameField, winTitle, action){
		var nameTitle="选择"+$("[name='"+nameField.replace("*",0)+"']").attr("subject");
		var buttons = [ {
		...
		var tempUrl = '/fssc/common/resource/jsp/common-template.jsp?modelName=' + modelName + '&_key=dialog_' + idField+'&_name='+nameField;
		...
		var id = setInterval(function(){
			if(dialogObj.element&&dialogObj.element.find('iframe').length>0){
				var win = dialogObj.element.find('iframe').get(0).contentWindow;
				if(!win||!win.getData)return;
				dialogObj.element.find('iframe').get(0).contentWindow.$parent = window;
				dialogObj.element.find('iframe').get(0).contentWindow.getData();
				clearInterval(id);
			}
		},500)
	};
