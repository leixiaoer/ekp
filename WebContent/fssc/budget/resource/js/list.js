...
seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/dialog_common','lang!fssc-budget','lui/util/env','lang!'], function($, dialog , topic, dialogCommon,lang,env,comlang){
    var option = window.listOption;
    function addDoc(){
    	var fdSchemeId = Com_GetUrlParameter(window.parent.location.href,"fdSchemeId");
        Com_OpenWindow(option.contextPath + option.basePath + "?method=add&fdSchemeId="+fdSchemeId);
    }
    ...
    function addSCategoryMain(){
    	var fdSchemeId = Com_GetUrlParameter(location.href,'fdSchemeId');
    	dialog.simpleCategoryForNewFile(option.templateName, option.basePath + '?method=add&i.docTemplate=!{id}&i.fdBudgetScheme='+fdSchemeId,
    			false,null,null,getValueByHash("docTemplate"));
    }
    ...
    window.deleteAllByAuth=function(){
        var selected = [];
        $("input[name='List_Selected']:checked").each(function(){
            selected.push($(this).val());
        });
        ...
        dialog.confirm(option.lang.comfirmDelete,function(ok){
            if(ok==true){
                ...
                $.ajax({
                	url:option.contextPath + option.basePath+'?method=checkDeleteAll',
                	data:$.param(param,true),
                	dataType:'json',
                	type:'POST',
                	success:function(data){
                	...
                	error:function(req){
                		if(req.responseJSON){
                			var data = req.responseJSON;
                			if(!data.status){
                				dialog.failure(lang['tips.deleteall.error']);
                			}
                		}else{
                			dialog.failure('操作失败');
                		}
     ...
     */
    window.deleteAllBudgetData=function(){
        var selected = [];
        $("input[name='List_Selected']:checked").each(function(){
            selected.push($(this).val());
        });
        ...
        dialog.confirm(option.lang.comfirmDelete,function(ok){
            if(ok==true){
                ...
                $.ajax({
                	url:option.contextPath + option.basePath+'?method=deleteAllBudgetData',
                	data:$.param(param,true),
                	dataType:'json',
                	type:'POST',
                	success:function(data){
                	...
                	error:function(req){
                		if(req.responseJSON){
                			var data = req.responseJSON;
                			if(!data.status){
                				dialog.failure(lang['tips.deleteallBudgetData.error']);
                			}
                		}else{
                			dialog.failure('操作失败');
                		}
  ...
  //预算结转
	window.transferBudget=function(){
		var url = "/fssc/budget/resource/jsp/fsscBudgetTransfer.jsp";
		window.dia = dialog.iframe(url,lang['fsscBudget.button.budget.transfer'],
				function(data) {
					if(!data){
						return;
					}
		        	...
		        	$.ajax({
		        		url:env.fn.formatUrl(listOption.basePath+'?method=transferBudget'),
		        		data:{fdCompanyIds:data.fdCompanyIds,startMonth:data.startMonth,endMonth:data.endMonth},
		        		dataType:'json',
	...
	//修改预算状态：暂停、重启、关闭
	window.alterBudgetStatus = function(operation){
		var values = [];
		$("input[name='List_Selected']:checked").each(function(){
