Com_AddEventListener(window,'load',function(){
	seajs.use(['lui/jquery','lui/dialog','fssc/common/resource/js/dialog_common','lui/util/str','lang!fssc-budget','lui/util/env'], function($, dialog, dialogCommon,strutil,lang,env){
		...
		window.dialogSelect=function(mul, key, idField, nameField,action){
			...
			if((idField.indexOf('*')>-1||idField.indexOf('[')>-1) && window.DocListFunc_GetParentByTagName){
				//明细表
				dialogSelectForDetail(mul, key, idField, nameField,action);
			}else{
		...
		function dialogSelectForDetail(mul, key, idField, nameField,action){
						...
						$.ajax({
							  ...
							  success: function(result){
								  ...
								  if(result){
									  ...
									  if(allPeriod){
										  $("input[name='fdDetailList_Form["+index+"].fdBudgetInfo']").val(JSON.stringify(allPeriod));
									  }
									}else{
										validateFlag=false;
									}
