seajs.use(['lui/jquery','lui/dialog','lui/dialog_common','lui/util/str','lui/util/env','lang!fssc-budget','lang!'], function($, dialog, dialogCommon,strutil,env,lang,comlang){
	...
	window.downTemplate=function(){
		...
		if(!fdCompanyId){
			dialog.alert(lang['fssc.budget.import.company.message']);
			return false;
		}
		var url=env.fn.formatUrl('/fssc/budget/fssc_budget_main/fsscBudgetMain.do?method=downTemplate&fdSchemeId='+fdSchemeId+'&fdCompanyId='+fdCompanyId)
						    ...
						    for(var key in dataJson){
					            ...
					            if(key=='fdYearRule'||key=='fdQuarterRule'||key=='fdMonthRule'||key=='fdRule'
					            	...
					            	if(value=='3'){
					            		...
					            		$("[name='fdDetailList_Form["+index+"].fdElasticPercent']").attr("validate","required  range(0,100)");
