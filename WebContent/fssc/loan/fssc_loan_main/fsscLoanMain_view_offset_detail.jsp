        ...
        <tr>
            <sunbor:columnHead htmlTag="td">
                ...
                <sunbor:column property="fsscLoanExecuteDetail.fdModelSubject">
                    <bean:message bundle="fssc-loan" key="fsscLoanMain.offset.details.docSubject"/>
                </sunbor:column>
                <sunbor:column property="fsscLoanExecuteDetail.fdModelNumber">
                    <bean:message bundle="fssc-loan" key="fsscLoanMain.offset.details.docNumber"/>
                </sunbor:column>
                <sunbor:column property="fsscLoanExecuteDetail.fdType">
                    <bean:message bundle="fssc-loan" key="fsscLoanMain.offset.details.docStatus"/>
                </sunbor:column>
                <sunbor:column property="fsscLoanExecuteDetail.fdMoney">
                    <bean:message bundle="fssc-loan" key="fsscLoanMain.offset.details.fdMoney"/>
                </sunbor:column>
            </sunbor:columnHead>
        </tr>
        <c:forEach items="${queryPage.list}" var="fsscLoanExecuteDetail" varStatus="vstatus">
            <c:choose>
                <c:when test="${fsscLoanExecuteDetail.fdModelName == 'com.landray.kmss.fssc.loan.model.FsscLoanRepayment'}">
                    <tr kmss_href="<c:url value="/fssc/loan/fssc_loan_repayment/fsscLoanRepayment.do" />?method=view&fdId=${fsscLoanExecuteDetail.fdModelId}">
                </c:when>
                <c:when test="${fsscLoanExecuteDetail.fdModelName == 'com.landray.kmss.fssc.expense.model.FsscExpenseMain'}">
                    <tr kmss_href="<c:url value="/fssc/expense/fssc_expense_main/fsscExpenseMain.do" />?method=view&fdId=${fsscLoanExecuteDetail.fdModelId}">
                </c:when>
                <c:otherwise>
                    <tr align="center">
                </c:otherwise>
