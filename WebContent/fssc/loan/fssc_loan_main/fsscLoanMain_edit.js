...
//		var formValiduteObj = $KMSSValidation(document.forms[formOption.formName]);
	...
	Com_Parameter.event.submit.push(function(){
		...
		var fdLoanMoney=$("input[name='fdLoanMoney']").val();  //本次借款金额
		//无事前不作校验
		if(!fdFeeId){
			return true;
		}
		...
		$.ajax({
			...
			success:function(rtn){
				...
				if(rtn.result=='1'){//正确返回结果
					if(rtn.rtnMoney-fdLoanMoney<0){//返回金额小于借款金额，说明不允许提交
...
window.FSSC_ReloadCostCenter = function(){
	...
	//重新带出成本中心
	var data = new KMSSData();
	data.AddBeanData("fsscBaseCostCenterService&authCurrent=true&flag=defaultCostCenter&fdCompanyId="+fdCompanyId+"&fdPersonId="+$("[name=fdLoanPersonId]").val());
...
function  selectFdAccountAreaCallback(rtnData){
	if(rtnData && rtnData.length > 0){
		var obj = rtnData[0];
		$("input[name='fdAccountAreaCode']").val(obj.fdProvincialCode);//开户行账号
...
function onValueChangeFdLoanPerson(){
    ...
    var fdIsErasable = $('input[name=fdIsErasable]').val();
    // 清空关联事前申请值
	$("[name=fdFeeMainId]").val("");
	$("[name=fdFeeMainName]").val("");

    $.ajax({
        url: Com_Parameter.ContextPath+"fssc/loan/fssc_loan_main/fsscLoanMain.do?method=loadPersonInfo&fdPersonId="+fdLoanPersonId,
        dataType:"json",
    ...
    if(fdLoanPersonId=='' || fdLoanPersonId == oldFdLoanPersonId || fdIsErasable=='false'){
...
window.FSSC_SelectAccount = function(){
	dialogSelect(false,'fssc_base_account_fdAccount','fdAccPayeeId','fdAccPayeeName',null,null,function(rtnData){
		if(rtnData && rtnData.length > 0){
			var obj = rtnData[0];
			$("input[name='fdPayeeAccount']").val(obj.fdBankAccount);//开户行账号
			$("input[name='fdPayeeBank']").val(obj.fdBankName);//开户行
