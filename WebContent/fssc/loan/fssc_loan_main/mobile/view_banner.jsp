...
<%@ taglib uri="/WEB-INF/KmssConfig/sys/person/person.tld" prefix="person"%>
		<div class="muiFlowInfoW muiFormContent">
				<div class="muiFormSubject">
				 ...
				 <c:if test="${fsscLoanMainForm.docStatus eq '30'}">
                 ...
                 <c:if test="${fsscLoanMainForm.docStatus eq '00'}">
					...
					<tr>
						<td rowspan="2" class="muiFlowIconTd">
							 <div class="muiProcessIcon">
			                    <img class="muiProcessImg" src='<person:headimageUrl contextPath="true"  personId="${fsscLoanMainForm.docCreatorId}" size="m" />'/>
			                </div>
						...
						<td class="muiFlowSummary">
							<div>${lfn:message('fssc-loan:fsscLooanMain.docTemplate') }：<c:out value="${fsscLoanMainForm.docTemplateName}" /> </div>
							<div>${lfn:message('fssc-loan:fsscLooanMain.docNumber') }： <c:out value="${fsscLoanMainForm.docNumber}" /></div>
							<div>${lfn:message('fssc-loan:fsscLooanMain.docCreateTime') }： <c:out value="${fsscLoanMainForm.docCreateTime}" /></div>
						</td>
					</tr>
				</table>
			</div>
