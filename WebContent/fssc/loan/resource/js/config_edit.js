seajs.use(['lui/jquery','lui/dialog','fssc/common/resource/js/dialog_common','lui/util/str','lang!fssc-fee'], function($, dialog, dialogCommon,strutil,lang){
	window.dialogSelect=function(mul, key, idField, nameField,action,modleParam, detailParam,targetWin){
		...
		if(dialogCfg){
			var params='';
			if(modleParam){
				for(var i in modleParam){
					params+='&'+i+"="+modleParam[i];
				}
			}
			if(detailParam){
				for(var i in detailParam){
					params+='&'+i+"="+$("[name='"+detailParam[i].replace("*",rowIndex)+"']").val();;
				}
			}
...
//配置页对话框
function dialogSelect1(mul, key, idField, nameField, action, modleParam, detailParam){
	var rowIndex;
