Com_AddEventListener(window,'load',function(){
	seajs.use(['lui/jquery','lui/dialog','fssc/common/resource/js/dialog_common','lui/util/str','lang!fssc-loan'], function($, dialog, dialogCommon,strutil,lang){
		var formValiduteObj = $KMSSValidation(document.forms[formOption.formName]);
		window.formValiduteObj=formValiduteObj;
			...
			'checkExpectedDate(day)':{
				...
				test:function(v,e,o){
					...
					if(isNaN(Date.parse(v))){
						this.getValidator('checkExpectedDate').error = lang['msg.expected.date.invalid'];
						return false;
					}
					var controlDay = parseInt(o.day);
					var fdExpectedDate = Date.parse(v);
