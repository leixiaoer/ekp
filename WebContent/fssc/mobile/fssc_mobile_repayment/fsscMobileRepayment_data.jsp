...
<head>
    ...
    $(document).ready(function(){
	       var template='${repayTemplateList}'; //分类模板
	        if(template){
	        	template=JSON.parse(template);
	        }
	        var repayTemplatePicker = new Picker({
	            data: [template]
	         });
	        repayTemplatePicker.on('picker.select', function (selectedVal, selectedIndex) {
	        	window.open("${LUI_ContextPath}/fssc/loan/fssc_loan_repay_mobile/fsscLoanRepayMobile.do?method=add&i.docTemplate="+selectedVal,'_self');
	          });
	
	        repayTemplatePicker.on('picker.change', function (index, selectedIndex) {
	            console.log(index);
	          });
	
	        repayTemplatePicker.on('picker.valuechange', function (selectedVal, selectedIndex) {
	            console.log(selectedVal);
	          });
	        var createExpenseElement = document.getElementById('createExpense');
	        createExpenseElement.addEventListener('click', function () {
	        	repayTemplatePicker.show();
	          });
	        //点击加号
	        $("#createExpense").click(function (e) {
	        	...
	        	$.ajax({
     	           type: 'post',
     	           url:'${LUI_ContextPath}/fssc/loan/fssc_loan_repay_mobile/fsscLoanRepayMobile.do?method=findRepayTemplate',
     	           data: {"keyword":''},
     	       }).success(function (data) {
     	    	   console.log('获取信息成功');
     	    	   var rtn = JSON.parse(data);
     	    	   repayTemplatePicker.refillColumn(0, rtn);
     	       }).error(function (data) {
     	    	   console.log('获取信息失败');
     	       })
	    	});
...
<body>
    <div class="ld-application-form-list">
        ...
        <ul id="list_data">
        	<c:forEach var="fsscLoanRepayment" items="${queryPage.list}" varStatus="status">
	        	<li onclick="viewRepay('${fsscLoanRepayment.fdId}');">
                <div class="ld-application-form-list-item-top">
                	<div style="width:60%;word-break: break-all;">
	                    <c:if test="${fn:length(fsscLoanRepayment.docSubject)>40 }">${fn:substring(fsscLoanRepayment.docSubject,0,37)}...</c:if>
	                    <c:if test="${fn:length(fsscLoanRepayment.docSubject)<=40 }">${fsscLoanRepayment.docSubject}</c:if>
                    </div>
                     <div style="width:40%;text-align:right;">
                     	<c:if test="${not empty fsscLoanRepayment.fdRepaymentMoney}">
                     		<kmss:showNumber value="${fsscLoanRepayment.fdRepaymentMoney}" pattern="##0.00"/>
                     	</c:if>	
                        <c:if test="${ empty fsscLoanRepayment.fdRepaymentMoney}">
                        		0.00
                        </c:if>
                    </div>
                </div>
                <div class="ld-application-form-list-item-time">
                    ...
                    <div style="width:40%;text-align:right;float:right;">
                      	<span class="ld-list-status ld-list-status-status${fsscLoanRepayment.docStatus}"><sunbor:enumsShow value="${fsscLoanRepayment.docStatus}" enumsType="common_status" /></span>
                    </div>
                </div>
            </li>
        	</c:forEach>
        ...
        <c:if test="${queryPage.list== null or fn:length(queryPage.list) == 0}">
				<div style="text-align:center; line-height:50;">
					${lfn:message('fssc-mobile:fssc.mobile.no.data')}
				</div>
        </c:if>
    </div>
    <!-- 加载中 -->   
	  ...
	  loadDownFn : function(me){
              ...
              if((totalrows-rowsize*pageno)>0){
		  	  ...
		  	  if(isContinue){
				  ...
				  //上拉加载更多需要调用的函数				
				  $.ajax({
						url: '${LUI_ContextPath}/fssc/loan/fssc_loan_repay_mobile/fsscLoanRepayMobile.do?method=moreData&pageno='+pageno+'&rowsize=${queryPage.rowsize}',
						type: 'post',
						error: function(data){
		...
		function viewRepay(id){
			window.open("${LUI_ContextPath}/fssc/loan/fssc_loan_repay_mobile/fsscLoanRepayMobile.do?method=view&fdId="+id,'_self');
		}
    </script>
</body>
</html>