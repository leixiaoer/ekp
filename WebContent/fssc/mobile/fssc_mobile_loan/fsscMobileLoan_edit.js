...
function selectObject(id,name,dataSource,callback){
	...
	if(docTemplateId){
		$.ajax({
			url: dataSource,
			type: 'post',
			data:{fdCompanyId:$("input[name='fdCompanyId']").val(),fdPersonId:fdPersonId},
			async:false,
		}).error(function(data){
        ...
        if(fdLoanPersonId&&fdOldLoanPersonId&&fdLoanPersonId!=fdOldLoanPersonId){
        	$.ajax({
                ...
                success: function(data){
                    ...
                    $('input[name="fdExchangeRate"]').val(data['fdExchangeRate']);
                    ...
                    $("input[name='fdBankId']").val(data['fdBankId']);
...
//预计还款日期不能早于填单日期
  ...
  function FSSC_reloadBaseCurreny(){
      ...
      if (data.length > 0 && data[0].fdExchangeRate != null) {
    	  ...
    	  jqtoast(fsscLang['fssc-loan:fsscLoanMain.exchangeRateNotExist']);
  ...
  window.FSSC_ChangeStandardMoney= function(){
		var fdApplyMoney = $("[name='fdLoanMoney']").val();
