...
//关联明细的、附件的数据
	...
	function selectExpenseItem(fdParent,order) {
    	...
    	var keyword = $("#searchItem").val();
    	 $.ajax({
             type: 'post',
             url:Com_Parameter.ContextPath + 'fssc/mobile/fssc_mobile_note/fsscMobileNote.do?method=getFsscBaseExpenseItem',
  ...
  //取消
    ...
    //-----------------------------------选择成本中心satart--------------------------------------//
	 ...
	 function selectCostCenter(){
			var nameEl = document.getElementById('fdCostCenterName');
			var selectedIndex = [0, 0]; // 默认选中
			...
			$.ajax({
				type : 'post',
				url : Com_Parameter.ContextPath +'fssc/mobile/fssc_mobile_note/fsscMobileNote.do?method=getCostCenter',
			}).success(function(data) {
				console.log('获取信息成功');
				var rtn = JSON.parse(data);
				if(rtn.result=='success'){
					costCenterArr=rtn.dataArr;
					first = [];
					second = [];
					creatList(costCenterArr, first);
					if (costCenterArr[selectedIndex[0]].hasOwnProperty('sub')) {
					  creatList(costCenterArr[selectedIndex[0]].sub, second);
					} else {
					  second = [{text: '', value: 0}];
					}
					
					if (costCenterArr[selectedIndex[0]].sub[selectedIndex[1]].hasOwnProperty('sub')) {
					  creatList(costCenterArr[selectedIndex[0]].sub[selectedIndex[1]].sub, '');
					}
			...
			picker.on('picker.select', function (selectedVal, selectedIndex) {
				  ...
				  var text2 = second[selectedIndex[1]].text;
				   nameEl.value = text2 ;
				  $("[name='fdCompanyId']").val(first[selectedIndex[0]].id);
				  $("[name='fdCostCenterId']").val(second[selectedIndex[1]].id);
				  $("[name='fdCostCenterName']").val(text2);
				});
				...
				picker.on('picker.change', function (index, selectedIndex) {
				  if (index === 0){
					...
					var firstCity = costCenterArr[selectedIndex];
					if (firstCity.hasOwnProperty('sub')) {
						creatList(firstCity.sub, second);
		
						var secondCity = costCenterArr[selectedIndex].sub[0]
