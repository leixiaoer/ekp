				...
				if(fieldName){
					...
					if(nameObj&&nameObj.length>0){
						...
						if(changeFunc){
							...
							$(nameObj).attr('onchange',"matchBudget('main',null);");
						}
					}
				}
			}
 	  ...
 	  if(id==tab_id){//费控的明细表用费控的样式显示
 		 var displayList=formInitData["displayList"];
 		  ...
 		  if(!currency_code){
 			  ...
 			  kms.AddBeanData("fsscBaseCurrencyService&authCurrent=true&type=localCurrencyInfo&fdCompanyId="+fdCompanyId);
...
//匹配标准
  function matchStandard(index,editFlag){
	...
	var method=$("[name='method_GET']").val();
  	...
  	if(fdBerthId){
  		var kms = new KMSSData();
		  kms.AddBeanData("fsscBaseBerthService&authCurrent=true&fdBerthId="+fdBerthId);
		  kms = kms.GetHashMapArray();
		  if(kms&&kms.length>0){
...
   }    
  ...
  function setCostCenterByCompany(params,target){
	  ...
	  if(fdCompanyId&&fdPersonId){
		  var kms = new KMSSData();
		  kms.AddBeanData("fsscBaseCostCenterService&authCurrent=true&flag=defaultCostCenter&fdCompanyId="+fdCompanyId+"&fdPersonId="+fdPersonId);
		  kms = kms.GetHashMapArray();
		  if(kms&&kms.length>0){
			  if(kms[0].id){
...
//明细切换币种，重新赋值汇率
  function setRate(params){
	  ...
	  var fdCurrencyId=$("[name$='"+currency_field+")']").val();
	  	var kms = new KMSSData();
		kms.AddBeanData("fsscBaseExchangeRateService&authCurrent=true&type=getRateByCurrency&fdCurrencyId="+fdCurrencyId+"&fdCompanyId="+fdCompanyId);
		kms = kms.GetHashMapArray();
		if(kms&&kms.length>0){
  ...
  Com_Parameter.event.submit.push(function(){
	...
	var fdMappFeild=$("[name='fdMappFeild']").val();
 	...
 	var standardStatusId=fdMappFeild["fdStandardId"];
  	...
  	for(var i=0;i<len;i++){
  		if($("[name*='."+i+"."+fdMappFeild['fdExpenseItemId'].replace(/.+\.(.+)/,"$1")+")']").length==0){
  			continue;
  		}
