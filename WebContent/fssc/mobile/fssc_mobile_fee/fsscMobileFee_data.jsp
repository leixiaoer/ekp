...
<head>
    ...
    $(document).ready(function(){
	       var template='${feeTemplateList}'; //分类模板
	        if(template){
	        	template=JSON.parse(template);
	        }
	        var feeTemplatePicker = new Picker({
	            data: [template]
	         });
	        feeTemplatePicker.on('picker.select', function (selectedVal, selectedIndex) {
	        	window.open("${LUI_ContextPath}/fssc/fee/fssc_fee_mobile/fsscFeeMobile.do?method=add&i.docTemplate="+selectedVal,'_self');
	          });
	
	        feeTemplatePicker.on('picker.change', function (index, selectedIndex) {
	            console.log(index);
	          });
	
	        feeTemplatePicker.on('picker.valuechange', function (selectedVal, selectedIndex) {
	            console.log(selectedVal);
	          });
	        var createExpenseElement = document.getElementById('createExpense');
	        createExpenseElement.addEventListener('click', function () {
	        	feeTemplatePicker.show();
	          });
	        //点击加号
	        $("#createExpense").click(function (e) {
	        	...
	        	$.ajax({
     	    	   ...
     	    	   feeTemplatePicker.refillColumn(0, rtn);
     	       }).error(function (data) {
     	    	   console.log('获取信息失败');
     	       })
	    	});
