    		...
    		setTimeout(function(){
            	...
            	if(ua.match(/MicroMessenger/i) == "micromessenger"
            		||(ua.match(/MicroMessenger/i) == 'micromessenger' && ua.match(/wxwork/i) == 'wxwork')){ //个人微信或者企业微信
          		  	...
          		  	// 获取微信签名
          			$.ajax({ 
          				url : Com_Parameter.ContextPath + 'fssc/base/fssc_base_data/fsscBaseData.do?method=getWxSign',
          				data : _data,
          				async:false,
        				type:"post",
		        ...
		        if (res.checkResult.getLocalImgData) {
		            ...
		            wx.uploadImage({
		                ...
		                success: function (res) {
		                    ...
		                    // 获取微信服务器图片base64
		          			$.ajax({ 
		          				url : Com_Parameter.ContextPath + 'fssc/base/fssc_base_data/fsscBaseData.do?method=getLocalImgData',
   ...
   //-------------------------------------上传附件end----------------------------------------//
    ...
    function listData(){
    	window.open(formInitData['LUI_ContextPath']+"/fssc/mobile/fssc_mobile_authorize/fsscMobileAuthorize.do?method=data", '_self');
    }