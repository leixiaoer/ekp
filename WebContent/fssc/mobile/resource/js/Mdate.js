(function() {
  ...
  //  includeCss("/fssc/mobile/resource/css/Mdate.css");
    var dateopts = {
        beginYear: 2010,
        beginMonth: 1,
        beginDay: 1,
    ...
    var Mdate = function(el, opts) {
        ...
        this.acceptName = opts.acceptName;
        ...
        this.dateBoxShow(el, opts);
    };
    Mdate.prototype = {
        constructor: Mdate,
        dateBoxShow: function(el, opts) {
        	console.log("el   "+el)
        	console.log("opts   "+opts)
        	console.log("selectorId    "+this.selectorId)
            var that = this;
            that.selectorId.onclick=function() {
            	console.log(that.selectorId);
                that.createDateBox();
                that.dateSure(el, opts)
            }
        },
        ...
        dateSure: function(el, opts) {
            ...
            sureBtn.onclick = function() {
                if (that.format == "YMD") {
                    ...
                    $("[name='"+that.acceptName+"']").val(yearTag + that.format + that.dateForTen(monthTag) + that.format + that.dateForTen(dayTag));
                }
                ...
                if(this.action){//选择完时间回调函数
                	this.action(opts.params);
                }
            };
        ...
        DateDiff: function(){
        	...
        	if(this.acceptName=='fdBeginDate' || this.acceptName=='fdEndDate'){
        		var sDate1 = d.getElementById("fdBeginDate"+index).value;
            	var sDate2 =  d.getElementById("fdEndDate"+index).value;
    		...
    		       oDate2  =  new  Date(aDate[1]  +  '/'  +  aDate[2]  +  '/'  +  aDate[0])  
    			   if(oDate1>oDate2){
    				   ...
    				   d.getElementById('days'+index).value ='';
    				   this.acceptId.value='';
    				   return ;
    			   }
    		       days  =  parseInt(Math.abs(oDate1  -  oDate2)  /  1000  /  60  /  60  /24)    //把相差的毫秒数转换为天数 
    			   d.getElementById('days'+index).value=(days+1)+"天";
