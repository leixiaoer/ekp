 ...
 **********************************************************************/
    ...
    function changeFdClaimant(person){
    	var fdPersonId='';
    	if(formInitData['fdIsAuthorize']=='1'){//启用授权
    			...
    			function(data){
    				...
    				if(data&&data.length>0){
	    		    	...
	    		    	$("[name='fdAccountsList_Form[0].fdAccountAreaName']").val(data[0].fdAccountAreaName);
	    		       	...
	    		       	if(fdCompanyId){
	    		       		data = new KMSSData();
	    		       		data.AddBeanData("fsscBaseCompanyService&fdCompanyId="+fdCompanyId+"&type=getStandardCurrencyInfo");
	    		       		data = data.GetHashMapArray();
	    		       		if(data&&data.length>0){
   ...
   //选择公司
	...
	function refreshTravelIndex(val){
		...
		for(var i=0;i<index;i++){
			var value=$("input[name='fdDetailList_Form["+i+"].fdTravel']").val();
			if(val==value){
				$("input[name='fdDetailList_Form["+i+"].fdTravel']").val("");
			}
		}
	}
  ...
  //按拼音排序
    ...
    function setIsDeductFromSelectNote(index){
    	...
    	var data = new KMSSData();
		data = data.AddBeanData("fsscBaseInputTaxService&authCurrent=true&fdExpenseItemId="+fdExpenseItemId);
		data = data.GetHashMapArray();
		if(data&&data.length>0){
...
//    		)
   ...
   function saveExpeseDeteil() {
    ...
    function  FSSC_ChangeIsDeduct(init){
		var data = new KMSSData();
		data = data.AddBeanData("fsscBaseInputTaxService&authCurrent=true&fdExpenseItemId="+$("[name='fdExpenseItemId']").val());
		data = data.GetHashMapArray();
		if(data&&data.length>0){
    ...
    function changeCurrency(){
    		...
    		var fdCompanyId = $("#fdCompanyId").val();
    		data.AddBeanData('fsscBaseExchangeRateService&authCurrent=true&type=getRateByCurrency&fdCurrencyId='+fdCurrencyId+'&fdCompanyId='+fdCompanyId);
    		data = data.GetHashMapArray();
		if(data.length>0){
			$("#fdExchangeRate").val(data[0].fdExchangeRate);
			$("#fdBudgetRate").val(data[0].fdBudgetRate);
     ...
     */
     ...
     function afterSelectAccount(data){
    	 ...
    	 $("#fdAccountAreaName").val(data.fdAccountArea);
     }
    	...
    	var index = $("[name='row-invoice-index']").val();
      	...
      	var fdInvoiceType = $("#fdInvoiceType").val();
	    	...
	    	if(fdInvoiceType==''){
	    		jqtoast("发票类型不能为空");
	    		return ;
	    	}else if(fdExpenseTypeId==""){
	    		jqtoast("费用类型不能为空");
	    		return ;
	    	}else if((fdInvoiceTypeId=="10100" || fdInvoiceTypeId=="30100") && fdInvoiceCode==""){
	    		jqtoast("发票代码不能为空");
	    		return ;
	    	} else if((fdInvoiceTypeId=="10100" || fdInvoiceTypeId=="30100") && fdInvoiceNumber==""){
	    		jqtoast("发票号码不能为空");
	    		return ;
	    	} else if((fdInvoiceTypeId=="10100" || fdInvoiceTypeId=="30100") && fdInvoiceDate==""){
	    		jqtoast("发票日期不能为空");
	    		return ;
	    	}else if((fdInvoiceTypeId=="10100" || fdInvoiceTypeId=="30100") && fdTaxValue==""){
	    		jqtoast("税率不能为空");
	    		return ;
	    	}else if((fdInvoiceTypeId=="10100" || fdInvoiceTypeId=="30100") && fdInvoiceMoney=="" ){
	    		if(fdInvoiceMoney==""){
	    			jqtoast("税价合计不能为空");
	       		return ;
		...
		for(var i=0 ; i<data.length ;i++){
			...
			var src =Com_Parameter.ContextPath+ "fssc/mobile/resource/images/specialTicket.png";
	 		var html1 = $("<div><div><img src="+src+" alt=''><span  name='fdOffsetList_span["+i+"].docSubject' class='subject'>"+data[i].docSubject+"</span></div>" +
	 					"<div><span class=\"ld-newApplicationForm-loan-top-div\">"+fsscLang['fssc-loan:fsscLoanMain.fdLoanMoney']+":</span>" +
    ...
    *************************************************************************/
    function selectObject(id,name,dataSource,callback){
		...
		$.ajax({
			...
			type: 'post',
			async:false,
			data: {"fdCompanyId":fdCompanyId,"categoryId":categoryId,"flag":"expense","keyword":"","fdExpenseItemId":fdExpenseItemId,fdTemplateId:categoryId, fdPersonId:fdPersonId},
		}).error(function(data){
				console.log("获取信息失败"+data);
		}).success(function(data){
			 console.log("获取信息成功");
