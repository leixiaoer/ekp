...
<c:choose>
	<c:when test="${ param.dtable == 'true'}">
		<c:import url="/resource/html_locate/sysAttMain_view_dt_swf.jsp" charEncoding="UTF-8">
	...
	<c:otherwise>
		<c:import url="/resource/html_locate/sysAttMain_view_swf.jsp" charEncoding="UTF-8">
			<c:param name="formBeanName" value="${ param.formBeanName }" />	
			<c:param name="fdModelName" value="${ param.fdModelName }" />
			<c:param name="fdModelId" value="${ param.fdModelId }" />
			<c:param name="fdMulti" value="${ param.fdMulti }" />
