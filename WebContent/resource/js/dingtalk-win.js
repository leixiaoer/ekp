...
Com_IncludeFile("dingtalk-pc-api.js");
seajs.use(['lui/parser', 'lui/jquery', 'lui/dialog', 'theme!common', 'theme!icon', 'theme!iconfont'],
    ...
    window.showTipWindow = function (url) {
    	...
    	//alert(url);
        // 添加用户token
        //var outToken = "<%=SecureUtil.BASE64Encoder(UserUtil.getUser().getFdId())%>";
        // 必须编码，防止#导致参数无法接收
...
//        }
        ...
        var innerHTML = "<div class='model'>" +
            ...
            "	<div class='pic'><img src='" + Com_Parameter.ContextPath + "resource/images/no_support_ding.png'/></div>" +
        ...
        if (typeof $(".model").html() == "undefined") {
            ...
            $(".no-support-msg .btn").on("click", function () {
                ...
                //alert("22"+url);
                try{
                	var signUrl = Com_Parameter.ContextPath+'third/ding/jsapi.do?method=jsapiSignature';
                	$.ajax({
             		   type: "POST",
             		   url: signUrl,
             		   async:true,
      						...
      						try{
      							DingTalkPC.runtime.permission.requestAuthCode({
          	       	   		        ...
          	       	   		        onSuccess: function (info) {
          	       	   		        	var code=info.code;
          	       	   		        	//alert(code);
          	       	   		        	url = Com_SetUrlParameter(url, "code", code);
          	       	   		        	window.open(url, '_blank');
          	       	   		        },
          	       	   		        onFail: function (err) {
      							...
      							console.log("requestAuthCode error,"+JSON.stringify(e2));
  	       	   		        	//alert("requestAuthCode error,"+JSON.stringify(e2));
  	       	   		        	window.open(url, '_blank');
      						}
      						
             			  }else{
             				 console.log("signInfo null");
             				 //alert("signInfo null");
	       	   		         window.open(url, '_blank');
             			  }
             		   },
             		   error:function(err3){
             			  console.log("jsapiSignature fail,"+JSON.stringify(err3));
	       	   		      window.open(url, '_blank');
             		   }
             		});
