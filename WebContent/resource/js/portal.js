...
Com_AddEventListener(window,"resize",Portlet_OnResize);
...
function Portlet_ResetText(){
	...
	for(i=0; i<textObj.length; i++)
	{
		...
		**********************************************************************/
		wordNum = Math.round((pObj.offsetWidth-lastObj.offsetLeft+textObj[i].nextSibling.offsetLeft-textObj[i].offsetLeft)/wordWidth*2)-4;
		//设置展现的文字信息
...
function showMultiTab(uuid,index,type){
	...
	for(var i=0;i<tabs.length;i++){
		if(i==index){
			...
			var timeId = portletDiv.getAttribute("LKS_Curr_TAB_TIME");
			...
			if(html==""){
				...
				portletDiv.setAttribute("LKS_Curr_TAB_UUID",tabs[i]["UUID"]);
				if(tabs[i].RefreshTime && tabs[i].RefreshTime!=0){
					var timeId = setTimeout("Portlet_Refresh(\""+portletDiv.uniqueID+"\");", tabs[i].RefreshTime*60000);
					portletDiv.setAttribute("LKS_Curr_TAB_TIME",timeId);
				} 
			}
...
function getTabTitleHTML(obj,index,type){
	...
	if(obj.MoreURL!="" && !S_PortalInfo.DesignMode){
		...
		html.push('<a onfocus=this.blur() href="'+formatUrl(moreUrl)+'" target='+obj.MoreTarget+'>');
	}
