...
(function(){
	...
	// 数据格式化
	$form.str =  function(value, type){
		if(util.isArray(value)){
			var rtnVal = [];
			for(var i=0; i<value.length; i++){
				rtnVal.push(str(value[i], type));
			}
			return rtnVal;
		}
		return str(value, type);
	};
	$form.format = function(value, type){
		if(util.isArray(value)){
			var rtnVal = [];
			for(var i=0; i<value.length; i++){
				rtnVal.push(format(value[i], type));
			}
			return rtnVal;
		}
	...
	function str(value, type){
		...
		if(type=='datetime' || type=='date' || type=='time'){
			return date2str(value, getDatePattern(type));
		}
		return value + '';
	}
	// 转基础类型
	function format(value, type){
		...
		case 'datetime':
			if(typeof value == 'string'){
				return str2date(value, getDatePattern(type));
			}
	...
	function getDatePattern(type){
		...
		if(type=='date' && Com_Parameter.Date_format){
