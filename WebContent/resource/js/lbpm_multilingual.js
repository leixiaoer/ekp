...
function changeLang(event, obj){
	...
	//隐藏前一个点击的区域
	preArea = currentArea;
	if(preArea && preArea != obj){
		$(preArea).children().find(".lang_item").css("display","none");
		$(preArea).children().find(".lang_icon_other").css("display","");
		//失去焦点，修改官方语言的input的样式，避免被遮住
		$(preArea).children().find("input.lang_input_official").css("padding-right",(25 * (langs.length+1)) + 'px');
		checkLbpmLangStatus1(preArea);
	}
	if(currentArea != obj)
		currentArea = obj;
	...
	if($(src)[0].localName==='input'||src.nodeName==='INPUT'){
		if($(src).attr("lang")===official){
			...
			checkLbpmLangStatus1(obj);
		}
	}else if($(src)[0].localName==='textarea'||src.nodeName==='TEXTAREA'){
		if($(src).attr("lang")===official){
			//官方语言，进行展示
			if($(src).attr("disabled") && $(src).attr("disabled") == "disabled"){
				return;
...
$(document).click(function(event){
	...
	if(!langObjs || langObjs.length <= 0){
		if(currentArea){
			$(currentArea).children().find(".lang_item").css("display","none");
			$(currentArea).children().find(".lang_icon_other").css("display","");
			//失去焦点，修改官方语言的input的样式，避免被遮住
			$(currentArea).children().find("input.lang_input_official").css("padding-right",(25 * (langs.length+1)) + 'px');
			checkLbpmLangStatus1(currentArea);
		}
		if(preArea){
			$(preArea).children().find(".lang_item").css("display","none");
			$(preArea).children().find(".lang_icon_other").css("display","");
...
$(function(){
	...
	var intervalNum = setInterval(function(){
		...
		$(".multiLang").each(function(index,element) {
			...
			var multiLangTextareas = $(element).children().find("textarea.lang_textarea_official");
			for(var i=0; i<multiLangTextareas.length; i++){
				if($(multiLangTextareas[i]).attr("disabled") || $(multiLangTextareas[i]).attr("disabled") == 'disabled'){
					$(element).children().find("span[name='lang_desc']").css("display","none");
					//将按钮隐藏
					$(element).children().find(".multi_lang_icon").hide();
				}
...
//监听input的值变化进行状态更新
//function inputLangValue(obj){
//	var langMul = $(obj).parents(".multiLang")[0];
//	checkLbpmLangStatus1(langMul);
//}

/*检查多语言是否有值来初始化按钮*/
function checkLbpmLangStatus1(langMul) {
	if($(langMul).children().find("input[lang='" + official + "']") && $(langMul).children().find("input[lang='" + official + "']").length > 0){
		($(langMul).children().find("input[lang='" + official + "']").val()) ? changeLangStatus1(
				'.multi_lang_icon.' + official, true, langMul)
				: changeLangStatus1('.multi_lang_icon.' + official, false, langMul);
		for(var i=0; i<langs.length; i++){
			var t = langs[i];
			($(langMul).children().find("input[lang_pull='" + t + "']")
					.val()) ? changeLangStatus1('.multi_lang_icon.' + t,
					true, langMul) : changeLangStatus1('.multi_lang_icon.'
					+ t, false, langMul);
		}
		/*langs
				.forEach(function(t) {
	...
	//增加文本框的
	if($(langMul).children().find("textarea[lang='" + official + "']") && $(langMul).children().find("textarea[lang='" + official + "']").length > 0){
		($(langMul).children().find("textarea[lang='" + official + "']").val()) ? changeLangStatus1(
				'.multi_lang_icon.' + official, true, langMul)
				: changeLangStatus1('.multi_lang_icon.' + official, false, langMul);
		for(var i=0; i<langs.length; i++){
			var t = langs[i];
			($(langMul).children().find("textarea[lang_pull='" + t + "']")
					.val()) ? changeLangStatus1('.multi_lang_icon.' + t,
					true, langMul) : changeLangStatus1('.multi_lang_icon.'
					+ t, false, langMul);
		}
...
function changeLangStatus1(langClassName, flag, langMul) {
	if (flag) {
