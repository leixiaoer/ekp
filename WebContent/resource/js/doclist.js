...
***********************************************/
function DocList_AddRow(optTB, content, fieldValues, noTrigger){
	if(optTB==null)
			...
			for(var i=0; i<fields.length; i++){
				...
				if(pObj!=null){
					switch(fields[i].tagName){
						case "INPUT":
							if(fields[i].type=="radio"){
								...
								if(fields[i].value==value || (datatype && datatype == 'Double' && parseFloat(fields[i].value) == parseFloat(value))){
					...
					if(Address_isNewAddress(fields[i])) {
						...
						/* #159311-流程表单【明细表】数据无法导入-开始 */
//						if (!fieldValues[idName] && addressName) {
//							idName = addressName.replace(".name", ".id");
//						}
						/* #159311-流程表单【明细表】数据无法导入-结束 */
	...
	//用于性能模式下, 兼容一些场景
	$(optTB).trigger($.Event("table-add-performance"),{'row':newRow,'vals':fieldValues,'table':optTB});
...
function DocList_CopyRow(optTR){
	...
	$(optTB).trigger($.Event("table-copy-new"),{'row':newOptTB});
...
function DocList_exportExcel(fdId,itemName,modelName,isXform,fieldNameArray){
	...
	if(fieldNameArray){
		...
		fieldArray = DocList_GetDetailsTableXformflag("TABLE_DL_"+itemName,null,false);
	}
...
function DocList_XformHandleControl(type,xformFlag){
	if(type){
		var expControl = ['company','costCenter','expenseItem','project','area','vehicle','innerOrder','wbs','currency'];//费控的控件
		...
		}else if($.inArray(type,expControl)>-1){
			var input = xformFlag.find("[name*='"+ control.id +"_name']");
			if(input.length > 0){
				control.value += input.val();
			}
		}else if(type == "xform_fSelect"){
			var labelText = [];
			xformFlag.find('.fs-option.selected').each(function(i, el) {
                labelText.push($(el).find('.fs-option-label').text());
            });
			control.value = labelText.join(";");
		}else if(type == "placeholder"){
			//业务关联控件
			var input = xformFlag.find("[name*='"+ control.id +"_text']");
			if(input.length > 0){
				control.value += input.val();
			}
		}else{
					...
					for(var i = 0;i < formElement.length;i++){
						...
						if($(e).attr("name") ){
							...
							if(isAddress){
								...
								var expControl = ['company','costCenter','expenseItem','project','area','vehicle','innerOrder','wbs','currency','dayCount','accountMoney'];//费控的控件
								if($(e).css('display') != 'none' && ($(e).attr('readOnly') == null || $(e).attr('readOnly') == false)||$.inArray(controlType,expControl)>-1){
									hasInput = true;
									break;
								}
							}
	...
	if(isXform == 'true'){
		...
		for(var i=0;i<dataJson.length;i++){
			if($.inArray(dataJson[i].fieldType,expControl)>-1||dataJson[i].fieldType=='area'){
				var div = $("[name$='"+dataJson[i].fieldId+")']").parent();
				var fdCompanyId = div[0].onclick.toString().replace(/[\s\S]+fdCompanyId\:[\'\"]([^\'^\"]+)[\'\"][\s\S]+/,'$1')
				if(dataJson[i].fieldType=='area'){
					fdCompanyId = fdCompanyId.replace(/[\s\S]+[\"\']([^\'^\"]+)[\"\']\,[\s\S]+/,'$1');
				}
				fdCompanyId = $("[name*="+fdCompanyId+"]").val();
				if(!fdCompanyId){
					alert('请先填写'+$("[name*="+fdCompanyId+"_name]").attr("subject"));
					return;
				}
			}
		}
...
function DocList_importSingleData(optTBId,jsonArray,fieldNames,temp,isXform){
	...
	if(isXform && isXform != null && isXform+'' == 'true'){
		...
		for(var fileName in jsonArray){
			for(var j = 0 ;j < fieldNames.length; j++){
				...
				if(fieldNames[j].lastIndexOf(fileName+")") > -1){
					...
					var xformflag = DocListFunc_GetParentByTagName("XFORMFLAG",fieldObj[0]);
					var type = xformflag?$(xformflag).attr('flagtype'):null;
					//特殊字段的处理
					if(fileName.indexOf(".name") == -1){ //地址本的name字段是只读的，在这里需排除
						//输入的dom为只读时，不处理;输入的dom为计算控件，不处理;
						if(fieldObj && fieldObj[0] && fieldObj[0].readOnly == true && ( fieldObj[0].getAttribute("calculation") != "true" || fieldObj[0].getAttribute("autocalculate") != "true")&&!(type!=null&&$.inArray(type,expControl)) ){
							break;
						}
					}
		...
		var noTrigger = $(tbObj.DOMElement).attr("performance") == "true" ? true : false;
		DocList_AddRow(optTBId,null,fieldValues, noTrigger);
	}else{
		for(var fileName in jsonArray){
			//这里或许应该弄个特殊字段的处理接口
			for(var j = 0 ;j < fieldNames.length; j++){
...
function DocListFunc_RefreshIndex(tbInfo,rowIndex, index){
	...
	DocListFunc_RefreshFormFieldIndex(tbInfo, rowIndex, "xformflag", "property");
    DocListFunc_RefreshFormFieldIndex(tbInfo, rowIndex, "DIV", "textid");
    DocListFunc_RefreshFormFieldIndex(tbInfo, rowIndex, "DIV", "myid");
	// id不更新的话，可能以后有问题
	...
	DocListFunc_RefreshFormFieldIndex(tbInfo, rowIndex, "DIV", "name");//这种结构的下标未刷新
}
...
function DocListFunc_RefreshFieldIndex(tbInfo, index, tagName){
	...
	for(var i=0; i<fields.length; i++){
		if(fields[i].name){
			...
			//添加条件 (j == -1 && $(fields[i]).closest("xformflag").length > 0)，为了替换name值有下划线的索引
			if((j>-1 && fields[i].name!=fieldName) || (j == -1 && $(fields[i]).closest("xformflag").length > 0)) {
				//添加条件 (j == -1 && $(fields[i]).closest("xformflag").length > 0)，为了替换name值有下划线的索引
				if(j == -1 && $(fields[i]).closest("xformflag").length > 0){
