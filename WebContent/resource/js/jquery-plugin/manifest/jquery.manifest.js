...
(function (factory) {
  ...
  $.widgetAddress('mf.manifest', {
    ...
    _marcoPoloOptions: function () {
      ...
      return {
        ...
        onSelect: function (mpData, $mpItem) {
          ...
          if(!options.multi && self._itemCount() >= 1)
            $input.blur();
        },
    ...
    _setOption: function (option, value) {
      ...
      $.WidgetAddress.prototype._setOption.apply(self, arguments);
    },
    ...
    _bindContainer: function () {
      ...
      var bindParentContainer = function() {
    	  ...
    	  if($parent.length > 0) {
    		  ...
    		  setTimeout(bindParentContainer,100);
    	  }
      };
    ...
    add: function (data, $mpItem, clearInputValue, initial,flag) {
      var self = this,
          $input = self.$input,
          options = self.options,
          ...
          buildItem = function(formatDisplay, formatRemove, formatValue) {
            ...
            $.when(self._trigger('add', [value, $item, !!initial,flag]))
             .then(function (add) {
               if (add !== false) {
      ...
      //单选时有值了就不能新增了
      if(!options.multi && self.values().length > 0) {
    	  return false;
      }
    ...
    remove: function (selector,flag) {
      ...
      var nowIndex = 1; //当前删除的元素下标，从1开始
      var maxIndex = $items.length; //需要删除的元素的总数
      $items.each(function () {
        var $item = $(this),
            data = $item.data('manifest');
        // 当删除的是最后一个元素同时传入的flag(控制是否触发onValueChange)为true时
        // 才触发onValueChange函数
        var triggered = flag && nowIndex===maxIndex;
        $.when(self._trigger('remove', [data, $item, triggered]))
    ...
    //获得完整的数据，values仅仅获得ID的数组
    datas: function() {
        var self = this,
            $list = self.$list,
            values = [];

        $list.find('li.mf_item').each(function () {
          values.push($(this).data('manifest'));
        });
    ...
    destroy: function () {
      ...
      $.WidgetAddress.prototype.destroy.apply(self, arguments);
    },
    ...
    _resizeInput: function () {
      ...
      var iWidth = Math.min(textWidth, self._maxInputWidth());
      $input.width(iWidth<10?10:iWidth);
      
      //
     
      var cHeight = $container.height();
      if(cHeight == 0 && $container.is(':visible')) {
    	  setTimeout(function() {
    		  self._resizeInput();
    	  },100);
      }else {
    	  var isText = $container.parents('div.inputselectsgl').length > 0;
    	
    	  if(isText) {
        	  var containerHeight = Math.max(24,cHeight);
              $container.parents('div.inputselectsgl').css({height:containerHeight+'px'});
          }else {
        	  $input.height(24).css({'overflow':'hidden','line-height':'24px'});
        	  //var containerHeight = Math.max(90,cHeight+10);
              //$container.parents('div.inputselectmul').css({height:containerHeight+'px'});
    ...
    cuxAddHighlight: function($item) {
    	this._addHighlight($item);
    },
