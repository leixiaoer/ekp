...
(function (factory) {
  ...
  $.widgetAddress('mp.marcoPolo', {
    ...
    _setOption: function (option, value) {
      ...
      $.WidgetAddress.prototype._setOption.apply(this, arguments);
    ...
    _showList: function () {
      ...
      if (this.$list.children().length) {
        //this.$list.css({left:this.$input.position().left,top:this.$input.position().top+this.$input.height()}).show();
    	this.$list.css({left:0,top:this.$input.position().top+this.$input.height()}).show();
    ...
    _buildResultsList: function (q, data) {
      ...
      for (var i = 0, length = data.length; i < length; i++) {
        datum = data[i];
        if(datum.name){
        	datum.name = datum.name.replace(/[<>&"]/g,function(c){
