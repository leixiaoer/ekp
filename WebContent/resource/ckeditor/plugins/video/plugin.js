...
CKEDITOR.plugins.add('video', {
  ...
  onLoad: function() {
    ...
    CKEDITOR.addCss('img.cke-video{background:#f8f8f8 url(' + url + ') center center no-repeat;outline:1px solid #ccc;outline-offset:-1px;min-width:192px;min-height:108px;max-width:100%;width:auto;height:auto;}');
  },
      ...
      dataFilter.addRules({
        elements: {
          'cke:video': function(element) {
            ...
            if(attributes.c != null && attributes.c != undefined){
            	element.attributes.controls = "controls";
            }
            element = editor.createFakeParserElement(element, 'cke-video', 'video', false);
            if(width && height){
            	element.attributes.style = "height:"+height+"px;width:"+width+"px";
            }
            element.attributes.src = "data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw==";
            return element;
          }
        }
      });
