/*
 * Copyright (c) 2003-2013, CKSource - Frederico Knabben. All rights
 * reserved. For licensing, see LICENSE.md or http://ckeditor.com/license
 */
;(function() {
  function z(a) {
    for (var a = a.toUpperCase(), c = A.length, b = 0, f = 0; f < c; ++f)
      for (
        var d = A[f], e = d[1].length;
        a.substr(0, e) == d[1];
        a = a.substr(e)
      )
        b += d[0]
  ...
  function B(a) {
    for (
      var a = a.toUpperCase(), c = C.length, b = 1, f = 1;
      0 < a.length;
      f *= c
    )
      (b += C.indexOf(a.charAt(a.length - 1)) * f),
        (a = a.substr(0, a.length - 1))
    return b
  }
  var D = CKEDITOR.htmlParser.fragment.prototype,
    o = CKEDITOR.htmlParser.element.prototype
  D.onlyChild = o.onlyChild = function() {
    var a = this.children
  ...
  D.firstChild = o.firstChild = function(a) {
  ...
  var H = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i,
    E = /^(?:\b0[^\s]*\s*){1,4}$/,
    y = {
      ol: {
        decimal: /\d+/,
    ...
    u = null,
    x,
    F = (CKEDITOR.plugins.pastefromword = {
      utils: {
                  ...
                  F.filters.stylesFilter([
                    ...
                    [
                      ...
                      function(a) {
                        ...
                        !t && (null !== u && a > u) && (t = a - u)
                        u = a
                        c['cke:indent'] = (t && Math.ceil(a / t) + 1) || 1
                      }
                    ],
                    [
                      /^mso-list$/,
                      ...
                      function(a) {
                        ...
                        1 == a && (b !== x && (c['cke:reset'] = 1), (x = b))
                        c['cke:indent'] = a
                      }
                    ]
                  ])(c.style, a) || ''),
              c['cke:indent'] || ((u = 0), (c['cke:indent'] = 1)),
            ...
            if (((e = d[h]), e.name in CKEDITOR.dtd.$listItem)) {
              ...
              F.filters.stylesFilter([
                [
                  ...
                  function(a) {
                    ;(a = a.split(' ')[1].match(H)) &&
                      (u = CKEDITOR.tools.convertToPx(a[0]))
                  }
                ],
                1 == c
                  ? [
          ...
          for (
            ...
            if (((b = c[n]), 'cke:li' == b.name))
              if (
                ...
                d != e && (m = g = null)
                if (i) {
                  if (m && y[m][g].test(i[1])) (l = m), (k = g)
                  else
                    for (var r in y)
                      for (var v in y[r])
                        if (y[r][v].test(i[1]))
                          if ('ol' == r && /alpha|roman/.test(v)) {
                            if (
                              ((g = /roman/.test(v) ? z(i[1]) : B(i[1])),
                              !p || g < p)
                            )
                              (p = g), (l = r), (k = v)
                          } else {
                ...
                if (j) {
                  if (d > e)
                    ...
                    if (d < e) {
                      e -= d
                      for (var s; e-- && (s = j.parent); ) j = s.parent
                    }
                    j.add(b)
                  }
                  c.splice(n--, 1)
      ...
      getRules: function(a, c) {
        var b = CKEDITOR.dtd,
          ...
          r = this.utils.resolveList,
          v = function(a) {
            a = CKEDITOR.tools.convertToPx(a)
            return isNaN(a) ? a : a + 'px'
          },
          s = this.utils.getStyleComponents,
          u = this.utils.listDtdParents,
          o = !1 !== d.pasteFromWordRemoveFontStyles,
          t = !1 !== d.pasteFromWordRemoveStyles
        return {
          ...
          elements: {
            ...
            $: function(a) {
              ...
              w in f &&
                e.style &&
                (e.style = j([[/^(:?width|height)$/, null, v]])(e.style) || '')
              if (w.match(/h\d/)) {
                a.filterChildren(c)
                if (r(a)) return
                g(d['format_' + w])(a)
              } else {
                if (-1 != w.indexOf(':') && -1 == w.indexOf('cke')) {
                  a.filterChildren(c)
                  if ('v:imagedata' != w && 'img' != w) {
                    delete a.name
                  }
                }
                if ('v:imagedata' == w || 'img' == w) {
                  ...
                  var _src = e.src
                  if (_src.indexOf('file:') < 0) return
                  var _src = _src.substring(8)

                  if (!d.webOffice) {
                    return
                  }

                  var fileName = _src.substring(_src.lastIndexOf('\\') + 1)
                  /***********************************************
                   * 金格控件上传图片--开始
                   **********************************************/
                  var fileUrl = ''
                  try {
                    var webOffice = d.webOffice
                    webOffice.WebSetMsgByName('rtf_file_name', fileName)
                    webOffice.WebSetMsgByName('upload_type', 'rtf')
                    if (webOffice.WebPutFile(_src, fileName)) {
                      fileUrl = webOffice.WebGetMsgByName('fileUrl')
                    }
                  } catch (e) {
            ...
            style: j(
              t
                ? [
                        ...
                        if (
                          ...
                          b =
                            'ltr' == d.contentsLangDirection
                              ? 'margin-left'
                              : 'margin-right'
                          if ('margin' == c) a = s(c, a, [b])[b]
                          else if (c != b) return null
                          if (a && !E.test(a)) return [b, a]
                        }
                        return null
                      }
                    ],
                    [/^clear$/],
                    [
                      /^border.*|margin.*|vertical-align|float|background.*$/,
                      null,
                      function(a, b) {
                        return a
                      }
