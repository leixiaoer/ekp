...
CKEDITOR.dialog.add("smiley", function(f) {
	for (var e = f.config, a = f.lang.smiley, h = e.smiley_images, g = e.smiley_columns
			|| 8, i, k = function(j) {
		...
		if ("a" == b)
			...
			return;
		var b = c.getAttribute("cke_src"), a = c.getAttribute("title"), c = f.document
				.createElement("img", {
							attributes : {
								src : b,
		...
		var a = new CKEDITOR.dom.event(a), c = new CKEDITOR.dom.element(c), b;
		b = a.getKeystroke();
		...
		switch (b) {
			...
			case d ? 39 : 37 :
				if (b = c.getParent().getPrevious())
					...
					b = b.getLast().getChild(0), b.focus(), a
							.preventDefault(!0)
		}
	}), d = CKEDITOR.tools.getNextId() + "_smiley_emtions_label", d = [
			'<div><span id="' + d + '" class="cke_voice_label">' + a.options
					+ "</span>",
			'<table role="listbox" aria-labelledby="'
					+ d
					+ '" style="width:100%;height:100%;border-collapse:separate;" cellspacing="2" cellpadding="2"',
			CKEDITOR.env.ie && CKEDITOR.env.quirks ? '' : "", "><tbody>"], l = h.length, a = 0; a < l; a++) {
		...
		d
				.push(
						...
						CKEDITOR.env.ie
								...
								+ e.smiley_descriptions[a] + "</span></a>",
						"</td>");
		a % g == g - 1 && d.push("</tr>")
	}
	if (a < g - 1) {
	...
	var style = ['width:',
			(f.config.smiley_width) ? (f.config.smiley_width) + "px" : "100%",
			';height:', (f.config.smiley_height || 120) + "px;",
			"overflow : scroll;"].join('');
	return {
