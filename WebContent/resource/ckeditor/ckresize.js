...
CKResize.____resetWidth____ = function(name, imgReader) {
	...
	function magnifyBimg(___bdiv, ___bimg) {
		...
		if(c != 'none' && c) {
			...
			if(needReset) {
				___bdiv.css({
					'overflow' : 'auto',
					'line-height':___bimg[0]['naturalWidth']+'px'
				});	
			}else {
				___bdiv.css({
					'overflow' : 'auto'
	...
	function shrinkBimg(___bdiv, ___bimg) {
		...
		if(IEVersion() != '-1') {
			___bimg.css('max-height',(orgMaxHeight!= null)? orgMaxHeight : CKResize.dialog_height)
				...
				CKResize.top.seajs.use('lui/dialog',
					function(__dialog) {
						...
						$dialog.on('show',function() {
									...
									initImgSizeZoom(CKResize.___bimg[0]);
									if (top.location.href && top.location.href.indexOf('km_agreement_card') > -1) {
										//多层iframe 嵌套图片预览背景设置
										var bgColor = '';
										if (IEVersion() == -1) {
										...
										this.element.find(".ckeditor_kanban").css('background-color',bgColor);
									}
