...
<head>
    <script>
    	...
    	function search(){
        	...
        	if(value==""){
            	alert("${lfn:message('sys-ftsearch-db:search.search.warning')}");
            	return;
        	}
...
<body>
	...
	<div class="search">
		<input type="text" class="txt_input" name="keyword" onkeydown="if(event.keyCode==13) search();" /><input type="button" class="btn_search" value="${lfn:message('sys-ftsearch-db:ftsearch.advanced.button.search')}" onclick="search();" />
	 </div>
</body>
</html>
