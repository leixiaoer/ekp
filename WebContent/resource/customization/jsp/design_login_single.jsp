...
<div class="option" data-hover="isLogoActive" @mouseenter="enter" @mouseleave="leave">
	...
	<c:forEach items="${langs }" var="lang">
		<div class="content">
			<div class="input">
				<input placeholder="${lang.value }" title="${lang.value }" type="text" v-model="single_logo_text_${lang.key }"/>
			</div>
		</div>
	</c:forEach>
	<div>
...
<!-- 产品链接和图标 -->
<div class="option" data-hover="isProductActive" @mouseenter="enter" @mouseleave="leave">
    <div class="title"><bean:message bundle="sys-profile" key="sys.profile.design.product.linkAndLogo"/></div>
    <div class="content">
        <div class="limit">62*62 png/jpg/jpeg</div>
        <form id="single_im_icon" enctype="multipart/form-data" method="post">
			<span class="btn-file">
		        <span class="f12"><bean:message bundle="sys-profile" key="sys.profile.design.selectImg"/></span>
		        <input type="hidden" name="configKey"/>
		        <input type="hidden" name="fileName"/>
		        <input type="file" name="file" accept=".png,.jpg,.jpeg" onclick="this.value='';" onchange="uploadImg('single_im_icon');"/>
		    </span>
        </form>
    </div>
    <c:forEach items="${langs }" var="lang">
        <div class="content">
            <div class="input">
                <input placeholder="${lang.value }" title="${lang.value }" type="text" v-model="single_im_${lang.key }"/>
            </div>
        </div>
    </c:forEach>
    <hr/>
    ...
    <div class="content">
        <div class="limit">62*62 png/jpg/jpeg</div>
        <form id="single_kms_icon" enctype="multipart/form-data" method="post">
			<span class="btn-file">
		        <span class="f12"><bean:message bundle="sys-profile" key="sys.profile.design.selectImg"/></span>
		        <input type="hidden" name="configKey"/>
		        <input type="hidden" name="fileName"/>
		        <input type="file" name="file" accept=".png,.jpg,.jpeg" onclick="this.value='';" onchange="uploadImg('single_kms_icon');"/>
		    </span>
        </form>
    </div>
    <c:forEach items="${langs }" var="lang">
        <div class="content">
            <div class="input">
                <input placeholder="${lang.value }" title="${lang.value }" type="text" v-model="single_kms_${lang.key }"/>
            </div>
        </div>
    </c:forEach>
    <hr/>
    ...
    <div class="content">
        <div class="limit">62*62 png/jpg/jpeg</div>
        <form id="single_ekp_icon" enctype="multipart/form-data" method="post">
			<span class="btn-file">
		        <span class="f12"><bean:message bundle="sys-profile" key="sys.profile.design.selectImg"/></span>
		        <input type="hidden" name="configKey"/>
		        <input type="hidden" name="fileName"/>
		        <input type="file" name="file" accept=".png,.jpg,.jpeg" onclick="this.value='';" onchange="uploadImg('single_ekp_icon');"/>
		    </span>
        </form>
    </div>
    <c:forEach items="${langs }" var="lang">
        <div class="content">
            <div class="input">
                <input placeholder="${lang.value }" title="${lang.value }" type="text" v-model="single_ekp_${lang.key }"/>
            </div>
        </div>
    </c:forEach>
</div>
<script>
	...
	function uploadImg(id) {
		seajs.use(['lui/jquery','lui/dialog'],function($,dialog) {
			...
			if(id=='single_im_icon' || id=='single_kms_icon' || id=='single_ekp_icon'){
                extFileNames = 'png,jpg,jpeg';
            }
