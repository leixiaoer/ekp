...
<c:import url="/sys/recycle/import/redirect.jsp">
	...
	function fn_dialog(url){
		...
		Dialog_PopupWindow(url,width,height);
	}
	//max_window();
					...
					<tr>
						<td colspan="4">
							<c:choose>
								...
								<c:if test="${editStatus == true}">
								    <c:set var="canEditStatus" value="true"/>
								</c:if>
								<c:if test="${editStatus != true}">
								    <c:set var="canEditStatus" value="false"/>
								</c:if>
								  <c:import url="/sys/attachment/sys_att_main/sysAttMain_edit.jsp" charEncoding="UTF-8">
										...
										<c:param  name="canEdit"  value="${canEditStatus}"/>
	...
	if("${fdNoId}" !=""){
	    ...
	    dialog.iframe(path,"文件编号",function(rtn){
		  if(rtn!="undefined"&&rtn!=null){
   		   ...
   		   if(Attachment_ObjectInfo['editOnline'] && Attachment_ObjectInfo['editOnline'].ocxObj && typeof(Attachment_ObjectInfo['editOnline'].setBookmark) == 'function'){
