		...
		<center>
			<html:form action="/km/carmng/km_carmng_user_fee_info/kmCarmngUserFeeInfo.do">
				<ui:tabpanel id="kmCarmngUserFeeInfoCountPanel" layout="sys.ui.tabpanel.list">
					<ui:content id="kmCarmngUserFeeInfoCountContent" title="${ lfn:message('km-carmng:kmCarmng.excel.exportQueryDriverExcel') }">
			...
			<div style="height: 300px;">
				...
				<list:listview id="listview">
					<ui:source type="AjaxJson" cfg-commitType="POST">
           				{url:''}
       				</ui:source>
					<!-- 列表视图 -->
		...
		<script type="text/javascript">
			...
			seajs.use([ 'lui/jquery', 'lui/dialog', 'lui/topic' ], function($, dialog, topic) {
				...
				window.loadList = function() {
					window.chgTimeSelect();
					var fdDriverId = $("input[name='fdDriverId']").val();
					...
					if (null == fdDriverId || fdDriverId == '') {
						dialog.alert("请选择驾驶员！");
						return;
					}
					// 动态传参调用统计（并使用POST传参请求）
					var url = '/km/carmng/km_carmng_user_fee_info/kmCarmngUserFeeInfo.do?method=queryDriverCount';
					var source = LUI("listview").source;
					source.params = {
							'timeType': timeType,
							'fdDriverId': fdDriverId,
				...
				window.exportExcel = function() {
					...
					if (fdStartDate && fdEndDate) {
						// 创建下载表单
						var exportForm = $('<form action="${KMSS_Parameter_ContextPath}km/carmng/km_carmng_user_fee_info/kmCarmngUserFeeInfo.do?method=exportQueryDriverExcel" method="post"/>');
						exportForm.appendTo("body");
						exportForm.append($("<input type='hidden' name='fdDriverId' />").val(fdDriverId));
