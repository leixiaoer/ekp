...
<template:include ref="default.simple">
	<template:replace name="title">${lfn:message('km-carmng:portlet.selectKmCarmngMotorcade')}</template:replace>
	...
	seajs.use(['lui/dialog'], function(dialog) {
		window.selectKmCarmngMotorcade = function(){
			var data = {};
			...
			if(selectNodes!=null){
				...
				dialog.alert("${lfn:message('km-carmng:portlet.select')}");
			}
		}
	});
	</script>
	...
	function generateTree(){
		LKSTree = new TreeView(
			"LKSTree",
			"${lfn:message('km-carmng:portlet.select')}",
			document.getElementById("treeDiv")
		);
		LKSTree.isShowCheckBox = true;
		LKSTree.isMultSel = false;
		LKSTree.treeRoot.AppendBeanData('kmCarmngMotorcadeSetTreeService&parentId=!{value}&portlet=true', null, null, null, null);
		LKSTree.Show();
	}
	</script>
	<table class="tb_normal" style="margin:20px auto;width:95%;height:460px;">
		...
		<tr><td align="center">
			<ui:button text="${lfn:message('button.ok')}" onclick="selectKmCarmngMotorcade();" /> 
			&nbsp;&nbsp;&nbsp;&nbsp;
