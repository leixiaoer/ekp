define([
	...
	var item = declare("km.carmng.list.item.ProcessItemMixin", [ItemBase, OpenProxyMixin], {
		...
		buildInternalRender : function() {
			...
			var titleNode = domConstruct.create("div",{className:"muiCarListContentTopTitleBox"},TopNode);
			domConstruct.create("span",{className:"muiCarListContentTopTitle",innerHTML:this.fdAppNames},titleNode);
			domConstruct.create("span",{className:"muiFontSizeS muiCarListContentStatus muiCarListContentStatusType" + this.fdFlag,innerHTML:this.status},titleNode);
			
			var bottomNode = domConstruct.create("div",{className:"muiCarListContentBottom muiFontSizeS muiFontColorMuted"},this.contentNode);
			domConstruct.create("span",{innerHTML:Msg["kmCarmngDispatchersInfo.fdCarInfoName"]+"："+this.fdCarInfoNames},bottomNode);
			domConstruct.create("span",{innerHTML:Msg["kmCarmngDispatchersInfo.docCreateTime"]+"："+this.fdStartTime},bottomNode);
		},
		startup:function(){
