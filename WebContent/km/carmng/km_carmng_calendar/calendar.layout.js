seajs.use(['lang!km-carmng'],function(lang){
	...
	function createHeader(){
		...
		$('<div class="lui_calendar_header_today" cal-opt="today">'+lang['kmCarmng.today']+'</div>').appendTo(leftTd).click(function(){
			calendar.today();
		});
		...
		//日试图
		$('<div class="lui_calendar_header_month" cal-opt="day">'+lang['kmCarmng.day']+'</div>').appendTo(rightTd).click(function(){
			calendar.changeMode('day');
		});
		//周视图
		$('<div class="lui_calendar_header_month" cal-opt="week">'+lang['kmCarmng.week']+'</div>').appendTo(rightTd).click(function(){
			calendar.changeMode('week');
		});
		
		$('<span class="meeting_calendar_label_unhold">&nbsp;</span><span class="meeting_calendar_label_text" >'+lang['kmCarmng.status.publish.unHold']+'</span>').appendTo(rightTd);
		
		$('<span class="meeting_calendar_label_holding">&nbsp;</span><span class="meeting_calendar_label_text">'+lang['kmCarmng.status.publish.holding']+'</span>').appendTo(rightTd);
		
		$('<span class="meeting_calendar_label_hold">&nbsp;</span><span class="meeting_calendar_label_text">'+lang['kmCarmng.status.publish.hold']+'</span>').appendTo(rightTd);
	...
	function displayChange(info){
		...
		if(view!=null){
			...
			if(view.start.getTime() <= today && today < view.end.getTime() || (view.start.getTime() == today && today == view.end.getTime())){
