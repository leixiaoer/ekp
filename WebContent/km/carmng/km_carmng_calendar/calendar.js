define(function(require, exports, module) {
	...
	var lang=require('lang!km-carmng');
	...
	var MeetingCalendarMode=Calendar.AbstractCalenderMode.extend({
		...
		initialize:function($super,_config){
			...
			this.selectedCategories = 'all'; //选中的会议室分类,字符串以;分割ID。特殊字符all代表全部选中
		...
		_getLoadContanier : function(){
			...
			this.loadMoreDiv = $('<div class="meeting_calendar_loadMore">'+'<span>'+lang['kmMeeting.calendar.loadMore']+'</span>'+ '<img src="'+Com_Parameter.ContextPath+'km/carmng/resource/images/showMore.png"/>' + '</div>').appendTo(container);
			this.nodataDiv = $('<div class="meeting_calendar_nodata">'+lang['kmCarmng.calendar.nodata']+'</div>').appendTo(container);
		...
		_getTH:function(){
			...
			resTD.append($('<div class="meeting_calendar_category_title"/>').append(lang['kmCarmngInfomation.fdMotorcade']));
			
			//分类选择框
			var categoryDialog=$('<div class="category_dialog" />')
			categoryDialog.width(this.calendarDiv.width()-20);
			var categoryCkAll=$('<div class="category_ckAll"><input type="checkbox" checked="check" class="meeting_dialog_ck_all" />'+lang['kmCarmng.calendar.selectAll']+'</div>')
