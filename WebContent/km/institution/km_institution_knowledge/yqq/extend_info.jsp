...
function dialogSelect(mul, idField, nameField,targetWin){
	seajs.use(['lui/jquery','lui/dialog','lui/dialog_common','lui/util/str'], function($, dialog, dialogCommon,strutil){
		var url = "/elec/yqqs/elec_yqqs_acc_info/elecYqqsAccInfo.do?method=accoutList&moduleSource=km-institution&fdUserName=${fdSigner.fdName}";
	...
	function sendYqq(){
		seajs.use(['lui/jquery','lui/dialog'], function($, dialog){
			...
			if(phone==''||phone==null){
				...
				var ajaxUrl = Com_Parameter.ContextPath+'km/institution/km_institution_knowledge/kmInstitutionKnowledge.do?method=sendYqq&signId=${param.signId}&phone='+phone+'&fdEnterprise='+fdEnterprise;
				$.ajax({
					url : ajaxUrl,
					type : 'post',
					data : {},
					...
					success : function(data) {
						...
						if(data){
							...
							if("true"==yqqSendCode){
								var url=Com_Parameter.ContextPath+'km/institution/km_institution_knowledge/yqq/yqq_loading.jsp?signId='+signId;
...
<div align="left">
	&nbsp;&nbsp;
		<c:import url="/sys/attachment/sys_att_main/sysAttMain_view.jsp" charEncoding="UTF-8">
			...
			<c:param name="fdModelId" value="${kmInst.fdId}" />
		</c:import>	
</div>
