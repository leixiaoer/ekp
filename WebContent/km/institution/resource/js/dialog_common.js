define(function(require, exports, module) {
		...
		var dialogObj = dialog.build({
			config:{
				...
				content : {
					...
					scroll : false,
					type : "iframe",
					url : '/km/institution/resource/js/common-template1.jsp?_key=template_select',
					params:null
				}
			}
		});
	...
	var dialogSelect = function(modelName, mulSelect, source, params, idField, nameField, winTitle, action){
		...
		var tempUrl = '/km/institution/resource/js/common-template.jsp?modelName=' + modelName + '&_key=dialog_' + idField;
		if(mulSelect==true){
		...
		window.top['__dialog_' + idField + '_dataSource'] = function(){
			if(idField==null){
				return strutil.variableResolver(source ,params);
			}else{
