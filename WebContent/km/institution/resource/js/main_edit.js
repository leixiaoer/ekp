Com_AddEventListener(window,'load',function(){
	seajs.use(['lui/jquery','lui/dialog','lui/dialog_common','lui/util/str',initData.contextPath+'/km/institution/resource/js/dialog_common'], function($, dialog, dialogCommon,strutil, dialogCommon1){
		...
		function createNewFormBlankCate(){
			...
			var fdType = Com_GetUrlParameter(url,'fdType');
			var param = null;
			if(fdType){
				param ={fdType:fdType};
			}
			...
			if((cateId==null || cateId=='') && method=='add' && formOption.mode.substring(0,5)=='main_'){
				...
				}else if(formOption.mode=='main_template'){dialogSelect
					dialog.categoryForNewFile(formOption.templateName, url,
				                false,null,function(rtn) {
						if (!rtn){
							window.opener = null;
							window.open("", "_self");
							window.close();
						}
					},null,'_self',true,null,null,param);
		...
		window.dialogSelect=function(mul, key, idField, nameField,targetWin,func){
		...
		function dialogSelectForDetail(mul, key, idField, nameField){
			...
			var f1 = idField.replace(".*","[*]"); 
			var f1Txt = f1.replace("*",tr.rowIndex-tbInfo.firstIndex); 
			var f2 = nameField.replace(".*","[*]"); 
			var f2Txt = f2.replace("*",tr.rowIndex-tbInfo.firstIndex); 
    		...
    		if(dialogCfg){
    			...
    			if(inputs){
    				for(var i=0;i<inputs.length;i++){
    					...
    					if(argu["value"].indexOf('*')>-1){
    						//入参来自明细表
    						var modelVal=$form(argu["value"],f1Txt).val();
        					if(modelVal==null||modelVal==''){
        						if(argu["required"]=="true"){
        							var errorInfo="当前对话框缺失必须传递的参数【"+argu["label"]+"】，请检查表单数据或相关配置";
	    							alert(errorInfo);
    			...
    			params=encodeURI(params);
    			window.top['__dialog_' + refIdField + '_dataSource'] = function(){
                    return strutil.variableResolver(dialogCfg.sourceUrl+params ,null);
                }
    			dialogCommon.dialogSelect(dialogCfg.modelName,
    					mul,dialogCfg.sourceUrl+params, null, refIdField, refNameField,null,function(data){
    				if (key == 'km_relative_main_relativelist') {
        				$form(f1,f1Txt).val(data[0]['fdId']);
        				$form(f2,f1Txt).val(data[0]['fdRelativeName']);
    				} else {
    					$form(f1,f1Txt).val(data[0]['fdId']);
    					$form(f2,f1Txt).val(data[0]['fdContactName']);
    				}
    				var outputs=getDialogOutputs(idField);
    				if(outputs){
						if(data.length==1){
							for(var i=0;i<outputs.length;i++){
								var output=outputs[i];
								$form(output["value"],f1Txt).val(data[0][output["key"]]);
								var tmpObj = output["value"].replace("[*]",".*");
								tmpObj = tmpObj.replace("*",tr.rowIndex-tbInfo.firstIndex); 
								$form(tmpObj).val(data[0][output["key"]]);
        					}
						}
    				}
    			});
		...
		window.getDialogInputs=function(idField){
		...
		window.getDialogOutputs=function(idField){
			var dialogLinks=formOption.dialogLinks;
			if(dialogLinks==null||dialogLinks.length==0){
				return null;
			}
		...
		window.submitForm = function(status, method, isDraft){
			...
			//修复合同申请驳回后编辑页暂存流程直接提交
			if(status==='11'&& 'kmAgreementApplyForm'==formOption.formName){
		...
		window.validateScore=function(){
			var flag=true;
			$("#TABLE_DocList_fdEval_Form input[name*='fdLevel']").each(function(){
				if(!$(this).val()){
					dialog.alert("评价平均分不能为空");
					flag=false;
				}
			});
			return flag;
		...
		window.validateDetail=function(){
			//金额长度校验
			var checkFlag = true;
			if (typeof thouFields != 'undefined' && thouFields != null && thouFields instanceof Array) {
				for(var i=0;i<thouFields.length;i++){
					if (checkFlag) {
						var prop=thouFields[i];
						var source=$("#"+prop).val();
						var len = 0;
						if(source){
							len = source.length;
							if (len > 21) {
								dialog.alert("金额超出21个英文字符限制");
								checkFlag = false;
							}
						}
					}
			...
			if(details!=null){
				for(var i=0;i<details.length;i++){
					...
					var num=2;
					if(detail=="TABLE_DocList_fdStage_Form"){
						num=1;
					}
					if(trSize<=num){
						dialog.alert(Data_GetResourceString("km-agreement:kmAgreement.list")+"【"+message+"】"+Data_GetResourceString("km-agreement:kmAgreement.notNull"));
						return false;
					}else{
						var flag=true;
