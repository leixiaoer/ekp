...
<script  type="text/javascript">
		seajs.use([ 'lui/jquery','lui/parser','lui/dialog'],function($,parser,dialog) {
			...
			window.clickOK=function(){
				...
				if(field.value==""){
					dialog.alert("<kmss:message key="errors.required" argKey0="km-institution:kmInstitutionKnowledge.setTop.topDays" />");
					return;
				}
				var rtn = field.value;
			    var g = /^[1-9]*[1-9][0-9]*$/;
				if(!g.test(rtn)){
					dialog.alert("<kmss:message key="errors.integer" argKey0="km-institution:kmInstitutionKnowledge.setTop.topDays" />");
					return;
				}
				$dialog.hide(rtn);
			};
...
<bean:message bundle="km-institution" key="kmInstitutionKnowledge.setTop.topDays" />
