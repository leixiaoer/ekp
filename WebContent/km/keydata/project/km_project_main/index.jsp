...
<template:include file="/sys/profile/resource/template/list.jsp">
	<template:replace name="content">
		...
		<list:criteria id="criteria">
		     <list:cri-ref key="fdName" ref="criterion.sys.docSubject" title="${lfn:message('km-keydata-project:kmProjectMain.fdName') }">
			</list:cri-ref>
		</list:criteria>
		<!-- 操作栏 -->
			...
			<div class="lui_list_operation_sort_btn">
				...
				<div class="lui_list_operation_sort_toolbar">
					<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" >
						<list:sort property="docCreateTime" text="${lfn:message('km-keydata-project:kmProjectMain.docCreateTime') }" group="sort.list" value="down"></list:sort>
					</ui:toolbar>
				</div>
			</div>
			<!-- 分页 -->
					...
					<ui:toolbar id="Btntoolbar">
						...
						<c:import url="/sys/right/doc_right_change_button.jsp"
						 ...
						 <c:import url="/sys/right/import/cchange_doc_right_button.jsp" charEncoding="UTF-8">
							<c:param name="modelName" value="com.landray.kmss.km.keydata.project.model.KmProjectMain" />
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/km/keydata/project/km_project_main/kmProjectMain.do?method=listChildren&fdIsAvailable=${param.fdIsAvailable}&categoryId=${param.categoryId}'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" 
			     rowHref="/km/keydata/project/km_project_main/kmProjectMain.do?method=showUsed&fdId=!{fdId}">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				<list:col-auto props="fdName,fdCode,fdExecutiveDept.fdName,fdStatus,docCreator.fdName,operations"></list:col-auto>
			</list:colTable>
		</list:listview>
		<br>
		<!-- 分页 -->
	 	...
	 	<script type="text/javascript">
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
		 		...
		 		window.addByDialog = function(){
			 		...
			 		var modelName = 'com.landray.kmss.km.keydata.project.model.KmProjectCategory';
			 		var urlParam = '/km/keydata/project/km_project_main/kmProjectMain.do?method=add&fdTemplateId=!{id}&fdTemplateName=!{name}';
		 		...
		 		window.add = function(){
		 			Com_OpenWindow('<c:url value="/km/keydata/project/km_project_main/kmProjectMain.do" />?method=add&fdTemplateId=${param.categoryId}');
			 	}

		 		window.edit = function(id){
					if(id){
						Com_OpenWindow('<c:url value="/km/keydata/project/km_project_main/kmProjectMain.do?method=edit&fdId=" />'+id,'_blank');
					}
				};
		 		...
		 		window.disableAll = function(id){
					...
					if(id){
						values.push(id);
						$.post('<c:url value="/km/keydata/project/km_project_main/kmProjectMain.do?method=disableAll"/>',
				...
				window.enableAll = function(id){
					...
					if(id){
						values.push(id);
						$.post('<c:url value="/km/keydata/project/km_project_main/kmProjectMain.do?method=enableAll"/>',
								$.param({"List_Selected":values},true),delCallback,'json');
					}else{
						$("input[name='List_Selected']:checked").each(function(){
							values.push($(this).val());
