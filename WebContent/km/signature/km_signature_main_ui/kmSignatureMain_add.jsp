...
<template:include ref="default.edit" sidebar="no">
	...
	<template:replace name="title">
		<c:out value="${ lfn:message('km-signature:signature.createSig') } - ${ lfn:message('km-signature:module.km.signature') }"></c:out>	
	</template:replace>
	<%--操作按钮--%>
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float">
			<%-- 保存 --%>
			<ui:button text="${lfn:message('button.save') }" order="2" onclick="Com_Submit(document.kmSignatureMainForm, 'save');">
			</ui:button>
			<%-- 保存并新建
			<ui:button text="${lfn:message('button.saveadd') }" order="2" onclick="Com_Submit(document.kmSignatureMainForm, 'saveadd');">
			</ui:button>
			 --%>
			<%--关闭--%>
	...
	<template:replace name="path">
		<ui:combin ref="menu.path.simplecategory">
				<ui:varParams 
					...
					autoFetch="false"
					categoryId="${KmSignatureMainForm.fdTempId}" />
		</ui:combin>
	</template:replace>
	<%--左侧主内容--%>
	<template:replace name="content">
		...
		$(function(){
			...
			onChangeDocType("${JsParam.docType}",true);
		});
		function Check_add() {
		...
		function onChangeFreeSign(opt){
			var $opt= $(opt);
			passwordShowOrHide($opt.selector);
		}
		...
		function passwordShowOrHide(selector){
			...
			var $fdPassword = $("#fdPassword");
			var $fdPassword1 = $("#fdPassword1");
		...
		function onChangeDocType(opt,delayed){
			var $fdIsFreeSignTr = $("#fdIsFreeSignTr"); 
			var $fdIsDefaultSpan = $("#fdIsDefaultSpan"); 
			if (opt && (opt=='2')){ //如果是单位签章则要隐藏设置个人默认、隐藏免密签名
				$fdIsFreeSignTr.hide();
				$fdIsDefaultSpan.hide();
				var $fdIsDefault = $("input[type='checkbox'][name$='fdIsDefault']");
				$fdIsDefault.removeAttr("checked");
				
				$("input:radio[name='fdIsFreeSign']")[0].checked = true;
				passwordShowOrHide("2");		
			}else{
				$fdIsFreeSignTr.show();
				$fdIsDefaultSpan.show();
			}
			//var fdUsersIds = document.getElementsByName("fdUsersIds");
			//var fdUsersNames = document.getElementsByName("fdUsersNames");
			//fdUsersIds[0].value = "";
			//fdUsersNames[0].value = "";
			/* $("input[name='fdUsersIds']").val("");
			$("input[name='fdUsersNames']").val(""); */
			//$("li[class='mf_item']").remove();
			if(!(delayed===true))
				initDialogAddress();
		}
		...
		function initDialogAddress(){			
			...
			if (fdDocType==1){ //个人签章
				...
				orgType = ORG_TYPE_ORG|ORG_TYPE_DEPT|ORG_TYPE_POST|ORG_TYPE_PERSON;
			}
			$("input[name='fdUsersIds']").parent().find("div[class=orgelement]").attr("onclick",address);
			var address = Address_GetAddressObj("fdUsersNames");
			address.reset(';',orgType,mulSelect,[]);
		}
			...
			<table class="tb_normal" width=95%>
				...
				<tr>
					<td class="td_normal_title" width="15%">
						...
						<c:if test="${docTypeFlag == '3'}">
							...
							<xform:radio property="fdDocType" onValueChange="onChangeDocType" subject="${ lfn:message('km-signature:signature.marktype') }" value="${HtmlParam.docType }" required="true" >
							...
							<c:if test="${param.docType != 1 && param.docType != 2 }">
							<xform:radio property="fdDocType" onValueChange="onChangeDocType" subject="${ lfn:message('km-signature:signature.marktype') }" value="1" required="true" >
								<xform:simpleDataSource value="1">
									<bean:message bundle="km-signature" key="signature.fdDocType.handWrite"/>
								</xform:simpleDataSource>
								<xform:simpleDataSource value="2">
				...
				<tr id="fdUsersArea">
					<td class="td_normal_title" width="15%">
						...
						<%
							...
							int _docTypeFlag = (Integer)request.getAttribute("docTypeFlag");
							if(_docTypeFlag == 3) {
								String _docType = request.getParameter("docType");
								if("1".equals(_docType) || "2".equals(_docType)) {
									fdDocType = Integer.parseInt(_docType);
								}
							}else {
								fdDocType = _docTypeFlag;
							}
							if(fdDocType == 1) {
								isMulti = false;
								orgType = "ORG_TYPE_PERSON";
						...
						%>
						<xform:address propertyId="fdUsersIds" propertyName="fdUsersNames" orgType="<%=orgType %>"
						 style="width:98%" nameValue="<%=fdUserName%>" idValue="<%=fdId%>" mulSelect="<%=isMulti %>"></xform:address>
						<span class="txtstrong"><bean:message bundle="km-signature" key="signature.warn14"/></span>
					</td>
				</tr>
				<!-- 是否免密签名 -->
				<c:if test="${(docTypeFlag == '1') || (docTypeFlag == '3')}">
				<tr id="fdIsFreeSignTr">
					<td class="td_normal_title" width="15%">
						<bean:message bundle="km-signature" key="signature.fdIsFreeSign"/>
					</td>
					<td width="85%" colspan="3">
						<xform:radio property="fdIsFreeSign" onValueChange="onChangeFreeSign" subject="${ lfn:message('km-signature:signature.fdIsFreeSign') }" value="0" required="true" >
				...
				<tr id="passwordArea">
					<td class="td_normal_title" width="15%">
						...
						<input id="fdPassword" type="password" name="fdPassword" size="50" maxlength="32" required="required" class="inputsgl">
				...
				<tr id="confirmPasswordArea">
					<td class="td_normal_title" width="15%">
						...
						<input id="fdPassword1" type="password" name="fdPassword1" size="50" maxlength="32"  required="required" class="inputsgl" 
