...
<%!
  ...
  String DocNo,UserName,Security,Draft,Check;
...
if(fdId!=null){
	...
	//集合排序
	NodeDefinition p = null;
	NodeDefinition p1 = null;
	NodeDefinition p2 = null;
	for(int i=0;i<tempSize&&tempSize>0;i++){      
       for(int j=0;j<tempSize;j++){  
           if(j == 0){  
               continue;  
           }  
           p1 = (NodeDefinition)(tempNodes.get(j-1));  
           p2 = (NodeDefinition)(tempNodes.get(j));  
           //这是升序  
           if(p1.getY()-p2.getY() >= 0){  
               ...
               if(tempNodes.get(j) != null){  
            	   tempNodes.remove(j);  
            	   tempNodes.add(j, p);  
               }  
               if(tempNodes.get(j-1) != null){  
            	   tempNodes.remove(j-1);  
            	   tempNodes.add(j-1,p1);  
               }  
          }  
    	}
	}
	...
	if(tempSize==5){
		...
		object3=((NodeDefinition)tempNodes.get(2)).getId();
		object4=((NodeDefinition)tempNodes.get(3)).getId();
		object5=((NodeDefinition)tempNodes.get(4)).getId();
		request.setAttribute("object1", object1);
		request.setAttribute("object2", object2);
		request.setAttribute("object3", object3);
		request.setAttribute("object4", object4);
		request.setAttribute("object5", object5);
	} 
}
...
if (ObjConnBean.OpenConnection() && fdId!=null) {
	...
	while (rs1.next()) {
	  //fileValues += rs1.getString("FieldValue")+"#";
	  fileValues += rs1.getString("fdFieldName")+"$"+rs1.getString("fdFieldValue")+"#";
	}
	rs1.close();
...
if(!"".equals(fileValues)){
	...
	for(int j=0;j<len;j++){
		...
		if(id.equals(object1)){fileValue1=val;}
		if(id.equals(object2)){fileValue2=val;}
		if(id.equals(object3)){fileValue3=val;}
		if(id.equals(object4)){fileValue4=val;}
		if(id.equals(object5)){fileValue5=val;}
	}
}
%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" height="100%">	
	 <tr>
		...
		<tr>
	   		<td height="100" colspan="2" style="border-bottom:1px dashed;border-color:#999999; border-top:1px dashed;border-color:#999999">
            <OBJECT name="<%=object5%>" classid="clsid:2294689C-9EDF-40BC-86AE-0438112CA439" codebase="iWebRevision.cab#version=6,5,0,275" width=100% height=100%>
              <param name="WebUrl" value="${KMSS_Parameter_ContextPath}km/signature/km_signature_document_main/iWebServer.jsp">    <!-- WebUrl:系统服务器路径，与服务器交互操作，如打开签章信息 -->
              <param name="RecordID" value="${kmReviewMainForm.fdId}">    <!-- RecordID:本文档记录编号 -->
              <param name="FieldName" value="<%=object5%>">         <!-- FieldName:签章窗体可以根据实际情况再增加，只需要修改控件属性 FieldName 的值就可以 -->
              <param name="UserName" value="<%=UserUtil.getUser().getFdName()%>">    <!-- UserName:签名用户名称 -->
              <c:choose>
          		<c:when test="${object5 eq nodeId && not empty nodeId}">
          			<param name="Enabled" value="1">  <!-- Enabled:是否允许修改，0:不允许 1:允许  默认值:1  -->
          		</c:when>
          		<c:otherwise>
          			<param name="Enabled" value="0">
              ...
              <param name="PenWidth" value="2">     <!-- PenWidth:笔的宽度，值:1 2 3 4 5   默认值:2  -->
              <param name="FontSize" value="11">    <!-- FontSize:文字大小，默认值:11 -->
			  <param name="value" value="<%=fileValue5%>">
              <param name="SignatureType" value="0">    <!-- SignatureType:签章来源类型，0表示从服务器数据库中读取签章，1表示从硬件密钥盘中读取签章，2表示从本地读取签章，并与ImageName(本地签章路径)属性相结合使用  默认值:0 -->
	           <param name="InputList" value="同意\r\n不同意\r\n请上级批示\r\n请速办理">
	          </OBJECT>
              </td>
...
<OBJECT name="<%=object5%>" classid="clsid:2294689C-9EDF-40BC-86AE-0438112CA439" codebase="iWebRevision.cab#version=6,5,0,275" width=100% height=100%>
            <param name="Enabled" value="0">  <!-- Enabled:是否允许修改，0:不允许 1:允许  默认值:1  -->           
            <param name="BorderStyle" value="0">    <!-- BorderStyle:边框，0:无边框 1:有边框  默认值:1  -->
            <param name="FieldName" value="<%=object5%>"> 
            <param name="value" value="<%=fileValue5%>">
</OBJECT>
</div>
</c:if>
