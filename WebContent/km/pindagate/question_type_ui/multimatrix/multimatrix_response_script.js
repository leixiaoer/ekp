Response_RegisterJsFile(Com_Parameter.ContextPath+"km/pindagate/question_type/multimatrix/multimatrix_response_script.js");
...
function multiMatrix_getContent_row(index){
	...
	for(var itemskey in fdQuestionDef[index].items){
		$('#multimatrixquestionTable'+index).append('<tr id="multimatrixquestionTr'+index+itemskey+'"></tr>');
		for(var optionskey in fdQuestionDef[index].options){
			$('#multimatrixquestionTr'+index+itemskey).append('<td align="center"><input type="checkbox" id="option'+index+
    ...
    for(var key in fdQuestionDef[index].options){
		...
		$("#multimatrixfirstTr"+index).append("<td>"+fdQuestionDef[index].options[key][0]+imgHTML+"</td>");
	}
	for(var key in fdQuestionDef[index].items){
		...
		$('#multimatrixquestionTr'+index+key).prepend("<td>"+ fdQuestionDef[index].items[key][0]+imgHTML+"</td>");
	}
...
function multiMatrix_getContent_col(index){
	$('#multimatrixquestion'+index).append("<table class='question_tb' id='multimatrixquestionTable"+index+"' ></table>");
	for(var optionskey in fdQuestionDef[index].options){
		$('#multimatrixquestionTable'+index).append('<tr id="multimatrixquestionTr'+index+optionskey+'"></tr>');
	}
	for(var itemskey in fdQuestionDef[index].items){
		for(var optionskey in fdQuestionDef[index].options){
			$('#multimatrixquestionTr'+index+optionskey).append('<td align="center"><input type="checkbox" id="option'+index+
					'" name="option'+index+'_'+itemskey+'" value="'+itemskey+'_'+optionskey+'"/></td>');
			}
	}
	$('#multimatrixquestionTable'+index).prepend('<tr id="multimatrixfirstTr'+index+'"></tr>');
	 for(var key in fdQuestionDef[index].items){
		 ...
		 var imgHTML = "";
			...
			$("#multimatrixfirstTr"+index).append("<td>"+fdQuestionDef[index].items[key][0]+imgHTML+"</td>");
		}
	 for(var key in fdQuestionDef[index].options){
		 var option = fdQuestionDef[index].options[key];
		 var imgHTML = "";
			...
			$('#multimatrixquestionTr'+index+key).prepend("<td>"+ fdQuestionDef[index].options[key][0]+imgHTML+"</td>");
		}
	 $('#multimatrixfirstTr'+index).prepend('<td></td>');
}
/************************************************
...
function multiMatrix_changeAnswerVal(index){
	...
	multiMatrix_setHasAnswerVal(index,answerVal);
}
/**************************************************
...
function multiMatrix_setValidateResultVal(index,answerVal){
  ...
  * 格式：true|false
  *************************************************/
 function multiMatrix_setHasAnswerVal(index,answerVal){
	 var hasAnswer = "true";
	 for(var key in fdQuestionDef[index].items){
		 var currSelectNum = 0;//单个问题选项，当前选择数
		$("input:checkbox[name='option"+index+'_'+key+"']").each(function(){
