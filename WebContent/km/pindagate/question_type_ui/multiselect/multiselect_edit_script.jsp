...
<script type="text/javascript">
	...
	*/
	$(document).ready(function(){
		 ...
		 if(obj!=null&&obj.value!=null&&obj.value!=""){
			...
			$("#minSelectNumber").val(selecttype.minSelectNumber);
			$("#selectReasonText").val(selecttype.selectReasonText);
			if(selecttype.maxSelectNumber){
				$("#maxSelectNumber").val(selecttype.maxSelectNumber);
			}
	...
	*/
	function save(){
		...
		if($('#willAnswer').prop("checked")==true){
			if(validateIsNull($("#minSelectNumber").val())){
				_dialog.alert("<bean:message bundle='km-pindagate' key='kmPindagateQuestion.minSelectNumber'/><bean:message bundle='km-pindagate' key='kmPindagateQuestion.validate.notNull'/>");
				return;
			}
			if(!validateIsNumber($("#minSelectNumber").val())){
				_dialog.alert("<bean:message bundle='km-pindagate' key='kmPindagateQuestion.minSelectNumber'/><bean:message bundle='km-pindagate' key='kmPindagateQuestion.validate.mustBeInteger'/>");
				return;
			}else{
				if(parseInt($("#minSelectNumber").val()) > $('input:text[name="itemtext"]').length){
					_dialog.alert("<bean:message bundle='km-pindagate' key='kmPindagateQuestion.minSelectNumber'/><bean:message bundle='km-pindagate' key='kmPindagateQuestion.validate.lessThanOrEqual'/>");
					return;
				}
			}
			if(!validateIsNull($("#maxSelectNumber").val())){
				if(!validateIsNumber($("#maxSelectNumber").val())){
					_dialog.alert("<bean:message bundle='km-pindagate' key='kmPindagateQuestion.maxSelectNumber'/><bean:message bundle='km-pindagate' key='kmPindagateQuestion.validate.mustBeInteger'/>");
					return;
				} else if(parseInt($("#maxSelectNumber").val()) > $('input:text[name="itemtext"]').length) {
					_dialog.alert("<bean:message bundle='km-pindagate' key='kmPindagateQuestion.maxSelectNumber'/><bean:message bundle='km-pindagate' key='kmPindagateQuestion.validate.lessThanOrEqual'/>");
					return;
				} else if(parseInt($("#maxSelectNumber").val()) < parseInt($("#minSelectNumber").val())) {
					_dialog.alert("<bean:message bundle='km-pindagate' key='kmPindagateQuestion.validate.maxMustLargerThanMin'/>");
					return;
				}
			}
		}
		...
		if($('#autoAddSelectReason').prop("checked")==true){
		 ...
		 if(obj != null ){
			  selecttype = {
					 ...
					 "minSelectNumber":$("#minSelectNumber").val(),
					 "maxSelectNumber":$("#maxSelectNumber").val(),
		...
		var str = '<tr><td>'
		 ...
		 for(var i=0; i<imgArr.length; i++){
			if(imgArr[i] != ""){
				...
				imgHtml += '<img src="'+imgArr[i]+'" border="1"  onclick="previewTooltip(event, this.src)" style="cursor:pointer;" width="25px" hight="25px" style="cursor:pointer;"/><img src="${KMSS_Parameter_StylePath}icons/delete.gif" width=16 height=16 onclick="deleteImg(\''+_id+'\','+i+')" title="'+"<bean:message bundle='km-pindagate' key='kmPindagateQuestion.deletePic'/>"+'" style="cursor:pointer;">&nbsp;';
			}
		 }
