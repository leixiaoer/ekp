...
function function_${JsParam.index}(){
	...
	$('#'+divId).prepend(multiselect_getTableHTML(index));
	//画“参与”、“跳过”进度条
	var totalNum = 10;
	...
	if(fdQuestionDef[index].statisticPic == "pie")
		multiselect_graphPie(index);//饼图
	else 
		multiselect_graphHistogram(index);//柱状图
	if(fdQuestionDef[index].autoAddOther){
		var otherDiv = '<div style="text-align: left;"><a href="javascript:void(0);" onclick="multiselect_viewOtherAnswer('+index+');" style="cursor: pointer;"><bean:message bundle="km-pindagate" key="kmPindagateResponse.view"/>“';
...
function multiselect_getTableHTML(index){
	...
	tableHTML += multiselect_getProgressHTML(index);
	tableHTML += '</table>';
	return tableHTML;
}
...
function multiselect_getProgressHTML(index){
...
//饼图
function multiselect_graphPie(index){
...
//柱状图
function multiselect_graphHistogram(index){
...
* 查看其他答案
*/
function multiselect_viewOtherAnswer(index){
