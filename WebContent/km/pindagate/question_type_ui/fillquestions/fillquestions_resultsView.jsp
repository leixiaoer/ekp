...
function function_${JsParam.index}(){
	...
	$('#'+divId).prepend(fillquestions_getTableHTML(index));
	//画“参与”、“跳过”进度条
	var totalNum = 10;
	...
	var otherDiv = '<div class="pi_otherInfo"><bean:message bundle="km-pindagate" key="kmPindagateResponse.allInfo"/>:“';
	otherDiv += '<a href="javascript:void(0);" onclick="fillquestions_viewOtherAnswer('+index+');" style="cursor: pointer;"><bean:message bundle="km-pindagate" key="kmPindagateResponse.view"/></a>”</div>';
...
function fillquestions_getTableHTML(index){
	...
	tableHTML += fillquestions_getProgressHTML(index);
	tableHTML += '</table>';
	return tableHTML;
}
...
function fillquestions_getProgressHTML(index){
...
* 全部信息:“查看”
*/
function fillquestions_viewOtherAnswer(questionId){
