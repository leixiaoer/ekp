...
<script type="text/javascript">
	...
	*/
	function save(){
		...
		try{
			...
			var selectScores = $("input:text[name^='selectScore']",$("#selectitemlist"));
			...
			$( selectScores).each(function(index){
	...
	*/
		 ...
		 var imgArr = imgsrc.split(";");
			...
			for(var i=0; i<imgArr.length; i++){
				if(imgArr[i] != ""){
					...
					imgHtml += '<img src="'+imgArr[i]+'" border="1"  onclick="previewTooltip(event, this.src)" style="cursor:pointer;" width="25px" hight="25px"/><img src="${KMSS_Parameter_StylePath}icons/delete.gif" width=16 height=16 onclick="deleteImg(\''+_id+'\','+i+')" title="'+"<bean:message bundle='km-pindagate' key='kmPindagateQuestion.deletePic'/>"+'" >&nbsp;';
				}
			}
	...
	function addSelectItem(text,selectScore){
		var rowCount = $('#selectitemlist tr').length;
		var str = "<tr><td><img src=\"${KMSS_Parameter_StylePath}icons/delete.gif\"  onclick=\"deleteSelectItem(this);\"/></td><td>"+rowCount+"</td><td><input type=\"text\" style=\"width:245px\" class=\"inputsgl\" name=\"itemtext\" value=\""
