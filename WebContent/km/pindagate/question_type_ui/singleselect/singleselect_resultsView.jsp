...
function function_${JsParam.index}(){
	...
	if(fdQuestionDef[index].autoAddOther){
		var otherDiv = '<div class="pi_otherInfo"><a href="javascript:void(0);" onclick="singleSelect_viewOtherAnswer('+index+');" style="cursor: pointer;"><bean:message bundle="km-pindagate" key="kmPindagateResponse.view"/>“';
		otherDiv += fdQuestionDef[index].otherText;
		otherDiv += '”</a></div>';
		$('#'+divId).append(otherDiv);
	}
	if(fdQuestionDef[index].autoAddSelectReason){
		var selectReasonDiv = '<div class="pi_otherInfo"><a href="javascript:void(0);" onclick="singleSelect_viewSelectReasonAnswer('+index+');" style="cursor: pointer;"><bean:message bundle="km-pindagate" key="kmPindagateResponse.view"/>“';
