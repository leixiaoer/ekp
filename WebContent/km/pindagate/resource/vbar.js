...
html5jp.graph.vbar.prototype.draw = function(items, inparams) {
	...
	var params = {
		...
		gBackgroundColor: "#aaaaaa",
		gGradation: true,
		barShape: "square",
		barColors: null,
		_barColors: ["rgb(11,122,207)", "rgb(214,44,0)", "rgb(248,154,5)", "rgb(87,203,46)", "rgb(33,100,50)", "rgb(33,41,107)", "rgb(115,0,90)", "rgb(132,0,0)", "rgb(165,99,0)", "rgb(24,123,0)"],
		barGradation: true,
		barAlpha: 0.7,
		borderAlpha: 0.2,
		xAxisWidth: 1,
	...
	if(params.xScale == true || params.x.length > 1) {
		...
		cpos.x_scale_y = cpos.y0 + x_scale_text_size.h * 1.5;
	}
...
html5jp.graph.vbar.prototype._draw_legend = function() {
	...
	for(var i=0; i<item_num; i++) {
		/* 文字 */
		this._drawText(lpos.cx, lpos.y, this.items[i][0], p.legendFontSize, p.legendFontFamily, p.legendColor,lpos.cw);
		/* 記号（背景） */
		this.ctx.save();
...
html5jp.graph.vbar.prototype._draw_x_scale_label = function(pos) {
	...
	if( p.x.length > 0 ) {
		...
		if(text != "") {
			...
			/* x軸キャプションテキストを描画 */
			//this._drawText(0, p.cpos.x_caption_y, text, p.xCaptionFontSize, p.xCaptionFontFamily, p.xCaptionColor);
			var div = document.createElement('DIV');
			div.appendChild( document.createTextNode(text) );
			div.style.fontSize = p.xCaptionFontSize;
			div.style.fontFamily = p.xCaptionFontFamily;
			div.style.color = p.xCaptionColor;
			div.style.maxWidth=this.canvas.width+"px";
			var canvasParentNode=this.canvas.parentNode;
			if(canvasParentNode.parentNode){
				canvasParentNode.parentNode.insertBefore(div,canvasParentNode);
			}else{
				canvasParentNode.appendChild(div);
			}
			
		}
	}
	if( p.xScale == true && p.x.length > 1 ) {
		var _x_scale_label_left=0;
		var _x_scale_label_top=0;
		var _x_scale_font_width=parseInt(p.xScaleFontSize.replace(/[^0-9]+/,''));
		var _y_scale_font_width=parseInt(p.yScaleFontSize.replace(/[^0-9]+/,''));
		
		for(var i=0; i<pos.length; i++) {
			if(i + 1 > p.x.length - 1) { break; }
			/* x軸目盛テキスト */
			var text = p.x[i+1].toString(),
				length = p.x.length+2 || 3,
				perWidth = this.canvas.width/length;
			if(text == "") { continue; }
			/* x軸目盛テキスト領域のサイズを算出 */
			var s = this._getTextBoxSize(text, p.xScaleFontSize, p.xScaleFontFamily);
			/* x軸目盛テキストを描画すべき左上端の座標を算出 */
			var x = Math.round( pos[i] - perWidth / 2 );
			/* x軸目盛テキストを描画 */
			this._drawText(x, p.cpos.x_scale_y, text, p.xScaleFontSize, p.xScaleFontFamily, p.xScaleColor,perWidth);
		}
	}
};
/* ------------------------------------------------------------------
...
文字列を描画
* ---------------------------------------------------------------- */
html5jp.graph.vbar.prototype._drawText = function(x, y, text, font_size, font_family, color,width) {
	var div = document.createElement('DIV');
	var showText = text.length > (width / 14) ? text.substring(0,width / 14)+'...' : text;
	div.appendChild( document.createTextNode(showText) );
