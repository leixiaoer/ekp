	...
	<template:replace name="body"> 
		...
		seajs.use(['lui/jquery', 'lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
			...
			window.addDoc = function() {
				dialog
						.categoryForNewFile(
								...
								'/km/pindagate/km_pindagate_main/kmPindagateMain.do?method=add&fdTemplateId=!{id}',
								false, null, null,
								'${JsParam.categoryId}',
								null, null, true);
			};
						...
						.confirm(
								...
								function(value) {
									if (value == true) {
										...
										$
												.post(
														'<c:url value="/km/pindagate/km_pindagate_main/kmPindagateMain.do?method=deleteall"/>',
														$
																.param(
																		{
	     ...
	     <list:tab-criterion title="" key="docStatus">
			   	<list:box-select>
			   		 	<list:item-select type="lui/criteria/select_panel!TabCriterionSelectDatas" cfg-enable="false" cfg-required="false"  cfg-if="param['join'] == 'all'||param['join'] == 'mine'">
			...
			<list:cri-auto
				...
				property="docCreator;docStartTime;docFinishedTime" />
		</list:criteria>
					...
					<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left">
						<list:sortgroup>
						   <list:sort property="docCreateTime"
							text="${lfn:message('km-pindagate:kmPindagateMain.docCreateTime') }"
							group="sort.list" value="down"></list:sort>
						    <list:sort property="docStatus"
							text="${lfn:message('km-pindagate:kmPindagateMain.docStatus') }"
							group="sort.list"></list:sort>
						<list:sort property="docFinishedTime"
							text="${lfn:message('km-pindagate:kmPindagateMain.docFinishedTime') }"
							group="sort.list"></list:sort>
						</list:sortgroup>
					</ui:toolbar>
				</div>
				...
				<div style="display: inline-block; vertical-align: middle;">
					<ui:toolbar id="toolbar">
						...
						<ui:button text="${lfn:message('button.export')}" id="export"
							onclick="listExport('${LUI_ContextPath}/sys/transport/sys_transport_export/SysTransportExport.do?method=listExport&fdModelName=com.landray.kmss.km.pindagate.model.KmPindagateMain')"
							order="3"></ui:button>
						</kmss:authShow>
						<kmss:auth
							requestURL="/km/pindagate/km_pindagate_main/kmPindagateMain.do?method=deleteall&status=${param.status}&categoryId=${param.categoryId}">
							<ui:button text="${lfn:message('button.deleteall')}" order="4"
...
<!-- 			列表视图 -->
			<list:colTable
				url="${LUI_ContextPath }/sys/profile/listShow/sys_listShow/sysListShow.do?method=getSort&modelName=com.landray.kmss.km.pindagate.model.KmPindagateMain"
				isDefault="true" layout="sys.ui.listview.columntable"
				rowHref="/km/pindagate/km_pindagate_response/kmPindagateResponse.do?method=add&pindageteId=!{fdId}"
				name="columntable">
