...
(function(window){
	...
	//绘制单选题目
	singleselect.init=function(questionJson){
		...
		if(questionJson.fdQuestionDef.autoAddOther){
			var otherDiv = '<div class="pi_otherInfo"><a href="javascript:void(0);" onclick="singleSelect_viewOtherAnswer(\''+questionJson.questionId+'\');" style="cursor: pointer;">'+Data_GetResourceString("km-pindagate:kmPindagateResponse.view")+'“';
			otherDiv += questionJson.fdQuestionDef.otherText;
			otherDiv += '”</a></div>';
			div.append(otherDiv);
		}
		if(questionJson.fdQuestionDef.autoAddSelectReason){
			var selectReasonDiv = '<div class="pi_otherInfo"><a href="javascript:void(0);" onclick="singleSelect_viewSelectReasonAnswer(\''+questionJson.questionId+'\');" style="cursor: pointer;">'+Data_GetResourceString("km-pindagate:kmPindagateResponse.view")+'“';
	...
	function getTitleHTML(questionJson){
		var titleHTML = '<tr><td>';
		titleHTML += '<div class="pi_txtTitle pi_txtTitle_no">'+questionJson.serialNum+'、</div>';
		...
		if(questionJson.fdQuestionDef.tip != ''){
			...
			titleHTML +=Data_GetResourceString("km-pindagate:kmPindagateQuestion.preview.titleDescription")+ '<div class="pi_subjectExplain_content">'+tip+'</div>';
			titleHTML += '</div></td></tr>';
		}
		return titleHTML;
	}
	...
	function graphPie(questionJson){
		var cg = new html5jp.graph.circle("vbar" + questionJson.index);
		if(!cg){
			return ;
		}
		var items = [];
	...
	window.singleSelect_viewOtherAnswer=function(questionId){
	...
	window.singleSelect_viewSelectReasonAnswer=function(questionId){
