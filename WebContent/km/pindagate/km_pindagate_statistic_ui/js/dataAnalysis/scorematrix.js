...
(function(window){
	...
	scorematrix.init=function(questionJson){
	...
	function getDataAnalysisHTML(questionJson){
		...
		for(var key in questionJson.fdQuestionDef.options){
			dataAnalysisHTML += '<td align="center">'+questionJson.fdQuestionDef.options[key][0]+'('+questionJson.fdQuestionDef.options[key][1]+Data_GetResourceString("km-pindagate:kmPindagateResponse.points")+')'+'</td>';
		}
		if(questionJson.fdQuestionDef.showAverage)
		dataAnalysisHTML += '<td align="center">'+Data_GetResourceString("km-pindagate:kmPindagateResponse.resultsDetail.Average")+'</td>';
		dataAnalysisHTML += '</tr>';
		for(var key in questionJson.fdQuestionDef.items){
			...
			if(questionJson.fdQuestionDef.showAverage){
			var sum=0 ,average =0, participateNumber =questionJson.fdStatistic.participateNumber;
			for(var i=0,j=0;i<questionJson.fdStatistic.items.length,j<questionJson.fdQuestionDef.options.length;i++,j++){
				sum +=(questionJson.fdStatistic.items[i][parseInt(key)+1]) *(questionJson.fdQuestionDef.options[j][1]);
			}
			average =parseFloat(sum/participateNumber).toFixed(1);
			dataAnalysisHTML += '<td align="center">'+average+'</td>';
			dataAnalysisHTML += '</tr>';		
			}
		}
