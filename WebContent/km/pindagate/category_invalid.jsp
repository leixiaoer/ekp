	...
	<head>
		...
		if('${fdIsAvailable}' == 'false'){
			seajs.use(['lui/dialog'],function(dialog) {
				dialog.confirm('<bean:message  bundle="km-pindagate" key="kmPindagateTemplate.msg.notAvailable"/>',function(flag){
					if(flag==true){
					dialog.categoryForNewFile(
							'com.landray.kmss.km.pindagate.model.KmPindagateTemplate',
							'/km/pindagate/km_pindagate_main/kmPindagateMain.do?method=add&fdTemplateId=!{id}',false,null,function(rtn){
								window.opener=null;
					    		window.open('','_self');
					    		window.close();
