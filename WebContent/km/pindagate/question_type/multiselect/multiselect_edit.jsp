...
<table class="tb_normal" width=90%>
	...
	<tr>
		<td><bean:message bundle="km-pindagate" key="kmPindagateQuestion.quetItem"/></td>
	</tr>
	<tr>
		<td>
	...
	<tr>
		<td>
			...
			<div id="div_minSelectNumber">
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<bean:message bundle="km-pindagate" key="kmPindagateQuestion.minSelectNumber"/><input type="text" name="minSelectNumber" id="minSelectNumber" value=""  class="inputSgl"><br>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<bean:message bundle="km-pindagate" key="kmPindagateQuestion.maxSelectNumber"/><input type="text" name="maxSelectNumber" id="maxSelectNumber" value=""  class="inputSgl"><br>
			</div>
	...
	if($('#willAnswer').attr("checked")==true){
		...
		if(!validateIsNull($("#maxSelectNumber").val())){
			if(!validateIsNumber($("#maxSelectNumber").val())){
				alert('<bean:message bundle="km-pindagate" key="kmPindagateQuestion.maxSelectNumber"/><bean:message bundle="km-pindagate" key="kmPindagateQuestion.validate.mustBeInteger"/>');
				return;
			}
		}
	}
...
function addQuestionItem(text,imgsrc){
	...
	var str = '<tr><td>'
		...
		str+='<td>'+rowCount+'</td>';
		str+='<td><input type="text" style="width:245;" class="inputSgl" name="itemtext" value="'+text+'"/></td>';
...
function  deleteImg(rowCount,k){
	...
	for(var i=0; i<imgArr.length; i++){
		if(i!=k){
			if(imgArr[i] != ""){
				imgHtml += '<img src="'+imgArr[i]+'" border="1" onMouseOver="resizeToSmail(this);" onMouseOut="resizeToLarge(this);" width="25px" hight="25px"/><img src="${KMSS_Parameter_StylePath}icons/delete.gif" width=10 height=10 onclick="deleteImg('+imgValueId.replace("questionitemimg","")+','+i+')" title="删除此图片" >&nbsp;';
			}
			newUrl +=  (imgArr[i] + ";");
		}
	}
