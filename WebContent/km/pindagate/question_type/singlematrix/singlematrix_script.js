Response_RegisterJsFile(Com_Parameter.ContextPath+"km/pindagate/question_type/singleselect/singleMatrix_script.js");
...
function singleMatrix_getContent_row(index){
	...
	for(var key in fdQuestionDef[index].options){
		...
		optionsHTML += '<td align="center">'+fdQuestionDef[index].options[key][0]+imgHTML+'</td>';
	}
	optionsHTML += '</tr>';
	for(var itemskey in fdQuestionDef[index].items){
		...
		for(var optionskey in fdQuestionDef[index].options){
			optionsHTML += '<td align="center"><input type="radio" id="option'+index+'" name="option'+index+"_"+itemskey+
...
function singleMatrix_getContent_col(index){
	 var optionsHTML = '<table>';
		...
		for(var optionskey in fdQuestionDef[index].options){
			...
			optionsHTML += '<tr>';
			optionsHTML += '<td align="center">'+fdQuestionDef[index].options[optionskey][0]+imgHTML+'</td>';
			for(var itemskey in fdQuestionDef[index].items){
				optionsHTML += '<td align="center"><input type="radio" id="option'+index+'" name="option'+index+"_"+itemskey+
				'" value="'+itemskey+'_'+optionskey+'"/></td>';
			}
...
function singleMatrix_changeAnswerVal(index){
	...
	if(fdQuestionDef[index].willAnswer){
		singleMatrix_setValidateResultVal(index,answerVal);
	}
}
