...
function singleMatrix_getTableHTML(index){
	...
	tableHTML += '<span onclick="singleMatrix_edit('+index+');" style="position:absolute;top: 0px;right: 30px;cursor: pointer;color:red;"><bean:message key="button.edit"/></span>';
...
function singleMatrix_getTitleHTML(index){
	...
	titleHTML += '</td></tr>';
	if(singleMatrix_fdQuestionDef.attachmentIds != ''){
	...
	if(singleMatrix_fdQuestionDef.tip != ''){
		...
		titleHTML += '<bean:message bundle="km-pindagate" key="kmPindagateQuestion.preview.titleDescription"/>'+$("<div></div>").append(singleMatrix_fdQuestionDef.tip).text();
		titleHTML += '</div></td></tr>';
	}
	return titleHTML;
}
...
function singleMatrix_getContent_row(index){
	...
	for(var key in singleMatrix_fdQuestionDef.options){
		...
		optionsHTML += '<td align="center">'+singleMatrix_fdQuestionDef.options[key][0]+imgHTML+'</td>';
	}
	optionsHTML += '</tr>';
	for(var itemskey in singleMatrix_fdQuestionDef.items){
		...
		optionsHTML += '<tr>';
		optionsHTML += '<td align="center">'+singleMatrix_fdQuestionDef.items[itemskey][0]+imgHTML+'</td>';
...
function singleMatrix_getContent_col(index){
	 var optionsHTML = '<table class="question_tb">';
		...
		for(var key in singleMatrix_fdQuestionDef.items){
			...
			optionsHTML += '<td align="center">'+singleMatrix_fdQuestionDef.items[key][0]+imgHTML+'</td>';
		}
		optionsHTML += '</tr>';
		for(var optionskey in singleMatrix_fdQuestionDef.options){
			...
			optionsHTML += '<tr>';
			optionsHTML += '<td align="center">'+singleMatrix_fdQuestionDef.options[optionskey][0]+imgHTML+'</td>';
			for(var itemskey in singleMatrix_fdQuestionDef.items){
				optionsHTML += '<td align="center"><input type="radio" name="option'+index+itemskey+'"/></td>';
			}
			optionsHTML += '</tr>';
...
function singleMatrix_edit(index){
	...
	var url = getContextPath()+"km/pindagate/question_type/singlematrix/singlematrix_edit.jsp";
	if(obj == null)
	...
	if(obj.value != null && obj.value != ""){
		...
		tableHTML += '<span onclick="singleMatrix_edit('+index+');" style="position:absolute;top: 0px;right: 30px;cursor: pointer;"><bean:message key="button.edit"/></span>';
