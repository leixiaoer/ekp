...
function function_${JsParam.index}(){
	...
	$('#'+divId).prepend(singleMatrix_getTableHTML(index));
	$('#content'+index).append(singleMatrix_getDataAnalysisHTML(index));
	//画“参与”、“跳过”进度条
	var totalNum = 10;
...
function singleMatrix_getTableHTML(index){
	...
	tableHTML += singleMatrix_getProgressHTML(index);
	tableHTML += '</table>';
	return tableHTML;
}
...
function singleMatrix_getProgressHTML(index){
...
function singleMatrix_getDataAnalysisHTML(index){
