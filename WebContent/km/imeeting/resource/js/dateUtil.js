define(function(require, exports, module) {
	...
	var fixDate=function(d, check) {
		if (+d) {
			while (d.getDate() != check.getDate()) {
				d.setTime(+d + (d < check ? 1 : -1) * TIME_MS['hour']);
			}
		}
	};
	...
	/**
	 * 将传入时间对象拆解成时分，如{"hour":1.5} —>{"hour":1,"minute":30 }
	 */
	dateUtil.splitTime=function(timeObj){
		var timeMs=0;
		for(var key in timeObj){
			if( TIME_MS[key] ){
				timeMs=TIME_MS[key]*timeObj[key];
			}
			break;
		}
		var hour=parseInt(timeMs/(1000*60*60));
		var minute=parseInt( timeMs %(1000*60*60)/(1000*60));
		return {"hour":hour,"minute":minute};
	};
	
	/**
	 * 将传入时间对象合并成指定类型，如{ "hour":1,"minute":30 },"hour"—>1.5、{ "hour":1,"minute":30 },"minute"—>90
	 */
	dateUtil.mergeTime=function(timeObj,type){
		var timeMs=0;
		for( var key in timeObj){
			if( TIME_MS[key] ){
				timeMs+=TIME_MS[key]*timeObj[key];
		...
		if(TIME_MS[type] ){
			return (timeMs/TIME_MS[type]).toFixed(1);
		}else{
			return null;
		}
	};
