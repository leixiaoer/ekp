...
$(document).ready(function (){
	var val ="${kmImeetingEquipmentForm.fdIsAvailable}";  
	if(val=='1'||val=='true'){
...
<div id="optBarDiv">
	<c:if test="${kmImeetingEquipmentForm.method_GET=='edit'}">
		<input type=button value="<bean:message key="button.update"/>"
			onclick="Com_Submit(document.kmImeetingEquipmentForm, 'update');">
	</c:if>
	<c:if test="${kmImeetingEquipmentForm.method_GET=='add'}">
		<input type=button value="<bean:message key="button.save"/>"
			onclick="Com_Submit(document.kmImeetingEquipmentForm, 'save');">
		<input type=button value="<bean:message key="button.saveadd"/>"
			onclick="Com_Submit(document.kmImeetingEquipmentForm, 'saveadd');">
	</c:if>
...
<table class="tb_normal" width=95%>
 ...
 <%-- 所属场所 --%>
     <c:import url="/sys/authorization/sys_auth_area/sysAuthArea_field.jsp" charEncoding="UTF-8">
         <c:param name="id" value="${kmImeetingEquipmentForm.authAreaId}"/>
     </c:import>   
