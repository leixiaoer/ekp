...
<script  type="text/javascript">
		seajs.use([ 'lui/jquery','lui/parser','lui/dialog'],function($,parser,dialog) {
			...
			window.clickOK=function(){
				...
				if(field.value==""){
					dialog.alert("<kmss:message key="errors.name.required" bundle="km-imeeting" />");
					return;
				}
				var rtn = field.value;
				$dialog.hide(rtn);
...
<bean:message bundle="km-imeeting" key="py.fdName" />
<xform:text property="fdName" required="true" showStatus="edit" value="${param.fdName}"></xform:text>
<br><br>
