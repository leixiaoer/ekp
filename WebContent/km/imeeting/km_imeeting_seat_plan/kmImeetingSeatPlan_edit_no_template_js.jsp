...
<script>
	...
	seajs.use([ 'lui/topic', 'lui/dialog','km/imeeting/resource/js/seatPlan', 
			...
			function (topic, dialog,SeatPlan,Seat,SeatUtil,ElementList,PersonList,TopicList) {	
		...
		**********************************************************/
		...
		var elementList,hostList,participantList,summaryList,attendList,reportersList,reporterList
			,attendUnitList,listenUnitList,topicList,seatPlan;
		var isInit = false;
	 	...
	 	window.checkIsPlan = function(){
	    	...
	    	if(isTopicPlan == 'true'){
	    		...
	    		$.each(allPersonData || [], function(_, p) {
	    			var index =$.inArray(p.elementId,planElement);
					if(index == -1){
						flag = false;
					}
	    		});
