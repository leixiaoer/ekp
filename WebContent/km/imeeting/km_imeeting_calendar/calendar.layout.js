seajs.use(['lang!km-imeeting'],function(lang){
	...
	//生成固定表头
	window.createFixedContent = function (){
		setTimeout(function(){
			let calendarValue = $('.lui_calendar_header_right>.lui_calendar_header_button_on').attr('cal-opt');
			if(calendarValue === "day"){
				$('.lui_calendar_fixed_box').remove();
				$('.lui_calendar_header').after($('<div class="lui_calendar_fixed_box"/></div>'));
				$('.lui_calendar_fixed_box').append($(".lui_calendar_content[data-lui-mark='calender.content.inside.day']").clone(true));
			}else{
				$('.lui_calendar_fixed_box').remove();
				$('.lui_calendar_header').after($('<div class="lui_calendar_fixed_box"/></div>'));
				$('.lui_calendar_fixed_box').append($(".lui_calendar_content[data-lui-mark='calender.content.inside.week']").clone(true));
			}
		},2000);
	}
	window.showFixedContent = function(){
		if( $("#meetingPlace",window.top.document).length > 0 ){
			let boxTop = $(window.top).scrollTop();
			if(boxTop>102){
				$('.lui_calendar_fixed_box').css('top',boxTop-40).show()
			}else{
				$('.lui_calendar_fixed_box').hide()
			}
		}
	};
	...
	$(document).ready(function(){
		$('.lui_calendar_header_right>.lui_calendar_header_button_on').click();
		createFixedContent();
	});
