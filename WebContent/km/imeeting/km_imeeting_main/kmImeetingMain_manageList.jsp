	...
	<template:replace name="content">  
				...
				<div style="display: inline-block;vertical-align: middle;">
					<ui:toolbar count="4" id="Btntoolbar">
						...
						<c:if test="${JsParam.nodeType=='TEMPLATE'}">	
							<kmss:auth
								requestURL="/km/imeeting/km_imeeting_main/kmImeetingMain.do?method=add&fdTemplateId=${JsParam.categoryId}&fdModelId=${JsParam.fdModelId}&fdModelName=${JsParam.fdModelName}"
								requestMethod="GET">
								<ui:button title="${lfn:message('button.add')}" text="${lfn:message('button.add')}" onclick="Com_OpenWindow('${LUI_ContextPath }/km/imeeting/km_imeeting_main/kmImeetingMain.do?method=add&fdTemplateId=${JsParam.categoryId}&fdProjectId=${JsParam.fdProjectId}&fdModelName=${JsParam.fdModelName}');" order="2"></ui:button>
							</kmss:auth>
						</c:if>
						<kmss:auth
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/km/imeeting/km_imeeting_main/kmImeetingMain.do?method=manageList&categoryId=${JsParam.categoryId}&nodeType=${JsParam.nodeType}'}
			</ui:source>
