				...
				<%
					if(pageContext.getAttribute("kmImeetingMain")!=null){
						...
						Date now=new Date(),
							...
							earlyFinishDate	=kmImeetingMain.getFdEarlyFinishDate();
						...
						if ((finishDate != null && finishDate.getTime() < now.getTime())
							||(earlyFinishDate !=null && earlyFinishDate.getTime() < now.getTime())) {
							isEnd = true;
						}
			...
			<%
			 	KmImeetingMain kmImeetingMain = (KmImeetingMain)pageContext.getAttribute("kmImeetingMain");
			  	...
			  	if(StringUtil.isNotNull(count)) {
			  		...
			  		if ("en_US".equals(ResourceUtil.getLocaleByUser().toString())) {
			  			String year = until.substring(5, 9);
				  		String month = until.substring(9, 11);
				  		String day = until.substring(11, 13);
			  			until = "Until " + month +"/" + day + "/" + year; 
			  		}
			  		out.print(until);
			  	} else {
