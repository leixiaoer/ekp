	...
	<template:replace name="body">
		...
		<list:criteria id="attendCriteria">
			<list:tab-criterion title="" key="meetingStatus">
				<list:box-select>
					<list:item-select  cfg-if="(param['mymeeting'] == 'myCreate')" cfg-enable="true" cfg-required="${(JsParam.required==null)?'':(JsParam.required)}" cfg-defaultValue="${(JsParam.defaultValue==null)?'':(JsParam.defaultValue)}" type="lui/criteria/select_panel!TabCriterionSelectDatas">
			...
			<list:tab-criterion title="" key="meetingStatus">
				<list:box-select>
					<list:item-select  cfg-if="(param['mymeeting'] == 'myApproved')" cfg-enable="true" cfg-required="${(JsParam.required==null)?'':(JsParam.required)}" cfg-defaultValue="${(JsParam.defaultValue==null)?'':(JsParam.defaultValue)}" type="lui/criteria/select_panel!TabCriterionSelectDatas">
						<ui:source type="Static">
							[
							{text:'${ lfn:message('km-imeeting:kmImeeting.status.reject') }', value:'11'},
							{text:'${ lfn:message('km-imeeting:kmImeeting.status.publish') }', value:'30'}
			...
			<list:cri-criterion title="${ lfn:message('km-imeeting:kmImeetingMain.docStatus') }" key="meetingStatus">
				<list:box-select>
					<list:item-select id="status_for_myCreate" cfg-enable="true" cfg-if="(param['mymeeting'] == 'myCreate' || param['mymeeting'] == 'myApproval' || param['mymeeting'] == 'myApproved')"> 
						<ui:source type="Static">
					...
					<ui:toolbar count="3" id="Btntoolbar">
					    ...
					    <kmss:authShow roles="ROLE_KMIMEETING_CREATE">
					    	...
					    	<ui:button title="${lfn:message('km-imeeting:table.kmImeetingVideo') }"
					       		text="${lfn:message('km-imeeting:table.kmImeetingVideo') }"  cfg-if="param['j_path']!='/abandom/myMeeting'"
					       		onclick="window.open('${LUI_ContextPath }/km/imeeting/km_imeeting_main/kmImeetingMain.do?method=add&noTemplate=true')" order="2" >
					       	</ui:button>
					       <%} %>	
						   <ui:button title="${lfn:message('button.add')}" text="${lfn:message('button.add')}" onclick="addDoc()" order="2" 
						   cfg-if="param['j_path']!='/abandom/myMeeting'"></ui:button>
						</kmss:authShow>
						<kmss:auth
	...
	<%-- 列表视图 --%>
		<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/km/imeeting/km_imeeting_main/kmImeetingMain.do?method=listChildren&categoryId=${JsParam.categoryId}&nodeType=${JsParam.nodeType}&cycle=true'}
			</ui:source>
...
/* 			//删除
			...
			window.delDoc = function() {
				...
				var url = '<c:url value="/km/imeeting/km_imeeting_main/kmImeetingMain.do?method=deleteall"/>';
				dialog.confirm(
