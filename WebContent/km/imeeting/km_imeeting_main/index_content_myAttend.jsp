	...
	<template:replace name="body">
		...
		<list:criteria id="attendCriteria">
			<list:tab-criterion title="" key="mymeeting">
				<list:box-select>
					<list:item-select id="status_for_imeeting" cfg-enable="true"  cfg-required="true"   type="lui/criteria/select_panel!TabCriterionSelectDatas">
			...
			<%-- <list:cri-criterion title="我的" key="mywork">
				<list:box-select>
					<list:item-select>
						<ui:source type="Static">
							[{text:'${ lfn:message('km-imeeting:kmImeeting.Report.my') }', value:'myReport'}
	...
	<%-- 列表视图 --%>
		<list:listview id="listview">
			...
			<list:colTable url="${LUI_ContextPath }/sys/profile/listShow/sys_listShow/sysListShow.do?method=getSort&modelName=com.landray.kmss.km.imeeting.model.KmImeetingMain" isDefault="true" layout="sys.ui.listview.columntable" 
				rowHref="/km/imeeting/km_imeeting_main/kmImeetingMain.do?method=view&fdId=!{fdId}&path=myAttend" name="columntable">
				<list:col-checkbox name="List_Selected"></list:col-checkbox>
				<list:col-serial title="${ lfn:message('page.serial')}"></list:col-serial>
				<list:col-auto props="fdName,fdHost,fdPlace,fdDate,fdHoldDate,fdFinishDate,docCreator.fdName,docDept.fdName,docStatus"></list:col-auto>
			</list:colTable>
		</list:listview>
