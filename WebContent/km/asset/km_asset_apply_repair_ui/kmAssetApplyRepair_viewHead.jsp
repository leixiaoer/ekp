...
<template:replace name="toolbar">
	 ...
	 <c:if test="${kmAssetApplyRepairForm.docStatus=='10' || kmAssetApplyRepairForm.docStatus=='11'|| kmAssetApplyRepairForm.docStatus=='20'}">
		<kmss:auth requestURL="/km/asset/km_asset_apply_repair/kmAssetApplyRepair.do?method=edit&fdId=${param.fdId}" requestMethod="GET">
		    <ui:button text="${ lfn:message('button.edit') }" order="2" 
						onclick="Com_OpenWindow('kmAssetApplyRepair.do?method=edit&fdId=${JsParam.fdId}','_self');">
		    </ui:button>
		</kmss:auth>
	</c:if>
	<kmss:auth requestURL="/km/asset/km_asset_apply_repair/kmAssetApplyRepair.do?method=delete&fdId=${param.fdId}" requestMethod="GET">
	    <ui:button text="${ lfn:message('button.delete') }" order="2" 
						onclick="if(!confirmDelete())return;Com_OpenWindow('kmAssetApplyRepair.do?method=delete&fdId=${JsParam.fdId}','_self');">
		 </ui:button>
	</kmss:auth>
	<c:if test="${kmAssetApplyRepairForm.docStatus!='10'}">
		<kmss:auth
			requestURL="/km/asset/km_asset_apply_repair/kmAssetApplyRepair.do?method=print&fdId=${param.fdId}"
			requestMethod="GET">
			<ui:button text="${lfn:message('button.print') }" order="4" 
			    onclick="Com_OpenWindow('kmAssetApplyRepair.do?method=print&fdId=${JsParam.fdId}','_blank');">
		    </ui:button>
		</kmss:auth>
   </c:if>
...
<template:replace name="path">
	<ui:combin ref="menu.path.category">
		<ui:varParams  moduleTitle="${ lfn:message('km-asset:module.km.asset') }" 
		    ...
		    autoFetch="false" 
		    href="/km/asset/#j_path=/kmAsset_repair" 
