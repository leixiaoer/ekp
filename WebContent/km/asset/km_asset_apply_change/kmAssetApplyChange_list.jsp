...
<html:form action="/km/asset/km_asset_apply_change/kmAssetApplyChange.do">
	<div id="optBarDiv">
		<kmss:auth requestURL="/km/asset/km_asset_apply_change/kmAssetApplyChange.do?method=add">
			<input type="button" value="<bean:message key="button.add"/>"
				onclick="addByApplyTemplate();">
		</kmss:auth>
		<kmss:auth requestURL="/km/asset/km_asset_apply_change/kmAssetApplyChange.do?method=deleteall">
			<input type="button" value="<bean:message key="button.deleteall"/>"
				onclick="if(!List_ConfirmDel())return;Com_Submit(document.kmAssetApplyChangeForm, 'deleteall');">
		</kmss:auth>
	</div>
...
<c:if test="${queryPage.totalrows>0}">
	...
	<table id="List_ViewTable">
			...
			<sunbor:columnHead htmlTag="td">
				...
				<!--标题-->
				<sunbor:column property="kmAssetApplyChange.docSubject">
					<bean:message bundle="km-asset" key="kmAssetApplyBase.docSubject"/>
				</sunbor:column>
				<!--编号-->
				<sunbor:column property="kmAssetApplyChange.fdNo">
		...
		<c:forEach items="${queryPage.list}" var="kmAssetApplyChange" varStatus="vstatus">
			<tr
				kmss_href="<c:url value="/km/asset/km_asset_apply_change/kmAssetApplyChange.do" />?method=view&fdId=${kmAssetApplyChange.fdId}">
				<td>
					<input type="checkbox" name="List_Selected" value="${kmAssetApplyChange.fdId}">
				</td>
				<td>
					${vstatus.index+1}
				</td>
				...
				<td>
					<c:out value="${kmAssetApplyChange.docSubject}" />
				</td>
					<td>
					<c:out value="${kmAssetApplyChange.fdNo}" />
				</td>
				<td>
					<c:out value="${kmAssetApplyChange.fdCreator.fdName}" />
				</td>
					<td>
				...
				<td>
					<c:out value="${kmAssetApplyChange.fdApplyTemplate.fdName}"/>
				</td>		
			</tr>
		</c:forEach>
	</table>
