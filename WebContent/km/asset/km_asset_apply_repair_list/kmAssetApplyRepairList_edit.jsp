		...
		if (null!= ids&& undefined !=ids&&ids!=""){
			...
			if (results.length > 0) {
				for(var i=0;i<results.length;i++){
					...
					trHTML += '</td>';
					trHTML += '<td>';
					trHTML += '<xform:text style="width:80%" subject="'+fdRepairType+'"  property="fdItems['+trIndex+'].fdRepairType" />';
					trHTML += '</td>';
					trHTML += '<td>';
					trHTML += '<xform:text style="width:80%" subject="'+fdMoney+'" property="fdItems['+trIndex+'].fdMoney"  value=""  onValueChange="calPercent('+trIndex+');void" validators="currency-dollar"/>';
					trHTML += '</td>';
					trHTML += '<td>';
					trHTML += '<input type="text"  class="inputread" style="width:80%" value=""  name="fdItems['+trIndex+'].fdDegree" readonly="readonly"/>';
					trHTML += '</td>';
					trHTML += '<td>';
					trHTML += '<xform:text style="width:80%" subject="'+fdForeignRepairAdd+'" property="fdItems['+trIndex+'].fdForeignRepairAdd" />';
					trHTML += '</td>';
...
<table class="tb_normal" width=100% id="RepairlistTB" >
	...
	<tr align="center">
		...
		<td> 
			<xform:text subject='<%=ResourceUtil.getString(request,"kmAssetApplyRepairList.fdRepairType","km-asset")%>' property="fdItems[${vindex.index}].fdRepairType" style="width:80%"/>
		</td>
		<td> 
			<xform:text subject='<%=ResourceUtil.getString(request,"kmAssetApplyRepairList.fdMoney","km-asset")%>' property="fdItems[${vindex.index}].fdMoney" value="" style="width:80%" validators="currency-dollar" onValueChange="calPercent(${vindex.index});void"/>
		</td>
		<td> 
			<xform:text subject='<%=ResourceUtil.getString(request,"kmAssetApplyRepairList.fdDegree","km-asset")%>' property="fdItems[${vindex.index}].fdDegree" value="" style="width:80%" htmlElementProperties="readonly='readonly'" className="inputread"/>
		</td>
		<td> 
			<xform:text subject='<%=ResourceUtil.getString(request,"kmAssetApplyRepairList.fdForeignRepairAdd","km-asset")%>' property="fdItems[${vindex.index}].fdForeignRepairAdd" style="width:80%"/>
		</td>
		<td>
