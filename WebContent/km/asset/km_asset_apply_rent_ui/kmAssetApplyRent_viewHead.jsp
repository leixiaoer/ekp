	...
	<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" var-navwidth="90%">
	 <c:if test="${kmAssetApplyRentForm.docStatus=='10' || kmAssetApplyRentForm.docStatus=='11'|| kmAssetApplyRentForm.docStatus=='20'}">
		<kmss:auth requestURL="/km/asset/km_asset_apply_rent/kmAssetApplyRent.do?method=edit&fdId=${param.fdId}" requestMethod="GET">
		    <ui:button text="${ lfn:message('button.edit') }" order="2" 
						onclick="Com_OpenWindow('kmAssetApplyRent.do?method=edit&fdId=${JsParam.fdId}','_self');">
		    </ui:button>
		</kmss:auth>
	 </c:if>
	 <kmss:auth requestURL="/km/asset/km_asset_apply_rent/kmAssetApplyRent.do?method=delete&fdId=${param.fdId}" requestMethod="GET">
	     <ui:button text="${ lfn:message('button.delete') }" order="2" 
						onclick="if(!confirmDelete())return;Com_OpenWindow('kmAssetApplyRent.do?method=delete&fdId=${JsParam.fdId}','_self');">
		 </ui:button>
	</kmss:auth>
  <c:if test="${kmAssetApplyRentForm.docStatus!='10'}">
		<kmss:auth
			requestURL="/km/asset/km_asset_apply_rent/kmAssetApplyRent.do?method=print&fdId=${param.fdId}"
			requestMethod="GET">
			<ui:button text="${lfn:message('button.print') }" order="4" 
			    onclick="Com_OpenWindow('kmAssetApplyRent.do?method=print&fdId=${JsParam.fdId}','_blank');">
		    </ui:button>
		</kmss:auth>
   </c:if>
