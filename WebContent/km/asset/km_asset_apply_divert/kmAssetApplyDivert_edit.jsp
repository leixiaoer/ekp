...
function commitMethod(method, saveDraft){
	...
	<div id="optBarDiv"><c:if
		test="${kmAssetApplyDivertForm.method_GET=='edit'}">
		...
		<c:if
			test="${kmAssetApplyDivertForm.docStatus=='10'||kmAssetApplyDivertForm.docStatus=='11'||kmAssetApplyDivertForm.docStatus=='20' }">
			<input type=button value="<bean:message key="button.submit"/>"
				onclick="commitMethod('update');">
		</c:if>
	</c:if> <c:if test="${kmAssetApplyDivertForm.method_GET=='add'}">
		<input type="button" value="<bean:message key="button.savedraft" />"
			...
			<table class="tb_normal" width=95%>
				<tr>
					...
					<td width="85%" colspan="5"><xform:text property="docSubject"
						style="width:85%" value="${kmAssetApplyDivertForm.docSubject}" /></td>
				</tr>
				<tr>
					<td class="td_normal_title" width=15%><bean:message
						bundle="km-asset" key="kmAssetApplyBase.fdApplyCategory" /></td>
					<td width="19%"><html:hidden property="fdApplyTemplateId"
						value="${kmAssetApplyDivertForm.fdApplyTemplateId}" /> <html:hidden
						property="fdApplyTemplateName" />
					${kmAssetApplyDivertForm.fdApplyTemplateName}</td>
					<td class="td_normal_title" width=15%><bean:message
						bundle="km-asset" key="kmAssetApplyBase.fdNo" /></td>
					<td width="51%" colspan="3"><c:choose>
				...
				<tr>
					...
					<td width="19%"><html:hidden property="fdCreatorId" /> <xform:text
						property="fdCreatorName"
						value="${kmAssetApplyDivertForm.fdCreatorName}" showStatus="view"></xform:text>
					</td>
					<td class="td_normal_title" width=15%><bean:message
						bundle="km-asset" key="kmAssetApplyBase.fdDept" /></td>
					<td width="19%"><html:hidden property="fdDeptId" /> <xform:text
						property="fdDeptName" value="${kmAssetApplyDivertForm.fdDeptName}"
						showStatus="view"></xform:text></td>
					<td class="td_normal_title" width=15%><bean:message
						bundle="km-asset" key="kmAssetApplyBase.fdCreateDate" /></td>
					<td width="19%"><html:hidden property="fdCreateDate" /> <xform:datetime
						property="fdCreateDate" dateTimeType="date" showStatus="view" /></td>
				</tr>
				<!-- 调拨明细 -->
				<tr>
					<td width="100%" colspan="6"><c:import
						url="/km/asset/km_asset_apply_divert_list/kmAssetApplyDivertList_edit.jsp"
						charEncoding="UTF-8">
					</c:import></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width="85%" colspan="5"><html:hidden property="fdExplain" />
