		...
		<table class="tb_normal" width=100%>
			<c:if test="${param.approveModel ne 'right'}">
				...
				<tr>
					...
					<td width="19%"><c:out value="${kmAssetApplyDivertForm.fdApplyTemplateName}"/></td>
					<td class="td_normal_title" width=15%><bean:message
			   ...
			   <c:import
				...
				<c:if test="${fn:length(myCards) > 0}">
					<c:import url="/km/asset/km_asset_card/kmAssetCard_mycard_import.jsp" charEncoding="UTF-8">
						<c:param name="fdResponsiblePerson" value="${kmAssetApplyDivertForm.fdCreatorId}"></c:param>
					</c:import>
				</c:if>
				</td>
			</tr>
	...
	<c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<ui:tabpanel suckTop="true" layout="sys.ui.tabpanel.sucktop" var-supportExpand="true" var-extend="true" var-average='false' var-useMaxWidth='true'>
				<c:choose>
					<c:when test="${kmAssetApplyDivertForm.docStatus>='30' || kmAssetApplyDivertForm.docStatus=='00'}">
				...
				<%--传阅机制--%>
				<c:if test="${kmAssetApplyDivertForm.fdCanCircularize eq 'true' }">
		...
		<template:replace name="barRight">
			<c:choose>
				<c:when test="${kmAssetApplyDivertForm.docStatus>='30' || kmAssetApplyDivertForm.docStatus=='00'}">
					<ui:accordionpanel>
						<!-- 基本信息-->
						<c:import url="/km/asset/km_asset_apply_divert_ui/kmAssetApplyDivert_viewBaseInfoContent.jsp" charEncoding="UTF-8">
						</c:import>
						<!-- 关联配置 -->
				...
				<c:otherwise>
					<ui:tabpanel layout="sys.ui.tabpanel.vertical.icon" id="barRightPanel" >
						...
						<c:import url="/sys/lbpmservice/support/lbpm_audit_note/lbpmAuditNote_list_content.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="kmAssetApplyDivertForm" />
							<c:param name="fdModelId" value="${kmAssetApplyDivertForm.fdId}" />
