...
<template:replace name="toolbar">
	<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" var-navwidth="90%">
	 ...
	 <c:if test="${kmAssetApplyDivertForm.docStatus!='10'}">
		<kmss:auth
			requestURL="/km/asset/km_asset_apply_divert/kmAssetApplyDivert.do?method=print&fdId=${param.fdId}"
			requestMethod="GET">
			<ui:button text="${lfn:message('button.print') }" order="4" 
			    onclick="Com_OpenWindow('kmAssetApplyDivert.do?method=print&fdId=${JsParam.fdId}','_blank');">
		    </ui:button>
		</kmss:auth>
   </c:if>
