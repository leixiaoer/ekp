				...
				<table class="muiSimple muiFlowTable" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td class="muiFlowSummary">
							<div>${lfn:message('km-asset:kmAssetApplyBase.fdDept') }：<c:out value="${_mainForm.fdDeptName}" /></div>
							<div>${lfn:message('km-asset:kmAssetApplyBase.fdNo') }： <c:out value="${_mainForm.fdNo}" /></div>
							<div>${lfn:message('km-asset:kmAssetApplyBase.fdCreateDate') }： <c:out value="${_mainForm.fdCreateDate}" /></div>
						</td>
					</tr>
				</table>
			</div>
