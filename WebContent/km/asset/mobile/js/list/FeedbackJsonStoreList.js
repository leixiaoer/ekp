define([
	...
	return declare("km.asset.list.FeedbackJsonStoreList", [JsonStoreList], {
		...
		handleOnPush: function(widget, handle) {
			...
			if(parentView.isVisible()){
				if(!this._loadOver){
					if('PersonView'==parentView.id){
						parentView.srcNodeRef.children[0].innerHTML="";
					}
		...
		onFeedbackCriteria:function(widget){
			if(this.url.indexOf("kmAssetApplyTaskDetail.do?method=manageList")>-1&&this.url.indexOf("inventoryType=todoInventory")>-1){
			criteriaType=widget.criteriaType || "";
			fdAssignUser=widget.fdAssignUser || "";
			personnelIds=widget.personnelIds || "";
			taskCreator=widget.taskCreator || "";
			this.url = this._setUrlParameter(this.url,"criteriaType",criteriaType);
			this.url = this._setUrlParameter(this.url,"fdAssignUser",fdAssignUser);
			this.url = this._setUrlParameter(this.url,"personnelIds",personnelIds);
			this.url = this._setUrlParameter(this.url,"taskCreator",taskCreator);
			this.reload();
			}else{
			this.reload();
			}
