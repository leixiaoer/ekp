...
<template:replace name="title">
	<c:choose>
		<c:when test="${ kmAssetApplyDealForm.method_GET == 'add' }">
			<c:out value="${lfn:message('km-asset:table.kmAssetApplyDeal.create') } - ${ lfn:message('km-asset:module.km.asset') }"></c:out>
		</c:when>
		<c:otherwise>
...
<template:replace name="toolbar">
	<ui:toolbar var-navwidth="90%" id="toolbar"
		layout="sys.ui.toolbar.float">
		<c:if test="${kmAssetApplyDealForm.method_GET=='edit'}">
			<%-- 提交 --%>
		...
		<%-- 增加--%>
		<c:if test="${kmAssetApplyDealForm.method_GET=='add'}">
			<%-- 暂存 --%>
			<ui:button text="${ lfn:message('button.savedraft') }" order="2"
