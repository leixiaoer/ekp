...
<script type="text/javascript">
	...
	function checkFile(){
		...
		if(file[0].value==null || file[0].value.length==0){
			alert("<bean:message bundle='km-asset' key='kmAssetCard.import.file.required'/>");
			return false ;
		}
		//添加参数:是否更新存在数据
...
<html:form action="/km/asset/km_asset_address/kmAssetAddress.do?method=importExcel" 
	enctype="multipart/form-data" onsubmit="return checkFile();">

	<p class="txttitle">
		<bean:message bundle="km-asset" key="kmAssetAddress.import.title" />
	</p>
	<br />
	<center>
		<table class="tb_normal" width=60%>
			<tr>
				...
				<td width=85%>
					...
					<a href="#" onclick="window.open('assetsAddress_template.xls');">
			...
			<tr>
				<td colspan="2" >
					<bean:message bundle="km-asset" key="kmAssetAddress.tip.upload" />
				</td>
			</tr>
		...
		<center>
			<input name="submit" type="submit" class="lui_form_button" value="<bean:message bundle="km-provider" key="kmProviderMain.btn.upload" />">
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" class="lui_form_button" value="<bean:message key="button.close"/>" onclick="window.close();">
		</center>
	</center>
