				...
				<ui:content title="${ lfn:message('km-review:kmReview.nav') }">
					<ui:combin ref="menu.nav.simple">
		  				<ui:varParam name="source">
		  					...
		  					{
								"text": "${ lfn:message('geesun-base:geesunBasePay.payConfirm') }",
								"href": "/listPayConfirm",
								"router" : true,
								"icon": "lui_iconfont_navleft_com_my_beapproval"
							},
		  					...
		  					<kmss:authShow roles="ROLE_GEESUNBASE_FINANCEQUERY">
		  					,{
		  						"text" : "${ lfn:message('geesun-base:geesunBaseBxsj.query.finance') }",
		  						"href" :  "/queryFinance",
								"router" : true,		  					
		  					...
		  					<kmss:ifModuleExist path="/dbcenter/echarts/">
								<kmss:authShow roles="ROLE_DBCENTERECHARTS_DEFAULT">
									<xform:Show bean="dbEchartsNavTreeShowService" mainModelName="com.landray.kmss.km.review.model.KmReviewTemplate" fdKey="kmReviewMainDoc">
										,{
		...
		<ui:tabpanel id="kmReviewPanel" layout="sys.ui.tabpanel.list"  cfg-router="true">
			...
			<list:criteria id="criteria1">
			    <list:tab-criterion title="" key="docStatus">
			   		 <list:box-select>
			   		 	<!-- 添加id="feedbackPanel"标记（流程反馈）菜单-#130243 -->
			   		 	<list:item-select type="lui/criteria/select_panel!TabCriterionSelectDatas" id="feedbackPanel" cfg-enable="false" cfg-required="true"  cfg-if="param['doctype'] == 'feedback'">
							<ui:source type="Static">
			     ...
			     <list:tab-criterion title="" key="docStatus">
			   		 <list:box-select>
			   		 	<!-- 添加id="examinePanel"标记（流程审核）菜单-#131393 -->
			   		 	<list:item-select type="lui/criteria/select_panel!TabCriterionSelectDatas" id="examinePanel" cfg-enable="false" cfg-required="true"  cfg-if="(param['doctype'] == 'examine' )&& param['docStatus'] !='00' && param['docStatus'] !='32' ">
							<ui:source type="Static">
				...
				<div style="float:right">
					<div style="display: inline-block;vertical-align: middle;">
						<ui:toolbar id="Btntoolbar">
							...
							<kmss:authShow roles="ROLE_GEESUNBASE_PAYING">
							 	<ui:button text="${lfn:message('geesun-base:geesunBasePay.button.payConfirm')}" id="payConfirm" onclick="window.moduleAPI.kmReview.pay_confirm();" order="1" cfg-if="param['mydoc'] == 'payConfirm'" ></ui:button>
							</kmss:authShow>
   	...
   	<template:replace name="script">
      	...
      	<script type="text/javascript">
      		...
      		seajs.use(['lui/framework/module','lui/criteria/criterion_calendar_init_date'],function(Module, init_date){
          		...
          		var hasBatchAudit = "${hasBatchAudit}";
      	...
      	<script type="text/javascript" src="${LUI_ContextPath}/km/review/resource/js/index.js?s_cache=${ LUI_Cache }"></script>
	</template:replace>
