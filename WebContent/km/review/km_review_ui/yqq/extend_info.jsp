...
<script type="text/javascript">
	...
	function dialogSelect(mul, idField, nameField,targetWin){
		seajs.use(['lui/jquery','lui/dialog','lui/dialog_common','lui/util/str'], function($, dialog, dialogCommon,strutil){
			var url = "/elec/yqqs/elec_yqqs_acc_info/elecYqqsAccInfo.do?method=accoutList&moduleSource=km-Review&fdUserName=${fdSigner.fdName}";
	...
	function sendYqq(){
		seajs.use(['lui/jquery','lui/dialog','lui/dialog_common','lui/util/str'], function($, dialog, dialogCommon,strutil){
			...
			if(phone==''||phone==null){
				...
				var ajaxUrl = Com_Parameter.ContextPath+'km/review/km_review_main/kmReviewMain.do?method=sendYqq';
				$.ajax({
					url : ajaxUrl,
					type : 'post',
					data : {
					...
					success : function(data) {
						...
						if(data){
							...
							if("true"==yqqSendCode){
								var url=Com_Parameter.ContextPath+'km/review/km_review_ui/yqq/yqq_loading.jsp?signId='+signId;
...
<div align="left">
	...
	<c:import url="/sys/attachment/sys_att_main/sysAttMain_view.jsp" charEncoding="UTF-8">
		...
		<c:param name="fdModelId" value="${kmReviewMain.fdId}" />
	</c:import>	
</div>
