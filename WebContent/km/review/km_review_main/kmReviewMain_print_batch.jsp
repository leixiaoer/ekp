	...
	<ui:toolbar  id="toolbar" layout="sys.ui.toolbar.float" count="9"> 
			<ui:button  id="zoomIn" text="${ lfn:message('km-review:button.zoom.in') }"   onclick="ZoomFontSize(2);">
		     ...
		     <ui:button id="zoomOut" text="${ lfn:message('km-review:button.zoom.out') }"   onclick="ZoomFontSize(-2);">
	 	   	 ...
	 	   	 <ui:button text="${ lfn:message('km-review:button.pageBreak') }"  onclick="ShowToPageBreak(event);">
	  		 </ui:button>
		    ...
		    <c:if test="${fn:length(kmReviewFormList) >0 }">
		     <ui:button text="${ lfn:message('km-review:button.printConfig') }"   onclick="ShowPrintList();">
		    </ui:button>
		    </c:if>
		    <%--
...
<c:forEach items="${kmReviewFormList }" var="kmReviewMainFormTemp" varStatus="vstatus">
...
<div>
	...
	<table class="tb_normal" width=100%>
		...
		<tr>
			...
			<td colspan=3><bean:write name="kmReviewMainFormTemp"
...
<div>
	...
	<c:if test="${kmReviewMainFormTemp.fdUseForm == 'false'}">
		<table name="info_content" class="tb_normal" width=100% >
			<tr>
				<td colspan="4">
					${kmReviewMainFormTemp.docContent}
				</td>
			</tr>
			<tr>
				...
				<td colspan=3>
					<c:import url="/sys/attachment/sys_att_main/sysAttMain_view.jsp" charEncoding="UTF-8">
						...
						<c:param name="fdModelId" value="${kmReviewMainFormTemp.fdId}"></c:param>
					</c:import>
				</td>
			</tr>
		</table>
	</c:if>
	<c:if test="${kmReviewMainFormTemp.fdUseForm == 'true' || empty kmReviewMainFormTemp.fdUseForm}">
		<table name="info_content" width=100% >
			<tr>
				<td name="_xform_detail">
					<ui:iframe cfg-dyniframe="true" src="${KMSS_Parameter_ContextPath}km/review/km_review_main/kmReviewMain.do?method=printBatch&fdId=${kmReviewMainFormTemp.fdId}&getXForm=1"></ui:iframe>
				</td>
			</tr>
		</table>
	 </c:if>
...
//清除表单信息
request.removeAttribute("kmReviewMainForm"); 
%> 
</kmss:auth>
</c:forEach>
...
<c:if test="${fn:length(printSubList) >0 }">
	<div class="batchPrintTip">
		<div><span style="font-weight: 900;font-size: 16px; color: #FF6000;text-align: left;"><bean:message bundle="km-review" key="kmReviewMain.printBatch.subject.notSurport" /></span></div>
		<span>
