...
function commitMethod(commitType, saveDraft){
	...
	$.post(url,$.param({"fdType":"${kmOitemsBudgerApplicationForm.fdType}"},false),function(results){
		if(results == null || results.length == 0){
			dialog.alert("${lfn:message('km-oitems:kmOitems.msg.kmOitemsTemplet')}");
		}else{
...
<c:if test="${param.approveModel ne 'right'}">
		<form name="kmOitemsBudgerApplicationForm" method="post" action ="${KMSS_Parameter_ContextPath}km/oitems/km_oitems_budger_application/kmOitemsBudgerApplication.do">
	</c:if>	
<p class="txttitle">
					...
					<c:choose>
						<c:when test="${kmOitemsBudgerApplicationForm.fdType eq '1'}">
							<td width=85% colspan="4">
								<c:choose>
									<c:when test="${kmOitemsBudgerApplicationForm.docStatus < '30' || kmOitemsBudgerApplicationForm.docStatus==null }">
										<div id="selectDiv">
										</div>
									</c:when>
									<c:otherwise>
		...
		function initKmOitemsType(val){
			...
			$.post(url,$.param({"fdType":val},true),function(results){
				...
				if(results.length ==1){
					...
					if('edit'=='${kmOitemsBudgerApplicationForm.method_GET}'){
						selectObj = '<select name="fdTemplateId" disabled="true" onchange="if(!onChangeKmOitemsTemplate(this.value))return;">'
					}
					var optionHtml = '';
...
<c:if test="${param.approveModel eq 'right'}">
		<template:replace name="barRight">
			<ui:tabpanel layout="sys.ui.tabpanel.vertical.icon" id="barRightPanel">
				<c:if test="${kmOitemsBudgerApplicationForm.docStatus>='30' || kmOitemsBudgerApplicationForm.docStatus=='00'}">
					<c:import url="/km/oitems/km_oitems_budger_application_ui/kmOitemsBudgerApplication_viewBaseInfoContent.jsp" charEncoding="UTF-8"></c:import>
				</c:if>
				<%--流程--%>
				<c:import url="/sys/workflow/import/sysWfProcess_edit.jsp" charEncoding="UTF-8">
					<c:param name="formName" value="kmOitemsBudgerApplicationForm" />
