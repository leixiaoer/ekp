...
<script type="text/javascript">
	...
	function checkFile(){
		...
		//添加参数:是否更新存在数据
		var form=document.getElementsByName("kmOitemsListingForm")[0];
	...
	function resetResult(){
		...
		uploadProcess.innerHTML="<bean:message bundle='km-oitems' key='kmOitems.title.uploadNotDo'/>";
...
<html:form action="/km/oitems/km_oitems_listing/kmOitemsListing.do?method=saveExcel" enctype="multipart/form-data"
	onsubmit="return checkFile();">

	<p class="txttitle"><bean:message bundle="km-oitems" 	key="kmOitemsWarehousingRecord.upload.file" /></p>
	<br />
	<center>
		<table class="tb_normal" width=60%>
			<tr>
				<td class="td_normal_title" width=15%>
					<bean:message  bundle="km-oitems" key="kmOitemsWarehousingRecord.file" />
				</td>
				<td width=85%><html:file property="file" style="width:90%" styleClass="inputsgl"/><br>
				<font color="red">
					<b><bean:message bundle="km-oitems" key="kmOitems.upload.note" /></b>
				</font>
				</td>
			</tr>
			<tr>
				<td  width=15%>
					<bean:message bundle="km-oitems" key="kmOitems.title.select" />
				</td>
				<td width=15%>
					<label><input type="checkbox" name="isAdd"  /></label>
					<bean:message bundle="km-oitems" key="kmOitems.tip.select" />
				</td>
			</tr>
			...
			<tr>
				<td colspan="2" >
					<bean:message bundle="km-oitems" key="kmOitems.tip.upload" />
				</td>
			</tr>
			...
			<tr>
				<td colspan="2">
					...
					<table class="tb_noborder" width="90%">
						...
						<tr height="25px">
							<td class="msglist">
								<bean:message bundle="km-oitems" key="kmOitems.title.successCount"/>
								<span id="div_successCount">${result.successCount}</span>
							</td>
							<td class="msglist" align="left">
									<bean:message bundle="km-oitems" key="kmOitems.title.ignoreCount"/>
									<span id="div_ignoreCount">${result.ignoreCount}</span>
							</td>
							<td class="msglist" align="left">
									<bean:message bundle="km-oitems" key="kmOitems.title.failCount"/>
		...
		<div style="padding-top:20px">
			<input name="submit" type="submit" value="<bean:message bundle="km-oitems" key="kmOitems.btn.upload" />">
			&nbsp;&nbsp;&nbsp;&nbsp;
