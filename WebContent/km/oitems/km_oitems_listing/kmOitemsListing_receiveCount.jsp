...
function setType(obj){
	...
	if( 1 == fdType.value ){
		...
		deptSelector.onclick = function(){
			Dialog_Address(true, 'fdDeptId','fdDeptName', ';',ORG_TYPE_PERSON|ORG_FLAG_AVAILABLEALL)
			};
		}else{
			fdDeptId.value = "";
			fdDeptName.value = "";
...
function checkContainPerson(){
	var deptSelector = document.getElementById("deptSelector");
	var fdType = document.getElementById("fdType");
    if(2== fdType.value){
		var containPersonFlag = document.getElementById("containPerson").checked;
		if(containPersonFlag){
			deptSelector.onclick = function(){
...
function doCount(){
    ...
    if(!fdDeptId.value||fdDeptId.value ==''){
    	dialog.alert('<bean:message bundle="km-oitems" key="kmOitemsMonthReport.noNull.dept"/>');
              return false;
          }
	
	var url = "KmOitemsCountListingService"
...
function afterCountListing(rtnData){
	if(rtnData.GetHashMapArray().length >= 1){
		...
		if(result['result']&&'success'==result['result']){
			    dialog.alert('<bean:message bundle="km-oitems" key="kmOitemsMonthReport.success"/>');
			}else{
				dialog.alert('<bean:message bundle="km-oitems" key="kmOitemsMonthReport.failure"/>');
				}
		}
}
...
<ui:tabpanel id="kmOitemsReceiveCountPanel" layout="sys.ui.tabpanel.list" >
	<ui:content id="kmOitemsReceiveCountContent" title="${ lfn:message('km-oitems:kmOitems.tree.receive.report') }">
		<div style="height:400px;padding:20px">
		<p class="txttitle">
		<bean:message bundle="km-oitems" key="kmOitems.tree.receive.report"/>
		</p>
		<center>
		<table class="tb_normal" width=70%>
			...
			<td class="td_normal_title" width=12% align="center">
					<bean:message bundle="km-oitems" key="kmOitemsMonthReport.docSubject"/></td>
			<td width="35%" colspan="4">
			<input type="text" id="docSubject" name="docSubject" class="inputsgl" style="width: 70%"><bean:message bundle="km-oitems" key="kmOitemsMonthReport.remark"/>
			</td>
			</tr>
			<tr>
			    <!-- 领用年份 -->
				<td class="td_normal_title" width=12% align="center">
					<bean:message bundle="km-oitems" key="kmOitemsMonthReport.year"/>
			...
			<tr>
			   <!-- 领用月份 -->
				<td class="td_normal_title" width=12% align="center">
					<bean:message bundle="km-oitems" key="kmOitemsMonthReport.month"/>
				</td><td width="35%" colspan="4">
			...
			<tr>
				<td class="td_normal_title" width=12% align="center">
					<bean:message bundle="km-oitems" key="kmOitemsMonthReport.receiveType"/>
				</td><td width="35%" colspan="4">
				    <div style="float:left">
				     ...
				     <bean:message bundle="km-oitems" key="kmOitemsMonthReport.receiveType.person"/>
					 <input type="radio" name="type" value="2"  onclick="setType(this);"/>
					 <bean:message bundle="km-oitems" key="kmOitemsMonthReport.receiveType.dept"/>
					</div>
					 <div id="containPersonTr" style="float:left;padding-left:12px;display: none">
						<input type="checkbox" id="containPerson" onclick="checkContainPerson()"  disabled="disabled">
						<bean:message bundle="km-oitems" key="kmOitemsMonthReport.isContainPerson"/>
					</div>
				</td>
			</tr>
			<tr>
			    <!-- 选择部门 -->
				<td class="td_normal_title" width=12% align="center">
					<bean:message bundle="km-oitems" key="kmOitemsMonthReport.receiveDept"/>
				</td><td width="35%" colspan="4">
					...
					<a href="#" style="cursor: hand; text-decoration: underlinel;" id="deptSelector"
					   onclick="Dialog_Address(true, 'fdDeptId','fdDeptName', ';',ORG_TYPE_PERSON|ORG_FLAG_AVAILABLEALL);"> 
			...
			<tr>
				...
				<td width=10% colspan="5" align="center">
					<ui:button text="${lfn:message('km-oitems:kmOitems.tree.reporting') }" order="2" onclick="doCount();">
				    </ui:button>
				</td>    
			</tr>
		</table>
