define([ "dojo/_base/declare", "dijit/_WidgetBase", "dijit/_Contained",
	...
	return declare("km.oitems.mobile.js.OitemSelectorListItem", [ WidgetBase, Contained, Container ], {
		...
		renderItem: function(){
			...
			if(ctx.type == ctx.TYPE_OITEM || ctx.type == ctx.TYPE_RECORD) {
				domCtr.create('div', {
					className: 'muiCateClick muiCateSelArea',
					innerHTML: '<div class="muiCateSel' + (ctx.isMul ? ' muiCateSelMul' : '') + '"></div>'
				}, muiCateContainer);
			}
			
			domCtr.create('div', {
				className: 'muiCateClick muiCateInfo',
				innerHTML: '<div class="muiCateName">' + ctx.text + '</div>'
			}, muiCateContainer);

			if(ctx.type == ctx.TYPE_CAT) {
				domCtr.create('div', {
					className: 'muiCateClick muiCateMore',
