...
<c:set var="kmOitemsBudgerApplicationForm" value="${requestScope[param.formBeanName]}" />
		<div class="muiFlowInfoW muiFormContent">
				<div class="muiFormSubject">
						<xform:text property="docSubject"></xform:text>
				</div> 
				 <c:if test="${kmOitemsBudgerApplicationForm.docStatus eq '30'}">
                 ...
                 <c:if test="${kmOitemsBudgerApplicationForm.docStatus eq '00'}">
					...
					<tr>
						<td rowspan="2" class="muiFlowIconTd">
							 <div class="muiProcessIcon">
			                    <img class="muiProcessImg" src='<person:headimageUrl contextPath="true"  personId="${kmOitemsBudgerApplicationForm.docCreatorId}" size="m" />'/>
			                </div>
						...
						<td class="muiFlowSummary">
							<div>${lfn:message('km-oitems:kmOitems.tree.application.type') }：<sunbor:enumsShow value="${kmOitemsBudgerApplicationForm.fdType}" enumsType="kmOitemsBudgerApplication_fdType" /></div>
							<div>${lfn:message('km-oitems:kmOitemsBudgerApplication.creator.dept') }： <xform:text property="docDeptName"></xform:text></div>
							<div>${lfn:message('km-oitems:kmOitemsBudgerApplication.docCreateTime') }： <xform:text property="docCreateTime"></xform:text></div>
						</td>
					</tr>
				</table>
			</div>
