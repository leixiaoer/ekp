...
define(	[
		...
		"dijit/registry", "dojo/window", "mui/i18n/i18n!sys-evaluation"
			...
			return declare("km.supervise.mixin._RemarkTabBarButtonNewMixin", null, {
				...
				scoreSelected : function(evt) {
					if (evt != null) {
						var val = evt.value;
						// 设置隐藏域分数值
						query('[name="fdFinishLevel"]')[0].value = parseInt(val);
						// 填写默认点评回复
				...
				doSubmit : function(evt) {
					...
					var promise = request
							.post(
									util
											.formatUrl("/km/supervise/km_supervise_main_remark/kmSuperviseMainRemark.do?method=save&fdMainId="+this.fdId),
									{
										data : fromData,
										timeout : 30000
									});
