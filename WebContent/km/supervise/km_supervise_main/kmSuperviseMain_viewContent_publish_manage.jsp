...
<%@ include file="/sys/ui/jsp/common.jsp"%>
	...
	<!-- 任务反馈情况统计及记录 -->
   	<div class="lui_supervise_tabpage_head lui_tabpage_float_header_title">
   		...
   		<div class="lui_supervise_tabpage_float_header_close" id="taskAndBackClose" onclick="superviseClose('taskAndBackContent','taskAndBackClose')" title="${lfn:message('km-supervise:py.open') }"></div>
   	</div>
    ...
    <div class="lui_supervise_tabpage_head lui_tabpage_float_header_title">
    	...
    	<div class="lui_supervise_tabpage_float_header_close" id="allBackClose" onclick="superviseClose('allBackFrame','allBackClose')" title="${lfn:message('km-supervise:py.open') }"></div>
	...
	<script>
		function superviseClose(contentId,closeId){
			var content = document.getElementById(contentId);
			var close = document.getElementById(closeId);
			var display = content.style.display;
			if(display == 'none'){
				content.style.display = 'block';
				close.title = '<bean:message bundle="km-supervise" key="py.close"/>';
			}else{
				content.style.display = 'none';
				close.title = '<bean:message bundle="km-supervise" key="py.open"/>';
			}
		}
		
		window.onload = function(){
			setTimeout(function(){
						superviseClose('taskAndBackContent','taskAndBackClose');
						superviseClose('allBackFrame','allBackClose');
					}, 2000);
