...
<script type="text/javascript">
	...
	function upload(){
		...
		if(file[0].value==null || file[0].value.length==0){
			...
			uploadProcess.innerHTML="<font color='blue'><bean:message bundle='km-supervise' key='kmSuperviseMain.title.uploadDoing'/></font>";
			var form=document.getElementsByName("kmSuperviseMainForm")[0];
	...
	function createTable(json) {
		...
		if(otherErrors && otherErrors.length>0) {
			...
			htmlStr += "<tr><td "+colspanStr+"><p><b><bean:message bundle='km-supervise' key='kmSuperviseMain.import.otherErrors'/></b></p>";
	...
	function resetResult(){
		var uploadProcess=document.getElementById("div_uploadProcess");
		uploadProcess.innerHTML="<bean:message bundle='km-supervise' key='kmSuperviseMain.title.uploadNotDo'/>";
...
<form name="downloadForm" action="${LUI_ContextPath }/km/supervise/km_supervise_main/kmSuperviseMain.do" method="post" target="_blank">
	<input type="hidden" name="docTemplate" value="${HtmlParam.docTemplate }"/>
</form>
<html:form action="/km/supervise/km_supervise_main/kmSuperviseMain.do?method=importSupervise&docTemplate=${HtmlParam.docTemplate }" 
	enctype="multipart/form-data"	onsubmit="return validateForm(this);">
	<iframe name="file_frame" style="display:none;"></iframe>	
<div id="optBarDiv">
	<input type="button" value="<bean:message bundle="km-supervise" key="kmSuperviseMain.import.template.download"/>" onclick="Com_SubmitNoEnabled(document.downloadForm,'downloadTemplate');">
	<input type="button" value="<bean:message bundle="km-supervise" key="kmSuperviseMain.button.upload"/>" onclick="upload();">
...
<p class="txttitle"><bean:message  bundle="km-supervise" key="kmSuperviseMain.dataImport"/></p>
<hr>
<table style="width:100%;">
	<tr>
		...
		<td>
			<div style="position:relative;height:34px;">
				<a href="javascript:void(0);" class="blueButton"><bean:message bundle="km-supervise" key="kmSuperviseMain.import.selectFile"/></a>  
	...
	<tr>
		<td colspan="2" style="line-height:28px;">
			<bean:message bundle="km-supervise" key="kmSuperviseMain.tip.uploadResult" /><br />
			<table class="tb_noborder" width="100%">
				<tr height="25px">
					<td class="msglist" colspan="3">
						<bean:message bundle="km-supervise" key="kmSuperviseMain.title.uploadProcess"/>
						<span id="div_uploadProcess">
								<bean:message bundle="km-supervise" key="kmSuperviseMain.title.uploadNotDo"/>
						</span>
					</td>
				</tr>
				<tr height="25px">
					<td class="msglist" colspan="3">
						...
						<bean:message bundle="km-supervise" key="kmSuperviseMain.title.detail"/>
