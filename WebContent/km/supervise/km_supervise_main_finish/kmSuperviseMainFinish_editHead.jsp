...
<%@ include file="/sys/ui/jsp/common.jsp"%>
	<template:replace name="head">
        ...
        <script type="text/javascript">
	        ...
	        function commitMethod(commitType, saveDraft){
				var formObj = document.kmSuperviseMainFinishForm;
	...
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float">
			<c:if test="${kmSuperviseMainFinishForm.method_GET=='edit' && kmSuperviseMainFinishForm.docStatus=='10'}">
				<ui:button text="${ lfn:message('button.savedraft') }" order="2" 
						onclick="commitMethod('save','true');">
				</ui:button>
			</c:if>
			<c:if test="${kmSuperviseMainFinishForm.method_GET=='edit' && kmSuperviseMainFinishForm.docStatus=='11'}">
				<ui:button text="${ lfn:message('button.savedraft') }" order="2" 
						onclick="commitMethod('save','true');">
				</ui:button>
			</c:if>
			<c:if test="${kmSuperviseMainFinishForm.method_GET=='edit'&&(kmSuperviseMainFinishForm.docStatus=='10'
						||kmSuperviseMainFinishForm.docStatus=='11'||kmSuperviseMainFinishForm.docStatus=='20')}">
				<c:choose>
			...
			<c:if test="${kmSuperviseMainFinishForm.method_GET=='add'}">
				<ui:button text="${ lfn:message('button.savedraft') }" order="2"  
						onclick="commitMethod('save','true');">
				</ui:button>
				<c:choose>
