...
<script>
	function checkOK(){
		...
		if(control_width&&!/^\d+%$|^\d+px$/g.test(control_width)){
			alert('<bean:message bundle="km-supervise" key="kmSupervise.fieldLayout.validate_width" />');
			return false;
		}
		var control_height=$("#control_height").val();
		if(control_height&&!/^\d+%$|^\d+px$/g.test(control_height)){
			alert('<bean:message bundle="km-supervise" key="kmSupervise.fieldLayout.validate_height" />');
			return false;
		}
		var control_content=$("#control_content").val();
		if(control_content&&!/^\d+$/g.test(control_content)){
			alert('<bean:message bundle="km-supervise" key="kmSupervise.fieldLayout.validate_content" />');
			return false;
		}
		return true;
	}
