        	...
        	<div class="lui_task_status_help">
		       <div class="lui_task_status_help_pop">
		           <div class="lui_task_status_item">
		           	<span class="lui_task_label">拖动可排序</span>
		           </div>
		       </div>
		   </div>
        </div>
    	...
    	<table id="TABLE_DocList" tbdraggable="true">
			...
			<tr KMSS_IsReferRow="1" style="display: none" align="center" class="task_add_class">
				<td >
					...
					<xform:text property="fdItems[!{index}].docSubject" style="width:80%" required="true"/>
				</td>
				<td>
					<xform:textarea property="fdItems[!{index}].docContent" style="width:85%" htmlElementProperties="data-actor-expand='true'" required="true"/>
				</td>
				<td>
				...
				<td>
					<xform:address propertyId="fdItems[!{index}].fdSponsorId" propertyName="fdItems[!{index}].fdSponsorName" orgType="ORG_TYPE_PERSON"  style="width:90%;" required="true"/>
				</td>
				<td>
					<c:choose>
						<c:when test="${kmSuperviseMainForm.fdSysUnitEnable eq 'true' }">
			...
			<c:forEach items="${kmSuperviseMainForm.fdItems}"  var="fdItem" varStatus="vstatus">
				<c:choose>
					...
					<td >
						...
						<xform:text property="fdItems[${vstatus.index}].docSubject" style="width:80%" required="true"/>
					</td>
					<td>
						<xform:textarea property="fdItems[${vstatus.index}].docContent" style="width:85%" htmlElementProperties="data-actor-expand='true'" required="true"/>
					</td>
					<td>
					...
					<td>
						<xform:address propertyId="fdItems[${vstatus.index}].fdSponsorId" propertyName="fdItems[${vstatus.index}].fdSponsorName" orgType="ORG_TYPE_PERSON" required="true" style="width:90%;" />
					</td>
					<td>
						<c:choose>
							<c:when test="${kmSuperviseMainForm.fdSysUnitEnable eq 'true' }">
					...
					<td >
						...
						<html:hidden property="fdItems[${vstatus.index}].fdOriginId" value="${fdItem.fdOriginId}"/>
						<html:hidden property="fdItems[${vstatus.index}].fdOrder" value="${vstatus.index}"/>
						<xform:text property="fdItems[${vstatus.index}].docSubject" style="width:80%" showStatus="readOnly"/>
					</td>
					<td>
						<xform:textarea property="fdItems[${vstatus.index}].docContent" style="width:85%" htmlElementProperties="data-actor-expand='true'" showStatus="readOnly"/>
					</td>
					<td>
						<xform:datetime property="fdItems[${vstatus.index}].fdPlanStartTime" style="width:93%;" subject="${lfn:message('km-supervise:kmSuperviseTask.fdPlanStartTime')}" dateTimeType="datetime"  showStatus="readOnly" validators="validateTime"/>
					</td>
					<td>
						<xform:datetime property="fdItems[${vstatus.index}].fdPlanEndTime"  style="width:93%;" subject="${ lfn:message('km-supervise:kmSuperviseTask.fdPlanEndTime')}" dateTimeType="datetime" showStatus="readOnly" validators="validateTime"/>
					</td>
					<td>
						<c:choose>
							<c:when test="${kmSuperviseMainForm.fdSysUnitEnable eq 'true' }">
							...
							<c:otherwise>
								<xform:address propertyId="fdItems[${vstatus.index}].fdUnitId" propertyName="fdItems[${vstatus.index}].fdUnitName" orgType="ORG_TYPE_DEPT"  style="width:90%;" showStatus="readOnly"/>
							</c:otherwise>
						</c:choose>
					</td>
					<td>
						<xform:address propertyId="fdItems[${vstatus.index}].fdSponsorId" propertyName="fdItems[${vstatus.index}].fdSponsorName" orgType="ORG_TYPE_PERSON"  style="width:90%;" showStatus="readOnly"/>
					</td>
					<td>
						<c:choose>
							<c:when test="${kmSuperviseMainForm.fdSysUnitEnable eq 'true' }">
							...
							<c:otherwise>
								<xform:address propertyId="fdItems[${vstatus.index}].fdOUnitIds" propertyName="fdItems[${vstatus.index}].fdOUnitNames" orgType="ORG_TYPE_PERSON"  style="width:90%;" mulSelect="true" showStatus="readOnly"/>
							</c:otherwise>
						</c:choose>
					</td>
					<!-- 删除、上移、下移按钮 -->
		...
		<table class="tb_simple">
    		<tr>
		    	<td class="td1">
	          	...
	          	<td class="td2">
	          		<xform:select property="fdBackType" showPleaseSelect="false" showStatus="edit" style="width:12%">
						<xform:enumsDataSource enumsType="km_supervise_task_back"></xform:enumsDataSource>
					</xform:select>
					<div style="display:inline-block;margin-left:5px" class="lui_supervise_right lui_text_primary" onclick="previewBackDay()">反馈日预览<i class="lui_arrow arrow_right"></i></div>
	          	</td>
			</tr>
			<tr>
