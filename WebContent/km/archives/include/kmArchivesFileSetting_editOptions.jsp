...
<%@page import="com.landray.kmss.km.archives.service.IKmArchivesFileTemplateService"%>
...
<%
	...
	IKmArchivesFileTemplateService service = (IKmArchivesFileTemplateService)SpringBeanUtil.getBean("kmArchivesFileTemplateService");
		...
		for (int i = 0; i < modelNames.length; i++) {
			...
			while(it.hasNext()) {
				...
				if((name+":"+key).equals(selected)) {
					out.println("<option value=\""+name+":"+key+"\" selected>"+messageKey+" - "+map.get(key)+"</option>");
				}else {
					out.println("<option value=\""+name+":"+key+"\">"+messageKey+" - "+map.get(key)+"</option>");
				}
			}
		}
	} else {
