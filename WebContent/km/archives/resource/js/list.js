...
seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/dialog_common'], function($, dialog , topic, dialogCommon){
    ...
    function addSCategoryMain(){
    	dialog.simpleCategoryForNewFile(option.templateName, option.basePath + '?method=add&fdTemplateId=!{id}&.fdTemplateId=!{id}&i.docTemplate=!{id}',
    			false,null,null,getValueByHash("docTemplate"));
    }
    ...
    function deleteAll(){
        ...
        var url=option.contextPath + option.basePath+'?method=deleteall';
		var config = {
				url : url, // 删除数据的URL
				data : $.param({"List_Selected":selected},true), // 要删除的数据
				modelName : option.modelName//"com.landray.kmss.km.archives.model.KmArchivesMain" // 主要是判断此文档是否有部署软删除
			};
			// 通用删除方法
