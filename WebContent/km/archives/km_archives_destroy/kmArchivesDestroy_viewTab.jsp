...
<template:replace name="content">
    ...
    <c:choose>
		<c:when test="${param.approveModel eq 'right'}">
			<c:import url="/km/archives/km_archives_destroy/kmArchivesDestroy_viewContent.jsp" charEncoding="UTF-8">
		...
		<template:replace name="barRight">
			<c:choose>
				<c:when test="${kmArchivesDestroyForm.docStatus>='30' || kmArchivesDestroyForm.docStatus=='00'}">
					<ui:accordionpanel>
						<!-- 基本信息-->
						<c:import url="/km/archives/km_archives_destroy/kmArchivesDestroy_viewBaseInfoContent.jsp" charEncoding="UTF-8">
						</c:import>
					</ui:accordionpanel>
				</c:when>
				<c:otherwise>
					<ui:tabpanel layout="sys.ui.tabpanel.vertical.icon" id="barRightPanel" >
						...
						<c:import url="/sys/lbpmservice/support/lbpm_audit_note/lbpmAuditNote_list_content.jsp" charEncoding="UTF-8">
							<c:param name="formName" value="kmArchivesDestroyForm" />
							<c:param name="fdModelId" value="${kmArchivesDestroyForm.fdId}" />
							<c:param name="fdModelName" value="com.landray.kmss.km.archives.model.KmArchivesDestroy" />
						</c:import>
						<!-- 基本信息-->
						<c:import url="/km/archives/km_archives_destroy/kmArchivesDestroy_viewBaseInfoContent.jsp" charEncoding="UTF-8">
						</c:import>
