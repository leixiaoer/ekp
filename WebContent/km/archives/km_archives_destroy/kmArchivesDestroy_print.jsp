...
<template:replace name="title">
	<%-- <bean:message bundle="km-archives" key="kmArchivesDestroy.printDestroyList" /> --%>
</template:replace>
...
<center>
	<div id="printTable" style="border: none;padding-left:30px;">
		<div class="txttitle"> 
	      	<bean:message bundle="km-archives" key="kmArchivesDestroy.details" />
	    </div>
	    <br/>
		<table class="tb_normal" width=100%>
			...
			<tr>
				...
				<td class="td_normal_title" width=30%>
					<bean:message bundle="km-archives" key="kmArchivesDestroy.fdDestroyIdea" />
				</td>
			</tr>
			<c:forEach items="${destroyList }" var="destroy" varStatus="varStatus">
				<tr>
					<td>${varStatus.index+1 }</td>
					<td>${destroy.fdArchivesName }</td>
					<td>${destroy.fdArchivesNumber }</td>
					<td>${destroy.docCreator.fdName }</td>
					<td><kmss:showDate value="${destroy.docCreateTime }" type="datetime"></kmss:showDate></td>
					<td>${destroy.fdDestroyIdea }</td>
				</tr>
			</c:forEach>
	</div>
</center>
