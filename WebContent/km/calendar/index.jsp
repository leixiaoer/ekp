...
<template:include ref="default.list" spa="true">
	...
	<template:replace name="content">
		<input type="hidden" name="fdId" value="<c:out value="${param.fdId}"/>">
		<input type="hidden" name="r" value="<c:out value="${param.r}"/>">
		<script type="text/javascript">	
			//不允许打印alert confirm 防止xss攻击
			...
			seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar','lui/util/env','lui/dateUtil'], function($,dialog , topic ,toolbar,env,dateutil) {
				LUI.ready(function() {
						...
						$.post('<c:url value="/km/calendar/km_calendar_main/kmCalendarMain.do?method=checkCalendarIsExists"/>',
								$.param({"fdId":fdId},true),function(data){
									...
									if(data!=null && data.status==true){
										...
										dialog.iframe('/km/calendar/km_calendar_main/kmCalendarMain.do?method=notifyView&fdId=' + fdId,
				...
				topic.subscribe('calendar.select',function(arg){
					...
					var now = new Date();
					$("[name='startHour']").val(dateTime.getHours());
			 	    $("[name='startMinute']").val(dateTime.getMinutes());
		 	      //  dateTime.setHours(now.getHours()+2);
		 	       var endDateTime=new Date(end);
		 	    	...
		 	    	$("[name='endHour']").val(endDateTime.getHours());
			 	    $("[name='endMinute']").val(endDateTime.getMinutes());
					//初始化标签
					if(LUI("label_edit")){
						...
						$.param(param,true),function(data){
							...
							if(data!=null && data.status==true){
								if(data.isRecurrence==true){//重复日程拖拽更新后直接刷新整个页面
									...
									if(dateutil.parseDate(arg.start).getTime() != dateutil.parseDate(arg.end).getTime()){
										arg.schedule.isChangeEndTime = "false";
									}else{
										arg.schedule.isChangeEndTime = "true";
									}
				...
				topic.subscribe('calendar.thing.click',function(arg){
 					...
 					$("#ul_label_edit").show();
 					$("#button_save_event").show();
 					$("#button_delete_event").show();
 					$("#button_view_detail").show();
					//题头:群组日程、我的日程
					...
					if("note"!=arg.schedule.type){
						...
						if(arg.schedule.content){
		                	$("#calendar_content").text(arg.schedule.content);
		                	$("#tr_content").show();
		                }else{
		                	$("#tr_content").hide();
		                }
						...
						if(arg.schedule.fdLocation){
		                	$("#calendar_location").text(arg.schedule.fdLocation);
		                	$("#tr_location").show();
		                }else{
		                	$("#tr_location").hide();
		                }
		              ...
		              //初始化提醒
						...
						if(arg.schedule.end!=null){
							var date = arg.schedule.end;
							console.log(date);
							if(arg.schedule.isChangeEndTime == "true"){
								var datetime =dateutil.parseDate(date).getTime();
								datetime = datetime - (60*1000);
								date = new Date(datetime);
							}
							console.log(date);
							DateString+="-"+kmcalendarDateUtil.formatDate(date,formatDate);
							}
