...
<template:include ref="mobile.edit" compatibleMode="true">
	...
	<xform:config orient="vertical">
		<html:form action="/km/calendar/km_calendar_main/kmCalendarMain.do" >
			...
			<html:hidden property="createdFrom" />
			
			<%--kk集成转日程 --%>
			<%@ include file="/km/calendar/mobile/kk/edit_kkinfo.jsp"%>
			...
			<div class="gray" data-dojo-type="mui/view/DocScrollableView" id="scrollView" data-dojo-mixins="mui/form/_ValidateMixin">
				<div class="muiImportBox">
					<div data-dojo-type="mui/form/Textarea" class="docSubjectTextArea"
						data-dojo-props="name:'docSubject',value:'${lfn:escapeHtml(subject)}',showStatus:'edit',subject:'<bean:message bundle="km-calendar" key="kmCalendarMain.docContent" />',required:true,validate:'required maxLength(500)',opt:false,orient:'vertical'"
						>
					</div>
					<table class="muiSimple" cellpadding="0" cellspacing="0">
						<tr>
							<td>
								<div data-dojo-type="mui/form/Input" class="locationInput"
									data-dojo-props="name:'fdLocation',value:'${lfn:escapeHtml(kmCalendarMainForm.fdLocation) }',showStatus:'edit',subject:'<bean:message bundle="km-calendar" key="kmCalendarMain.fdLocation" />',validate:'maxLength(200)',opt:false,orient:'vertical'">
								</div>
							</td>
						</tr>
						<tr>
							<td>
								<span id="ownerTip" style="padding-left: 1rem;color: red;display: none;">
									<bean:message bundle="km-calendar" key="kmCalendarMain.docOwner.tip" />
								</span>
								<%--所有者--%>
								<div data-dojo-type="mui/form/Select" id="docOwnerId"
								    data-dojo-props="name:'docOwnerId',mul:false,showStatus:'edit',subject:'<bean:message bundle="km-calendar" key="kmCalendarMain.docOwner" />',value:'${kmCalendarMainForm.docOwnerId}',showPleaseSelect:true,orient:'vertical',
										 store:[<c:forEach var="owner" items="${owners}" varStatus="vs">
													{text:'${owner[1] }',value:'${owner[0]}'}
												...
												<c:if test="${kmCalendarMainForm.method_GET == 'addEvent' }">
													<kmss:authShow roles="ROLE_KMCALENDAR_MULTI_CREATE">
														,{text:'<bean:message bundle="km-calendar" key="kmCalendarShareGroup.fdGroup"/>',value:'multiCreate'}
													</kmss:authShow>
											</c:if>]">
								</div>
							</td>
						...
						<tr>
							<td id="docOwnerIds" style="display:none;">
								<%--所有者(发起多人日程时以地址本选择)--%>
								<xform:address isLoadDataDict="false" mobile="true"
									showStatus="edit" propertyId="docOwnerIds"
									propertyName="docOwnerNames" orgType='ORG_TYPE_ALL'
									subject="${lfn:message('km-calendar:kmCalendarShareGroup.fdGroupMemberIds')}" mulSelect="true"></xform:address>
							</td>
						</tr>
						<tr>
							<td>
								<xform:radio property="fdAuthorityType" showStatus="edit" onValueChange="authorityTypeSelect" mobile="true">
						...
						<tr>
							<td class="event_auth">
								<xform:address htmlElementProperties="id='authReader'" showStatus="edit" propertyId="authReaderIds" propertyName="authReaderNames" orgType="ORG_TYPE_ALL" mulSelect="true" textarea="true" mobile="true" subject="${lfn:message('km-calendar:kmCalendarMain.authReader')}"></xform:address>
							</td>
						</tr>
						<tr>
							<td class="event_auth">
								<xform:address htmlElementProperties="id='authEditor'" showStatus="edit" propertyId="authEditorIds" propertyName="authEditorNames" orgType="ORG_TYPE_ALL" mulSelect="true" textarea="true" mobile="true" subject="${lfn:message('km-calendar:kmCalendarMain.authEditor')}"></xform:address>
							</td>
						</tr>
					</table>
				</div>
				...
				<div id="recurrenceView" data-dojo-type="dojox/mobile/View">
					...
					<div class="moreset">
						<div data-dojo-type="mui/form/Select"
							 data-dojo-props="name:'RECURRENCE_WEEKS',showStatus:'edit',subject:'<bean:message bundle="km-calendar" key="kmCalendarMain.recurrenceTimeType" />',value:'${kmCalendarMainForm.RECURRENCE_WEEKS }',showPleaseSelect:false,orient:'vertical',required:true,
				...
				<div id="labelView" data-dojo-type="dojox/mobile/View">
					<div class="muiCalendarLabelContainer">
						<div data-dojo-type="mui/form/Select"
							 data-dojo-props="name:'labelId',mul:false,showStatus:'edit',subject:'<bean:message bundle="km-calendar"  key="kmCalendarMain.docLabel"/>',value:'${kmCalendarMainForm.labelId eq 'myEvent' ? "" : kmCalendarMainForm.labelId}',showPleaseSelect:false,orient:'vertical',
							 store:[<c:forEach var="label" items="${labels}" varStatus="vs">
										{text:'${label[1] }',value:'${label[0]}'}
										<c:if test="${ vs.last==false }">,</c:if>
									</c:forEach>]" id="labelId">
						</div>
					</div>
				</div>
				...
				<ul data-dojo-type="mui/tabbar/TabBar" fixed="bottom" data-dojo-props='fill:"grid"'>
				  	...
				  	<c:if test="${ kmCalendarMainForm.method_GET == 'addEvent' }">	
						<li data-dojo-type="mui/tabbar/TabBarButton" class="muiBtnSubmit " 
						  	data-dojo-props='colSize:2,transition:"slide"' onclick="commitMethod('save')">
						  		<bean:message  key="button.submit"/>
						 </li>
					 </c:if>
					 <c:if test="${ kmCalendarMainForm.method_GET == 'edit'  }">	
					 <li data-dojo-type="mui/tabbar/TabBarButton" class="muiBtnSubmit " 
					  	data-dojo-props='colSize:2,transition:"slide"' onclick="commitMethod('update')">
...
require(['dojo/query','mui/util','dojo/topic','dijit/registry','dojo/ready','dojo/dom-style','dojo/dom-geometry','mui/dialog/Tip','mui/calendar/CalendarUtil','dojo/date/locale','mui/device/adapter','dojo/request','dojo/dom-attr',"mui/form/ajax-form"],
		function(query,util,topic,registry,ready,domStyle,domGeometry,Tip,cutil,locale,adapter,request,domAttr,ajaxForm){
	window.authorityTypeSelect=function(value){
	  ...
	  //隐藏公开属性
	    var publicNode = query('input[name="fdAuthorityType_group"][value="PUBLIC"]');
	    if(publicNode && publicNode.length>0){
	    	var p = publicNode.parent();
	    	domStyle.set(p[0],'display','none');
		}
	};
	var labelIdObj;
	var allLabelArr;
	...
	ready(function(){
		...
		var intStartHour,intEndHour;
			...
			if(date.getHours() == 23){
				var nextTime = cutil.parseDate(startTimeWidget.value); 
				nextTime.setDate(nextTime.getDate()+1);
				startTimeWidget.set("value",locale.format(nextTime,{
					selector : 'time',
					timePattern : "${ lfn:message('date.format.date') }"
				}));
				finishTimeWidget.set("value",locale.format(nextTime,{
					selector : 'time',
					timePattern : "${ lfn:message('date.format.date') }"
				}));
				intStartHour = '00';
		...
		//如果是同一天，最后一个小时是24.则把时间设置成23:59
		if(startTimeWidget && finishTimeWidget){
			if(startTimeWidget.value ==finishTimeWidget.value){
				if(endHour ==24){
					endHour =23;
					endMinute =59;
				}	
	...
	window.commitMethod=function(method){
		...
		if(___validate()){
			if(method == 'save'){
				...
				Com_Submit(document.forms[0],'updateEvent');
		}
	...
	function addCalendarToAdapter(){
		...
		var webCallback = function(){
			var url = Com_GetCurDnsHost()+'${LUI_ContextPath}/km/calendar/mobile/index.jsp';
			window.open(url, '_top');
		}
		var options = {
			title : title,
			loc : loc,
			webAdapterCallback : webCallback,
			notes : notes,
	...
	function ___validate(){
		...
		var muiAgendaNormalTr = query(".muiAgendaNormalTr");
		muiAgendaNormalTr.forEach(function(node,index){
			var fdBeforeTime = document.getElementsByName("sysNotifyRemindMainContextForm.sysNotifyRemindMainFormList["+index+"].fdBeforeTime")[0];
			var fdTimeUnit = document.getElementsByName("sysNotifyRemindMainContextForm.sysNotifyRemindMainFormList["+index+"].fdTimeUnit")[0];
			if(fdBeforeTime && fdTimeUnit) {
				fdBeforeTime=fdBeforeTime.value;
				fdTimeUnit= fdTimeUnit.value;
				if(fdTimeUnit=="minute"){
					fdBeforeTime *= 1;
				}
				if(fdTimeUnit=="hour"){
		...
		var alarms = options.alarms; //提前提醒时间（分钟）
		for(var i = 0;i < alarms.length;i++){
			if(alarms[i] !="") {
				var beforeT = alarms[i];
				var tempT = startTime - (beforeT*60000);
				if (tempT <= new Date().getTime()) { //如果提前时间早于当前时间，无法提醒
					Tip.fail({
						text: '提醒设置早于当前时间,请重新设置！'
					});
					return false;
				}
			}
	...
	//提交后返回查看页面
	/* Com_Submit.ajaxAfterSubmit=function(){ #137839 屏蔽此段代码 
		if('${ kmCalendarMainForm.method_GET}'=='edit'){
			setTimeout(function(){
 ...
 */	
	...
	function initKKIM(){
		//KK转日程
		var createdFrom = "${kmCalendarMainForm.createdFrom}";
		var methodGet = "${kmCalendarMainForm.method_GET}";
		var content = "${docSubject}";
		if(util.isExistFace(content)){
			document.getElementsByName("docSubject")[0].value='';
		}
		if(createdFrom=='KK_IM' && methodGet=='addEvent'){
			ajaxForm.ajaxForm("[name='kmCalendarMainForm']", {
	...
	//发送kk消息
	function sendKkNotify(){
		var fdId = query('input[name="fdId"]')[0].value;
		var subject = document.getElementsByName("docSubject")[0].value;;
		var fdIsAlldayevent = document.getElementsByName("fdIsAlldayevent")[0].value;
		var startT = document.getElementsByName("docStartTime")[0].value;
		var endT = document.getElementsByName("docFinishTime")[0].value;
		var startTime = document.getElementsByName("docStartHHmm")[0].value;
		var endTime = document.getElementsByName("docFinishHHmm")[0].value;
		var betweenTime = "";
		if(fdIsAlldayevent=="false"){
			betweenTime = startT==endT ? startT+" "+startTime+"~"+endTime:startT+" "+startTime+"~" + endT + " "+endTime;
		}else{
			betweenTime = startT==endT ? startT:startT+"~"+endT;
		}
		...
		var url = "/km/calendar/km_calendar_main/kmCalendarMain.do?method=view&fdId="+fdId;
		var summary = '<bean:message key="km-calendar:kmCalendarMain.docTime" />:'+betweenTime;
		var context = {
				dest:'kk',
				...
				title:'<bean:message key="km-calendar:kmCalendar.label.table.calender" />',
				bizType:2,
				summary:summary
			};
			console.log(context);
