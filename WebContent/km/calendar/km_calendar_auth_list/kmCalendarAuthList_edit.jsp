...
<template:include ref="default.edit" sidebar="no">
    ...
    <template:replace name="toolbar">
        <ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
       		<ui:button text="${ lfn:message('button.update') }" onclick="commitMethod('update');" />
			<%--关闭--%>
            ...
            <html:form action="/km/calendar/km_calendar_auth_list/kmCalendarAuthList.do">
						...
						<tr>
							...
							<td colspan="3">
								...
								<div id="_xform_fdAuthType" _xform_type="checkbox">
	                                <xform:checkbox property="fdAuthType" htmlElementProperties="id='fdAuthType'" showStatus="edit" notChangedValues="read" value="${fdAuthType }">
						...
						<tr>
							...
							<td colspan="3">
								...
								<div id="_xform_fdIsShare" _xform_type="radio">
									<xform:radio property="fdIsShare" htmlElementProperties="id='fdIsShare'" showStatus="edit" onValueChange="changeIsShare" value="${kmCalendarAuthListForm.fdIsShare ==null? 'false' :kmCalendarAuthListForm.fdIsShare}">
	                            ...
	                            <div id="_xform_fdShareDate" _xform_type="datetime" style="margin-top: 5px;display: none;">
	                                ...
	                                <xform:datetime property="fdShareDate" required="true" showStatus="edit" dateTimeType="date" style="width:20%;" />
	                            </div>
							</td>
						</tr>
					</table>
                ...
                <script>
			    	...
			    	window.onload = function(){
			    		// 初始化历史日程生效起始时间
			    		var fdIsShare = "${kmCalendarAuthListForm.fdIsShare}";
			    		var today = "${today }";
			    		if (!fdIsShare || fdIsShare == 'false') {
			    			$("input[name='fdShareDate']").val(today);
						}
			    		changeIsShare();
			    	};
