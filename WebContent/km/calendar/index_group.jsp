	...
	<c:import url="/km/calendar/import/nav.jsp" charEncoding="UTF-8">
			...
			seajs.use([
				...
				topic.subscribe('calendar.select',function(arg){
				    ...
				    if(!arg.personId){
						...
						$("#calendar_add #docOwnerName").html('<bean:message bundle="km-calendar" key="kmCalendarMain.docOwner.other" />');
						//显示群组地址本
						$("#multiOwner").show();
		...
		seajs.use(['lui/jquery','lui/topic'],function($,topic){
			...
			topic.subscribe("person_next_all",function(personIds){
				...
				$.ajax({
                    url:"${LUI_ContextPath}/km/calendar/km_calendar_share_group/kmCalendarShareGroup.do?method=listGroupSearch&groupId=${JsParam.groupId}&loadAll=true&keyword="+keyword+"&isShare=true",					success:function(response){
