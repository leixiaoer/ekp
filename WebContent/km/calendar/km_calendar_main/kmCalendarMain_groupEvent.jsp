...
<template:include ref="default.dialog">
	...
	<html:form action="/km/calendar/km_calendar_main/kmCalendarMain.do" styleId="eventform">
		...
		<table id="event_base_tb" class="tb_simple" width="100%">
			<tr>
	     		<%--日历--%>
	              ...
	              <td width="85%" colspan="3" >
	              		<bean:message bundle="km-calendar" key="kmCalendarMain.calendar.group" />
	              </td>
	         </tr>
	         <%--所有者--%>
	         <tr>
	              <td width="15%" class="td_normal_title" >
	              		<bean:message bundle="km-calendar" key="kmCalendarMain.calendar.group.docOwner" />
	              </td>
	              <td width="85%" colspan="3" >
	              		<input id="ownerIds" type="hidden" name="docOwnerIds" value="${docOwnerIds }" />
	              		<xform:textarea htmlElementProperties="id='ownerNames'" property="docOwnerNames" value="${docOwnerNames }" showStatus="readOnly" subject="成员" style="width:90%" />
	              		<br/>
	              		<c:if test="${kmCalendarMainForm.method_GET eq 'addGroupEvent' }">
		              		<div style="width: 90%;">
		              			<a onclick="Dialog_List(true,'docOwnerIds','_docOwnerNames',';','kmCalendarPersonGroupService&personGroupId=${personGroupId}');"><bean:message bundle="km-calendar" key="kmCalendarMain.calendar.group.docOwner.add" /></a>
		         				<a onclick="clearOwner();"><bean:message bundle="km-calendar" key="kmCalendarMain.calendar.group.docOwner.clear" /></a>
		              			<a onclick="addAllOwner();" style="float: right;"><bean:message bundle="km-calendar" key="kmCalendarMain.calendar.group.docOwner.addAll" /></a>
		              		</div>
	              		</c:if>
	              </td>
	         </tr>
			...
			<!-- <input type="hidden" name="authReaderIds" />
			<input type="hidden" name="authEditorIds" /> -->
...
<script>Com_IncludeFile("dialog.js|validator.jsp|validation.js|plugin.js|validation.jsp|eventbus.js|xform.js|form.js",null,"js");</script>
...
<script>
	...
	seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog,topic,toolbar) {
		...
		window.addAllOwner = function(){
			$("#ownerIds").val('${docOwnerIds}');
			$("#ownerNames").val('<c:out value="${docOwnerNames}" />');
		};
		...
		window.deleteDoc=function(){
			...
			var isGroupCalendar = '${isGroupCalendar}';
			var isGroupAdmin = '${isGroupAdmin}';
			var fdMainIds = '${fdMainIds}';
			var config = {};
			if(isGroupCalendar == 'true' && isGroupAdmin == 'true'){
				var _url ="${LUI_ContextPath}/km/calendar/km_calendar_main/kmCalendarMain.do?method=deleteGroupEvent";
				config.url = _url;
				config.type = 'POST';
				config.data = {fdMainIds:fdMainIds};
				Com_Parameter.ComfirmDelete = "<bean:message bundle='km-calendar' key='kmCalendarMain.groupEvent.delete.msg1' />";
			}else{
				var _url = "${LUI_ContextPath}/km/calendar/km_calendar_main/kmCalendarMain.do?method=delete&fdId"+fdId;
				config.url = _url;
				config.type = 'GET';
				if(isGroupCalendar == 'true')
					Com_Parameter.ComfirmDelete = "<bean:message bundle='km-calendar' key='kmCalendarMain.groupEvent.delete.msg2' />";
			}
			config.modelName = 'com.landray.kmss.km.calendar.model.KmCalendarMain';
