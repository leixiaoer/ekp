...
<script>
	...
	seajs.use(['lui/jquery','lui/dialog','lui/dateUtil'],function($,dialog,dateutil){
		var control = true;
		...
		function handler4equal(satrt,end){
			if(satrt && satrt==end){
				var datetime =dateutil.parseDate(end).getTime();
				datetime = datetime + (60*1000);
				var date = new Date(datetime);
				end = dateutil.formatDate(date,"yyyy-MM-dd HH:mm");
			}
			return end;
		};
                		...
                		<div style="float:left">
                	  	<ui:dataview id="label_edit" style="float:left;">
							...
							<ui:render type="Template">
								<c:import url="/km/calendar/tmpl/add_event_label_select.jsp" charEncoding="UTF-8"></c:import>
							</ui:render>
						</ui:dataview>
						</div>
	        	...
	        	<%--所有者--%>
					...
					<td width="80%">
						<select id="docOwnerId" name="docOwnerId" onchange="changeOwner();">
							<option value="${KMSS_Parameter_CurrentUserId}">
								<bean:message bundle="km-calendar" key="kmCalendarMain.docOwner.self" />
							</option>
							<kmss:authShow roles="ROLE_KMCALENDAR_MULTI_CREATE">
								<%--是否发起多人日程--%>
								<option class="fdGroup" value="multiCreate">
									<bean:message bundle="km-calendar" key="kmCalendarMain.docOwner.other"/>
								</option>
							</kmss:authShow>
						</select>
		         ...
		         <tr id="multiOwner" style="display: none;">
		              ...
		              <td width="85%" colspan="3" >
		              		<div _xform_type="address">
		              			<xform:dialog subject="${lfn:message('km-calendar:kmCalendarShareGroup.fdGroupMemberIds') }"  
		              			style="width:90%;" propertyId="docOwnerIds" propertyName="docOwnerNames" showStatus="edit">
