define(function(require, exports, module) {
	...
	//群组日历模式
	var PersonGroupCalendarMode=Calendar.AbstractCalenderMode.extend({
...
//							debugger;
		...
		_afterRender : function(method){
			...
			//人名弹出框定位
			//$(".person_dialog").eq(0).css("top",$(".person_dialog").eq(0).parent().height());
			//$(".person_dialog").eq(0).css("width",this.calendarDiv.width());
			
			//#144559 修复 周共享日程筛选问题
			/*topic.subscribe("changeShowPersonGroup",function(data){
				var person = data.id;
				if(data.isChecked){
			...
			this.calendarDiv.on('click','td.lui_calendar_group_date',function(evt){
				if($(evt.target).closest('.lui_calendar_person_data').get(0) || $(evt.target).closest('.lui_calendar_personGroup_data').get(0)) {
					return;
				}
...
//			debugger;
		...
		dataRender:function(data,cellObj,days){
			...
			if(data.isGroup)
				divObj = $('<div class="lui_calendar_personGroup_data"/>').append(divObj);
			else
				divObj = $('<div class="lui_calendar_person_data"/>').append(divObj);
...
//				this.calSetting.eventRender(data,cellObj);
	...
	exports.PersonGroupCalendarMode = PersonGroupCalendarMode;
