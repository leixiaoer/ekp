...
//      var solarDate = new Date(baseDate.valueOf() + offset * 86400000);
        // 由于new Date(1900, 0, 31)在不同的浏览器得到的时区可能不一样，而导致计算出来的结果有时间差，这里固定使用GMT+0800时区的结果
        var solarDate = new Date(-2206425600000 + offset * 86400000);
        return solarDate;
    }
	
	function zodiacForLunarYear(lYear) {
		return DB.chineseZodiac[Math.abs(lYear - 1984) % 12];
	}
	...
	window.Term = Term;
	window.zodiacForLunarYear=zodiacForLunarYear;
	window.getSolarDate=getSolarDate;
	
	window.leapDays=leapDays;
	window.leapMonth=leapMonth;
	window.monthDays=monthDays;
})();
