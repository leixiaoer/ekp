	...
	var CalendarPortlet = base.Container.extend({
		...
		startup : function(){
			if (!this.layout) {
				this.layout = new layout.Template({
					src : require.resolve('../layout/calendar_portlet_direction_layout.jsp#'),
					parent : this
				});
				this.layout.startup();
		...
		_bindEvent : function(){
			...
			toggleDom.click(function (e) {
		        ...
		        self.element.find('.lui_tcd_table--tr').show();
		...
		showCurrentWeekCalendar:function(){
			...
			var height = this.element.find('.lui_calendar_dirction_content')[0].style.height;
			height = height.replace('px','') || 70;
			var currentTrNode = this.element.find('.lui_tcd_table--tr .today');
			var trNodes = this.element.find('.lui_tcd_table--tr');
			if(!isSameYearMonth(currentDate,new Date())){
				currentTrNode = this.element.find('.lui_tcd_table--tr .first td');
			}
			if(height<100){
				trNodes.show();
				currentTrNode.parent('.lui_tcd_table--tr').show();
			}else{
				trNodes.show();
			}
		},
		...
		userCalendarDataRender : function(){
			...
			if(this.datas && this.datas[currentStr] && this.datas[currentStr].length > 0){
				for(var i = 0;i < this.datas[currentStr].length; i++){
					...
					$('<i class="lui_tct_ui__i" />').appendTo($li);
					...
					$content.append($li);
					$content.append($title).appendTo(footerDom);
					lastLiNode = $content;
				...
				this.element.find('.lui_calendar_portlet_dirction_data').hide();
				$ls = $("<div class='lui_calendar_portlet_dirction_nodata'><div class='lui_pce_iconStyle'>"+
				...
				var nodataNode = this.element.find('.lui_calendar_portlet_dirction_left');
		...
		hideNoDataTip :function(){
			this.element.find('.lui_calendar_portlet_dirction_data').show();
			this.element.find('.lui_calendar_portlet_dirction_nodata').hide();
		},
