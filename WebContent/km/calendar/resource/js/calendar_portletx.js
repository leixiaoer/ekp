...
define( function(require, exports, module) {
	...
	var CalendarPortlet = base.Container.extend({
		...
		startup : function(){
			if (!this.layout) {
				this.layout = new layout.Template({
					src : require.resolve('../layout/calendar_portlet_layout.jsp#'),
					parent : this
				});
				this.layout.startup();
		...
		openCalendarDialog : function(url,method,type){
			...
			var startDate = util.fn.formatDate(self.currentDate,seajs.data.env.pattern.date);
		...
		_stuff : function(){
			this._stuffHeader();//填充头部
			this._stuffWeekHeader();//填充星期栏
			this._stuffBody();//填充日期
			this._fetchData().then(function(){
				this._stuffFooter();
				this._processHasData();
			});
		...
		_stuffHeader : function(){
			var dateTextDom = this.element.find('[data-lui-mark="calendar_datetext"]'),
				myTextDom = this.element.find('[data-lui-mark="calendar_monthyeartext"]'),
			...
			if(Com_Parameter.Date_format == 'dd/MM/yyyy')
				myTextDom.text( month + '-' + year );
			else
				myTextDom.text( year + '-' + month );
		},
		
		/**
		 * 填充星期栏
		 */
		_stuffWeekHeader : function(){
		...
		_stuffFooter : function(){
			...
			if(this.datas && this.datas[currentStr] && this.datas[currentStr].length > 0){
				for(var i = 0;i < this.datas[currentStr].length; i++){
					if(i == this.rowsize) break;//只取5条
					var data = this.datas[currentStr][i],
						$li = $('<li class="lui_calendar_portlet_item" />'),
						$colorLabel = $('<div class="lui_calendar_portlet_item_color_label" />').css('background-color',data.color).appendTo($li),
						$header = $('<div class="lui_calendar_portlet_item_header" />'),
						$content = $('<div class="lui_calendar_portlet_item_content" />');
					...
					var label = $('<span class="label" />').text(labelName).css('color',data.color);
					var link = $('<a/>').attr('title',data.title).attr('target','_blank').append(label).append($('<span />').text(data.title));
					if(data.relationUrl){
					...
					$header.append($('<span class="time" />').text(date));
					$content.append(link);
					$li.append($header).append($content).appendTo(footerDom);
				}
				footerDom.css('height','auto');
			}else{
				$li = $('<li/>').addClass('lui_calendar_portlet_item_nodata');
				var $liText = $('<li/>').addClass('lui_calendar_portlet_item_nodata_text').text(clang['kmCalendarMain.portlet.nodata']);
				footerDom.append($li);
				footerDom.append($liText);
				footerDom.css('height','100%');
			}
		},
