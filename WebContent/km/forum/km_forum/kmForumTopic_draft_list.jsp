...
<html:form action="/km/forum/km_forum/kmForumTopic.do">
	...
	<table id="List_ViewTable">
		...
		<c:forEach items="${queryPage.list}" var="kmForumTopic" varStatus="vstatus">
			<tr
					kmss_href="<c:url value="/km/forum/km_forum/kmForumPost.do" />?method=viewDraft&fdForumId=${kmForumTopic.kmForumCategory.fdId}&fdTopicId=${kmForumTopic.fdId}"
					kmss_target="_blank">
				<td><input type="checkbox" name="List_Selected"
					value="${kmForumTopic.fdId}"></td>
					<td>${vstatus.index+1}</td>
					<td width="40%" align="left" title="${kmForumTopic.docSubject }">
						<c:if test="${kmForumTopic.fdSticked==true}">
							<img src="${KMSS_Parameter_StylePath}forum/stick.gif" border="0" height="18" width="19">
							<b>[<bean:message bundle="km-forum" key="kmForumTopic.button.stick"/>]</b>
						</c:if>				
						<c:if test="${kmForumTopic.fdPinked==true}">
							<img src="${KMSS_Parameter_StylePath}forum/pink.gif" border="0" height="18" width="19">
							<b><font color="blue">[<bean:message bundle="km-forum" key="kmForumTopic.pink.title"/>]</font></b>
						</c:if>
						<c:if test="${kmForumTopic.fdReplyCount>='10'}">
							<img src="${KMSS_Parameter_StylePath}forum/hot.gif" border="0" height="18" width="19">
							<b><font color="red">[<bean:message bundle="km-forum" key="kmForumTopic.hot.title"/>]</font></b>
						</c:if>		
						<%	
				...
				<td width="15%">
					<c:if test="${!empty kmForumTopic.fdLastPoster.fdName}">
