...
<script  type="text/javascript">
		seajs.use([ 'lui/jquery','lui/parser','lui/dialog'],function($,parser,dialog) {
			...
			window.clickOK=function(){
				...
				if(field.value==""){
					dialog.alert("<kmss:message key="errors.required" argKey0="km-forum:kmForumTopic.button.stickDays" />");
					return;
				}
				var rtn = field.value;
				if(isNaN(rtn)){
					dialog.alert("<kmss:message key="errors.integer" argKey0="km-forum:kmForumTopic.button.stickDays" />");
					return;
				}
				var g = /^[1-9]*[1-9][0-9]*$/;
		        if(!g.test(rtn)){
		        	dialog.alert("<kmss:message bundle="km-forum" key="kmForumTopic.button.stickDays.tips" argKey0="kmForumTopic.button.stickDays"/>");
					return;
				}
				$dialog.hide(rtn);
			};
...
<bean:message bundle="km-forum" key="kmForumTopic.button.stickDays" />
