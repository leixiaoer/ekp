...
<template:replace name="content">
		<p class="txttitle">${lfn:message('tic-rest-connector:ticRestMain.selectExtendParam.check.note')}'${param.title}'${lfn:message('tic-rest-connector:ticRestMain.selectExtendParam.define')}</p>
		...
		<table id="TABLE_DocList" class="tb_normal" width=95% >
			...
			<tr KMSS_IsReferRow="1" style="display:none">
				<td width="32%">
					<input validate="required" class="inputsgl" readonly subject="<bean:message bundle="tic-rest-connector" key="ticRestMain.fdReqBizParam.set.name"/>" style="width:75%" name="param[!{index}].name">
				</td>
				<td width="32%">
					<input validate="required" class="inputsgl" readonly subject="<bean:message bundle="tic-rest-connector" key="ticRestMain.fdReqBizParam.set.title"/>" style="width:75%" name="param[!{index}].title">
				</td>
				<td width="24%">
					<input type="hidden" name="param[!{index}].children" value="">
					<select validate="required" onchange="doParamsTypeChange(this,'param');" disabled subject="<bean:message bundle="tic-rest-connector" key="ticRestMain.fdReqBizParam.set.title"/>" style="width:72%" name="param[!{index}].type">
				...
				<td width="12%">
					<center>
							<img class="editPd" style="display:none;" src="${KMSS_Parameter_ContextPath}resource/style/default/icons/edit.gif" title="${lfn:message('tic-rest-connector:ticRestMain.selectExtendParam.check.note1')}" onclick="doExendParamsDefine('param');" style="cursor:pointer">
					</center>
				</td>
			</tr>
		</table>
