...
<div class="lui_tree_operation">
   <ui:toolbar id="toolbar" style="float:right;margin-right:15px" count="7">
    ...
    <c:if test="${1!=param.rightFlag}"> <%-- 这里控制有些类别不需要权限控制，传入rightFlag=1，不需要权限 --%>
	   <ui:button text="${lfn:message('sys-right:right.change.title')}"  onclick="updateCateRight();" order="5" ></ui:button>
    </c:if>
...
function generateTree()
{
	...
	var fdAppType="${JsParam.fdAppType}"
	n1.authType = "01"
	n2 = n1.AppendSimpleCategoryData(modelName,"", "","", "","","","ticCoreBusiCate");
	LKSTree.Show();
}

function addCategory(){
	var url = s_contextPath + "${JsParam.actionUrl}?method=add&fdModelName=${JsParam.modelName}&fdAppType=${JsParam.fdAppType}";
