...
var modelName = Com_GetUrlParameter(parentHref,'modelName');
...
function getFuncParamsToJson(){
	...
	if($("input[name=splitTableExeFunc]").prop("checked")){
		var splitTableExeFunc = {};
		splitTableExeFunc.isEnable = $("input[name=splitTableExeFunc]").prop("checked");
		splitTableExeFunc.hidden_value = $("#table_id_hidden").val();
		splitTableExeFunc.display_value = $("#table_id_display").val();
		rtnJson.splitTableExeFunc = splitTableExeFunc;
	}
	
	var splitTableSetTag = true;
	if($("input[name=splitTableExeFunc]").prop("checked")){
		if(!$("#table_id_hidden").val()){
			splitTableSetTag = false;
		}
	}
	...
	if (!$("input[name=xformRest]").prop('checked')) {
			...
			for (var i = 0; i < trans.length; i++) {
				...
				trans_info.display_value = $(
						$("#" + trans[i].id.replace(".", "-") + "").find(
								"textarea")[0]).val();
				trans_info.hidden_value = $(
						$("#" + trans[i].id.replace(".", "-") + "").find(
								"input")[0]).val();
	...
	function recursionGetValueToJson(paramJson, rtnParamJson, id) {
		...
		if (paramJson.children) {
			...
			var displayValue = $(
					$("#" + id.replace(":", "_")).find('textarea')[0]).val();
			var hiddenValue = $($("#" + id.replace(":", "_")).find('input')[0])
					.val();
	...
	function initFuncOption() {
		...
		var nodeContent = parent.NodeContent;
		nodeContent = nodeContent.replace("import\"","import_\"").replace("export\"","export_\"");
		//alert(nodeContent);
		// 获得内容对象
		var json = eval('(' + nodeContent + ')');
		//alert(json.import_);
		//console.log(json);
		if (json) {
			$("input[name=xformRest]").attr('checked', json.xformRest);
			if(json.splitTableExeFunc){
				$("input[name=splitTableExeFunc]").attr('checked',
						json.splitTableExeFunc.isEnable);
				$("#table_id_hidden").val(json.splitTableExeFunc.hidden_value);
				$("#table_id_display").val(json.splitTableExeFunc.display_value);
			}
			splitTableExeFunc_select();
			...
			if (!json.xformRest) {
				...
				data.SendToBean("ticCoreFindFormulaJsonService&funcId="
						+ json.fdFunctionId, after_initFuncOption);
				//异常设置信息回显
	...
	var outParamArray;
	function after_initFuncOption(rtn) {
		//alert(rtn);
		if (!rtn) {
			return;
		}
		...
		//alert(fieldList.length + "---" + outParamJson.length);
	...
	function createTableInElementHTML(tr_list) {
		...
		for (var i = 0; i < tr_list.length; i++) {
			...
			if (tr.type == "array" || tr.type == "detailsTable") {
				var tr_new = $("<tr id='"+tr.id+"'><td>" + tr.id + "</td><td>"
						+ tr.label + "</td><td colspan='2' ></td></tr>");
			} else if (tr.id.indexOf(".") != -1) {
				tr.id = tr.id.replace(".", "-");
				var parentId = tr.id.substr(0, tr.id.indexOf('.'));
				var tr_new = $("<tr id='"+tr.id+"' class='child-of-"+parentId+"'><td>"
						+ tr.id
						+ "</td><td>"
						+ tr.label
						+ "</td><td><textarea style='width: 160px;height: 25px' readonly='readonly' name='"+tr.id+"_input'></textarea><input type='hidden' name='"+tr.id+"_hidden'></input></td><td align='center'><img  class='td_img' src='${KMSS_Parameter_StylePath}icons/edit.gif' border='0' onclick=\"mapping('"
						+ tr.id
						+ "_hidden','"
						+ tr.id
						+ "_input','in');\"></td></tr>");
			} else {
				var tr_new = $("<tr id='"+tr.id+"'><td>"
						+ tr.id
						+ "</td><td>"
						+ tr.label
						+ "</td><td><textarea style='width: 160px;height: 25px' readonly='readonly' name='"+tr.id+"_input'></textarea><input type='hidden' name='"+tr.id+"_hidden'></input></td><td align='center'><img  class='td_img' src='${KMSS_Parameter_StylePath}icons/edit.gif' border='0' onclick=\"mapping('"
			...
			if (tr.display_value) {
				$($("#" + tr.id).find('textarea')[0]).val(
						tr.display_value.replace(/\\n/g, "\n"));
				$($("#" + tr.id).find('input')[0]).val(
						tr.hidden_value.replace(/\\n/g, "\n"));
			}
		}
		$("#table_paraIn").treeTable({
			initialState : true,
			indent : 15
		}).expandAll();
	}
	...
	function getFormFieldInfo() {
		...
		for (var i = 0, length = fieldList.length; i < length; i++) {
			if (fieldList[i].controlType == "inputText") {
				rtnResult.push({
					id : fieldList[i].name,
					businessType : fieldList[i].controlType,
	...
	function after_dialogSelect(rtn) {
		...
		$("#table_paraOut")
				.append(
						"<tr><td class='td_normal_title' width='40%'>转换函数出参</td><td class='td_normal_title' width='20%'>参数说明</td><td class='td_normal_title' width='20%'>表单映射字段</td><td class='td_normal_title' width='20%'><a href='javascript:void(0)' style='color:#47b5ea;' onclick='adapt();' title='自动匹配映射关系'>适配</a></td></tr>");
		$("#table_paraIn")
				.append(
						"<tr><td class='td_normal_title' width='40%'>转换函数入参</td><td class='td_normal_title' width='20%'>参数说明</td><td class='td_normal_title' width='20%'>映射名称</td><td class='td_normal_title' width='20%'><span style='color:#47b5ea' onclick='expand();'> 展开 </span></td></tr>");
	...
	function createTableOutElementHTML(infoObj) {
		...
		for (i = 0; i < trs.length; i++) {
			...
			{
				tr_new = $("<tr id='"+tr.id+"'><td>" + tr.name + "</td><td>"
						+ tr.title + "</td><td colspan='2'></td></tr>");
			} else {
				tr_new = $("<tr id='"+tr.id+"'><td>"
						+ tr.name
						+ "</td><td>"
						+ tr.title
						+ "</td><td><textarea style='width: 160px;height: 25px' readonly='readonly' name='"+tr.id+"_input'></textarea><input type='hidden' name='"+tr.id+"_hidden'></input></td><td align='center'><img  class='td_img' src='${KMSS_Parameter_StylePath}icons/edit.gif' border='0' onclick=\"mapping('"
	...
	function mapping(idField, nameField, type) {
		...
		if(type=="out"){
			...
			idFieldObj = $("#table_paraOut").find("input[name='"+idField+"']").get(0);
			nameFieldObj = $("#table_paraOut").find("textarea[name='"+nameField+"']").get(0);
		...
		Formula_Dialog(idFieldObj, nameFieldObj, varInfo, 'Object', null, null, modelName);
	}
	...
	function genVarInfo(field, varInfo, parentName, parentLabel) {
		...
		m.len = 200;
		m.length = 0;
		m.type = field.type.substring(0, 1).toUpperCase()
	...
	function updateMapping() {
		...
		var data = new KMSSData();
		data.SendToBean("ticCoreFindFormulaJsonService&funcId=" + fdFunctionId,
				after_updateMapping);
	}
	function after_updateMapping(rtn) {
		...
		$.each(outParamArray, function() {
			$("#table_paraOut").find("textarea[name=" + this.name + "_input]")
					.val(this.display_value);
			$("#table_paraOut").find("input[name=" + this.name + "_hidden]")
					.val(this.hidden_value);
		});
	}
		...
		$.each(paraIn, function(idx, obj) {
			...
			if (resourceObj) {
				...
				if (obj.children && resourceObj.children) {
					updateAndSetResourceValue(obj.children,
	...
	function splitTableExeFunc_select() {
		if ($("input[name=splitTableExeFunc]").prop("checked")) {
			$('#table_id_span').show();
		} else {
			$('#table_id_span').hide();
	...
	function importFormFields() {
		seajs.use([ 'lui/dialog', 'lui/jquery' ], function(dialog, $) {
			dialog.iframe("/tic/core/mapping/xformRestTree.jsp", "", function(
					rtnData) {
				if (rtnData)
		...
		for (var i = 0; i < tr_list.length; i++) {
			...
			if (tr.type != "arrayObject" && tr.type != "object") {
				var tr_new = $("<tr id='"+id+"'><td><input type='text' name='name' validate='required'  value='"+tr.name+"'  class='inputsgl'></input></td><td><input type='text' name='title' validate='required'  value='"+tr.title+"'  class='inputsgl'></td><td><select name='type'>"
						...
						+ "<td align='center'><img  class='td_img' src='${KMSS_Parameter_StylePath}icons/edit.gif' border='0' onclick=\"mapping('xform_"
						+ id
						+ "_hidden','xform_"
						+ id
						+ "_input','out');\"> <img  src=\"${KMSS_Parameter_StylePath}icons/delete.gif\" title=\"删除\" onClick=\"deleteFormField(this);\"/></td></tr>");
			} else {
			...
			tr_new
					...
					.on(
							...
							function() {
								var type = $(this).val();
								var img_td = $(this).parent().parent().find(
										"img").parent();
								img_td.find('#img_add').remove();
								if (type == "object" || type == "arrayObject") {
									img_td
											.prepend("<img  id='img_add' src=\"${KMSS_Parameter_StylePath}icons/add.gif\" title=\"新增\" onClick=\" addFormField(this);\"/> ");
								}
							});
			if (tr.parentId)
				tr_new.addClass("child-of-" + tr.parentId);
			...
			if (tr.display_value) {
				$($("#" + id).find('textarea[name^=xform_]')[0]).val(
						tr.display_value);
				$($("#" + id).find('input[name^=xform_]')[0]).val(
						tr.hidden_value);
			} else {
				$($("#" + id).find('textarea[name^=xform_]')[0]).val(
						'$' + tr.title + '$');
	...
	function addFormField(parent) {
		var val = $(parent).parent().parent().find('[name="type"]').val()
				|| "object";
		...
		tr_new
				...
				.on(
						...
						function() {
							var type = $(this).val();
							var img_td = $(this).parent().parent().find("img")
									.parent();
							img_td.find('#img_add').remove();
							if (type == "object" || type == "arrayObject") {
								img_td
	...
	function deleteFormField(obj) {
		var id = "tr.child-of-" + $(obj).parent().parent().attr("id");
		id = id.replace(new RegExp("/","g"), "\\/");
		$(obj).parent().parent().siblings(id).each(
				function(index) {
					$(this).remove();
				});
	...
	function buildParaJson() {
		var paras = [];
		$("#table_xform").find("tbody tr").each(
				function(index) {
					if (index > 0) {
	...
	function buildParaField(node) {
		...
		if ((field.type == "object" || field.type == "arrayObject")
				&& !field.children) {
	...
	function getChildrenOf(node) {
		var id = "tr.child-of-" + node.attr("id");
		id = id.replace(new RegExp("/","g"), "\\/");
...
<table width="100%" class="tb_normal">
     ...
     <tr >
       <td colspan='2' style="display:table-cell; vertical-align:middle"><input type="checkbox" style="vertical-align: middle;" name="splitTableExeFunc"  onclick="splitTableExeFunc_select();">
           表单明细数据拆分调用函数 <span id="table_id_span">明细表:
           <textarea id="table_id_display" style='width: 100px;height: 22px' readonly='readonly' name='table_id_display'></textarea>
           <input id="table_id_hidden" type='hidden' name="table_id_hidden"></input>
           <img  class='td_img' src='${KMSS_Parameter_StylePath}icons/edit.gif' border='0' onclick="mapping('table_id_hidden','table_id_display','mingxi');"></span>
       </td>
     </tr>
    <tr>
