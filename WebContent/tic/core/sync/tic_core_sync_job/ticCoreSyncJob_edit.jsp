...
<%@page import="com.landray.kmss.sys.quartz.scheduler.CronExpression"%><script
	type="text/javascript">
	Com_IncludeFile("doclist.js|jquery.js|json2.js|dialog.js");
</script>
<script
	src="${KMSS_Parameter_ContextPath}tic/core/resource/js/custom_validations.js"
...
<script language="javascript">
	...
	$(function() {
		...
		$("#Include_Custom_Validations_Span_Id").load(
				Com_Parameter.ContextPath
						+ "tic/core/resource/jsp/custom_validations.jsp",
				sync_callValid);
	});

	function sync_callValid() {
		FUN_AddValidates("fdSubject:required");
	}
</script>
<script>
	var errorInteger = "<kmss:message key="errors.integer" />";
	var errorRange = "<kmss:message key="errors.range" />";
	var fieldMessages = "<bean:message bundle="sys-quartz" key="sysQuartzJob.cronExpression.fields"/>"
			.split(",");
	...
	function buildCronExpression() {
		...
		//构造CronExpression
		try {
			var expression = formatCronExpressionField(second,
					fieldMessages[0], 0, 59)
					+ " ";
			if (frequency == "every")
				expression += minute + " ";
			else
				expression += formatCronExpressionField(minute,
						fieldMessages[1], 0, 59)
						+ " ";
			expression += formatCronExpressionField(hour, fieldMessages[2], 0,
					23)
					+ " ";
			if (frequency == "week") {
				expression += "? ";
				expression += formatCronExpressionField(month,
						fieldMessages[4], 1, 12)
						+ " ";
				expression += week;
			} else {
				expression += formatCronExpressionField(day, fieldMessages[3],
						1, 31)
						+ " ";
				expression += formatCronExpressionField(month,
						fieldMessages[4], 1, 12)
			...
			if (year != "")
				expression += " "
						+ formatCronExpressionField(year, fieldMessages[6],
								1970, 2099);
		} catch (e) {
	...
	function setCronExpressionField(data) {
		document.getElementsByName("fdYear")[0].value = data.year == null ? ""
				: data.year;
		document.getElementsByName("fdMonth")[0].value = data.month == null ? ""
				: data.month;
		document.getElementsByName("fdDay")[0].value = data.day == null ? ""
				: data.day;
		setSelectFieldValue(document.getElementsByName("fdWeek")[0], data.week);
		document.getElementsByName("fdHour")[0].value = data.hour == null ? 0
				: data.hour;
		document.getElementsByName("fdMinute")[0].value = data.minute == null ? 0
				: data.minute;
		document.getElementsByName("fdSecond")[0].value = data.second == null ? 0
				: data.second;
		setSelectFieldValue(document.getElementsByName("fdEvery")[0],
				data.every);
		setSelectFieldValue(document.getElementsByName("fdFrequency")[0],
				data.frequency);
	}
	...
	function submitForm(type) {
		...
		Com_Submit(document.ticCoreSyncJobForm, type);
	}

	function onRunTypeChange() {
		document.getElementsByName("fdRunServer")[0].style.display = document
				.getElementsByName("fdRunType")[0].value == "2" ? "" : "none";
	}
</script>
<html:form action="/tic/core/sync/tic_core_sync_job/ticCoreSyncJob.do">
	<div id="optBarDiv">
		<input type=button value="${lfn:message('home.help')}"
			onclick="Com_OpenWindow(Com_Parameter.ContextPath+'tic/core/sync/help/SyncJobHelp.html','_blank');" />
		<c:if test="${not empty param.fdId}">
			<input type="button"
		...
		<c:if test="${ ticCoreSyncJobForm.fdEnabled=='true'}">
			<input type="button"
				value="<bean:message bundle="sys-quartz" key="sysQuartzJob.button.run"/>"
		...
		<c:if test="${ticCoreSyncJobForm.method_GET=='edit'}">
			<input type=button value="<bean:message key="button.update"/>"
				onclick="submitForm('update');">
		</c:if>
		<c:if test="${ticCoreSyncJobForm.method_GET=='add'}">
			<input type=button value="<bean:message key="button.save"/>"
				onclick="submitForm('save');">
			<%-- 
		<input type=button value="<bean:message key="button.saveadd"/>"
			onclick="Com_Submit(document.ticCoreSyncJobForm, 'saveadd');">
		--%>
		</c:if>
	...
	<p class="txttitle">
		<bean:message bundle="tic-core-sync" key="table.ticCoreSyncJob" />
	</p>

	<center>
		<table id="Label_Tabel" width=95%>
			<tr
				LKS_LabelName="<bean:message key='ticCoreSyncJob.fdCoreMsg' bundle='tic-core-sync'/>">
				<td>

					<table class="tb_normal" width=95% id="TB_MainTable">
						<tr>
							<td class="td_normal_title"><bean:message
									bundle="tic-core-sync" key="ticCoreSyncJob.fdSubject" /></td>
							<td><c:if test="${ticCoreSyncJobForm.fdIsSysJob=='true'}">
									<kmss:message key="${ticCoreSyncJobForm.fdSubject}" />
								</c:if> <c:if test="${ticCoreSyncJobForm.fdIsSysJob!='true'}">
									<xform:text property="fdSubject" style="width:85%" />
								</c:if></td>
							<td class="td_normal_title" width=15%><bean:message
									bundle="tic-core-sync" key="ticCoreSyncJob.fdCategory" /></td>
							<td width="35%"><html:hidden property="fdCategoryId" /> <xform:text
									property="fdCategoryName" required="true" style="width:34%" />
								<a href="#"
								onclick="Dialog_Tree(false, 'fdCategoryId', 'fdCategoryName', ',', 'ticCoreBusiCateTreeService&parentId=!{value}&fdAppType=${param.fdAppType}', 
				'函数分类', null, null, 
				'', null, null, 
				'函数分类');" />
						...
						<tr>
							<td width=15% class="td_normal_title"><bean:message
									bundle="tic-core-sync" key="ticCoreSyncJob.fdCronExpression" />
							</td>
							<td width=35%><c:import
									url="/sys/quartz/sys_quartz_job/sysQuartzJob_showCronExpression.jsp"
									charEncoding="UTF-8">
									<c:param name="value"
										value="${ticCoreSyncJobForm.fdTempCronExpression}" />
								</c:import></td>
							<td width=15% class="td_normal_title"><bean:message
									bundle="tic-core-sync" key="ticCoreSyncJob.nextTime" /></td>
							<td width=35%>
								<%
									CronExpression expression = new CronExpression(ticCoreSyncJobForm.getFdTempCronExpression());
						...
						<tr>
							<td class="td_normal_title"><bean:message
									bundle="tic-core-sync" key="ticCoreSyncJob.fdRunType" /></td>
							<td><sunbor:enums property="fdRunType" elementType="select"
									enumsType="sysQuartzJob_fdRunType" /></td>
							<td class="td_normal_title"><bean:message
									bundle="sys-quartz" key="sysQuartzJob.cronExpression.inputType" />
							</td>
							<td><input type="radio" name="fdInputType" value="0" checked
								onclick="changeInputType(value);">
							<bean:message bundle="sys-quartz"
									key="sysQuartzJob.cronExpression.inputType.normal" /> <input
								type="radio" name="fdInputType" value="1"
								onclick="changeInputType(value);">
							<bean:message bundle="sys-quartz"
									key="sysQuartzJob.cronExpression.inputType.cronExpression" /></td>
						</tr>
						<tr>
							<!-- kmss_inputtype="0" -->
							<td class="td_normal_title"><bean:message
									bundle="sys-quartz" key="sysQuartzJob.cronExpression.frequency" />
							</td>
							<td colspan="3"><select name="fdFrequency"
								onchange="refreshInpuType0();">
									...
									<option value="month"><bean:message
											bundle="sys-quartz"
									...
									<option value="every"><bean:message
											bundle="sys-quartz"
											key="sysQuartzJob.cronExpression.frequency.every" /></option>
							</select></td>
						</tr>
						<tr kmss_inputtype="1" id="TR_FrequencySetting"
							style="display: none">
							<td class="td_normal_title"><bean:message
									bundle="sys-quartz"
									key="sysQuartzJob.cronExpression.timeSetting" /></td>
							<td colspan="3"><span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.year1" /><input
									name="fdYear" size="4" class="inputSgl">
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.year2" /></span> <span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.month1" /><input
									name="fdMonth" size="2" class="inputSgl">
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.month2" /></span> <span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.day1" /><input
									name="fdDay" size="2" class="inputSgl">
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.day2" /></span> <span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.week1" /><select
									name="fdWeek">
										<c:forEach begin="0" end="6" var="i">
											<option value="${i+1}"><bean:message
													key="date.weekDay${i}" /></option>
										</c:forEach>
								</select>
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.week2" /></span> <span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.hour1" /><input
									name="fdHour" size="2" class="inputSgl">
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.hour2" /></span> <span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.minute1" /><input
									name="fdMinute" size="2" class="inputSgl">
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.minute2" /></span> <span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.second1" /><input
									name="fdSecond" size="2" class="inputSgl">
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.second2" /></span> <span><bean:message
										bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.every1" /><select
									name="fdEvery">
								...
								<bean:message bundle="sys-quartz"
										key="sysQuartzJob.cronExpression.field.every2" /></span></td>
						</tr>
						<tr kmss_inputtype="1" style="display: none">
							<td class="td_normal_title">CronExpression</td>
							<td colspan="3"><html:text property="fdCronExpression"
									style="width:80%" /> <a href="#"
								onclick="isShowHelp=!isShowHelp; refreshInpuType1();"> <bean:message
										bundle="sys-quartz" key="sysQuartzJob.cronExpression.help" />
							</a></td>
						</tr>
						<tr kmss_inputtype="1" id="TR_CronExpressionHelp"
							style="display: none">
							<td colspan="4"><%@ include
									file="/sys/quartz/sys_quartz_job/sysQuartzJob_cronExpressionHelp.jsp"%>
								<br>
							<bean:message bundle="sys-quartz"
									key="sysQuartzJob.cronExpression.helpLink" /> <a
						...
						<tr>
							<td class="td_normal_title" width=15%><bean:message
									bundle="tic-core-sync" key="ticCoreSyncJob.fdLink" /></td>
							<td colspan="3"><xform:text property="fdLink"
									style="width:85%" /></td>

						</tr>
						<tr>
							<td class="td_normal_title" width=15%><bean:message
									bundle="tic-core-sync" key="ticCoreSyncJob.fdUseExplain" /></td>
							<td colspan="3"><xform:text property="fdUseExplain"
									style="width:85%" /></td>
			...
			<!-- =============================================== -->
			<%@ include
				file="/tic/core/sync/tic_core_sync_job/ticCoreSync_tempEdit.jsp"%>
		</table>
