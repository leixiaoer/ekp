...
// *************** END OF PLACEHOLDER DOC ENTRIES FOR NATURAL DOCS ***********************************************************		
		...
		this.connect = function(params, referenceParams) {
			...
			// dynamic anchors. backwards compaticility here: from 1.2.6 onwards you don't need to specify "dynamicAnchors".  the fact that some anchor consists
...
between this method and jsPlumb.reset).  
		...
		/*
		  Function: detach 
		  Detaches and then removes a <Connection>.  Takes either (source, target) (the old way, maintained for backwards compaticility), or a params
...
about the parameters allowed in the params object.
		...
		this.detach = function(source, target) {
			...
			// the connect method does.
				...
				if (arguments[0].constructor == Connection) {
					...
					var _p = jsPlumb.extend( {}, source); // a backwards compaticility hack: source should be thought of as 'params' in this case.
		...
		this.makeAnchor = function(x, y, xOrientation, yOrientation, xOffset, yOffset) {
			// backwards compaticility here. we used to require an object passed
			// in but that makes the call very verbose. easier to use
			// by just passing in four/six values. but for backwards
			// compaticility if we are given only one value we assume it's a
			...
			if (arguments.length == 1) {
				...
				// last we try the backwards compaticility stuff.
...
// ***************************** END OF PLACEHOLDERS FOR NATURAL DOCS *************************************************
			...
			*/
			 ...
			 * @deprecated
			 */
			this.removeConnection = this.detach; // backwards compaticility
...
// ********************************* OVERLAY DEFINITIONS ***********************************************************************    
 ...
 // ********************************* END OF OVERLAY CANVAS RENDERERS ***********************************************************************
	...
	if (document.createStyleSheet) {			
		...
		// 
		// try to avoid IE8.  it is recommended you set X-UA-Compaticle="IE=7" if you can.
		//
