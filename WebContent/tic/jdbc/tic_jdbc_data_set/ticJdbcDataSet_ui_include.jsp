...
<template:include file="/tic/core/tic_ui_list.jsp">
	...
	<template:replace name="content">
		<%-- 筛选器 --%>
		<list:criteria>
			<list:cri-ref key="fdName" ref="criterion.sys.docSubject"  title="${ lfn:message('tic-jdbc:ticJdbcDataSet.docSubject') }" >
			</list:cri-ref>
				...
				<div style="display: inline-block; vertical-align: middle;">
						<ui:toolbar>
							...
							<kmss:auth requestURL="/tic/jdbc/tic_jdbc_data_set/ticJdbcDataSet.do?method=deleteall">
								<ui:button text="${lfn:message('button.deleteall')}" order="4" onclick="delDoc('${LUI_ContextPath}/tic/jdbc/tic_jdbc_data_set/ticJdbcDataSet.do?method=deleteall')"></ui:button>
							</kmss:auth>
						</ui:toolbar>						
              </div>
          </div>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/tic/jdbc/tic_jdbc_data_set/ticJdbcDataSetIndex.do?method=list&categoryId=${param.categoryId }&fdAppType=${JsParam.fdAppType}&fdEnviromentId=${JsParam.fdEnviromentId}'}
			</ui:source>
			<%--列表形式--%>
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/tic/jdbc/tic_jdbc_data_set/ticJdbcDataSet.do?method=view&fdId=!{fdId}&fdAppType=${param.fdAppType}" name="columntable">
		...
		seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog','lui/topic'], function($, strutil, dialog , topic) {
			...
			window.addDocByCate = function(){
				window.open(Com_Parameter.ContextPath+"tic/jdbc/tic_jdbc_data_set/ticJdbcDataSet.do?method=add&fdAppType=${JsParam.fdAppType}&fdEnviromentId=${JsParam.fdEnviromentId}");
			};
			//根据筛选器分类异步校验权限
