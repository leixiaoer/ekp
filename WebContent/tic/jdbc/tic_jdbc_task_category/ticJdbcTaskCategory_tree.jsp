...
{
	LKSTree = new TreeView("LKSTree", "<bean:message key="ticJdbcTaskCategory.ticJdbcTaskCategory" bundle="tic-jdbc"/>", document.getElementById("treeDiv"));
	LKSTree.isShowCheckBox=true;
	LKSTree.isMultSel=true;
	LKSTree.isAutoSelectChildren = false;
	var n1, n2;
	n1 = LKSTree.treeRoot;
	n2 = n1.AppendBeanData("ticJdbcTaskCategoryTreeService&parentId=!{value}");//选取树数据的bean
	//TODO 数据显示不出来
	LKSTree.Show();
	var  iframeObj=document.getElementById("iframeSearch");//搜索结果页面
 	var url="<c:url value='/tic/jdbc/tic_jdbc_task_category/ticJdbcTaskCategory.do?method=add'/>"; 
	iframeObj.src=url;
}
...
function editCategory(){
	var url = "<c:url value='/tic/jdbc/tic_jdbc_task_category/ticJdbcTaskCategory.do' />?method=edit&mainModelName=${param.mainModelName}";
	var checkedNode = LKSTree.GetCheckedNode();
	if(List_CheckSelect()){
	if(checkedNode.length>1)
	   {
...
function viewCategory(id) {
	...
	if(node!=null && node.value!=null) {
		var url = "<c:url value='/tic/jdbc/tic_jdbc_task_category/ticJdbcTaskCategory.do' />?method=view&fdId="+node.value;
		Com_OpenWindow(url);
	}
}
// 删除
function deleteCategories(){
	...
	for(var i=selList.length-1;i>=0;i--){
		...
		document.ticJdbcTaskCategoryForm.appendChild(input);	
		
	}
	Com_Submit(document.ticJdbcTaskCategoryForm, 'deleteall');	
}
//选择
