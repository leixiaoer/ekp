	...
	<template:replace name="path">
		<ui:combin ref="menu.path.tic.simplecategory">
			<ui:varParams 
				...
				cateTitle="${ lfn:message('tic-jdbc:table.ticJdbcMappManage') }" 
				href="javascript:parent.ticLoadList('/tic/jdbc/tic_jdbc_mapp_manage/ticJdbcMappManage_ui_include.jsp?categoryId=!{value}');" 
	...
	<template:replace name="content">
		...
		<list:criteria>
			...
			<list:cri-ref ref="criterion.sys.simpleCategory" key="docCategory" multi="false" title="${lfn:message('tic-core-common:ticCoreCommon.categoryNavigation')}" expand="true">
			  <list:varParams modelName="com.landray.kmss.tic.jdbc.model.TicJdbcMappCategory"/>
			</list:cri-ref>
			<list:cri-auto modelName="com.landray.kmss.tic.jdbc.model.TicJdbcMappManage" property="fdIsEnabled"/>
		</list:criteria>
					...
					<td align="right">
						<ui:toolbar>
							...
							<kmss:auth requestURL="/tic/jdbc/tic_jdbc_mapp_manage/ticJdbcMappManage.do?method=deleteall">
								<ui:button text="${lfn:message('button.deleteall')}" order="4" onclick="delDoc('${LUI_ContextPath}/tic/jdbc/tic_jdbc_mapp_manage/ticJdbcMappManage.do?method=deleteall')"></ui:button>
							</kmss:auth>
						</ui:toolbar>						
					</td>
				</tr>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/tic/jdbc/tic_jdbc_mapp_manage/ticJdbcMappManageIndex.do?method=list&categoryId=${param.categoryId }'}
			</ui:source>
			<%--列表形式--%>
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/tic/jdbc/tic_jdbc_mapp_manage/ticJdbcMappManage.do?method=view&fdId=!{fdId}" name="columntable">
		...
		seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog','lui/topic'], function($, strutil, dialog , topic) {
			...
			window.addDocByCate = function(){
				parent.addDoc('${LUI_ContextPath}/tic/jdbc/tic_jdbc_mapp_manage/ticJdbcMappManage.do?method=add&fdtemplatId='+ cateId);
			};
			//根据筛选器分类异步校验权限
