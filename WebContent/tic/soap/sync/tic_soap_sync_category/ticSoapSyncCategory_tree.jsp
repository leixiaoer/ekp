...
<html:form action="/tic/soap/sync/tic_soap_sync_category/ticSoapSyncCategory.do">
	...
	<%-- 删除 --%> <input type="button"
	 ...
	 <div style="width: 100%"><iframe id='iframeSearch' name="iframeName" width=100%
		height=120px frameborder=0 scrolling=no marginheight="0"> </iframe></div>
...
{
	LKSTree = new TreeView("LKSTree", "<bean:message key="table.ticSoapSyncCategory" bundle="tic-soap-sync"/>", document.getElementById("treeDiv"));
	LKSTree.isShowCheckBox=true;
	LKSTree.isMultSel=true;
	LKSTree.isAutoSelectChildren = false;
	var n1, n2;
	n1 = LKSTree.treeRoot;
	n2 = n1.AppendBeanData("ticSoapSyncCategoryTreeService&parentId=!{value}");
	//TODO 数据显示不出来
	LKSTree.Show();
	  var  iframeObj=document.getElementById("iframeSearch");//搜索结果页面
 	var url="<c:url value='/tic/soap/sync/tic_soap_sync_category/ticSoapSyncCategory.do?method=add'/>"; 
	iframeObj.src=url;
}
...
//新建
	...
	if(checkedNode.length>0){
	   ...
	   {
	   alert("<bean:message key="error.messageone" bundle="tic-soap-sync"/>");
	   return false;
	   }
...
function editCategory(){
	var url = "<c:url value="/tic/soap/sync/tic_soap_sync_category/ticSoapSyncCategory.do" />?method=edit&mainModelName=${param.mainModelName}";
	var checkedNode = LKSTree.GetCheckedNode();
	if(List_CheckSelect()){
	if(checkedNode.length>1)
	   {
...
function deleteCategories(){
	...
	for(var i=selList.length-1;i>=0;i--){
		...
		document.ticSoapSyncCategoryForm.appendChild(input);	
		
	}
	Com_Submit(document.ticSoapSyncCategoryForm, 'deleteall');	
}
//选择
