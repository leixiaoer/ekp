...
<%@page import="com.landray.kmss.sys.quartz.scheduler.CronExpression"%><script type="text/javascript">
...
$(function(){
	...
	$("#Include_Custom_Validations_Span_Id").load(Com_Parameter.ContextPath +
			"tic/core/resource/jsp/custom_validations.jsp", sync_callValid);
});
...
function submitForm(type){
	...
	Com_Submit(document.ticSoapSyncJobForm, type);
}
...
<div id="optBarDiv">
	<c:if test="${ticSoapSyncJobForm.method_GET=='edit'}">
		<input type=button value="<bean:message key="button.update"/>"
		onclick="submitForm('update');">
	</c:if>
	<c:if test="${ticSoapSyncJobForm.method_GET=='add'}">
		<input type=button value="<bean:message key="button.save"/>"
			onclick="submitForm('save');">
		<input type=button value="<bean:message key="button.saveadd"/>"
			onclick="Com_Submit(document.ticSoapSyncJobForm, 'saveadd');">
	</c:if>
	<input type="button" value="<bean:message key="button.close"/>" onclick="Com_CloseWindow();">
</div>

<p class="txttitle">SOAP<bean:message bundle="tic-soap-sync" key="table.ticSoapSyncJob"/></p>

<center>
<table
		id="Label_Tabel"
		width=95%>
		<tr LKS_LabelName="<bean:message key='ticSoapSyncJob.fdSoapMsg' bundle='tic-soap-sync'/>">
			<td>
...
<table class="tb_normal" width=95% id="TB_MainTable">
	...
	<td class="td_normal_title" width=15%>
		...
		<html:hidden property="docCategoryId" /> <xform:text
				property="docCategoryName" required="true" style="width:34%" />
		 <a href="#" onclick="Dialog_Tree(false, 'docCategoryId', 'docCategoryName', ',', 'ticSoapSyncCategoryTreeService&parentId=!{value}', 
	...
	<tr>
		<td width=15% class="td_normal_title">
			...
			<c:import url="/sys/quartz/sys_quartz_job/sysQuartzJob_showCronExpression.jsp" charEncoding="UTF-8">
				<c:param name="value" value="${ticSoapSyncJobForm.fdTempCronExpression}" />
			</c:import>
		</td>
		<td width=15% class="td_normal_title">
			...
			<%
				CronExpression expression = new CronExpression(ticSoapSyncJobForm.getFdTempCronExpression());
	...
	<tr>
		...
		<td>
			<input type="radio" name="fdInputType" value="0" checked onclick="changeInputType(value);"><bean:message bundle="sys-quartz" key="sysQuartzJob.cronExpression.inputType.normal"/>
			<input type="radio" name="fdInputType" value="1" onclick="changeInputType(value);"><bean:message bundle="sys-quartz" key="sysQuartzJob.cronExpression.inputType.cronExpression"/>
		</td>
	</tr>
	<tr ><!-- kmss_inputtype="0" -->
		<td class="td_normal_title">
	...
	<tr kmss_inputtype="1" id="TR_FrequencySetting" style="display:none">
		<td class="td_normal_title">
	...
	<!-- =============================================== -->
<%@ include file="/tic/soap/sync/tic_soap_sync_job/ticSoapSync_tempEdit.jsp"%>	
		</table>
