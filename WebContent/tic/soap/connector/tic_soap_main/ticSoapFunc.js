...
function cleanCache(serviceId) {
	...
	data.SendToBean("ticSoapCleanCache&serviceId=" + serviceId,
			after_cleanCache);
}
...
function loadOptions() {
	...
	// 获取数据并校验
	var serviceId = $("select[name='wsServerSettingId']").val();
	var data = new KMSSData();
	var bean_str = "ticSoapSelectOptionsBean&serviceId=!{serviceId}&soapversion=SOAP 1.1";
	bean_str = bean_str.replace("!{serviceId}", serviceId);
...
//debugger;
	...
	var data = new KMSSData();
	var beanStr = "ticSoapBindFuncImpl&serviceId=!{serviceId}&wsBindFunc=!{wsBindFunc}&soapversion=SOAP 1.1&curId=!{fdId}";
	beanStr = beanStr.replace("!{serviceId}", serviceId).replace(
			"!{wsBindFunc}", b_func).replace("!{fdId}", fdId);
	...
	$("#Include_Span_Id")
			.load(
					Com_Parameter.ContextPath
							+ "tic/soap/connector/resource/js/resource_properties.jsp",
					function(rtnData) {
						callBackLoad(xmldom, flag);
					});
	//alert(ERP_parser.IEVersion());
}
...
function callBackLoad(xmldom, flag) {
	...
	var params=m_info_out.info['tbody'];
	var options=[];
	for(var i=0;i<params.length;i++){
		if(!params[i].hasNext){
			var index =params[i].nodeName.indexOf(":")+1;
			var name = getParents(params,params[i],"")+params[i].nodeName.substring(index);
	...
	$("#returnFlag").append(options);
}

function getParents(info,obj,name){
	for(var i=0;i<info.length;i++){
		if(obj.root){
			return name;
		}else if(obj.parentKey==info[i].nodeKey){
			var index =info[i].nodeName.indexOf(":")+1;
			name=info[i].nodeName.substring(index)+"."+name;
			return getParents(info,info[i],name);
		}
	}	
}
...
function erp_toggle(elem, nodekey) {
	if(elem.checked){
		...
		$("#returnFlag").hide();
		$('select[name="fdReturnValue"]').val('');
		$('input[name="fdSuccess"]').val('');
		$('input[name="fdFail"]').val('');
	}
