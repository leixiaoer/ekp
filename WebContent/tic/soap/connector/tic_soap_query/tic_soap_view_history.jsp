...
<c:set var="ticSoapMainForm" value="${requestScope[param.formName]}" />
...
function func_history_LoadIframe(){
	...
	/*if(iframe.src==""){
		iframe.src = "<c:url value="/tic/soap/connector/tic_soap_query/ticSoapQuery.do" />?method=list&funcId=${ticSoapMainForm.fdId}";
	}*/

	if($(iframe).attr("src").length<1){
		 var srcValue="<c:url value="/tic/soap/connector/tic_soap_query/ticSoapQuery.do" />?method=list&funcId=${param.fdId}";
...
<tr LKS_LabelName="<bean:message bundle="tic-soap-connector" key="ticSoapQuery.history"/>" style="display:none">
	<td>
