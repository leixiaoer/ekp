...
<list:criteria channel="channel_common_approved" id="criteria1" expand="false"
	multi="false">
	<%--模块--%>
	<list:cri-criterion
		...
		<list:box-select>
			<list:item-select cfg-enable="true">
				<ui:source type="Static">
											[
											{text:'${ lfn:message('sys-lbpmperson:lbpmperson.status.draft')}',value:'10'},
...
<div class="lui_list_operation">
	...
	<div style="float: left">
		<div style="display: inline-block; vertical-align: middle;">
			<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" count="6" channel="channel_common_approved">
				<list:sortgroup>
				<c:if test="${param.fdType!='finish'}">
	...
	<div style="float: left;">
		<list:paging layout="sys.ui.paging.top" channel="channel_common_approved">
		</list:paging>
	</div>

</div>
<ui:fixed elem=".lui_list_operation"></ui:fixed>
<list:listview id="list_approved" channel="channel_common_approved">
	<ui:source type="AjaxJson">
													{url:'/sys/lbpmperson/sys_lbpmperson_myprocess/SysLbpmPersonMyProcess.do?method=listApproved&modelName=${JsParam.modelName }&fdStatus=${JsParam.fdStatus}&fdType=${JsParam.fdType}'}
											</ui:source>
...
<list:paging channel="channel_common_approved"></list:paging>
<script>
//解决多子系统页面显示不全问题
domain.autoResize();
	seajs.use([ 'lui/topic' ],function(topic) {
		...
		topic.subscribe('successReloadPage', function() {
			window.setTimeout(function() {
				topic.channel("channel_common_approved").publish('list.refresh');
			}, 2000);
		});
	});
