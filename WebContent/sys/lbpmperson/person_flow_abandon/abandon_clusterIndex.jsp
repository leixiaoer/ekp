...
<%@ page import="java.util.*"%>
 <% List<SysClusterGroup> groups = ClusterDiscover.getInstance().getGroupByFunc("lbpmMonitorServer");
    if(groups.size()<=0){
   		request.getRequestDispatcher("/sys/lbpmperson/person_flow_abandon/abandon_index.jsp").forward(request, response); 
    }else{
	    ...
	    <div style="width:100%">
	    	   ...
	    	   <c:forEach items="${groups}" var="group">
			       <ui:content title="${group.fdName}" style="padding:0;background-color:#f2f2f2;" id="${group.fdKey}">
			            ...
			            <ui:event event="show">
			                var fdUrl = "${group.fdUrl}/sys/lbpmperson/person_flow_abandon/abandon_index_person.jsp"
