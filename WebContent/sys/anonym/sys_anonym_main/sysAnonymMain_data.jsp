    ...
    <list:data-columns var="sysAnonymMain" list="${queryPage.list}" varIndex="status" custom="false">
		...
		<list:data-column col="operations" title="${ lfn:message('list.operation') }" escape="false">
			<c:if test="${sysAnonymMain.fdStatus == 1 }" >
				<a class="btn_txt"  href="javascript:anonymCancel('${sysAnonymMain.fdId}','${sysAnonymMain.fdModelName}')">${lfn:message('sys-anonym:sysAnonymMain.btn.cancelpub')}</a>
			</c:if>
			<c:if test="${sysAnonymMain.fdStatus == 0 }" >
				<a class="btn_txt"  href="javascript:anonymPublish('${sysAnonymMain.fdId}','${sysAnonymMain.fdModelName}')">${lfn:message('sys-anonym:sysAnonymMain.btn.repub')}</a>
			</c:if>
		</list:data-column>
