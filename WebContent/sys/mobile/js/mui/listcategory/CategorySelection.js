define([
  ...
  var selection = declare("mui.listcategory.CategorySelection", [WidgetBase], {
    //非必填项,用于初始值
    curIds: null,
    ...
    _popSelItem: function() {
    	...
    	var forwardCallback = lang.hitch(this, function(){
    		...
    		dialog = Dialog.element({
    	        ...
    	        scrollable: true,
    	        parseable: true,
    	        showClass: "muiCateSecPop",
    	        callback: lang.hitch(this, function() {
    	        	if(previousId){
    	        		listener.go({
    	        			historyId : previousId,
    	        			callback: function(){} //空操作
    	        		})
    	        	}
    	        	dialog = null
    	        	previousId = null;
    ...
    _initSelection: function() {
      ...
      if (this.curIds) {
        ...
        var pWeiget = _self.getParent()
        if (pWeiget && pWeiget.curIds) {
          var arrs = pWeiget.curIds.split(";")
          if (array.indexOf(arrs, _self.fdId) > -1) {
        ...
        if (_url) {
          ...
          promise.then(function(result) {
        	  var items = result['datas'] ? _self.formatResult(result['datas']) : result		  
              if (items.length > 0) {
	              array.forEach(items, function(item) {
	            	  if(_self.curIds == item.value){
    ...
    formatResult: function(datas){
    	var result = [];
    	for(var i=0;i<datas.length;i++){
    		var rowData = datas[i];
    		var newRowData = {};
    		for(var k=0;k<rowData.length;k++){
    			if(rowData[k].col=="fdId"){
    				newRowData["value"] = rowData[k].value;
    			}
    			if(!newRowData["text"] && rowData[k].col=="docSubject"){
    				newRowData["text"] = rowData[k].value;
    			}
    			if(!newRowData["text"] && rowData[k].col=="fdName"){
    				newRowData["text"] = rowData[k].value;
    			}
    		}
    ...
    buildIcon: function(iconNode, item) {
      if (item.icon) {
        ...
        iconUtils.setIcon("mui mui-file-text", null, null, null, iconNode)
      }
    },
