...
define([ "dojo/_base/declare",
	   ...
	   return declare("mui.form.checkbox.render.CheckBoxTable",[WidgetBase], {
			...
			*/
			_buildRender: function(obj){
		    	if(this.renderType == obj.renderType){
					this.checkboxNode = domConstruct.create(obj.tag, {
						className : "muiCheckBoxContainer muiCheckBoxTableItem" + (obj.unCheckedIcon?" "+obj.unCheckedIcon:""),
					}, obj.domNode, 'after');
					
					this.checkboxText = domConstruct.create('span', {
						className : 'muiCheckBoxTableText',
						innerHTML : obj.text&&obj.text.indexOf("&#039;")>0 ? obj.text.replace(/</g, "&lt;").replace(/>/g, "&gt;") : util.formatText(obj.text)
					}, this.checkboxNode, 'last');
					
					this.checkboxIcon = domConstruct.create('i', {
						className : 'fontmuis muis-form-selected-cor muiCheckBoxTableChecked'
					}, this.checkboxNode);
		    	}
