define([
  ...
  return declare("mui.property.PropertyFilterListMixin", null, {
    ...
    // 白名单
    journalist: ["q.docCategory", "q.kmsExpertType"],

    inJournalist: function(item) {
      for (var i = 0; i < this.journalist.length; i++) {
        var journa = this.journalist[i]
        if (item.indexOf(journa) >= 0) {
          return true
        }
      }
      ...
      array.forEach(
        ...
        function(view) {
          ...
          for (var i = 0; i < paramArr.length; i++) {
            if (
              paramArr[i].indexOf("q.") > -1 &&
              !this.inJournalist(paramArr[i])
            ) {
