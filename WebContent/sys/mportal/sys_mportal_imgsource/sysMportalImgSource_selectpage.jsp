...
<template:include ref="default.simple">
	<template:replace name="title">选择幻灯片</template:replace>
			...
			<td valign="top">
				<list:listview id="listview">
					<ui:source type="AjaxJson">
						{"url":"/sys/mportal/sys_mportal_imgsource/sysMportalImgSource.do?method=listdata&orderby=docAlterTime&ordertype=down"}
					</ui:source>
					<list:colTable sort="false" onRowClick="selectLink('!{fdId}','!{name}')">
						<list:col-checkbox></list:col-checkbox>
						<list:col-serial style="5%;"></list:col-serial>
						<list:col-html title="标题" style="width:35%;">
							{$<div style="overflow: hidden;text-overflow: ellipsis;">{%row['name']%}</div>$}
						</list:col-html>
						<list:col-html title="链接" style="width:35%;">
							{$<div style="overflow: hidden;text-overflow: ellipsis;">{%row['fdUrl']%}</div>$}
						</list:col-html>
						<list:col-html title="更新时间" style="width:18%;">
							{$ {%row['docAlterTime']%}  $}
						</list:col-html>
					</list:colTable>
					<%-- 列表加载后更新状态和绑定事件 --%>
