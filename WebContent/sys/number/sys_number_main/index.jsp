	...
	<template:replace name="content">
		...
		<div class="lui_list_operation">		
			...
			<div class="lui_list_operation_sort_toolbar">
				<ui:toolbar layout="sys.ui.toolbar.sort">
					<list:sortgroup>
					    <list:sort property="fdOrder" text="${lfn:message('sys-number:sysNumberMain.fdOrder') }" group="sort.list" value="up"></list:sort>
						<list:sort property="docCreateTime" text="${lfn:message('sys-number:sysNumberMain.docCreateTime') }" group="sort.list"></list:sort>
						<list:sort property="fdName" text="${lfn:message('sys-number:sysNumberMain.fdName') }" group="sort.list"></list:sort>
					</list:sortgroup>
				</ui:toolbar>
			</div>
					...
					<ui:toolbar id="Btntoolbar">
					    ...
					    <kmss:auth requestURL="/sys/number/sys_number_main/sysNumberMain.do?method=add&modelName=${param.modelName}" requestMethod="GET">
							...
							<c:import url="/sys/profile/common/change_order_num.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.sys.number.model.SysNumberMain"></c:param>
		...
		<list:listview>
			...
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" 
			     rowHref="/sys/number/sys_number_main/sysNumberMain.do?method=view&fdId=!{fdId}&modelName=${param.modelName}&fdKey=${param.fdKey}">
		...
		<!-- 分页 -->
	 	...
	 	<script type="text/javascript">
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
			 	// 增加
		 		window.add = function() {
		 			Com_OpenWindow('<c:url value="/sys/number/sys_number_main/sysNumberMain.do" />?method=add&isSysnumberAdmin=${JsParam.isSysnumberAdmin}&modelName=${JsParam.modelName}&isCustom=2');
		 		};
		 	   // 编辑
		 		window.edit = function(id) {
			 		if(id)
		 				Com_OpenWindow('<c:url value="/sys/number/sys_number_main/sysNumberMain.do" />?method=edit&modelName=${JsParam.modelName}&fdId=' + id);
		 		};
					...
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.ajax({
								url: url,
								type: 'POST',
								data:$.param({"List_Selected":values,"modelName":'${JsParam.modelName}'},true),
								dataType: 'json',
								error: function(data){
