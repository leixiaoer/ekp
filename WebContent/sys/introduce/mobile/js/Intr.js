					...
					[ widgetBase ],
					{
						...
						doSubmit : function(evt) {
							if (!this._validate()){
								...
								if(val.length<1){
									tip.warn({text : Msg['sysIntroduceMain.mobile.introduce.content.notNull.tip']});
								}else{
									tip.warn({text : Msg['sysIntroduceMain.mobile.introduce.type.notNull.tip']});
								}
								return;
							}
							var fromData = {};
						...
						_validateInput : function() {
							var flag = false;
							if(!this.inputNode || !this.inputNode[0]){
								return flag;
							}
							var val = this.inputNode[0].value.trim();
							var leng = val.length;
						...
						_validateEssence : function(){
							...
							this.EssenceDom = query('[name="fdIntroduceToEssence"]');

							if(this.EssenceDom && this.EssenceDom.length > 0){
							
								// 没有选择推荐到精华库并没有选择推荐个人，校验不通过
								if(this.EssenceDom[0].value=="true")
						...
						_validateAddress : function() {
							...
							// 推荐到精华库允许没有推荐目标
							if(this.EssenceDom && this.EssenceDom.length > 0 && this.EssenceDom[0].value=="true"){
								return true;
							}
