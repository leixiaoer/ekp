...
<c:if test="${sysIntroduceForm.introduceForm.fdIsShow=='true'}">
		...
		<ui:content title="<div id='intr_label_title'  name='introviewnames'>${lfn:message('sys-introduce:sysIntroduceMain.tab.introduce.label')}${sysIntroduceForm.introduceForm.fdIntroduceCount}</div>"
			...
			<%ISysIntroduceMainService sysIntroduceMainService= (ISysIntroduceMainService)SpringBeanUtil.getBean("sysIntroduceMainService") ;
			        ...
			        String whereBlock="sysIntroduceMain.fdModelId=:id and sysIntroduceMain.fdModelName=:name and sysIntroduceMain.fdIntroduceToEssence is false and sysIntroduceMain.fdIntroduceToPerson is false and sysIntroduceMain.fdIntroduceToNews is false ";  
			        info.setWhereBlock(whereBlock);
			        info.setParameter("id", id);
			        info.setParameter("name", name);
			      List list=  sysIntroduceMainService.findList(info);
			...
			<ui:content title="${lfn:message('sys-introduce:sysIntroduceMain.introduce.title') }">
				...
				<kmss:auth
	               requestURL="/sys/introduce/sys_introduce_main/sysIntroduceMain.do?method=cancelIntro&fdModelName=${param.fdModelName}&fdModelId=${param.fdModelId }">
				...
				<div id="introduceContent" style="min-height:150px">
					<list:listview channel="intr_ch3">
						...
						<list:rowTable channel="rowtable" isDefault="true" target="_blank" cfg-norecodeLayout="simple">
							...
							<list:row-template>
								{$
									<div class="intr_record_msg"><div class="intr_record_content intr_title_color">${lfn:message('sys-introduce:sysIntroduceMain.cancel.button') }</div>
									<div class="intr_summary">
										<span class="intr_evaler">{%row['fdIntroducer.fdName']%}</span>
										<span class="intr_summary_color">|&nbsp;{%row['fdIntroduceTime']%}</span>
...
<kmss:auth
	requestURL="/sys/introduce/sys_introduce_main/sysIntroduceMain.do?method=cancelIntro&fdModelName=${fdModelName}&fdModelId=${fdModelId}">
	<script>
	...
	<c:if
		test="${baseModel.docIsIntroduced!= null &&baseModel.docIsIntroduced}">
		<ui:button parentId="toolbar" id="cancelIntroduce"
