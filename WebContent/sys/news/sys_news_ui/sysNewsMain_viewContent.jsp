		...
		<div class='lui_form_title_frame'>
			...
			<div class='lui_form_baseinfo'>
				...
				<c:if test="${sysNewsMainForm.fdIsWriter==false}">
					<span>
						<ui:person personId="${sysNewsMainForm.fdAuthorId}"
							personName="${sysNewsMainForm.fdAuthorName}" layer="true">
						</ui:person>
					</span>
				</c:if>
					...
					<c:otherwise>
						...
						<c:if test="${sysNewsMainForm.fdContentType=='word'}">
							<c:choose>
								...
								<c:when test="${_isWpsWebOffice == 'true'}">
								  <div>
										<%
				    	 ...
				    	 <%
						  ...
						  <div class="lui_form_content_frame">
								...
								<!-- #135859  经过排查由于1200高度引起，删去后显示正常-->
						  		<iframe id="pdfFrame"  src="<c:url value="/sys/attachment/sys_att_main/sysAttMain.do?method=view&fdId=${attId}&inner=yes"/>"  width="100%"  style="min-height:565px;/* height:1200px */"  frameborder="0">
								</iframe>
						   </div>
						  <%
						   }else{
