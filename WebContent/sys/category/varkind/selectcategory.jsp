...
<template:include ref="default.simple">
	<template:replace name="title"><bean:message bundle="sys-category" key="dialog.maintree.title" /></template:replace>
	...
	<script>
		function selectSimpleCategory(){
			...
			if(selectNodes!=null){
				...
				alert('<bean:message bundle="sys-category" key="dialog.maintree.title" />');
			}
		}
	</script>
	<script>
	function generateTree(){
		...
		LKSTree = new TreeView(
			"LKSTree",
			'<bean:message bundle="sys-category" key="dialog.maintree.title" />',
			document.getElementById("treeDiv")
		);
		LKSTree.isShowCheckBox = true;
		LKSTree.isMultSel = false;
		LKSTree.treeRoot.AppendBeanData('sysCategoryTreeService&showType=0&getTemplate=2&nodeType=!{nodeType}&authType=00&modelName=' + modelName, null, null, null, null);
		LKSTree.Show();
	}
	</script>
