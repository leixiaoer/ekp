			...
			<div class="conf_show_more_w">
				<div class="conf_btn_edit">
					...
					<kmss:auth requestURL="/sys/notify/sys_notify_todo/sysNotifyTodo.do?method=deleteall" requestMethod="POST">
						<!-- 置为已办事项 -->
						<a class="btn_txt" href="javascript:__del('deleteall','1','${sysNotifyTodo.fdId}')">${lfn:message('sys-notify:sysNotify.todo.finish')}</a>
					</kmss:auth>
					</c:if>
					...
					<kmss:auth requestURL="/sys/notify/sys_notify_todo/sysNotifyTodo.do?method=mngDelete" requestMethod="POST">
						<!-- 置为已办事项 -->
						<a class="btn_txt" href="javascript:__del('mngDelete','1','${sysNotifyTodo.fdId}')">${lfn:message('sys-notify:sysNotify.todo.finish')}</a>
					</kmss:auth>
					</c:if>
					<a class="btn_txt" href="javascript:setNotifySelfTitle('1','${sysNotifyTodo.fdId}')">  ${ lfn:message('sys-notify:sysNotifySelfTitleSetting.self.notify') }</a>
				</div>
			</div>
			<!--操作按钮 结束-->
		</list:data-column>
