...
<table id='tagContent'>
	...
	<tr>
		...
		<c:if test="${param.fdType == 1 || empty param.fdType && empty param.finish}">
			<a target="_blank"
				onclick="Com_OpenNewWindow(this)" data-href='<c:url value="/moduleindex.jsp?nav=/sys/notify/tree.jsp&main=%2Fsys%2Fnotify%2Fsys_notify_todo%2FsysNotifyTodo.do%3Fmethod%3DmngList%26oprType%3Ddoing%26fdType%3D13%26s_path%3D!{message(sys-notify:sysNotify.todo)}"/>'>
		...
		<c:if test="${param.fdType == 2}">
			<a target='_blank'
				onclick="Com_OpenNewWindow(this)" data-href='<c:url value="/moduleindex.jsp?nav=/sys/notify/tree.jsp&main=%2Fsys%2Fnotify%2Fsys_notify_todo%2FsysNotifyTodo.do%3Fmethod%3DmngList%26oprType%3Ddoing%26fdType%3D2%26s_path%3D!{message(sys-notify:sysNotify.todo)}"/>'>
			<span class="trspan1"><bean:message bundle="sys-notify" key="sysMail.home.youHave" />
		...
		<c:if test="${param.finish == 1}">
			<a target='_blank' style='font-weight:bold;color:#0066CC'
				onclick="Com_OpenNewWindow(this)" data-href='<c:url value="/moduleindex.jsp?nav=/sys/notify/tree.jsp&main=%2Fsys%2Fnotify%2Fsys_notify_todo%2FsysNotifyTodo.do%3Fmethod%3DmngList%26oprType%3Ddone%26s_path%3D!{message(sys-notify:sysNotifyTodo.done.item)}" />'>
	...
	<tr>
		...
		<a target="_blank"<c:if test="${not empty model.todo.fdLink}"> onclick="Com_OpenNewWindow(this)" data-href="<c:url value="${model.todo.fdLink}"/>"</c:if>>
