		...
		<div style="width:100%">
		  <ui:tabpanel id="notifyTabpanel" layout="sys.ui.tabpanel.list">
			...
			width: 85%;
			 ...
			 <ui:content id="notifySysToviewContent" title="${ lfn:message('sys-notify:sysNotifyTodo.dataType.todo') }" cfg-route="{path:'/sysUnread'}">
			  <list:criteria id="systoview" channel="list_systoview">
				...
				<list:cri-criterion title="${lfn:message('sys-notify:sysNotifyTodo.todo.category') }" key="fdCateId" multi="false">
					<list:box-select>
						<list:item-select type="lui/criteria!CriterionSelectDatas"  id="fdCateId_systoview">
							<ui:source type="Static">
								${cateJsonArr}
							</ui:source>
							<ui:event event="selectedChanged" args="evt">
								...
								if (vals.length > 0 && vals[0] != null) {
									LUI('moduleNames_systoview').setEnable(false);
									
								}else{
									LUI('moduleNames_systoview').setEnable(true);
								}
							</ui:event>
						</list:item-select>
					</list:box-select>
				...
				<list:cri-criterion title="${lfn:message('sys-notify:sysNotifyTodo.moduleName') }" key="fdModelName" multi="true">
					<list:box-select>
						<list:item-select type="lui/criteria!CriterionSelectDatas"  id="moduleNames_systoview">
				...
				<list:cri-criterion title="${lfn:message('sys-notify:sysNotifyTodo.todo.system') }" key="fdAppName" multi="false">
					<list:box-select>
						<list:item-select type="lui/criteria!CriterionSelectDatas"  id="appNames_systoview">
							<ui:source type="AjaxJson" >
				...
				<div class="lui_list_operation">
					...
					<div class="lui_list_operation_order_btn">
						<list:selectall channel="list_systoview"></list:selectall>
					</div>
					<!-- 分割线 -->
					...
					<div class="lui_list_operation_sort_btn">
						...
						<div class="lui_list_operation_sort_toolbar">
							<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" channel="list_systoview">
								<list:sortgroup>
					...
					<div class="lui_list_operation_page_top">
						<list:paging layout="sys.ui.paging.top"  channel="list_systoview"> 
						</list:paging>
					</div>
				...
				<list:listview id="listview3" channel="list_systoview">
					<ui:source type="AjaxJson">
							{url:'/sys/notify/sys_notify_todo/sysNotifySystemIndex.do?method=list&dataType=systoview&fdType=4&fdCateId=${JsParam.fdCateId}${starCon }'}
					</ui:source>
					<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
						 name="columntable_3" id="fixedForTable_systoview">
						...
						<list:col-html   title="${ lfn:message('sys-notify:sysNotifyTodo.fdSubject') }" style="text-align:left">
							{$ <a class="lui_notify_alink" onclick="onNotifyClick(this,'{%row._fdType%}')" data-href="${KMSS_Parameter_ContextPath}sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=view&fdId={%row.fdId%}" target="_blank">{%row['todo.subject4View']%} </a>$}
						</list:col-html>
					...
					<list:rowTable isDefault="false"
						rowHref="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=view&fdId=!{fdId}" name="rowtable_3" >
							<list:row-template>
						  {$
						  <div class="clearfloat lui_listview_rowtable_summary_content_box">
							<dl>
					         ...
					         <dl>		
					            <dt>
									<a onclick="onNotifyClick(this,'{%row._fdType%}')" data-href="${KMSS_Parameter_ContextPath}sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=view&fdId={%row.fdId%}" class="textEllipsis com_subject" 
									{%row.read%}
			 	...
			 	<list:paging channel="list_systoview"></list:paging>
			  </ui:content>

				<ui:content id="notifySysToviewdoneContent" title="${ lfn:message('sys-notify:sysNotifyTodo.dataType.done') }" cfg-route="{path:'/sysRead'}" >
				  <list:criteria id="systoview_done" channel="list_systoview_done">
					...
					<list:cri-criterion title="${lfn:message('sys-notify:sysNotifyTodo.todo.system') }" key="fdAppName" multi="false">
						<list:box-select>
							<list:item-select type="lui/criteria!CriterionSelectDatas"  id="appNames_systoview_done">
								<ui:source type="AjaxJson" >
									{url: "/sys/notify/sys_notify_category/sysNotifyCategory.do?method=getAppNames&fdAppName=${JsParam.fdAppName}"}
								</ui:source>
									<ui:event event="selectedChanged" args="evt">
			...
			<script type="text/javascript">
				seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/framework/router/router-utils'], function($, dialog , topic,routerUtils) {
					...
					window.mngDelete = function(){
						...
						dialog.confirm('<bean:message bundle="sys-notify" key="sysNotifyTodo.confirm.finish"/>',function(value){
							if(value==true){
								...
								$.post('<c:url value="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=deleteall"/>',
					...
					window.SdelCallback = function(data){
						...
						if(data!=null && data.status==true){
							topic.channel('list_systoview').publish('list.refresh');
					...
					topic.subscribe('successReloadPage', function() {
						topic.channel('list_systoview').publish('list.refresh');
						topic.channel('list_systoview_done').publish("list.refresh");
						topic.publish('portal.notify.refresh');
					});
				...
				window.doStar = function(star){
					...
					$.post('<c:url value="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=doStar"/>',
