		...
		<c:choose>
			<c:when test="${param.finish == 1}">
				<span id="lui_notify_title">
					<a title='<c:out value="${model.subject4View}"/>' class="lui_dataview_classic_link"  target="_blank"<c:if test="${not empty model.fdLink}"> onclick="Com_OpenNewWindow(this)" data-href="<c:url value="${model.fdLink}"/>"</c:if>>
