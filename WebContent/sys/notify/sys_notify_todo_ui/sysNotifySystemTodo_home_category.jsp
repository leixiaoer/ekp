...
<c:if test="${cateListTotal>0 }">
				...
				<ui:content title="${cate.fdName} ${cate.cateCount} ${_cateCountTxt}">
					...
					<ui:dataview>
						<ui:source type="AjaxJson">
							{"url":"/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=listDatasByCate&home=${JsParam.home}&finish=${JsParam.finish}&rowsize=${JsParam.rowsize}&cateFdId=${cate.fdId}&dataType=${JsParam.dataType}&LUIID=${JsParam.LUIID}&sortType=${JsParam.sortType}&displayMode=${JsParam.displayMode}"}
						</ui:source>
						<ui:render type="Template">
							{$ <div class="lui_dataview_classic"> $}
								...
								if(data.length==0){
									...
									}else if(dataType=="systoview"){
										{$<bean:message bundle="sys-notify" key="sysNotifyTodo.toView.info" />$}
									}else if(dataType=="tododone" || dataType=='done' || dataType=='tododone;systoviewdone'){
										{$<bean:message bundle="sys-notify" key="sysNotifyTodo.done.label.2" />$}
									}else if(dataType=="systoviewdone"){
									...
									if(dataType=="tododone" || dataType=='done' || dataType=='tododone;systoviewdone' || dataType=="systoviewdone"){
										_isDone = true;
									}
								}
								...
								for(var i = 0;i<data.length;i++){
									...
									$}
										if(_isDone==true){
											...
											if(data[i].fdLink){
												{$ onclick="Com_OpenNewWindow(this)" data-href="${LUI_ContextPath}<c:url value='{%data[i].fdLink%}'/>" $}
											}
										}else{
											{$ <a title='{%data[i].subject4View%}' class="lui_dataview_classic_link" target="_blank" data-href="${LUI_ContextPath}/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=view&fdId={%data[i].fdId%}" onclick="onNotifyClick(this,'{%data[i].fdType%}');onNotifyCountClick('${cate.fdId }','{%data[i].fdType%}')" $}
										}
	...
	<c:when test="${ displayMode=='noData' }">
		...
		<ul class="lui_toolbar_db_list">
			...
			<c:if test="${HtmlParam.dataType=='systoview'}">
				<c:set var="criCate" value="&cri.list_systoview.q=fdCateId:" />
				<c:set var="catePagePath" value="#j_path=%2Fsystem&dataType=systoview"></c:set>
			</c:if>
			<c:if test="${HtmlParam.dataType=='systoviewdone'}">
				<c:set var="criCate" value="&cri.list_systoview_done.q=fdCateId:" />
				<c:set var="catePagePath" value="#j_path=%2Fsystem&dataType=systoviewdone"></c:set>
			</c:if>
			...
			<c:forEach var="cate" items="${cateList}">
				<li><span><a target="_blank" onclick="Com_OpenNewWindow(this)" data-href="${LUI_ContextPath}/sys/notify/index.jsp${pageScope.catePagePath}${criCate}${cate.fdId}" ><em>${cate.cateCount}</em>${cate.fdName}</a></span></li>	       
			</c:forEach>
		</ul>
	...
	<c:when test="${ displayMode=='singleCategory' }">
		...
		<ui:dataview>
			<ui:source type="AjaxJson">
				{"url":"/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=listDatasByCate&home=${JsParam.home}&finish=${JsParam.finish}&rowsize=${JsParam.rowsize}&cateFdId=${JsParam.notifyCategoryId}&dataType=${JsParam.dataType}&LUIID=${JsParam.LUIID}&sortType=${JsParam.sortType}&displayMode=${JsParam.displayMode}"}
			</ui:source>
			<ui:render type="Template">
				{$ <div class="lui_dataview_classic"> $}
					var dataType = "${JsParam.dataType}";
					...
					for(var i = 0;i<data.length;i++){
						...
						$}
							if(_isDone==true){
								...
								{$ <a title='{%data[i].subject4View%}' class="lui_dataview_classic_link" target="_blank" data-href="${LUI_ContextPath}/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=view&fdId={%data[i].fdId%}" onclick="onNotifyClick(this,'{%data[i].fdType%}');onNotifyCountClick('${JsParam.notifyCategoryId}','{%data[i].fdType%}')" $}
							}
