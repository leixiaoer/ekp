...
<c:choose>
    <c:when test="${portletType=='graphic'}"> <% /**  图文类型的门户组件   **/ %>
            ...
            <c:set var="has_systoview" value="false"></c:set>
            <c:set var="has_systoviewdone" value="false"></c:set>
			...
			<c:forEach items="${requestScope.dataTypeList}" var="dType">
				 ...
				 <c:if test="${dType eq 'systoview'}"> <c:set var="has_systoview" value="true" /> </c:if>
				 <c:if test="${dType eq 'systoviewdone'}"> <c:set var="has_systoviewdone" value="true" /> </c:if>
				 <c:if test="${dType eq 'done'}"> <c:set var="has_done" value="true" /> </c:if>
			</c:forEach>

		    <c:choose>
			    <c:when test="${pageScope.has_todo==true && pageScope.has_systoview==false}">
			        <!-- 需处理待办 -->
			        <c:set var="infoTip" value="sysNotifyTodo.toDo.info"></c:set>
			        <!-- 是否需要显示“快速审批” （是） -->
					<c:set var="fastTodo" value="true"></c:set> 
			    </c:when>
			    <c:when test="${pageScope.has_todo==false && pageScope.has_systoview==true}">
			        <!-- 待阅读事项 -->
					<c:set var="infoTip" value="sysNotifyTodo.toView.info"></c:set>
			    </c:when>
			    <c:when test="${pageScope.has_tododone==false && pageScope.has_systoviewdone==true}">
			        <!-- 已阅读事项 -->
			    ...
			    <c:when test="${has_done==true || has_tododone==true}">
			        <!-- 已处理事项 -->
...
<div class="lui_dataview_classic_heading">
	...
	<span class="lui_notify_count_tip">
		...
		<c:if test="${not empty infoTip}">
			<a class="lui_notify_link_txt" onclick="Com_OpenNewWindow(this)" data-href="${LUI_ContextPath }/sys/notify/index.jsp${pageScope.itemPagePath}${lfn:escapeHtml(_fdAppName)}${crqPrefix}" target="_blank">
	...
	<c:if test="${fastTodo && isCanFastTodo}">
		  <span class="lui_fastApproveIcon" title='<bean:message bundle="sys-notify" key="sysNotifyTodo.tab.title5" />'>
			  <a onclick="Com_OpenNewWindow(this)" data-href="${LUI_ContextPath }/sys/notify/index.jsp#j_path=%2Ffastview${lfn:escapeHtml(_fdAppName)}${crqPrefix}" target="_blank">
