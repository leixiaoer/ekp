...
<c:if test="${param.isShowBtLable!=0}">
	<div id='buttonDiv'>
		<nobr>
			...
			<input type="button" value='<bean:message bundle="sys-notify" key="sysNotifyTodo.type.toDo" />(${toDoCount})'
				...
				onclick='window.open("<c:url value="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=list&home=1&rowsize=10&forKK=${JsParam.forKK}" />","_self");'>
			<input type="button" id="notifyBtn2" value='<bean:message bundle="sys-notify" key="sysNotifyTodo.type.toView" />(${toViewCount})'
				...
				onclick='window.open("<c:url value="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=list&fdType=2&rowsize=10&home=1&forKK=${JsParam.forKK}" />","_self");'>
			...
			<input type="button" value='<bean:message bundle="sys-notify" key="sysNotifyTodo.type.done" />'
				...
				onclick='window.open("<c:url value="/sys/notify/sys_notify_todo/sysNotifySystemTodo.do?method=list&finish=1&rowsize=10&home=1" />","_self");'>
			</c:if>
		</nobr>
	...
	<div class="lui_dataview_classic" >
			...
			<c:if test="${empty portletType || portletType eq '' || portletType=='graphic' || (portletType=='category' && displayMode=='singleCategory') }">
                 ...
                 <%@ include file="/sys/notify/sys_notify_todo_ui/sysNotifySystemTodo_home_remind.jsp"%>
			</c:if>
			...
			<c:choose>
				<c:when test="${portletType=='graphic' }"> <% /**  图文列表    **/ %>
                     <%@ include file="/sys/notify/sys_notify_todo_ui/sysNotifySystemTodo_home_graphic.jsp"%>
				</c:when>
				<c:when test="${portletType=='category'}"> <% /**  分类列表    **/ %>
                     <%@ include file="/sys/notify/sys_notify_todo_ui/sysNotifySystemTodo_home_category.jsp"%>
				</c:when>
				<c:otherwise> <% /**  默认列表    **/ %>
                     <%@ include file="/sys/notify/sys_notify_todo_ui/sysNotifySystemTodo_home_default.jsp"%>
				</c:otherwise>
			</c:choose>
...
<script type="text/javascript">
	function onNotifyClick(hrefObj,notifyType){
        // 是否仅显示单个分类(系统通知类)
		var isShowSingleCategory = ${portletType=='category' && displayMode=='singleCategory'?"true":"false"}; 
		//待阅点击后及时消失
		if(notifyType=='2'||isShowSingleCategory){
			//当前行顶级节点
