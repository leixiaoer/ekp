...
<c:if test="${not empty requestScope.dataType}">
    ...
    <c:if test="${requestScope.dataType eq 'toview' }"> <% /**  待阅   **/ %>
        <c:set var="itemPagePath" value="#j_path=%2Fread&dataType=${lfn:escapeHtml(dataType)}"></c:set>
    </c:if> 
    <c:if test="${requestScope.dataType eq 'tododone' }"> <% /**  已办   **/ %>
        <c:set var="itemPagePath" value="#j_path=%2Fprocess&dataType=${lfn:escapeHtml(dataType)}"></c:set>
    </c:if> 
    <c:if test="${requestScope.dataType eq 'toviewdone' }"> <% /**  已阅   **/ %>
        <c:set var="itemPagePath" value="#j_path=%2Fread&dataType=${lfn:escapeHtml(dataType)}"></c:set>
    </c:if>              
</c:if>
