...
.Ellipsis
{
 ...
 <ui:content title="${ lfn:message('sys-notify:sysNotifyTodo.tab.title2') }" >
  <list:criteria id="toview" channel="list_toview">
	...
	<c:if test="${empty HtmlParam.fdCateId && empty HtmlParam.fdAppName}">
	...
	<c:if test="${not empty HtmlParam.fdAppName}">
		<list:cri-criterion title="${lfn:message('sys-notify:sysNotifyTodo.moduleName') }" key="fdAppName" multi="false">
			<list:box-select>
				<list:item-select type="lui/criteria!CriterionSelectDatas"  >
					<ui:source type="AjaxJson" >
	...
	<div class="lui_list_operation">
		...
		<div style="float:left;">	
			<list:paging channel="list_toview" layout="sys.ui.paging.top" > 		
			</list:paging>
		</div>
	...
	<list:listview id="listview3" channel="list_toview">
		<ui:source type="AjaxJson">
				{url:'/sys/notify/sys_notify_todo/sysNotifyMainIndex.do?method=list&dataType=toview&fdType=2&fdCateId=${JsParam.fdCateId}'}
		</ui:source>
		<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
			...
			<list:col-html   title="${ lfn:message('sys-notify:sysNotifyTodo.fdSubject') }" style="text-align:left">
				{$ <a class="lui_notify_alink" style="display: inline-block;" onclick="onNotifyClick(this,'{%row._fdType%}')" data-href="${KMSS_Parameter_ContextPath}sys/notify/sys_notify_todo/sysNotifyTodo.do?method=view&fdId={%row.fdId%}" target="_blank">{%row['todo.subject4View']%} </a>$}
			</list:col-html>
