...
function getVars(){
	...
	for(var i=0; i<varInfo.length; i++){
		...
		var nameArr = varInfo[i].name.split(".");
		var pNode = this;
		var node;
		for(var j=0; j<textArr.length; j++){
			node = Tree_GetChildByText(pNode, nameArr[j]+"("+textArr[j]+")");
			if(node==null){
				node = pNode.AppendChild(nameArr[j]+"("+textArr[j]+")");
			}
			pNode = node;
		}
		...
		node.title = varInfo[i].name +"(" + varInfo[i].label + ")";
