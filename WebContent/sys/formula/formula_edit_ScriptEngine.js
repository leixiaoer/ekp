...
function getFdHandlerRoleInfoIds(dataObj,dialogObj){
	...
	data.SendToBean("sysFormulaSimulateByJS", action);
}
...
function createVariableArea(varis,varTable){
	...
	for (var i = 0; i < varis.length; i++){
		...
		$(input).attr("name",varis[i].name+".scriptJstype");
		$(input).val(varis[i].type);
		$(td).append(input);
	}
	//必填校验
	for (var i = 0; i < varis.length; i++){
...
function startSimulateFormula(src){
	...
	for (var i = 0; i < varInputs.length; i++){
		if($(varInputs[i]).attr("hidden")){//类型隐藏值不处理
			continue;
		}
	...
	for (var key in obj){
		info[key] = obj[key];
		var scriptKey=key+".scriptJstype"
		var scriptJstype=$("input[name='"+ scriptKey +"']").val();
		if(scriptJstype){
			info[scriptKey]=scriptJstype;
		}
	}
