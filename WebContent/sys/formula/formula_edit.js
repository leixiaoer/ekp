...
//	$(a).attr("onclick","Dialog_Address(false, '" + varInfo.name + "_id','"  + varInfo.name + "_name'," + "null, " + orgType + ");");
	$(a).attr("onclick","Dialog_Address("+((varInfo.type && varInfo.type.indexOf("[]")>-1)?"true":"false")+", '" + varInfo.name + "_id','"  + varInfo.name + "_name'," + "null," +  orgType + ",function(value,name){" +
			"getFdHandlerRoleInfoIds(value,this)});");
	$(a).text("选择");
	parent.appendChild(a);
	if(varInfo.type == "com.landray.kmss.sys.organization.model.SysOrgElement"){
		//起草人身份
...
function startSimulateFormula(src){
		...
		if (varInputs[i].name){
			...
			if (/_id$/.test(varInputs[i].name)){
				...
				if (roleId != null){
					...
					var idParam = [];
					idParam.push(obj[varInputs[i].name.replace("_id","")]);
					idParam.push($("[name='" + varInputs[i].name.replace("_id","_name") + "']",$(varInputs[i]).parent()).val());
					obj[varInputs[i].name.replace("_id","")] = idParam;
				}
			}
	...
	if(window.showModalDialog){//ie模态窗口
		...
		}else if(dialogObject.formulaParameter.model){
			//兼容业务建模
	...
	if(flowChartObject && flowChartObject.isModeling) {
		info["modelId"] = flowChartObject.FdAppModelId;
	}
...
function validateFormula(action){
	...
	//兼容金额字段（BigDecimal_Money），金额字段实质上就是BigDecimal型  by 洪健 2021-8-12
	if(info["returnType"] && info["returnType"].indexOf('BigDecimal_') > -1){
		info["returnType"] = 'BigDecimal';
	}
...
function writeBack(rtnVal) {
	...
	if(success=="1"){
		...
		//若是参数中又强制保存的标志，则进行强制保存
		if(dialogObject.formulaParameter && rtnVal.GetHashMapArray()[0].forceMsg && (dialogObject.formulaParameter.isForceSave == true || dialogObject.formulaParameter.isForceSave == 'true')){
			if(confirm(rtnVal.GetHashMapArray()[0].forceMsg)){
				dialogObject.rtnData = [validateResult];
				dialogObject.AfterShow();
				close();
			}else{
