...
<script type="text/javascript">
	...
	var preventFlag = false;//防止重复点击标识
	//父窗口存储的信息
	...
	function upload(){
		...
		for(var i=0;i<dataJson.length;i++){
			if($.inArray(dataJson[i].fieldType,expControl)>-1||dataJson[i].fieldType=='area'){
				...
				console.log(top.$("[name=docTemplateId]").val())
				break;
			}
		}
		...
		if(file[0].value.indexOf(".xls") == -1){
			...
			//form.action += parseJsonToUrlParam(_paramJsonExcelUpload);
			form.submit();
			$(div).remove();
		}
	}
	...
	function downTemplate(){
		...
		var url = Com_Parameter.ContextPath + "sys/transport/detailTableExport.do?method=deTableExportTemplate" + parseJsonToUrlParam("modelName","itemName","propertyName","isXform",_paramJsonExcelUpload);;	
		Com_OpenWindow(url,"_parent");
	}
</script>
<html:form action="/sys/transport/sys_transport_xform/SysTransportImportUpload.do?method=detailTableUpload" 
