...
var newOpen = {
  ...
  allPageFit: function() {
    ...
    if (
      ...
      newOpen.looper = setInterval(function() {
        ...
        //#96965 修复页面缩放外层高度和水印问题
        //if (newPageSize > allPageSize) {
          window.frameElement.style.height = newPageSize + 46 + "px"
       // } else {
  ...
  scrollHandler: function() {
    ...
    seajs.use("lui/jquery", function($) {
      ...
      // 记录日志
      newOpen.logPage(currentPageNum)
  ...
  loadPage: function(pageNum, otherParam) {
    ...
    if (!commonFuncs.isLoadPage(pageNum)) {
      ...
      seajs.use("lui/jquery", function($) {
        if (fileKeySufix == "-img") {
          //#136932 让ie第一次resize时可以触发重新加载 add by wangl
          commonFuncs.lastScreenWidth = 111
  ...
  setImgSize: function(objImg, pageNum) {
    ...
    if (parentFrame != null) {
      ...
      var height = (availableWidth * 0.96) / (fWidth / fHeight)
  ...
  setFrame: function(pageNum, otherParam) {
    ...
    var topWindow =
      (Com_Parameter && Com_Parameter.top) || frameObj.contentWindow.top
    ...
    if (frameObj && !window.opera) {
      ...
      if (parentFrame != null) {
        ...
        if (fWidth / playerW > 0.96) {
          ...
          seajs.use("lui/jquery", function($) {
            container.css("width", playerW * 0.96)
            container.css("height", (playerW * 0.96) / (fWidth / fHeight))
          })
          newOpen.pageHeights.put(
            pageNum,
        ...
        if (fWidth / availableWidth > 0.96) {
          ...
          newOpen.zoomEle(frameObj, scaleSize, scaleSize)
          seajs.use("lui/jquery", function($) {
            container.css("width", availableWidth * 0.96)
            container.css(
              "height",
      ...
      this.initSize(fWidth * scaleSize, fHeight * scaleSize)
  ...
  loging: false,
  // 记录页码
  logPage: function(pageNum) {
    if (newOpen.timer) {
      clearTimeout(newOpen.timer)
      newOpen.timer = null
    }

    if (newOpen.pageTmp == pageNum || newOpen.loging) {
      return
    }

    // 延迟5秒防止并发
    newOpen.timer = setTimeout(function() {
      newOpen.loging = true

      newOpen.pageTmp = pageNum
      logApi.log(fdId, pageNum, newOpen.type).then(function() {
        newOpen.loging = false
      })
    }, 1000)
  }
}
...
seajs.use("sys/attachment/viewer/resource/common/js/logApi", function(logApi) {
  window.logApi = logApi
  logApi.find(fdId, newOpen.type).then(function(data) {
    if (data.fdId && data.fdParam) {
      var pageNum = parseInt(data.fdParam)
      newOpen.goToPageNum = pageNum
      newOpen.pageTmp = pageNum
    }
