...
String p_fdModelName = attMainFrm.getFdModelName();
String p_fdKey = attMainFrm.getFdKey();
String p_fdFileName = fdFileName;
if ("com.landray.kmss.km.agreement.model.KmAgreementApply".equals(p_fdModelName)
		&& "mainOnline".equals(p_fdKey)) {
	p_fdFileName = StringUtil.escape(p_fdFileName);
}
...
<script type="text/javascript">
	Com_SetWindowTitle("${p_fdFileName}");
	var jg_loadFileName = "${p_fdFileName}";
	<c:if test="${sysAttMainForm.fdModelName eq 'com.landray.kmss.km.agreement.model.KmAgreementApply' && sysAttMainForm.fdKey eq 'mainOnline' }">
		jg_loadFileName = unescape(jg_loadFileName);
	</c:if>
	...
	<c:if test="${sysAttMainForm.fdModelName eq 'com.landray.kmss.km.agreement.model.KmAgreementApply' && docStatus!='' && docStatus eq '30' }">
	jg_attachmentObject_editonline.canPrint = false;
	</c:if>
	<c:if test="${not empty HtmlParam.inner}">
	...
	<c:if test="${sysAttMainForm.fdModelName eq 'com.landray.kmss.km.agreement.model.KmAgreementApply' && docStatus!='' && docStatus eq '30' }">
	jg_attachmentObject_editonline.canDownload = false;
	</c:if>
	...
	Com_AddEventListener(window, "load", function() {
		setTimeout(function(){
			jg_attachmentObject_editonline
					.load(encodeURIComponent(jg_loadFileName), "");
