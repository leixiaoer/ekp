...
<c:forEach items="${attForms.attachments}" var="sysAttMain"	varStatus="vstatus">
		...
		<c:set var="tmp_fdModelName" value="${sysAttMain.fdModelName}"/>
		<c:set var="tmp_FdKey" value="${sysAttMain.fdKey}"/>
</c:forEach>

<%
	String p_fdFileName = (String)pageContext.getAttribute("fdFileName");
	String tmp_fdModelName = (String)pageContext.getAttribute("tmp_fdModelName");
	String tmp_FdKey = (String)pageContext.getAttribute("tmp_FdKey");
	if ("com.landray.kmss.km.agreement.model.KmAgreementApply".equals(tmp_fdModelName)
			&& "mainOnline".equals(tmp_FdKey)) {
		//文件名称转换
		p_fdFileName = StringUtil.escape(p_fdFileName);
	}

	pageContext.setAttribute("fdFileName", p_fdFileName);
%>
...
if (fdKey == 'mainOnline' && fdModelName == 'com.landray.kmss.km.agreement.model.KmAgreementApply') {
	...
	fdDocFileName = unescape(fdDocFileName);
} else {
...
<ui:event event="show">
	if("${JsParam.loadJg}" == 'false'){
		...
		Attachment_ObjectInfo['${JsParam.fdKey}'].load();
