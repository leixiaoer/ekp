...
<%@page import="org.apache.commons.beanutils.PropertyUtils,com.landray.kmss.sys.attachment.plugin.HistoryVersionPlugin,com.landray.kmss.sys.attachment.model.SysAttMainHistoryConfig"%>
...
<script type="text/javascript">
	...
	<c:if test="${empty ____content____ }">
		...
		<%
		String viewKey=request.getParameter("fdKey");
		String viewDetailTableName=viewKey.split("\\.")[0];
		String viewFieldName=viewKey.split("\\.")[1];
		pageContext.setAttribute("viewFieldName", viewFieldName);
		%>
		//明细表内控件 有明细表的 table-add 事件触发初始化
		$(document).on('table-add','table[id$=<%=viewDetailTableName%>]',function(e,row){
			var uid="uid_"+parseInt(((new Date().getTime()+Math.random())*10000)).toString(16);
			$(row).find("input[name *='${viewFieldName}'][type='hidden']").val(uid);
