...
function WPSCloudOffice_AttachmentObject(fdId, fdKey,fdModelId,fdModelName, fdMode,hasHead, wpsPreviewIsLinux, previewCode) {
	this.fdId = fdId;
	...
	this.wpsPreviewIsLinux = wpsPreviewIsLinux;
...
function WPSCloudOffice_Attachment_Load() {
	...
	if((!self.hasLoad && !self.isLoading) || self.forceLoad){
		  _self.isLoading = true;
			if(_self.wpsPreviewIsLinux == 'true' && this.fdMode == 'read') {
				var url = Com_Parameter.ContextPath+"sys/attachment/sys_att_main/sysAttMain.do?method=getWpsPreView";
				$.ajax({
					type:"post",
					url:url,
					data:{fdId:this.fdId,fdMode:this.fdMode,contentFlag:this.contentFlag},
					async:false,    //用同步方式
					success:function(data){
						var wpsUrl =  data.requestUrl;
						if (wpsUrl != '') {
							var mode = 'normal';
							if(_self.hasHead=="simple"){
								mode=_self.hasHead;
							}
							if (_self.previewCode != undefined && _self.previewCode != '') {
								wpsUrl += "&wpsPreview=" + _self.previewCode;
							}
...
function WPSCloudOffice_Attachment_Submit() {
	//如果当前云文档没有显示，则不提交，为了不影响其他模块，加上模块判断
	//谷歌高版本浏览器会出问题，低版本不需要判断也可以
	if ('com.landray.kmss.km.agreement.model.KmAgreementApply' == this.fdModelName) {
