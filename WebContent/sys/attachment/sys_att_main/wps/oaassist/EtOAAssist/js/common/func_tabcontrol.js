...
function AddWorkbookEvent() {
	...
	/*去除文档关闭后事件
	 * try {
		wps.ApiEvent.AddApiEventListener("WorkbookAfterClose",
				OnWorkbookAfterClose);
	} catch (e) {

	}*/
	try {
		wps.ApiEvent.AddApiEventListener("WorkbookBeforePrint",
				OnWorkbookBeforePrint);
	} catch (e) {
...
function SetEtParamsValue(doc, Key, Value){
	if (!doc) return;
...
function OnUploadToServerSuccess(resp) {
	...
	if(resultJson.flag=="ok"){//上传成功
	    var l_doc = wps.EtApplication().ActiveWorkbook;
	    if(isNeedClose){
	    	l_doc.Close(-1); //保存文档后关闭
	    	isNeedClose = false;
        }else{
    	    if (wps.confirm("文件上传成功,是否关闭此窗口。") == true) {
    	        if (l_doc) {
...
function pCheckIfOADoc(doc1) {
	console.log(doc1)
    var doc = doc1||wps.EtApplication().ActiveWorkbook;
    if (!doc){
    	console.log("没有doc")
    	return false;
    }
