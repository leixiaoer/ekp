...
function OnWindowActivate() {
    ...
    setTimeout(activeTab, 1000); // 激活页面必须要页签显示出来，所以做1秒延迟
    return;
}
...
function OnWorkbookBeforeSave(doc) {
    ...
    if(!doc||!wps.EtApplication().ActiveWorkbook){
    	return ;
    }
    if(doc.FullName == wps.EtApplication().ActiveWorkbook.FullName){
...
function OnWorkbookBeforeClose(doc) {
    ...
    l_bIsOADoc = pCheckIfOADoc(doc); //判断是否OA文档要关闭
    if (l_bIsOADoc == false) { // 非OA文档不做处理
    	if(wps.EtApplication().Workbooks.Count==0){
    		closeEtIfNoDocument()
    	}
        return;
    }
    ...
    if (pISOADocReadOnly(doc) == false) {
        if (doc.Saved == false) { //如果OA文档关闭前，有未保存的数据
            ...
            if(!closeFileNoTip){
            	if (etCloseIsTip && wps.confirm("文档已修改，请确认是否保存到OA")) {
                    wps.ApiEvent.Cancel = true;
                    isNeedClose=true
                    OnBtnSaveToServer();//需要判断是否保存成功再关闭，得有一个返回值
                    return;
                }
            	etCloseIsTip=false;
            }
    ...
    pRemoveDocParam(l_fullName); // 关闭文档时，移除PluginStorage对象的参数
 ...
 * @param {*} doc 
 */
function pRemoveDocParam(fileName) {
    if (!fileName) return;
    var isExist=false;
    for(var i=1;i<wps.EtApplication().Workbooks.Count;i++){
    	if(wps.EtApplication().Workbooks.Item(i).FullName==fileName){
    		isExist=true;
    	}
    }
    if(!isExist){
    	wps.PluginStorage.removeItem(fileName);
    }
    return;
}
