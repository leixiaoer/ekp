...
//后台模板是否配置的是新的签约方的映射信息
function checkContractReletiveName(name) {
	if (name == '') {
		return false;
	}
	if(name.indexOf('.')>-1){
		var prefix = name.split('.')[0];
		if (['00','01','02','03'].indexOf(prefix) > -1) {
			return true;
		} else {
			return false;
		}
	...
	for(var cm = 1;cm <= bookmarkCount;cm ++){
		...
		if (cmBookMarkObj) {
			...
			if (typeof bookVal != 'undefined' && bookVal != null && bookVal != '==请选择==') {
				...
				if (initContainTable <= 0 || checkContractReletiveName(formFieldMapping[bookmarkname])) {
			...
			if (rows.Item(n).Range.Bookmarks.Count > 0) {
				...
				for (var v = 1; v <= rows.Item(n).Range.Bookmarks.Count; v ++) {
					...
					if (typeof mapping[bookmarkName] != 'undefined' && mapping[bookmarkName] != '') {
						//签约方的书签映射，即使书签在表格中，也不需要做处理
						if (checkContractReletiveName(mapping[bookmarkName])) {
							break;
						} else {
							//后台配置了书签映射
