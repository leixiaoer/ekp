...
//OA门户网站用接口,配置默认服务器接口
var prefix = wps.PluginStorage.getItem("ekp_server_prefix");
var OA_DOOR = {
    ...
    redHeadsPath: prefix+'/km/imissive/', //默认红头模板列表获取路径
}
 ...
 * @param {*} width 窗口宽
 */
function OnShowDialog(html, title, height, width, bModal,yqqUrl) {
...
function closeWpsIfNoDocument() {
    ...
    if (!docs || docs.Count == 0) {
        ...
        let params={closeWps:true}
        wps.WpsApplication().WindowState=2;//最小化
 ...
 */
function InsertFile(url,bookmark,activeDoc,callback){ 
    DownloadFile(url,function(url){
		...
		// 轮询插入书签
			if(bookmark!="redhead"){
				var imssiveRed = GetDocParamsValue(activeDoc, "bookMarks");
...
/*        if (bookMarks.Exists(bookmark)) {
         ...
         //activeDoc.Activate()
         selection.WholeStory()
         wps.WpsApplication().CommandBars.ExecuteMso("PasteMenu")
        //  doc.Close(0);
        callback&&callback();
		window.close();
    },undefined,true)
...
function CloseWpsClientByEkp(doc){
