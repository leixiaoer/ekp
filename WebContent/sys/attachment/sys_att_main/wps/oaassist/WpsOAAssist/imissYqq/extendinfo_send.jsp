...
<%
	...
	KmImissiveSendMain kmImissiveSendMain = (KmImissiveSendMain)kmImissiveSendMainService.findByPrimaryKey(signId);
	if (kmImissiveSendMain != null) {
		SysOrgPerson user = UserUtil.getUser();
		int processType = kmImissiveSendMainService.getProcessType(signId);
		List<SysOrgElement> signPersons = new ArrayList<SysOrgElement>();
		if (processType == 2) {// 会审
			List<SysOrgElement> currentHandlers = kmImissiveSendMainService.getCurrentHandlers(signId);
		...
		request.setAttribute("kmImissiveSendMain", kmImissiveSendMain);
	...
	function sendYqq(){
		...
		seajs.use(['lui/jquery','lui/dialog'], function($, dialog){
			...
			if(phone==''||phone==null){
				...
				dialog.alert("${modelId}");
				var ajaxUrl = Com_Parameter.ContextPath+'km/imissive/km_imissive_send_main/kmImissiveSendMain.do?method=sendYqq&signId=${signId}&phone='+phone;
				if(checkboxVal == '1'){
