...
<%
	...
	IKmImissiveReceiveMainService kmImissiveReceiveMainService = ((IKmImissiveReceiveMainService)SpringBeanUtil.getBean("kmImissiveReceiveMainService"));
	KmImissiveReceiveMain kmImissiveReceiveMain = (KmImissiveReceiveMain)kmImissiveReceiveMainService.findByPrimaryKey(signId);
	if (kmImissiveReceiveMain != null) {
		SysOrgPerson user = UserUtil.getUser();
		int processType = kmImissiveReceiveMainService.getProcessType(signId);
		List<SysOrgElement> signPersons = new ArrayList<SysOrgElement>();
		if (processType == 2) {// 会审
			List<SysOrgElement> currentHandlers = kmImissiveReceiveMainService.getCurrentHandlers(signId);
		...
		request.setAttribute("kmImissiveReceiveMain", kmImissiveReceiveMain);
...
<script type="text/javascript">
	...
	function dialogSelect(mul, idField, nameField,targetWin){
		seajs.use(['lui/jquery','lui/dialog','lui/dialog_common','lui/util/str'], function($, dialog, dialogCommon,strutil){
			var url = "/elec/yqqs/elec_yqqs_acc_info/elecYqqsAccInfo.do?method=accoutList&moduleSource=km-ImissiveReceive&fdUserName=${fdSigner.fdName}";
	...
	function sendYqq(){
		...
		seajs.use(['lui/jquery','lui/dialog'], function($, dialog){
			...
			if(phone==''||phone==null){
				...
				var ajaxUrl = Com_Parameter.ContextPath+'km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=sendYqq&signId=${signId}&phone='+phone;
				if(checkboxVal == '1'){
				...
				$.ajax({
					...
					success : function(data) {
						...
						if(data){
							...
							var yqqReceiveCode=json['sendStatus'];
							var signId=json['signId'];
							if("true"==yqqReceiveCode){
								var url=Com_Parameter.ContextPath+'km/imissive/km_imissive_receive_main/yqq/yqq_loading.jsp?signId='+signId;
...
<div align="left">
	&nbsp;&nbsp;
	<c:if test="${kmImissiveReceiveMain.fdNeedContent eq '1'}">
		<c:import url="/sys/attachment/sys_att_main/sysAttMain_view.jsp" charEncoding="UTF-8">
			...
			<c:param name="fdModelId" value="${kmImissiveReceiveMain.fdId}" />
		</c:import>	
	</c:if>
	<c:if test="${kmImissiveReceiveMain.fdNeedContent eq '0'}">
		<c:import url="/sys/attachment/sys_att_main/sysAttMain_view.jsp" charEncoding="UTF-8">
