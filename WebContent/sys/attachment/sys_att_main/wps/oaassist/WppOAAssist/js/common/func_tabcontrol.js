...
function SetWppParamsValue(doc, Key, Value){
	if (!doc) return;
 ...
 *  if (!wps.confirm("先保存文档，并开始上传到系统后台，请确认？")) {
    ...
    if (pIsOnlineOADoc(l_doc) == false) {
        ...
        try {
            //调用OA助手的上传方法
            wps.OAAssist.UploadFile(l_UploadName, l_DocPath, l_uploadPath, l_FieldName, "OnUploadToServerSuccess2", "OnUploadToServerFail");
        } catch (err) {
            alert("上传文件失败！请检查系统上传参数及网络环境！");
        }
    } else {
