        ...
        function startWpsInnder(tryCount) {
            ...
            xmlReq.ontimeout = xmlReq.onerror = function (res) {
                ...
                if (tryCount == options.tryCount && options.bPop) { //打开wps并传参
                	var iframe=null
                    try{  
                    	iframe = document.createElement('<iframe name="iframewpsoaassist"></iframe>');  
                    }catch(e){ 
                        iframe = document.createElement('iframe');  
                        iframe.name = 'iframewpsoaassist';  
                    }
                    iframe.src="ksoWPSCloudSvr://start=RelayHttpServer";
                    iframe.width=0;
                    iframe.height=0;
                    document.body.appendChild(iframe);
        ...
        if(isSendWpsOaassitMe){
        	if(navigator && navigator.sendBeacon){
				navigator.sendBeacon(options.url, options.sendData);
			}else{
//				var client = new XMLHttpRequest();
//			    client.open("POST", options.url, true); // 第三个参数表明是同步的 xhr
//			    client.setRequestHeader("Content-Type", "text/plain;charset=UTF-8");
//			    client.send(options.sendData);
    ...
    function WpsStartWrapExInner(options) {
        ...
        if (!options.wpsclient) {
            ...
            var isExistWpsClient=options.param.param.funcs[0];
            if(isExistWpsClient.hasOwnProperty("ExitWPS")||isExistWpsClient.hasOwnProperty("ExitEt")){
            	isSendWpsOaassitMe=true;
            }else
            	isSendWpsOaassitMe=false;
