    ...
    func(WpsInvoke.ClientType.et, // 组件类型
        ...
        function (result) { // 调用回调，status为0为成功，其他是错误
            if (result.status) {
                ...
                WpsInvoke.RegWebNotify(WpsInvoke.ClientType.et, "EtOAAssist", function (messageText) {
                	attWpsAssitHandleEditor(funcs,'unlock');
               	 console.log(messageText);
               	 let json = eval('(' + messageText + ')');
               	 isCloseEt=json.closeEt;
...
function CloseEtClient(docId){
	if(!isCloseEt){
		_EtInvoke([{
	        "ExitEt": {"ekpSysAttMainId":docId,"closeFileNoTip":true}
	    }]);
	}
