...
var server = app.listen(3888, function () {
	console.log(getNow() + "启动本地web服务(http://192.168.2.172:3888)成功！");
	let url="http://192.168.2.172:3888/index.html";
	let exec=cp.exec;
...
function configOemFileInner(oemPath, callback) {
	...
	if (!sup || !sup.JsApiPlugin || !sup.JsApiShowWebDebugger)
		needUpdate = true;
	if (!ser || !ser.JSPluginsServer || ser.JSPluginsServer != "http://192.168.2.172:3888/jsplugins.xml")
		needUpdate = true;
	if (!sup) {
		sup = {}
		config.Support = sup
	...
	ser.JSPluginsServer = "http://192.168.2.172:3888/jsplugins.xml"
	if (needUpdate) {
...
function getServerIPAdress() {
	...
	for (var devName in interfaces) {
		...
		for (var i = 0; i < iface.length; i++) {
			var alias = iface[i];
			if (alias.family === 'IPv4' && alias.address !== '192.168.2.172' && !alias.internal) {
