...
<%
  ...
  pageContext.setAttribute("_useWpsLinuxView_", new Boolean(SysAttWpsCloudUtil.getUseWpsLinuxView()));
%>
<link
...
<list:listview id="listview">
	...
	<list:colTable layout="sys.ui.listview.columntable" name="columntable">
		...
		<list:col-html
			...
			<span class="com_subject" style="cursor: pointer;" onClick="viewHistoryDoc('{% (row['fdFileName']) %}','{% (row['fdId']) %}')">{%row['fdFileName']%}</span> 
					$}
		</list:col-html>
...
<script>
	...
	function viewHistoryDoc(fileName,fileId){
		var isEnableWpsWebOffice = '${isEnableWpsWebOffice}';
		if(isEnableWpsWebOffice == 'true' && "${_useWpsLinuxView_}" != 'true'){
			openByWpsOaassist(fileId);
		}else{
			lisOpenView(fileName,fileId);
		}
	}
