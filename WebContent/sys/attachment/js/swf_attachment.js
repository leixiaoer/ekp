...
try {
	if( (Com_Parameter.top || window.top).window && typeof (Com_Parameter.top || window.top).window.Attachment_ObjectInfo == "undefined"){
		(Com_Parameter.top || window.top).window.Attachment_ObjectInfo = new Array();
	}
} catch (e) {
	if(typeof window.Attachment_ObjectInfo == "undefined"){
		window.Attachment_ObjectInfo = new Array();
	}	
}
...
function Swf_AttachmentObject(
	...
	this.disabledFileType = ".js;.bat;.exe;.sh";
	//是否显示下载次数,默认显示
	...
	*****************************************/
			...
			self.showButton(function(){
				...
				if(self.drawFunction != null){
					...
					if(self.renderFn == null){ 
						...
						setTimeout(function(){ 
							$.ajax(self.renderurl,{dataType:'text',type:'GET',success:function(result){
	...
	this.readDoc = function(docId) {
		...
		if(this.wpsoaassist=="true"&&this.checkOAassistView(docId)&&!this.getUseWpsLinuxView()){
	...
	this.editDoc = function(docId) {
		var doc = self.getDoc(docId);
		if(this.wpsoaassist=="true"){
			var fileName = doc.fileName;
			var fileExt = fileName.substring(fileName.lastIndexOf("."));
			if(fileExt==".docx"||fileExt==".doc"){
				var wpsParam = {};
				var books = "";
				if(this.wpsExtAppModel == "kmImissive"){
...
//				}
		...
		for (var i = 0, size = self.fileList.length; i < size; i++) {
			if (self.fileList[i].fileStatus > -1 && self.fileList[i].fileName == _fileName) {
				...
				var isConfirm = window.confirm(Attachment_MessageInfo["msg.fileName.confirm"].replace("{0}",_fileName));
				if(isConfirm){
	 ...
	 *****************************************/
     this.drawFrame = function(renderCallback){
		 ...
		 if(self.layoutFn == null){	 
			 //要求快速显示附件，取消延迟
			 $.ajax(self.layouturl,{dataType:'text',type:'GET',success:function(result){
