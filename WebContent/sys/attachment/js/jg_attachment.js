...
Com_IncludeFile("sysAttMain_showWindow.jsp",Com_Parameter.ContextPath	+ "sys/attachment/sys_att_main/jg/sysAttMain_showWindow.jsp?_="+new Date().getTime(),"js",true);
		...
		try {
			...
			if (navigator.userAgent.toUpperCase().indexOf("MSIE") < 0 && navigator.userAgent.toUpperCase().indexOf("TRIDENT") < 0) {
				...
				if(session['JSESSIONID']){
					if(OSType.indexOf("Win") > -1 && 
							typeof(eval(this.ocxObj.INetSetCookie)) == 'function'){
						...
						if(showDiv == null){
							...
							_download.onclick = function() {
								...
								if (attachmentObject.fdKey == 'mainOnline' 
									...
									|| attachmentObject.fdModelName == 'com.landray.kmss.km.imissive.model.KmImissiveAttTrack') {
									isAgreement = true;
								}
