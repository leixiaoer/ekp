define([
  ...
  return declare('sys.attachment.audio.AudioLogMixin', [AttachmentLogApi], {
    // 上次记录时间
    lastTime: 0,
    // 每多少秒更新一次
    sec: 10,
    ...
    timeupdate: function() {
      ...
      if (Math.abs(this.audioNode.currentTime - this.lastTime) >= this.sec) {
        this.lastTime = this.audioNode.currentTime
        this.setTime()
      }
    },
    ...
    findDone: function(data) {
      ...
      if (data && data.fdParam) {
        if (data.fdParam) {
          ...
          this.audioNode.currentTime = this.lastTime
        }
      }
    }
  })
