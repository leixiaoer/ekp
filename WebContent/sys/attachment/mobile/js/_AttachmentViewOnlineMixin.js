define([
  ...
  return declare("sys.attachment.mobile.js._AttachmentViewOnlineMixin", null, {
    ...
    getPool: function() {
      ...
      }else if(this.canRead && (this.wpsWebOfficeEnable || this.wpsCloudOfficeEnable) && (fdFileType == 'word' || fdFileType == 'excel' || fdFileType == 'ppt' || fdFileType == 'wps' || fdFileType == 'et' || fdFileType == 'dps')){
    	  pool.push({
              value: 1,
              text: msg["mui.sysAttMain.openfile"],
              callback: function() {
      ...
      if (this.canEdit) {
    	  var _self = this;
    	  if(this.canRead && (this.wpsWebOfficeEnable || this.wpsCloudOfficeEnableEdit || (this.wpsCloudOfficeEnableMobileEdit && devType == 12)) && (fdFileType == 'word' || fdFileType == 'excel' || fdFileType == 'ppt' || fdFileType == 'wps' || fdFileType == 'et' || fdFileType == 'dps')){
    		  pool.push({
                  value: 3,
    ...
    view: function() {
      ...
      if (fileType == "img") {
          ...
          try {
            if (attrObj && attrObj.getType() == "img") {
              if (has("ios") || attrObj.canRead) {
                url = util.formatUrl(
                  attrObj.viewPicHref + "&fdId=" + attrObj.fdId,
                  true
                )
        ...
        var curSrc = ""
        if (this.fdId) {
          curSrc = util.formatUrl(this.viewPicHref + "&fdId=" + this.fdId, true)
        } else {
          curSrc = this.href
        }
      ...
      } else if (fileType == "ofd" && devType == 10) {
        //安卓客户端才支持ofd
    ...
    canViewOnline: function() {
      ...
      return (
        this.hasViewer ||
        this.getType() == "img" ||
        this.canRead && (this.wpsWebOfficeEnable  || this.wpsCloudOfficeEnableEdit || (this.wpsPreviewIsLinux&&isOfficeAndWps) || (this.wpsPreviewIsWindows&&isOfficeAndWps) || canReadOfdOrPdf || (cvo_devType == '10' && this.wpsCloudOfficeEnableMobileEdit)) && isOfficeAndWps ||
