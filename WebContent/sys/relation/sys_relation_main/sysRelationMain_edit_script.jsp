...
<%SysRelationEntryForm sysRelationEntryForm = (SysRelationEntryForm) pageContext.getAttribute("sysRelationEntryForm");%>
	...
	%>
	relationTexts = {
		fdDescription:"<%=SysRelationUtil.replaceJsonQuotes(sysRelationTextForm.getFdDescription())%>"
	};
...
function addRelationEntryHTML(rtnRelaDate) {
	var rtnVal = null;
	if(rtnRelaDate==null){
	...
	content.push("<td style='width: 3%'><input type='checkbox' name='DocList_Selected' /></td>");
	content.push("<td width='5%' align='center'>"+(index+1));
	content.push("<input type=\"text\" name=\""+entryPrefix+"["+index+"].fdModuleName\" class=\"inputsgl\" value=\""+Relation_HtmlEscape(rtnVal.fdModuleName)+"\" style=\"width:90%;text-align: center;\"/>&nbsp;<span class=\"txtstrong\">*</span>" + cellOneHTML);
...
function editRelationEntryHTML() {
	...
	row.cells[2].innerHTML = "<input type=\"text\" name=\""+entryPrefix+"["+index+"].fdModuleName\" class=\"inputsgl\" value=\""+Relation_HtmlEscape(fdModuleName)+"\" style=\"width:90%;text-align: center;\">&nbsp;<span class=\"txtstrong\">*</span>" + cellOneHTML;	
	row.cells[3].innerHTML = getRelationTypeNameByType(rtnVal.fdType);
}
// 获取行号
