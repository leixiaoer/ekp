...
<template:include ref="default.simple">
	<template:replace name="body">
		<link rel="stylesheet" href="${LUI_ContextPath}/sys/relation/sys_relation_main/style/rela_edit.css">
		...
		function generateTree(){
			LKSTree = new TreeView(
					"LKSTree",
					"<bean:message bundle="sys-relation" key="sysRelationEntry.fdSearchScope2"/>",
					document.getElementById("treeDiv")
				);
			n1 = LKSTree.treeRoot;
			n2 = n1.AppendBeanData("sysRelationDocService&fdValue=!{value}");/* sysRelationDocService */
		...
		<div id="div_relation_Container" class="div_relation_Container">
			<div class="div_relation_wrapper div_relation_wrapper_clear">
				<div class="div_relation_wrapper_tree">
					<input type="hidden" name="fdIsTemplate" value="true" >
					<div class="div_relation_wrapper_tree_head">
						<div class="div_relation_wrapper_tree_head_can_selected">
							<span class="div_relation_wrapper_tree_head_can_selected_span">
								<bean:message bundle="sys-relation" key="relationOverView.canSelected"/>
							</span>
						</div>
						<div class="div_relation_wrapper_tree_head_checkAll">
							<label>
								<input id="checkAll" type="checkbox" onclick="selectAll(this)" >
								<span id='checkBoxAll' class="div_relation_wrapper_tree_head_checkAll_span">
									<bean:message bundle="sys-relation" key="relationOverView.selectAll"/>
								</span>
							</label>
				 		</div>
					</div>
					<div class="div_relation_wrapper_tree_data">
						<div id="treeDiv" class="treediv"></div>
					</div>
					<script>generateTree();</script>
				</div>
				
				<div class="div_relation_wrapper_selected_list">
					<div class="div_relation_wrapper_selected_list_head">
						<div class="div_relation_wrapper_selected_list_head_selected">
							<span class="div_relation_wrapper_selected_list_head_selected_span">
								<bean:message bundle="sys-relation" key="relationOverView.selected"/>
							</span>
						</div>
						<div class="div_relation_wrapper_selected_list_head_cancelAll">
							<label>
								<span id='cancelAll' class="div_relation_wrapper_selected_list_head_cancelAll_span" onclick="optionCancel(this,true);">
									<bean:message bundle="sys-relation" key="relationOverView.cancelAll"/>
								</span>
							</label>
					 	</div>
					</div>
			...
			<div>
				<div class="div_relation_docStatus">
					<span class="span_relation_docStatus">
						<bean:message bundle="sys-relation" key="relationOverView.docStatus"/>
					</span>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<span>
						<label>
							<input name="docStatus" type="checkbox" value="30" checked disabled="disabled" />
							<bean:message bundle="sys-relation" key="relationOverView.publish"/>
						</label>
					</span>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<span>
						<label>
							<input name="docStatus" type="checkbox" value="20"  />
							<bean:message bundle="sys-relation" key="relationOverView.examine"/>
						</label>
					</span>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<span>
						<label>
							<input name="docStatus" type="checkbox" value="00"  />
							<bean:message bundle="sys-relation" key="relationOverView.discard"/>
						</label>
					</span>
				</div>
				<% if ("true".equals(SysCommonSensitiveConfig.newInstance().getXformRelevanceEnable())) { %>
					<div class="div_relation_fdDiffusionAuth" >
						<span class="span_relation_fdDiffusionAuth">
							<bean:message bundle="sys-relation" key="sysRelationEntry.fdDiffusionAuth"/>
						</span>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<span>
							<label>
								<input type="checkbox" name="fdDiffusionAuth"/>
								<bean:message bundle="sys-relation" key="sysRelationEntry.fdDiffusionAuth.info"/>
							</label>
						</span>
					</div>
				<% } %>
				<center class="div_relation_opt_button">
		...
		<%@ include file="sysRelationDocPage_edit_script.jsp"%>
		<br>
	</template:replace>
</template:include>