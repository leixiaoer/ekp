...
<input type="hidden" name="fdOtherUrl">
	...
	<tr>	
	<td  style="
    text-align: center;" class="lui_rtp_tb_title"> <bean:message bundle='sys-relation' key='sysRelationEntry.text.des' /></td>
		<td  class="lui_rtp_tb_content">
			<div ><textarea style="width:99%;height:100px"  name="fdDesContent" class="rela_des_textarea"></textarea></div>
		</td>
	</tr>
...
function createRelationEntry(fdOtherUrl){
	return {fdId:"<%=com.landray.kmss.util.IDGenerator.generateID()%>",fdType:"6",fdModuleName:"<bean:message bundle='sys-relation' key='sysRelationEntry.text1' />",fdParameter:"",fdOtherUrl:fdOtherUrl};
}
function dyniFrameSize() {
	try {
		...
		if(window.frameElement!=null && window.frameElement.tagName=="IFRAME"){
			window.frameElement.style.height = (arguObj.offsetHeight + 150) + "px";
...
function doOK() {
	...
	var text= $('textarea[name=fdDesContent]').val();
	var relationTexts ={} ;
	relationTexts['fdDescription']=text;
	relationEntry.relationTexts = relationTexts;
	top.Com_DialogReturn(relationEntry);
}
var contentIndex = 0;
Com_AddEventListener(window, "load", function(){

	var relationTexts = relationEntry.relationTexts;
	if(relationTexts && relationTexts!=null){
		$('textarea[name=fdDesContent]').val("");
		var description= relationTexts.fdDescription;
		$('textarea[name=fdDesContent]').val(description);	
