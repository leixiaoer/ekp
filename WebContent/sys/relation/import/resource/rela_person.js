...
//czk2019
//人员选择配置
function RelationPersonSetting(params){
	this.relationType = "8";
	this.params = params;
	...
	this.onload = function(){
		...
		if(setting.fdId!=null){//编辑
			if(setting.fdType==self.relationType){
				...
				$("#fdPersonIds").val(setting.relationPersons['fdPersonIds']);
				$("#fdPersonNames").val(setting.relationPersons['fdPersonNames']);
			}
		}
	...
	this._rela_get_condition_cfg = function(){
		...
		var personContent={};
		personContent['fdPersonIds']=$("#fdPersonIds").val();
		personContent['fdPersonNames']=$("#fdPersonNames").val();
		setting.relationPersons=personContent;
		return  setting;
	};
