...
//精确搜索配置
function RelationTextSetting(params){
	this.relationType = "6";
	this.params = params;
	...
	this.onload = function(){
		...
		if(setting.fdId!=null){//编辑
			if(setting.fdType==self.relationType){
				...
				$("textarea").val(setting.relationTexts['fdDescription']);
			}
		}
	...
	this._rela_get_condition_cfg = function(){
		...
		var textContent={};
		textContent['fdDescription']=$("textarea").val()
		setting.relationTexts=textContent;
		return  setting;
	};
	//设置域对应值
