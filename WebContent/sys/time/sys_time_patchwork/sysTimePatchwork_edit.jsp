...
function submited(method){
    ...
    $.when(ajaxGetAll).then(function(all){
		...
		var endTime = $('[name="fdEndTime"]').val() || '';
		if(startTime && endTime){
			var date = compareDate(endTime, startTime);
			if( date < 0){
				alert('<bean:message  bundle="sys-time" key="sysTimePatchwork.validate"/>');
				return false;
			}
		...
		if($('input[name="timeType"]:checked').val() == '1') {
			...
			Com_Submit(document.sysTimePatchworkForm, method);
			...
			for(var i=0; true; i++){
				...
				var workOverType = document.getElementsByName("sysTimePatchworkTimeFormList[" + i + "].fdOverTimeType");

				var fdStartTime = document.getElementsByName("sysTimePatchworkTimeFormList[" + i + "].fdStartTime");
				var fdEndOverTimeType = document.getElementsByName("sysTimePatchworkTimeFormList[" + i + "].fdEndOverTimeType");
				var fdOverTime = document.getElementsByName("sysTimePatchworkTimeFormList[" + i + "].fdOverTime");
			...
			var color = $('[name="fdPatchWorkColor"]').val();
			if('${JsParam.method}' == 'edit' && color == '${sysTimePatchworkForm.fdPatchWorkColor}') {
				Com_Submit(document.sysTimePatchworkForm, method);
				return true;
...
<div id="optBarDiv">
	<c:if test="${sysTimePatchworkForm.method_GET=='edit'}">
		<input type=button value="<bean:message key="button.update"/>"
			onclick="submited('update')">
	</c:if>
	<c:if test="${sysTimePatchworkForm.method_GET=='add'}">
		<input type=button value="<bean:message key="button.save"/>"
			onclick="submited('save')">
		<input type=button value="<bean:message key="button.saveadd"/>"
			onclick="submited('saveadd')">
	</c:if>
...
<table class="tb_normal" width=95%>
	...
	<tr>
		...
		<td colspan="3" width="85%">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			<xform:datetime property="fdEndTime" dateTimeType="date" required="true" htmlElementProperties="readonly='true'"/>
		</td>
	</tr>
		...
		<td width="85%" colspan="3">
			<c:choose>
				<c:when test="${sysTimePatchworkForm.timeType != null && sysTimePatchworkForm.timeType != ''}">
					<xform:radio property="timeType" showStatus="edit" onValueChange="handleChangeType" value="${sysTimePatchworkForm.timeType }">
	...
	<tr id="commonClass">
		...
		<td width="85%" colspan="3">
			<xform:select property="sysTimeCommonId" showStatus="edit" onValueChange="handleChangeCommon">
	...
	<tr class="customClass" style="display: none;">
		...
		<td width="85%" colspan="3">
			<c:choose>
				<c:when test="${sysTimePatchworkForm.fdPatchWorkColor != null and sysTimePatchworkForm.fdPatchWorkColor != '' }">
					<input name="fdPatchWorkColor" value="${sysTimePatchworkForm.fdPatchWorkColor}" type="hidden"/>
				</c:when>
				<c:otherwise>
					<input name="fdPatchWorkColor" value="#5484ed" type="hidden"/>
				</c:otherwise>
			</c:choose>
			<ul class="clrfix color_ul">
                 <li class="select">
                 	<c:choose>
                 		<c:when test="${sysTimePatchworkForm.fdPatchWorkColor != null and sysTimePatchworkForm.fdPatchWorkColor != '' }">
                 			<a style="background-color: ${sysTimePatchworkForm.fdPatchWorkColor};"></a>
                 		</c:when>
                 		<c:otherwise>
			        ...
			        $(".color_ul li").each(function () {
			        	if($(this).prop("className") != 'select' && $(this).prop("className") != 'line'){
			        		 $(this).click(function () {
				                ...
				                $("input[name='fdPatchWorkColor']").val(color);
				            });
			        	}
			        ...
			        $('#selfdef').colorpicker({
			            ...
			            success: function (o, color) {
			                ...
			                if(color == '#FFFFFF' || color == '#FFF' || color == '#ffffff' ||color == '#fff'){
			                	 $("input[name='fdPatchWorkColor']").val("");
			                }else{
			                	$("input[name='fdPatchWorkColor']").val(color);
			                }
			            },
	...
	<tr class="customClass" style="display: none;" id="restRow">
		...
		<td width="35%">
			<div id="_xform_fdRestStartTime" _xform_type="datetime">
				<xform:datetime onValueChange="window.calcWorkHour" property="sysTimeCommonTimeForm.fdRestStartTime" subject="午休开始时间" 
					validators="checkStartEnd checkWithWorkTime" showStatus="edit" dateTimeType="time" style="width:95%;" />
			</div>
		</td>
		<td class="td_normal_title" width="15%">
			午休结束时间
		...
		<td width="35%">
			<div id="_xform_fdRestEndTime" _xform_type="datetime">
				<xform:datetime onValueChange="window.calcWorkHour" property="sysTimeCommonTimeForm.fdRestEndTime" subject="午休结束时间" 
					validators="checkStartEnd checkWithWorkTime" showStatus="edit" dateTimeType="time" style="width:95%;" />
			</div>
		</td>
	</tr>
			...
			<table class="tb_normal" width=100% id="TABLE_DocList" style="display: none;">
				...
				<tr KMSS_IsReferRow="1" style="display:none" class="time-row">
					...
					<td>	
					     ...
					     <input type="text" onchange="window.calcWorkHour();" class="inputSgl start-time" style="width:70%" name="sysTimePatchworkTimeFormList[!{index}].fdWorkStartTime" value = "" readonly/>
					...
					<td>
					<xform:select
									property="sysTimePatchworkTimeFormList[!{index}].fdOverTimeType"
					     ...
					     <input type="text" onchange="window.calcWorkHour();" class="inputSgl end-time" style="width:40%"   name="sysTimePatchworkTimeFormList[!{index}].fdWorkEndTime" value = "" readonly/>
					...
					<td style="width: 120px">
						<input type="text"  class="inputSgl begin-start-time" style="width:70%"
							   name="sysTimePatchworkTimeFormList[!{index}].fdStartTime" value = "" readonly/>
						<a href="#" onclick="selectTime('sysTimePatchworkTimeFormList[!{index}].fdStartTime');" styleClass="inputsgl" >
							<bean:message key="dialog.selectTime" /></a>
						<span class="txtstrong">*</span>
					</td>
						<%-- 最晚打卡 --%>
					<td style="">
						<xform:select
								property="sysTimePatchworkTimeFormList[!{index}].fdEndOverTimeType"
								showPleaseSelect="false"
								title="${ lfn:message('sys-time:sysTimeWork.hbmEndTime') }"
						...
						<input type="text"  class="inputSgl end-over-time" style="width:40%"
							   name="sysTimePatchworkTimeFormList[!{index}].fdOverTime" value = "" readonly/>
						<a href="#" onclick="selectTime('sysTimePatchworkTimeFormList[!{index}].fdOverTime');" styleClass="inputsgl" >
							<bean:message key="dialog.selectTime" /></a>
						<span class="txtstrong">*</span>
					</td>
				</tr>
				...
				<tr KMSS_IsContentRow="1" class="time-row">
					...
					<td>
						...
						<input type="text" onchange="window.calcWorkHour();" class="inputSgl start-time" style="width:70%" name="sysTimePatchworkTimeFormList[${vstatus.index}].fdWorkStartTime" value = "${sysTimePatchworkTimeForm.fdWorkStartTime}" readonly/>
					...
					<td>
					<xform:select
						    property="sysTimePatchworkTimeFormList[${vstatus.index}].fdOverTimeType"
						    showPleaseSelect="false"
						    title="${ lfn:message('sys-time:sysTimeWork.hbmEndTime') }"
						    onValueChange="window.calcWorkHour();"
						    style="width:30%;height:32px;margin-right:7px;">
					     ...
					     <input type="text" onchange="window.calcWorkHour();" class="inputSgl end-time" style="width:40%"   name="sysTimePatchworkTimeFormList[${vstatus.index}].fdWorkEndTime" value = "${sysTimePatchworkTimeForm.fdWorkEndTime}" readonly/>
						...
						<%-- 最早打卡 --%>
					<td >
						<input onchange="window._calcWorkHour();" type="text" class="inputSgl begin-start-time" style="width:70%"
							   name="sysTimePatchworkTimeFormList[${vstatus.index}].fdStartTime"
							   value = "${sysTimePatchworkTimeForm.fdStartTime}" readonly/>
						<a href="#" onclick="selectTime('sysTimePatchworkTimeFormList[${vstatus.index}].fdStartTime');" styleClass="inputsgl" >
							<bean:message key="dialog.selectTime" /></a>
						<span class="txtstrong">*</span>
					</td>
						<%-- 最晚打卡 --%>
					<td >
						<xform:select
								property="sysTimePatchworkTimeFormList[${vstatus.index}].fdEndOverTimeType"
								showPleaseSelect="false"
								title="${ lfn:message('sys-time:sysTimeWork.hbmEndTime') }"
								onValueChange="window._calcWorkHour();"
								style="width:30%;height:32px;margin-right:7px;"
						...
						<input type="text"  class="inputSgl end-over-time" style="width:40%"
							   value = "${sysTimePatchworkTimeForm.fdOverTime}"
							   name="sysTimePatchworkTimeFormList[${vstatus.index}].fdOverTime" readonly/>
						<a href="#" onclick="selectTime('sysTimePatchworkTimeFormList[${vstatus.index}].fdOverTime');" styleClass="inputsgl" >
							<bean:message key="dialog.selectTime" /></a>
						<span class="txtstrong">*</span>
					</td>
				</tr>	
...
<script language="JavaScript">
	...
	window.calcWorkHour = function() {
		...
		$('.time-row').each(function(index) {
			...
			var overTime = $('select[name="sysTimePatchworkTimeFormList['+index+'].fdOverTimeType"]').val();
	...
	//自定义必填校验器:校验午休开始结束时间
	validation.addValidator('checkStartEnd','午休开始时间不能晚于午休结束时间',function(v, e, o){
	...
	//自定义必填校验器:校验午休开始结束时间是否在工作时间范围内
	validation.addValidator('checkWithWorkTime','午休时间不能在工作时间范围外',function(v, e, o){
		if(window.checkRestRow()) {
			
			var restStartTime = $('input[name="sysTimeCommonTimeForm.fdRestStartTime"]').val();
			var restEndTime = $('input[name="sysTimeCommonTimeForm.fdRestEndTime"]').val();
			var workStartTime = $('input[name="sysTimePatchworkTimeFormList[0].fdWorkStartTime"]').val();
			var workEndTime = $('input[name="sysTimePatchworkTimeFormList[0].fdWorkEndTime"]').val();
			var overTimeType = $('select[name="sysTimePatchworkTimeFormList[0].fdOverTimeType"]').val();
	...
	if('${JsParam.method}'  == 'edit') {
		if('${sysTimePatchworkForm.timeType}' == '1') {
			handleChangeType('1');
			if('${sysTimePatchworkForm.sysTimeCommonId}') {
				handleChangeCommon('${sysTimePatchworkForm.sysTimeCommonId}');
			}
		} else {
			handleChangeType('2');
		}
