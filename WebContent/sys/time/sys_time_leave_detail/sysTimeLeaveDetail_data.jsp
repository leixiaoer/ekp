...
<list:data>
	<list:data-columns var="sysTimeLeaveDetail" list="${queryPage.list}" varIndex="status">
		<%
			...
			Float fdResumeTime = leaveDetail.getFdResumeTime();
			Float convertTime = SysTimeUtil.getConvertTime();
			if(Integer.valueOf(3).equals(leaveDetail.getFdStatType())){
				fdTotalTime = fdTotalTime/60;
				fdResumeTime = fdResumeTime/60;
			}else{
				fdTotalTime = convertTime * fdTotalTime/(60*24);
				fdResumeTime = convertTime * fdResumeTime/(60*24);
			}
			pageContext.setAttribute("fdTotalTime", fdTotalTime);
			pageContext.setAttribute("fdResumeTime", fdResumeTime);
		%>
		...
		<list:data-column col="fdLeaveTimeHour" escape="false"  title="${ lfn:message('sys-time:sysTimeLeaveDetail.fdLeaveTime') }(${ lfn:message('sys-time:sysTimeLeaveDetail.fdLeaveTime.hour') })">
			<%
					Float __fdTotalTime = (Float)pageContext.getAttribute("fdTotalTime");
					Float __fdResumeTime = (Float)pageContext.getAttribute("fdResumeTime");
					pageContext.setAttribute("fdTotalTimeStr", SysTimeUtil.formatLeaveTimeStr(0, __fdTotalTime));
					pageContext.setAttribute("fdResumeTimeStr", SysTimeUtil.formatLeaveTimeStr(0, __fdResumeTime));
				%>
			${fdTotalTimeStr}
		...
		<list:data-column col="fdLeaveTime" escape="false" title="${ lfn:message('sys-time:sysTimeLeaveDetail.fdLeaveTime') }(${sysTimeLeaveDetail.fdType=='2'?lfn:message('sys-time:sysTimeLeaveDetail.fdLeaveTime.hour'):lfn:message('sys-time:sysTimeLeaveDetail.fdLeaveTime.day') })">
			<%
				Float __fdTotalTime = (Float) pageContext.getAttribute("fdTotalTime");
				if(Integer.valueOf(2).equals(leaveDetail.getFdType())){
					pageContext.setAttribute("fdResumeTimeTxt", "");
					pageContext.setAttribute("fdTotalTimeTxt", SysTimeUtil.formatHourTimeStr(__fdTotalTime));
				}else {

					Float __fdResumeTime = (Float) pageContext.getAttribute("fdResumeTime");
					String fdTotalTimeStr = "";
					if (__fdTotalTime < 0) {
						__fdTotalTime = -__fdTotalTime;
						fdTotalTimeStr = "-";
					}
					pageContext.setAttribute("fdTotalTimeTxt", fdTotalTimeStr + SysTimeUtil.formatLeaveTimeStr(0f, __fdTotalTime));
					pageContext.setAttribute("fdResumeTimeTxt", SysTimeUtil.formatLeaveTimeStr(0f, __fdResumeTime));
				}
				%>
