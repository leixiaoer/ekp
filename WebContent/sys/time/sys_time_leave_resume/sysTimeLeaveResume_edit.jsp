			...
			<div class="lui_form_content_frame" style="padding-top:20px">
				<table class="tb_normal" width=100%>
					...
					<tr>
						...
						<td colspan="3">
							<xform:text property="fdLeaveTime" showStatus="noShow" style="width: 30%" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveResume.fdLeaveTime') }" validators="number min(0.5) max(365) fraction"></xform:text>
							<c:if test="${sysTimeLeaveResumeForm.fdDetailStatType !='3' }">
					...
					jQuery.ajax({
			            ...
			            success: function (data) {
			            	if(data && data.status==1){
			            		...
			            		if(statType!='3'){
			            			times = times/(60*24);
			            			timesTxt = times < 0 ? '' : times;
			            			$('#leaveTimeDayStr').html(data.leaveTimesStr);
			            		}else{
			            			times = times/(60);
			            			timesTxt = times < 0 ? '' : parseInt(times *10)/10;
			            			$('#leaveTimeHourStr').html(data.leaveTimesStr);
			            		}
			            		fdLeaveTime.val(timesTxt);
			            	}else{
			            		fdLeaveTime.val(0);
			            	}
			            },
