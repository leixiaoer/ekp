...
<%
	SysTimeLeaveResumeForm sysTimeLeaveResumeForm = (SysTimeLeaveResumeForm) request.getAttribute("sysTimeLeaveResumeForm");
	Float fdTotalTime = Float.valueOf(sysTimeLeaveResumeForm.getFdTotalTime());
	if("3".equals(sysTimeLeaveResumeForm.getFdDetailStatType())){
		fdTotalTime = fdTotalTime/60;
	}else{
		fdTotalTime = fdTotalTime/(60*24);
	}
						...
						<td colspan="3">
							<c:if test="${sysTimeLeaveResumeForm.fdDetailStatType == '3' }">
								<%
									...
									pageContext.setAttribute("fdTotalTimeTxt", SysTimeUtil.formatLeaveTimeStr(0f, __fdTotalTime));
								%>
								${fdTotalTimeTxt }
							</c:if>
							<c:if test="${sysTimeLeaveResumeForm.fdDetailStatType != '3' }">
								<fmt:formatNumber value="${fdTotalTime}" pattern="#.#"/>${ lfn:message('sys-time:sysTimeLeaveDetail.fdLeaveTime.day') }
							</c:if>
						</td>
					</tr>
					<tr>
