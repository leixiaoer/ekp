...
<list:data>
	<list:data-columns var="sysTimeLeaveAmount" list="${queryPage.list}" varIndex="status">
		<%
				...
				for(SysTimeLeaveAmountItem amountItem : fdAmountItems) {
					if(leaveRule.getFdSerialNo().equals(amountItem.getFdLeaveType())){
						...
						if(Integer.valueOf(3).equals(leaveRule.getFdStatType())) {
							...
							data.put("restDay", SysTimeUtil.formatLeaveTimeStr(leaveDay, leaveHour));
							totalRest += leaveDay;
							deltaHour += leaveHour;
						} else {
							totalRest += rest;
							data.put("restDay", SysTimeUtil.formatFloat(rest));	
						}
						isFind = true;
					}
				}
			...
			pageContext.setAttribute("totalRest", SysTimeUtil.formatLeaveTimeStr(totalRest, deltaHour));
