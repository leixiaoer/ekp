...
<table class="tb_normal" width=100%>
	<tr>
		...
		<td width="35%">
			<xform:address onValueChange="onChangePerson" propertyId="fdPersonId" propertyName="fdPersonName" orgType="ORG_TYPE_PERSON" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdPerson') }" validators="" style="width: 95%">
			</xform:address>
		</td>
		<%-- 年份 --%>
		<td width="15%" class="td_normal_title">
		...
		<td width="35%">
			<input type="hidden" name="_years" value="${_years }" />
			<xform:select property="fdYear" onValueChange="onChangeYear" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdYear') }" showPleaseSelect="false" validators="yearUnique" style="width: 45%">
				<%
					Calendar cal = Calendar.getInstance();
					int thisYear = cal.get(Calendar.YEAR);
					int startYear = thisYear - 10;
					int endYear = thisYear + 10;
				%>
				<c:forEach begin="<%=startYear%>" end="<%=endYear%>" var="curYear">
					<xform:simpleDataSource value="${curYear }">${curYear }</xform:simpleDataSource>
				</c:forEach>
			</xform:select>
		</td>
	</tr>
	...
	<c:forEach items="${sysTimeLeaveAmountForm.fdAmountItems }" var="amountItem" varStatus="vstatus">
		...
		<tr>
			...
			<td width="35%">
				<c:if test="${amountItem.fdIsAuto }">
					${amountItem.fdTotalDay eq null ?  0 : amountItem.fdTotalDay}${ lfn:message('sys-time:sysTimeLeaveAmount.day') }（${ lfn:message('sys-time:sysTimeLeaveAmount.auto.release') }）
					<html:hidden property="fdAmountItems[${vstatus.index }].fdTotalDay" value="${amountItem.fdTotalDay }" />
				</c:if>
				<c:if test="${amountItem.fdIsAuto eq null || !amountItem.fdIsAuto}">
					<xform:text property="fdAmountItems[${vstatus.index }].fdTotalDay" style="width: 95%" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdTotalDay') }" validators="number min(0) max(365) largerThanUsed" onValueChange="onChangeTotal"></xform:text>
				</c:if>
			</td>
			<%-- 失效日期 --%>
			<td width="15%" class="td_normal_title">
			...
			<td width="35%">
				...
				<c:if test="${amountItem.fdIsAuto eq null || !amountItem.fdIsAuto}">
					<xform:datetime property="fdAmountItems[${vstatus.index }].fdValidDate" dateTimeType="date" style="width: 95%" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdValidDate') }" validators="validDateRange" onValueChange="onChangeVDate"></xform:datetime>
				</c:if>
			</td>
		...
		<tr>
			<%-- 上周期剩余 --%>
			<td width="15%" class="td_normal_title">
				${ lfn:message('sys-time:sysTimeLeaveAmount.lastRestDays') }
			</td>
			<td width="35%">
			...
			<%-- 上周期失效日期 --%>
			<td width="15%" class="td_normal_title">
				${ lfn:message('sys-time:sysTimeLeaveAmount.lastValidDate') }
			</td>
			<td width="35%">
