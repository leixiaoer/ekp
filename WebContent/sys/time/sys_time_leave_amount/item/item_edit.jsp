		...
		<tr>
			...
			<td width="35%">
				...
				<c:if test="${amountItem.fdIsAuto eq null || !amountItem.fdIsAuto}">
					<xform:text property="fdAmountItems[${vstatus.index }].fdTotalDay" value="${_fdTotalDay }" style="width: 95%" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdTotalDay') }" validators="number min(0) max(365) largerThanUsed" onValueChange="onChangeTotal"></xform:text>
				</c:if>
			</td>
			<%-- 失效日期 --%>
			<td width="15%" class="td_normal_title">
			...
			<td width="35%">
				...
				<c:if test="${amountItem.fdIsAuto eq null || !amountItem.fdIsAuto}">
					<xform:datetime property="fdAmountItems[${vstatus.index }].fdValidDate" value="${amountItem.fdValidDate }" dateTimeType="date" style="width: 95%" required="true" subject="${ lfn:message('sys-time:sysTimeLeaveAmount.fdValidDate') }" validators="validDateRange" onValueChange="onChangeVDate"></xform:datetime>
				</c:if>
			</td>
