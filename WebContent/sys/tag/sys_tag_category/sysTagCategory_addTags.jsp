...
function validateEmpty() {
	var fdAddTagNames = document.getElementsByName("fdAddTagNames")[0];
	if(fdAddTagNames.value=="") {
		seajs.use(['lui/dialog'],function(dialog) {
			dialog.alert("<bean:message key="sysTagCategory.addTags.msg.tags" bundle="sys-tag"/>");
		});
		return false;
	}
	return true;
...
function dialog_tag(){
	...
	top.window.selectTagNames = $("input[name='fdAddTagNames']").val();
	var url = "/sys/tag/import/addTag.jsp?mulSelect=true&categoryNotFilter=1&catAddSign=1";
...
function after_cat_tag_add(datas) {
	if (datas != null && typeof (datas) != "undefined") {
		...
		$("input[name='fdAddTagNames']").val(str);
	}
}
...
<html:form action="/sys/tag/sys_tag_category/sysTagCategory.do">
	...
	<p class="txttitle" style="color:#3e9ece"><bean:message bundle="sys-tag" key="sysTagCategory.addTags.title" /></p>
	<center>
	<div>
		<div>
			
			<bean:message key="sysTagCategory.addTags.tags" bundle="sys-tag"/>
			
			<input type="text" property="fdAddTagNames" name="fdAddTagNames"  class="inputsgl" readonly>
	...
	<html:hidden property="fdAddTargetId" value="${HtmlParam.fdAddTargetId}"/>
