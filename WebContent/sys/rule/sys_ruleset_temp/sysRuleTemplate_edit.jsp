...
<c:if test="${param.useLabel != 'false'}">
	<tr LKS_LabelName="${lfn:message('sys-rule:sysRuleTemplate.title.1')}" style="display:none" id="sysRule_tab">
		<td>
						...
						<tr style="display:none;" KMSS_IsReferRow="1">
							...
							<td>
								...
								<c:if test="${!isLangSuport }">
									<input class="inputsgl" name="sysRuleTemplateForms.${key }.sysRuleTemplateEntrys[!{index}].fdName" validate="required" subject="${lfn:message('sys-rule:sysRuleTemplate.col.title.1') }" style="width:90%" onblur="sysRuleTemplate.changeName(this.value,this,${vstatus.index });"/>
								</c:if>
								<c:if test="${isLangSuport }">
									<rule:text property="sysRuleTemplateForms.${key }.sysRuleTemplateEntrys[!{index}].fdName" validators="required maxLength(200)" subject="${lfn:message('sys-rule:sysRuleTemplate.col.title.1') }" style="width:75%"  htmlElementProperties="onblur=sysRuleTemplate.changeName(this.value,this,${vstatus.index })"></rule:text>
								</c:if>
