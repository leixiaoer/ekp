...
<c:choose>
	<c:when test="${not empty param['j_content'] && param['j_content']  eq 'true' }">
		...
		<%@ include file="/sys/person/template/person_base_template_old/content.jsp"%>
	</c:when>
	<c:when test="${ (not empty param['j_iframe'] && param['j_iframe'] eq 'true') ||  (not empty param['j_rIframe'] && param['j_rIframe']  eq 'true' )  }">
		...
		<%@ include file="/sys/person/template/person_base_template_old/iframe.jsp"%>
	</c:when>
	<c:otherwise>
		<%-- 完整页面 --%>
		<%@ include file="/sys/person/template/person_base_template_old/all.jsp"%>
	</c:otherwise>
</c:choose>