	...
	<template:replace name="content">
		...
		<list:criteria id="criteria">
			<list:cri-ref key="fdName" ref="criterion.sys.docSubject"
				title="${lfn:message('sys-person:sysPersonSysNavCategory.fdName') }">
			</list:cri-ref>
		</list:criteria>
		<!-- 操作栏 -->
					...
					<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left">
						<list:sortgroup>
							<list:sort property="fdOrder"
								text="${lfn:message('sys-person:sysPersonSysNavCategory.fdOrder') }"
								group="sort.list" value="up"></list:sort>
							<list:sort property="docCreateTime"
								text="${lfn:message('sys-person:sysPersonSysNavCategory.docCreateTime') }"
								group="sort.list"></list:sort>
							<list:sort property="fdName"
								text="${lfn:message('sys-person:sysPersonSysNavCategory.fdName') }"
								group="sort.list"></list:sort>
						</list:sortgroup>
					</ui:toolbar>
				</div>
				...
				<div style="display: inline-block; vertical-align: middle;">
					<ui:toolbar id="Btntoolbar" count="5">
						<kmss:authShow roles="ROLE_SYSPERSON_PORTAL_ADMIN">
							<ui:button text="${lfn:message('sys-person:btn.start')}"
								onclick="PersonOnUpdateStatus(2);" order="1"></ui:button>
							<ui:button text="${lfn:message('sys-person:btn.stop')}"
								onclick="PersonOnUpdateStatus(1);" order="2"></ui:button>
							<ui:button text="${lfn:message('button.add')}" onclick="add();"
								order="3"></ui:button>
							<ui:button text="${lfn:message('button.deleteall')}"
								onclick="PersonOnDeleteAll();" order="4"></ui:button>
							<!-- 快速排序 -->
							<c:import url="/sys/profile/common/change_order_num.jsp"
								charEncoding="UTF-8">
								<c:param name="modelName"
									value="com.landray.kmss.sys.person.model.SysPersonSysNavCategory"></c:param>
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/sys/person/sys_person_sysnav_category/sysPersonSysNavCategory.do?method=list'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-auto
					props="fdOrder,fdName,fdShortName,fdStatus,docCreateTime,docCreator.fdName,operations"></list:col-auto>
			</list:colTable>
		...
		<list:paging />
		<div
			style="margin: 20px 0px; border-top: 1px #C0C0C0 dashed; text-align: center; padding-top: 10px;">
			<div style="display: inline; vertical-align: top;">
				<bean:message bundle="sys-person" key="nav.config.example" />
			</div>
			...
			seajs
					.use(
							...
							function($, dialog, topic) {
								...
								window.add = function() {
									Com_OpenWindow(
											'<c:url value="/sys/person/sys_person_sysnav_category/sysPersonSysNavCategory.do?method=add" />',
											'_blank');
								};
								// 编辑
								window.edit = function(id) {
									if (id)
										Com_OpenWindow('<c:url value="/sys/person/sys_person_sysnav_category/sysPersonSysNavCategory.do" />?method=edit&fdId='
												+ id);
								};
								window.PersonOnUpdateStatus = function(status,
										id) {
									var values = [];
									if (id) {
										values.push(id);
									...
									$
											.post(
													'<c:url value="/sys/person/sys_person_sysnav_category/sysPersonSysNavCategory.do?method=updateStatus"/>',
													$
															.param(
																	{
											...
											.confirm(
													...
													function(value) {
														if (value == true) {
															...
															$
																	.post(
																			'<c:url value="/sys/person/sys_person_sysnav_category/sysPersonSysNavCategory.do?method=deleteall"/>',
																			$
																					.param(
																							{
																								"List_Selected" : values
											...
											.confirm(
													...
													function(value) {
														if (value == true) {
															...
															$
																	.get(
																			'<c:url value="/sys/person/sys_person_sysnav_category/sysPersonSysNavCategory.do?method=delete"/>',
																			$
																					.param(
																							{
																								"fdId" : id
