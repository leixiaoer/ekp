...
function searchUseXML(){
	...
	var currentNode = dialogObject.tree.GetCurrentNode();
	var selectedId = ""
	if (currentNode){
		selectedId = currentNode.value || "";
	}
	beanURL = beanURL.replace(/!\{value\}/g, encodeURIComponent(selectedId));
...
window.onload = function(){
	...
	if(dialogObject.data){
		var _data = dialogObject.data;
		var _key = "";
		if(_data){
			_key = _data._key;
		}
		if ("MAINDATABUSINESS" === _key){
			document.getElementById("updateFunc").style.display = "none";
			document.getElementById("addFunc").style.display = "none";
		}
	}
}
