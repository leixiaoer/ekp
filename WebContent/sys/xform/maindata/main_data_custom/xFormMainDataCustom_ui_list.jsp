...
<list:data>
	<list:data-columns var="sysFormMainDataCustom" list="${queryPage.list }">
		<%--ID--%>
		...
		<list:data-column col="docSubject" title="${ lfn:message('sys-xform-maindata:sysFormJdbcDataSet.docSubject') }" style="text-align:center" escape="false">
			<span class="com_subject"><c:out value="${sysFormMainDataCustom.docSubject}" /></span>
		</list:data-column>
		...
		<list:data-column col="docCategory" title="${ lfn:message('sys-xform-maindata:sysFormJdbcDataSet.docCategory') }" style="text-align:center" escape="false">
			<c:out value="${sysFormMainDataCustom.docCategory.fdName}" />
		</list:data-column>
		
		<list:data-column headerClass="width80" property="docCreator.fdName" title="${ lfn:message('model.fdCreator') }"></list:data-column>
		
		<list:data-column headerClass="width120" property="docCreateTime" title="${ lfn:message('model.fdCreateTime') }"></list:data-column>
		...
		<list:data-column headerClass="width100" col="operations" title="${ lfn:message('list.operation') }" escape="false">
			...
			<div class="conf_show_more_w">
				<div class="conf_btn_edit">					
					<!-- 编辑 -->
					<a class="btn_txt" href="javascript:edit('${sysFormMainDataCustom.fdId}')">${lfn:message('button.edit')}</a>				
					<!-- 删除 -->
					<a class="btn_txt" href="javascript:delDoc('${sysFormMainDataCustom.fdId}')">${lfn:message('button.delete')}</a>					
				</div>
			</div>
			<!--操作按钮 结束-->
		</list:data-column>
