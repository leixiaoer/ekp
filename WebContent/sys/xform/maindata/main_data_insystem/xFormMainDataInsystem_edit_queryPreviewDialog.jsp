...
<link rel="stylesheet" href="${KMSS_Parameter_ContextPath}sys/xform/maindata/resource/css/xFormMainDataInsystem_edit_dialog.css">
	<script>
		...
		function xform_main_data_queryPreviewInit(){
			if(typeof parentWin.xform_main_data_detailWithAllData != 'undefined'){				
				dataJSON = parentWin.xform_main_data_detailWithAllData(false);
				
				if(!parentWin.xform_main_data_validateTr(dataJSON)){
				...
				if(selects){				
					for(var i = 0;i < selects.length;i++){
						...
						inputJSON.fieldText = parentWin.xform_main_data_getFieldsText("xform_main_data_whereTable",select.field,"fdAttrField");
						inputJSON.fieldOperator = parentWin.xform_main_data_getFieldsText("xform_main_data_whereTable",select.fieldOperator,"fdWhereSelectFieldOperator");
				...
				if(returnValue){
					...
					for(var i = 0;i < returnValue.length;i++){
						...
						td += parentWin.xform_main_data_getFieldsText("xform_main_data_returnValueTable",returnValue[i].field,"fdAttrField");
						td += "</td>";
						$title.append(td);
					}	
				}
		...
		function xform_main_data_queryPreview_search(pageno){
			...
			var fdModelName = parentWin.document.getElementsByName("fdModelName")[0];
			if(fdModelName && $(fdModelName).val() != ''){
				//组装
				...
				var url = Com_Parameter.ContextPath + "sys/xform/maindata/main_data_insystem/sysFormMainDataInsystem.do?method=executeQueryPreview&rowsize="+ _xform_main_data_queryPreview_rowSize +"&orderby=fdId&pageno="+pageno;
				$.ajax({
					url: url,
					type:"POST",
		...
		function init(){
			var interval = setInterval(function(){
				...
				parentWin = $dialog.___params.parentWin;
				xform_main_data_queryPreviewInit();
			}, "50");
		}
	</script>
