...
function buildSql(){
	...
	if(dbType == 'mysql'){
		...
		if(isPage == 'true'){
			...
			var tableblockSqlserver = "(select row_number() over(order by "+(isContainsFdId()?"fd_id":_mainData_extend_dialog_dbTable.datas[0].columnName)+" desc) as rn,* from "+ tableName +" where " + whereBlock;
			if(searchBlock != ''){
...
// 返回字段是否包含fd_id字段
function isContainsFdId(){
	if(!_mainData_extend_dialog_dbTable.datas){
		return false;
	}
	for(var i=0;i<_mainData_extend_dialog_dbTable.datas.length;i++){
		var col = _mainData_extend_dialog_dbTable.datas[i];
		if(col && "fd_id"==col.columnName){
			return true;
		}
	}
	return false;
