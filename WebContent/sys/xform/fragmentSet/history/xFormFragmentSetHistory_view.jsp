...
<script>
	...
	function Sys_Form_FragmentSet_loadHistory(templateId, key){
		...
		if(div.is(":hidden")) {
			...
			html.push('<td class="td_normal_title">' + '版本号' + '</td>');
			html.push('<td class="td_normal_title">' + '修改人' + '</td>');
			html.push('<td class="td_normal_title">' + '修改时间' + '</td>');		
			html.push('<td class="td_normal_title">' + '版本类型' + '</td>');
			html.push('</tr>');
			for(var i = 0; i < data.length; i++){
				if(data[i].fdId && data[i].fdId != ''){
					...
					if(i == 0){
						html.push('<td>' + "当前版本" + '</td>');
					}else{
						html.push('<td>' + "历史版本" + '</td>');
					}	
					html.push('</tr>');	
				}
			}
