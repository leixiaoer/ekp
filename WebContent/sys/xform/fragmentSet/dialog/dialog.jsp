...
dialogObject.tree = new TreeView("LKSTree","片段集");
var node = dialogObject.tree.treeRoot;
node.AppendBeanData("${JsParam.springBean}&serviceBean=${JsParam.infoBean}&selectId=!{value}&type=cate&fdMainModelName=${JsParam.fdMainModelName}",
		"${JsParam.springBean}&serviceBean=${JsParam.infoBean}&selectId=!{value}&type=func&fdMainModelName=${JsParam.fdMainModelName}"
);
node.parameter="${JsParam.springBean}&serviceBean=${JsParam.infoBean}&selectId=!{value}&type=func";
</script>
<script type="text/javascript">
initComParameter();
function initComParameter(){
	...
	/* var langArr = Data_GetResourceString("button.ok;button.cancel;dialog.selectNone;message.keyword;dialog.requiredKeyword;"+
		...
		"dialog.description;dialog.currentValue;dialog.requiredSelect;sys-xform-fragmentSet:sysFormFragmentSet.control.updateFunc;"+
		"sys-xform-fragmentSet:sysFormFragmentSet.control.requiredFuncSelect;sys-xform-fragmentSet:sysFormFragmentSet.control.requiredCateSelect;"+
		"sys-xform-fragmentSet:sysFormFragmentSet.control.addFunc"); */
	...
	var updateFunc = "${lfn:message('sys-xform-fragmentSet:sysFormFragmentSet.control.updateFunc')}";
	var requiredFuncSelect = "${lfn:message('sys-xform-fragmentSet:sysFormFragmentSet.control.requiredFuncSelect')}";
	var requiredCateSelect = "${lfn:message('sys-xform-fragmentSet:sysFormFragmentSet.control.requiredCateSelect')}";
	var addFunc = "${lfn:message('sys-xform-fragmentSet:sysFormFragmentSet.control.addFunc')}";
...
function Com_DialogReturn(value){
	if (value != null && value.length != 0) {
		var returnJson = {"_key" :value[0] ,"_keyName" : value[1]};
