	...
	<template:replace name="body">
		<script>
			...
			function submitSelected() {
				...
				dialog.confirm('<bean:message bundle="sys-xform" key="sysFormCommonTemplate.select.confirm"/>',function(value){
					if(value==true){
		...
		<div style="margin:40px auto 60px;width:93%;">
			<div style="border: 1px #e8e8e8 solid;display:none;">
				<div style="width: 98%;margin: 10px auto;">
					<div data-lui-type="lui/search_box!SearchBox">
						<script type="text/config">
						{
						placeholder: "${ lfn:message('sys-xform:sysFormCommonTemplate.select.inputq') }",
						width: '90%'
						}
						</script>
			...
			<div style="border: 1px #e8e8e8 solid;padding: 5px;height:auto;">
				<list:listview id="sourceList">
					<ui:source type="AjaxJson">
						{"url":"/sys/xform/fragmentSet/history/xFormFragmentSetHistory.do?method=getFragmentSetHistory&fdId=${JsParam.fdId}&rowsize=8&isHistory=${JsParam.isHistory}&fdModelName=${JsParam.fdModelName}&fdKey=${JsParam.fdKey}"}
					</ui:source>
					<list:colTable sort="false" onRowClick="selectPage('!{fdId}','!{fdName}')">
						...
						<list:col-auto props="fdName,fdTemplateEdition"></list:col-auto>
					</list:colTable>
					<%-- 列表加载后更新状态和绑定事件 --%>
...
<script type="text/javascript">
	...
	if(dialogObject.winTitle==null)
		dialogObject.winTitle = '<bean:message bundle="sys-xform-fragmentSet" key="table.sysFormFragmentSet" />';
