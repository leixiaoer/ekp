		...
		standardTable : {
			...
			insertValidate:function(cell, control) {
				...
				if (control.implementDetailsTable != true && cell) {
					return $(cell).parents("table[showStatisticRow]").length==0;
				}
				return true;
			},
			...
			domAttrs : {
				td : {
					chooseTable:{
						text: "<a href='javascript:void(0)' style='text-decoration:underline' onclick='javascript:if(Designer.instance.attrPanel.panel.control.onUnLock)Designer.instance.attrPanel.panel.control.onUnLock();Designer.instance.attrPanel.open();'>"+Designer_Lang.controlStandardTableOpenTableAttr+"</a>",
						type: 'help',
						align:'right'
					}
					,
...
function _Designer_Control_Select_OnDraw(parentNode, childNode){
	...
	if (itemsText.length == 0) {
		...
		var lab = '<label class="select_tag_face"';
		if (this.options.values.color){
...
function _Designer_Control_Rtf_OnDraw(parentNode, childNode){
	...
	$(domElement).append("<img src='style/img/rtf.jpg' toFilter='true' width='98%' height='100%'/>");
	if (this.options.values.required == 'true') {
