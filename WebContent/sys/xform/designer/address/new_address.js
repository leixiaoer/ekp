		...
		if(values.scope){
			...
			if(values.scope=='44'){
				...
				for(var i = 0;i<info.length;i++){
					if(info[i].controlType=='detailsTable'){
						continue;
					}
...
//组织架构dialog
function _Designer_Control_New_Address_Show_Dialog_Address(){
	Dialog_Address(true, "new_address_select_value_id", "new_address_select_value_name", ";", ORG_TYPE_ALL,_Designer_Control_New_Address_After_Select);
...
function _Designer_Control_New_Address_OnAttrLoad(form){
	...
	if($('input:radio[name="scope"]:checked').val()=='22'||$('input:radio[name="scope"]:checked').val()=='33'){
		$("input[name='_org_org']").prop('disabled','false');
		$("input[name='_org_dept']").prop('disabled',false);
		$("input[name='_org_post']").prop('disabled',false);
		$("input[name='_org_person']").prop('disabled',false);
...
function _Designer_Control_NewAddress_getDetailstableParent(control){
	if(control && control.parent && control.parent.type == "detailsTable"){
		return control.parent;
	}else if(control && control.parent && control.parent.parent){
		return _Designer_Control_NewAddress_getDetailstableParent(control.parent);
	}else{
		return;
	}
}
