...
<script type="text/javascript">
	...
	function generateTree() {
		LKSTree = new TreeView("LKSTree", "<bean:message bundle="sys-xform-base" key="Designer_Lang.controlNew_Address_templateField"/>", document.getElementById("treeDiv"));
	...
	function getVars(){
		...
		for(var i=0; i < varInfo.length; i++){
			...
			var enumType = varInfo[i].enumType;
			var isCustom = varInfo[i].isCustom;
			if (dataType.indexOf("com.landray.kmss")==0) {
				...
				node.FetchChildrenNode = setShowAndHide;
			}else{
				var textArr = varInfo[i].label.split(".");
				var pNode = this;
				var node;
	...
	function setShowAndHide(){
		...
		var pNode = this;
		var node;
		node = Tree_GetChildByText(pNode, 'showText');
		if(node == null){
			node = pNode.AppendChild('<bean:message bundle="sys-xform-base" key="Designer_Lang.relation_common_base_showText" />');
		}
		node.value = pNode.value+'-showText';
		if(valueData.id != "" && valueData.id == pNode.value+'-showText'){
			LKSTree.SetNodeChecked(node,true);
		}
		var node2;
		node2 = Tree_GetChildByText(pNode, 'hideValue');
		if(node2 == null){
			node2 = pNode.AppendChild('<bean:message bundle="sys-xform-base" key="Designer_Lang.relation_common_base_hiddeValue" />');
		}
		node2.value = pNode.value+'-hideValue';
		if(valueData.id != "" && valueData.id == pNode.value+'-hideValue'){
			LKSTree.SetNodeChecked(node2,true);
		}
	}
	...
	function selectRelatedCtr(){
		...
		for(var i=0; i < varInfo.length; i++){
			if(selectNodes!=null){
				if((selectNodes.value==varInfo[i].name+'-fdId'||selectNodes.value==varInfo[i].name+'-fdName'||selectNodes.value==varInfo[i].name+'-showText'||selectNodes.value==varInfo[i].name+'-hideValue')&&varInfo[i].name!='showText'&&varInfo[i].name!='hideValue'&&varInfo[i].name!='fdName'&&varInfo[i].name!='fdId'){
					bool=true;
		...
		if(selectNodes!=null){
			...
			if(bool){
				...
				if(selectNodes.parent && selectNodes.parent.enumType){
					info.enumType = selectNodes.parent.enumType;
				}
				if(selectNodes.parent && selectNodes.parent.isCustom && selectNodes.parent.isCustom=='true'){
					info.isCustom = selectNodes.parent.isCustom;
				}
				data.push(info);
			}else{
				info.id = selectNodes.value;
				info.name = selectNodes.text;
				if(selectNodes.isCustom && selectNodes.isCustom=='true'){
					info.isCustom = selectNodes.isCustom;
				}
				data.push(info);
			}
