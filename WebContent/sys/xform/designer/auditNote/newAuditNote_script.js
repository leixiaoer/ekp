...
function initialCommonUsages(name){
	...
	kmssData.SendToUrl(Com_Parameter.ContextPath + 'sys/lbpmservice/support/lbpm_usage/lbpmUsage.do?method=getUsagesIsAppend',
		function(http_request){
			var responseText = http_request.responseText;
	...
	if(lbpm && lbpm.canHideNextNodeTr && Lbpm_SettingInfo && Lbpm_SettingInfo.isHideOperationsRow == "true"){
		...
		if(operationAuditTDContent){
			...
			if (operatorInfo != null) {
				//生成即将流向节点html
				var html = newAuditNoteGenerateNextNodeInfo(null,id);
				// #149066 新审批操作控件，通过、转办、加签、补签操作均没有带出系统通知选项
				var currentNodeObj = lbpm.globals.getNextNodeObj(lbpm.nowNodeId);
