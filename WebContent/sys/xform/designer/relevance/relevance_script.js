...
$(function(){
	...
	$("xformflag[flagtype='relevance']:not([id*='!{index}'])").each(function(index,dom){
...
function relevance_main_update_buildControlData(){
	...
	if(validateEles.length>0){
		for(var j = 0;j<validateEles.length;j++){
			if(validateEles[j].reminder){
				validateEles[j].reminder.show();
			}
		}
		//定位到第一个提示地方
