...
function RelevanceDoc_Init(config){
	...
	Xform_ObjectInfo.Xform_Controls.relevanceObj.push(self);
}
...
function RelevanceDoc_OpenDialog(){
	...
	seajs.use(['lui/dialog'], function(dialog) {
		...
		var url = '/sys/xform/controls/relevance.do?method=relevanceDialog&mainModelName='+ Xform_ObjectInfo.mainModelName + '&fdControlId='+ self.parsedControlId + '&inputParams='+ encodeURIComponent(inputParams);
...
// 刷新数据列表
function RelevanceDoc_UpdateDataList(isEdit){
	var self = this;
	if(isEdit == null){
		isEdit = false;
	}
