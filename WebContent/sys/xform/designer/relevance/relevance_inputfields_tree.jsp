...
<script type="text/javascript">
	...
	function getVars(){
		...
		for(var i=0; i < varInfo.length; i++){
			...
			var dictType = varInfo[i].dictType;
	...
	function selectRelatedCtr(){
		...
		for(var i=0; i < varInfo.length; i++){
			if(selectNodes!=null){
				if((selectNodes.value==varInfo[i].name+'.fdId'||selectNodes.value==varInfo[i].name+'.fdName')&&varInfo[i].name!='fdName'&&varInfo[i].name!='fdId'){
					bool=true;
		...
		if(selectNodes!=null){
			...
			if(bool){
				...
				if(selectNodes.parent && selectNodes.parent.dictType){
					info.dictType = selectNodes.parent.dictType;
				}
				data.push(info);
			}else{
				info.id = selectNodes.value;
				...
				if(selectNodes.dictType){
					info.dictType = selectNodes.dictType;
				}
				data.push(info);
			}
