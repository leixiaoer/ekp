...
function _Designer_Control_Right_InsertValidate(div, control) {
	...
	if(control.type == 'relevance'){
		alert('关联控件不支持权限设置！');
		return false;
	}else if(control.type == 'bookticket'){
		alert('携程控件不支持权限设置！');
		return false;
	}
	//权限区段插入明细表后，不能插入明细表中的控件也不能插入该权限区段
	if (control.implementDetailsTable != true && div) {
		return $(div).parents("table[fd_type='detailsTable']").length==0;
	}
	return true;
}
		...
		if (nodes != null && nodes.length > 0) {
			...
			for (var i = 0; i < nodes.length; i ++) {
				...
				this.addNode(nid, 
						...
						//+(i == 0 ? ' checked ' : '')
						+'><label onclick="this.previousSibling.click();">' + node.value + ":" + node.name + '</label>', 
