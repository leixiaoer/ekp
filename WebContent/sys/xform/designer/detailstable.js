...
Designer_Config.controls.detailsTable = {
	...
	attrs : {
		...
		performance : {
            text: Designer_Lang.controlAttrPerformance,
            value: "",
            type: 'self',
            draw: _Designer_Control_DetailsTable_Performance_Options_Self_Draw,
            show: true,
        },
		width : {
...
function _Designer_Control_DetailsTable_OnDrawEnd() {
	...
	if (values) {
		...
		domElement.setAttribute('performance', values.performance);
...
function _Designer_Control_DetailsTable_Performance_Options_Self_Draw(name, attr, value, form, attrs, values,control){
    var html = '';
    html += "<label isfor='true'><input type='checkbox' name='performance' value='false'";
    var performance = control.options.values.performance || 'false';
    if(performance == 'true'){
    	html += " checked='checked'";
    }
    html += " onclick='_Designer_Control_DetailsTable_Performance_SetValue(this);'/><span class='performanceText' isChanged='true'>" + Designer_Lang.controlAttrPerformance + "</span></label>";
    html += " <span class='performanceTip'><img class='tipIcon' src='" + Com_Parameter.ContextPath + "sys/xform/designer/style/img/promptControl.png'></img><span class='tipContent'>" + Designer_Lang.controlAttrPerformanceDesc +"</span></span>";
    //延时赋值
    setTimeout(function(){
        if(document.getElementsByName("performance")[0]){
            document.getElementsByName("performance")[0].value = performance;
        }
    },0);
    return Designer_AttrPanel.wrapTitle(name, attr, value, html);
}

function _Designer_Control_DetailsTable_Performance_SetValue(dom){
    if(document.getElementsByName("performance")[0]){
        document.getElementsByName("performance")[0].value = dom.checked ? 'true':'false';
    }
}
