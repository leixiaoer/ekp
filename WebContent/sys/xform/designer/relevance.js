...
Designer_Config.controls['relevance'] = {
		...
		insertValidate: _Designer_Control_Relevance_InsertValidate, //插入校验，用于不支持权限控件
	...
	if(isOpenInSys){
		...
		setTimeout(function(){
			...
			if(document.getElementsByName("diffusionAuth")[0]){
				document.getElementsByName("diffusionAuth")[0].value = values.diffusionAuth || '';
			}
		},0);
		return Designer_AttrPanel.wrapTitle(name, attr, value, html.join(''));
	}
...
//获得映射字段内容
function _Designer_Control_Relevance_SetInfo(){
	var info = [];
	$.ajax({
		  url: Com_Parameter.ContextPath + "sys/xform/controls/relevance.do?method=getMappingFileds&modelRangeValue="+encodeURIComponent($("input[name='modelRangeValue']").val()),
		  type:'GET',
		  async:false,//同步请求
	...
	if(value){
		...
		for(var fid in inputParamsMapping){
			...
			if(param.fieldDataType && param.fieldDataTypeForm){
				...
				alert(Designer_Lang.relevance_mappingFiled + Designer_Lang.relevance_or + Designer_Lang.relevance_inputFiled + Designer_Lang.relevance_noExist);
				return false;
			}
		}
	}
