...
function RelationRuleRun(bindDomId,bindEvent,destDomIdsStr,opsStr,assemblyId){
			...
			for(var i=0;i<destIds.length;i++){
				...
				if(destDomId.indexOf(".")>=0){
					...
					$xformflag.each(function(){
						...
						setTimeout(
							function(){
							    self.setTimeoutExeOpt(tempControlId,tempOp)},200);
					});
				//非明细表处理
				}else{
	...
	this.getDetailTr = function(row){
		var tableObj = $(row).parents("table[fd_type='detailsTable']")[0];
		if(tableObj){
	...
	if(AttachXFormValueChangeEventById&&this.bindEvent=='change'){
		if ($("[xform_type][id='_" + bindDomId + "']").length > 0) { // 兼容属性变更作为触发控件
...
function __fixMaxLevel(baseField) {
	 ...
	 /*if(!baseField.target.element.is(":visible") && baseField.target.cache.maxLevel == 1 && baseField.target.element.closest("[fd_type='mutiTab']").length > 0){
