...
<body>
	...
	<table class="tb_normal" width=95%>
		...
		<tr>
			<td class="td_normal_title" width=40%><bean:message bundle="sys-xform-base" key="Designer_Lang.fieldLayout_height" /></td>
			<td><input type='text' id='control_height' class='inputsgl' style="width:80%" storage="true" value="80px"/></td>
		</tr>
		<tr>
			<td class="td_normal_title" width=40%><bean:message bundle="sys-xform-base" key="Designer_Lang.fieldLayout_contentLength" /></td>
...
function checkOK(){
	...
	if(control_height&&!/^\d+%$|^\d+px$/g.test(control_height)){
		alert('<bean:message bundle="sys-xform-base" key="Designer_Lang.fieldLayout_heightValidate" />');
		return false;
	}
	var control_content=$("#control_content").val();
	if(control_content&&!/^\d+$/g.test(control_content)){
		alert('<bean:message bundle="sys-xform-base" key="Designer_Lang.fieldLayout_contentLengthValidate" />');
		return false;
	}
	return true;
}
