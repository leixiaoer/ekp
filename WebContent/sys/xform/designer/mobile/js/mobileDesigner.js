...
(function(config){
	...
	function getUseDefaultLayout() {
		var $useDefaultLayout = $("[name='sysFormTemplateForms." + config.fdKey + ".fdUseDefaultLayout']");
	...
	$(document).ready(function() {
		...
		var _$useDefaultLayout = $("[name='_sysFormTemplateForms." + config.fdKey + ".fdUseDefaultLayout']");
		...
		if (isUseSubForm()) {
			...
			for (var key in allPcControls) {
				...
				for (var i = 0;i < formControls.length; i++) {
					...
					if (!isExit && !isDefault) {
						if (formControls[i].type === "detailsTable" && currFormId && currFormId != key) {
							continue;
						}
	...
	function preview() {
		...
		if (clientType == "pc") {
			...
			url += "&fdModelName=" + pcDesigner.fdModelName;
	...
	function __isAllowAdd(control,mobileDesigner) {
		...
		if (control.parent && control.parent.type === "detailsTable") {
	...
	function controlClick(self){
		...
		if (mobileDesigner) {
			...
			if (pcFormControl) {
				...
				if (!isAllowAdd) {
					if (pcFormControl.parent && pcFormControl.parent.type === "detailsTable") {
...
//		}
	...
	function shouldTransfer(transferControl,designer) {
		...
		if (!tableControl || tableControl.type === "detailsTable") {
				return false;
		} 
		if (tableControl) {
	...
	function createControl(selectedControl, currElement,callback) {
		...
		if (mobileDesigner.builder.isAddRow) {
			...
			if (tableControl) {
				...
				if (tableControl.type === "detailsTable") {
	...
	function _findStandardTableRow(currentElement) {
		...
		var $currentTable = $(currentElement).closest("table[formdesign='landray']");
		var isDetailTable = $currentTable.attr("fd_type") == "detailsTable";
		if (isDetailTable) {
	...
	function _isInSameScope(pcControl,currentElement) {
		var detailTableType = "detailsTable";
		var standardTableType = "standardTable";
		if(pcControl.type === detailTableType) {
			return true;
		}
		...
		if (pcTableType === detailTableType) {
			if (mobileTableType != detailTableType) { //pc明细表内的控件不允许添加到移动明细表外
				return false;
			}
			if (mobileTableType == detailTableType) {
				if (pcTableId != mobileTableId) { //pc明细表内的控件不允许添加到移动其它明细表内
					return false;
				} else {
		...
		if (pcTableType === standardTableType) {
			if (mobileTableType == detailTableType) { //pc明细表外的控件不允许添加到移动明细表内
				return false;
			}
		}
		return true;
	...
	function recoveryJSPContent($mobileHtmlDiv){
	 ...
	 function removeTdWidth(){
		 var mobileDesigner = this.getMobileDesigner();
		$(mobileDesigner.builderDomElement).find("td[width]").removeAttr("width");
	 }
...
//		$tr.attr("onmouseout","MobileDesigner.instance.bindEvent(event,'formMouseOut')");
	...
	function loadControls(){
		...
		for (var key in otherFormControls) {
			...
			var pControl;
			if (otherFormControls[key].parent && ("detailsTable" === otherFormControls[key].parent.type || 
	...
	function getCancidateControl(designer,curMobileFormId) {
		...
		//pc表单所有的控件
//		var $mobileFormTr = $("#" + curMobileFormId);
//		var pcFormId = getPcFormTrByMobileTr($mobileFormTr).attr("id");
	...
	function __buildControlHtml(control,mobileDesigner){
		...
		if (control.type === "detailsTable" || control.type === "composite" 
