	...
	for(var i = 0;i < executors.length;i++){
		...
		if(executor.attr('isrow')=='true'){
			var ___relateId = relateId.substring(relateId.lastIndexOf('.') + 1,relateId.length),
				relatedoms =$(GetXFormFieldById(relateId, true));// $('[name$="'+___relateId+')"]');
			var isExit = false;
			for(var j = 0;j < relatedoms.length;j++){
				if (XForm_DateFormatGetTableRr(executor[0]) === XForm_DateFormatGetTableRr(relatedoms[j])) {
		...
		for (var j = 0, m = elems.length; j < m; j ++) {
			...
			if (elem && elem.name != null && elem.getAttribute && elem.getAttribute("relatedid")!=null&&elem.getAttribute("relatedid").indexOf(varName) > -1&& elem.getAttribute("dateFormatType")!=null) {
				if ( elem.getAttribute("isrow") == 'true' && /\.(\d+)\./g.test(dom.name)) {
					if (XForm_DateFormatGetTableRr(elem) === XForm_DateFormatGetTableRr(dom)) {
						executors.push(elem) ;
					}
				}else{
					executors.push(elem);
...
//返回控件所在行
function XForm_DateFormatGetTableRr(dom) {
...
//附加Change的监听事件
function XForm_DateFormatOnLoad() {
...
Com_AddEventListener(window, 'load', XForm_DateFormatOnLoad);