define([
  ...
  var NewAddress = declare("sys.xform.mobile.controls.NewAddress", [Address], {
    ...
    startup: function() {
      this.inherited(arguments);
      var method = util.getUrlParameter(window.top.location.href, "method");
      //新建时有初始值和传参时自动加载
      if ((this.right=="edit") &&
        this.autoload &&
        this.autoload == "true"
      ...
      if (
        ...
        for (var i = 0; i < autoFiledIds.length; i++) {
          ...
          if (autoFiledIds[i].indexOf(".") > -1 && this._inDetailTable) {
            ...
            //地址本和所需赋值的控件都不在明细表
            var wgts = xUtil.getXformWidgetsBlur(null, autoFiledIds[i]);
    ...
    setOutputParams: function() {
      var outputParamsMapping = JSON.parse(
        this.outputParams.replace(/quot;/g, '"')
      );
      for (var fid in outputParamsMapping) {
        ...
        if (this.curIds != "") {
          ...
          $.ajax({
            url:
              ...
              "sys/xform/controls/address.do?method=getInfo&orgType=" +
              encodeURIComponent(this.type) +
              "&id=" +
              this.curIds +
              "&fd=" +
        ...
        if (idform.indexOf(".") > -1 && this._inDetailTable) {
          //地址本和所需赋值的控件都在明细表
          idform = idform.replace(".", rowIndex);
          var wgt = xUtil.getXformWidget("TABLE_DL_" + detailFromId, idform);
          this._filedSetValue(wgt, msg, idform);
        } else if (idform.indexOf(".") < 0 && !this._inDetailTable) {
          //地址本和所需赋值的控件都不在明细表
          var wgts = xUtil.getXformWidgetsBlur(null, idform);
          for (var i = 0; i < wgts.length; i++) {
            this._filedSetValue(wgts[i], msg, idform);
          }
        } else if (idform.indexOf(".") > -1 && !this._inDetailTable) {
          //地址本不在，所需赋值的控件在
          continue;
        } else if (idform.indexOf(".") < 0 && this._inDetailTable) {
          //地址本在，所需赋值的控件不在
    ...
    _clearValue: function(srcObj, argus) {
      if (srcObj != null && this != srcObj && this.isFormulaLoad) {
        ...
        if (
          ...
          this.set("curNames", "");
          this.set("curIds", "");
          var addressShow = query(".muiCateFiledShow", this.domNode);
          if (addressShow[0]) {
            domConstruct.empty(addressShow[0]);
          }
        }
      }
    },
