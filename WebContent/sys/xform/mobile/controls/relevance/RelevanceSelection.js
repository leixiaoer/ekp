define([ "dojo/_base/declare", "dijit/_WidgetBase", "dojo/_base/array",
		...
		function(declare, WidgetBase,array, ScrollableView,dom, domConstruct, domStyle, domClass,
			...
			var selection = declare("sys.xform.mobile.controls.relevance.RelevanceSelection",[ WidgetBase ],{
				//非必填项,用于初始值
				curIds : null,
				...
				_subSelItem:function(){
					if(this.cateSelArr.length>0){
						topic.publish("/sys/xform/relevance/main/submit" , this, this._calcCurSel());
					}else{
				...
				_initSelection:function(){
					if(this.curIds && this.curIds != "null"){
						...
						var names = this.curSubjects.split(";");
						var fdModelNames = this.curFdModelNames.split(";");
						var isCreators = this.curIsCreators.split(";");
						for(var i = 0;i<ids.length;i++){
							var item = {};
							item.fdId = ids[i];
							item.label = names[i];
							item.fdModelName = fdModelNames[i];
							item.isCreator = isCreators[i];
				...
				_calcCurSel:function(){
					...
					if(this.cateSelArr.length>0){
						...
						array.forEach(this.cateSelArr,function(selItem){
							ids += ';' + selItem.fdId;
							names += ';' + selItem.label;
							fdModelNames += ';' + selItem.fdModelName;
							isCreators += ';' + selItem.isCreator;
						});
						if(ids!=''){
							ids = ids.substring(1);
							names = names.substring(1);
							fdModelNames = fdModelNames.substring(1);
							isCreators = isCreators.substring(1);
							eCxt.curIds = ids;
							eCxt.subjects = names;
							eCxt.fdModelNames = fdModelNames;
							eCxt.isCreators = isCreators;
						}
					}
					return eCxt;
				}
