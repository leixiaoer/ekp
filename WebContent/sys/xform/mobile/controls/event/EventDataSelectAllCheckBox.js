	...
	return declare("sys.xform.mobile.controls.event.EventDataSelectAllCheckBox",
			[ WidgetBase ], {
				...
				buildRendering : function() {
					...
					this.selectAllCheckboxArea = domConstruct.create("div",{'className':'muiEventSelectAllCheckboxArea'},this.domNode);
					this.selectAllCheckbox = domConstruct.create("div",{'className':'muiEventSelectAllCheckbox'},this.selectAllCheckboxArea);
					/*domStyle.set(this.selectAllCheckbox,"display","inline-block");*/
					this.selectAll = domConstruct.create("span",{'innerHTML':Msg["mui.event.selectAll"]},this.selectAllCheckboxArea);
					domStyle.set(this.selectAll,"display","inline-block");
					if (this.params && !this.params.isMul) {
						domStyle.set(this.selectAllCheckboxArea,"display","none");
					}
				},

				postCreate : function() {
					this.inherited(arguments);
					this.subscribe("/sys/xform/event/checked","_checked");
					this.subscribe("/sys/xform/event/unchecked","unchecked");
					this.subscribe("/sys/xform/event/searchData","unchecked");
				...
				_select:function(evt){ //全选按钮点击事件
					...
					topic.publish("/sys/xform/event/selectAll",this); //发布全选事件
				}
		});
});
