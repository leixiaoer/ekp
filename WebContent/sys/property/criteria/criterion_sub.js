...
define(function(require, exports, module) {
	...
	require("sys/property/criteria/style/criterion.css");
	var SubSelectedValues = select_panel.CriterionSelectDatas.extend({
		...
		startup: function($super) {
			...
			if (!this.render) {
				this.setRender(new render.Template({src:require.resolve('./criterion-cell.jsp#'), parent: this}));
				this.render.startup();
			}
			if (!this.source) {
		...
		_buildRender : function(_panel) {
			...
			var _render = new render.Template({
				src : require.resolve('./criterion-cell.jsp#'),
				parent : _panel
			});
