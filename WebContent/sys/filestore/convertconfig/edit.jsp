...
///				$("textarea").removeAttr("readonly");//textarea 可以编辑
				...
				if(converterTypeValue == 'wps' || converterTypeValue == 'skofd' ||converterTypeValue == 'dianju')
				{
					$(":input[ name = 'fdConverterType' ]").attr("readonly","readonly");
					$(":input[ name = 'fdConverterType' ]").attr("disabled","disabled");
				}
			...
			function chooseModulName()
			{
				...
				Dialog_TreeList(true, "fdModelName", "fdModul",  "、", "sysModulsInfosTreeService&parent=", "${ lfn:message('sys-filestore:sysFilestore.conversion.modul.info') }", "sysModulsInfosTreeService&parent=!{value}");
			}
				...
				<tr>
						<td class="td_normal_title"  width="180px" align="center">
							...
							<label>
						         ...
						         <c:if test="${sysFileConvertConfigForm.fdConverterType != 'wps' && sysFileConvertConfigForm.fdConverterType != 'skofd' && sysFileConvertConfigForm.fdConverterType != 'dianju'}">
							     ...
							     <%--  <c:if test="${'true' eq sysFileConvertConfigForm.converter_yzdcs}">
						        <xform:simpleDataSource value="yozo">永中DCS</xform:simpleDataSource>
						       </c:if> --%>
						       </c:if>
						        <c:if test="${sysFileConvertConfigForm.fdConverterType == 'wps' }">
						            <xform:simpleDataSource value="wps"><bean:message
									key='sysFilestore.conversion.wps.online.ofd' bundle='sys-filestore' /></xform:simpleDataSource>
						            
						        </c:if>
						        <c:if test="${sysFileConvertConfigForm.fdConverterType == 'skofd'}">
						             <xform:simpleDataSource value="skofd"><bean:message
									key='sysFilestore.conversion.sk.ofd' bundle='sys-filestore' /></xform:simpleDataSource>
						        </c:if>
	<c:if test="${sysFileConvertConfigForm.fdConverterType == 'dianju'}">
									<xform:simpleDataSource value="skofd">点聚OFD转换</xform:simpleDataSource>
								</c:if>
				...
				<tr>
				<%-- ${ lfn:message('sys-filestore:sysFileConvertConfig.fdFileExtName') } --%>
						...
						<td colspan="3" width="300px">
							...
							<c:choose>
							   <c:when test="${sysFileConvertConfigForm.fdConverterType == 'wps' || sysFileConvertConfigForm.fdConverterType == 'skofd' || sysFileConvertConfigForm.fdConverterType == 'dianju'}"> 
							        <select readonly="readonly" disabled="disabled" id="fdConverterKey" name="fdConverterKey"  className="fdFileExtNameClass" required="required" style="width:130px;" onclick="chooseConverterKey();">
							</select>
							   </c:when>
							   <c:otherwise> 
								 ...
								 <tr>
								  ...
								  <td class="validation-advice-msg">
								   <span class="validation-advice-title"><bean:message
									key='sysFilestore.conversion.select.converter' bundle='sys-filestore' /></td>
								   </tr></tbody></table>
							</div>
					...
					<tr>
						...
						<td colspan="2" width="300px" style="border-right-style:none">
					     	...
					     	<c:choose>
							   <c:when test="${sysFileConvertConfigForm.fdConverterType == 'wps' || sysFileConvertConfigForm.fdConverterType == 'skofd' || sysFileConvertConfigForm.fdConverterType == 'dianju'}"> 
								       <xform:checkbox subject="" property="allModuls" dataType="boolean" showStatus ="readOnly" onValueChange="changeRequired">
						...
						<td style="border-left-style:none">
							  <label><c:if test="${sysFileConvertConfigForm.fdConverterType != 'wps' && sysFileConvertConfigForm.fdConverterType != 'skofd' && sysFileConvertConfigForm.fdConverterType != 'dianju'}">
			...
			<center style="margin-top: 10px;"> 		 							            		
              		<c:choose>
					   <c:when test="${sysFileConvertConfigForm.fdConverterType != 'wps' && sysFileConvertConfigForm.fdConverterType != 'skofd'}"> 
					   ...
					   <c:otherwise> 
					     	 <ui:button text="${lfn:message('button.close') }" height="35" width="80" onclick="$dialog.hide(null);"> </ui:button>
					   </c:otherwise>
				   </c:choose>
              		</center> 
		</html:form>
