		...
		seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
				...
				LUI('opinionView').source.get();
			 	...
			 	if(value != '10'){
					 ...
					 if(!LUI('remind') && "${remindAuth}" == 'true'){ 
		            	 var recallBtn = toolbar.buildButton({id:'remind',order:'2',text:'提醒',click:'remind()'});
    					 LUI('Btntoolbar').addButton(recallBtn);
	            	 }
					 if(!LUI('recall') && "${recallAuth}" == 'true'){
		            	 var recallBtn = toolbar.buildButton({id:'recall',order:'1',text:'撤回',click:'recall()'});
    					 LUI('Btntoolbar').addButton(recallBtn);
	            	 }
				 }
			};
			LUI.ready(function(){
			 ...
			 window.recall = function(){
				...
				dialog.confirm("您确定要对所选记录进行撤回吗？", function(value) {
					if (value == true) {
			...
			window.remind = function(){
				...
				dialog.confirm("您确定要对所选未阅对象进行提醒吗？", function(value) {
					if (value == true) {
						...
						$.ajax({
							...
							success : function(data) {
								...
								if(data.fail && data.fail.length > 0) {
									dialog.failure('<bean:message bundle="sys-circulation" key="button.remind.fail"/>' + data.fail.join("; "));
								} else {
									dialog.success('<bean:message bundle="sys-circulation" key="button.remind.success"/>');
								}
							},
							complete : function() {
								if (window.remind_load != null) {
