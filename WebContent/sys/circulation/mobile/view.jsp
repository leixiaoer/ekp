...
<template:include ref="mobile.view" compatibleMode="true">
	...
	<kmss:auth requestURL="/sys/circulation/sys_circulation_main/sysCirculationMain.do?method=recallCir&fdId=${sysCirculationMainForm.fdId}" requestMethod="GET">
		...
		<div id="scrollView" class="gray circulationView" data-dojo-type="mui/view/DocScrollableView">
			  <div>
				  <div class="muiFormContent">
				  	 <div class="circulateDetail">
					     <div class="circulateDetailHead"><b>传阅记录</b> </div>
					  </div>
				 ...
				 <div data-dojo-type="mui/list/StoreElementScrollableView" data-dojo-props="height:'500px'" id="storeElementScrollableView">
					 <div  data-dojo-type="mui/list/JsonStoreList" id="opinionList" 
						...
						data-dojo-props="url:'/sys/circulation/sys_circulation_opinion/sysCirculationOpinion.do?method=list&forward=listOpinion&fdMainId=${sysCirculationMainForm.fdId}&docStatus=all',lazy:false">
					</div>
				</div>
			</div>
				...
				if(list.length > 0){
					 req(util.formatUrl("/sys/circulation/sys_circulation_opinion/sysCirculationOpinion.do?method=addReminds"), {
						...
						if(results.fail && results.fail.length > 0) {
							Tip.fail({
								text: '<bean:message bundle="sys-circulation" key="button.remind.fail"/>' + results.fail.join("; ")
							});
						} else {
							Tip.success({
								text : '<bean:message bundle="sys-circulation" key="button.remind.success"/>'
							});
						}
					}));
				}else{
