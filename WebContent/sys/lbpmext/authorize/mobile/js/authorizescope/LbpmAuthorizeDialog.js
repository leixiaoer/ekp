define([ "dojo/_base/declare", "dojo/topic", "dojo/dom", "dojo/dom-construct", "dojo/dom-style",
          ...
          "dijit/registry","dojo/dom-class", "dijit/_WidgetBase"],
		function(declare, topic, dom, domConstruct, domStyle, lang, html, array, util, touch, css3, registry, domClass, WidgetBase) {
			var rDialog = declare("sys.lbpmext.authorize.mobile.js.authorizescope.LbpmAuthorizeDialog", [WidgetBase], {
				
				//模板地址
				templURL : "sys/lbpmext/authorize/mobile/js/authorizescope/lbpmauthorize.jsp",
				...
				_init: function(config) {
					topic.subscribe("/sys/lbpmext/authorize/mobile/js/authorizescope/submit",lang.hitch(this,"returnDialog"));
					this.key = config.key;
					this.curIds = config.curIds;
					this.curNames = config.curNames
				},
				
				returnDialog : function(srcObj , evt){
					if(evt){
						if(srcObj.key == this.key){
							topic.publish("/sys/lbpmext/authorize/mobile/js/authorizescope/setvalue",this,evt);
