define([
	...
	"dojo/dom",
	"dojox/mobile/_css3",
	"mui/i18n/i18n!sys-lbpmext-authorize"
	], function(declare, lang, topic, domCtr, query, viewRegistry, when, domStyle, array,dom,css3,Msg) {
	
	return declare("sys.lbpmext.authorize.mobile.js.authorizescope.LbpmAuthorizeNextPageViewMixin", null, {
		...
		_createPushDom: function() {
			if (!this._pushDom) {
				this._pushDom = domCtr.toDom('<div style="width:100%;text-align:center;"><span><i class="mui mui-loading mui-spin"></i>'+Msg["mui.authorize.loadMore"]+'</span></div>');
		...
		startup : function(){
			...
			this._createPushDom();
			this.set("push", false);
			this.subscribe("/sys/lbpmext/authorize/toTop", 'handleToTopTopic');
		},
