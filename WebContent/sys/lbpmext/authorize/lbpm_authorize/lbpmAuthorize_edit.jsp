...
<table class="tb_normal" width=95%>
	...
	<tr class="lbpm_authorize_process" id="lbpmAuthorizeProcess" style="display: none">
		...
		<td width=85% colspan=3>
			...
			<span id="processId">
				<a href="#" onclick="showDocSgl();">选择</a><span style="color:red"> *</span>
			</span>
		</td>
	</tr>
	<!-- 授权流程end -->
		...
		<td width=85% colspan=3>
			...
			<table class="tb_normal" width=100% id="TABLE_Setting_Info">
				...
				<tr style="display:none;" KMSS_IsReferRow="1">
					...
					<td>
						...
						<div style="margin-top:5px;min-height: 20px">
							<label style='display:none;<kmss:authShow roles="ROLE_LBPM_WORKFLOW_AUTHORITY_MANAGER;ROLE_LBPM_WORKFLOW_AUTHORIZE_ASSIGN">display:inline-block;</kmss:authShow>'>
								<input type="radio" name="authContents[!{index}].fdType" value="condition" onchange="switchSettingType(this)" checked>使用条件选择
							</label>
							<label style='display:none;<kmss:authShow roles="ROLE_LBPM_WORKFLOW_AUTHORITY_MANAGER;ROLE_LBPM_WORKFLOW_AUTHORIZE_ASSIGN">display:inline-block;</kmss:authShow>'>
								<input type="radio" name="authContents[!{index}].fdType" value="formula" onclick="switchSettingType(this)">使用高级公式
							</label>
							<label  class="add_condition_btn" style="float:right"><a href="javascript:void(0)" style="color: #4285f4;" onclick="addCondition(this);">新增条件</a></label>
						</div>
					...
					<td align="center">
						<span class="setting_del_btn" onclick="DocList_DeleteRow(this.parentNode.parentNode)">删除</span>
					</td>
				</tr>
				<%-- 内容行 --%>
				...
				<tr KMSS_IsContentRow="1">
					...
					<td>
						<div style="margin-top:5px;min-height: 20px;">
							<label style='display:none;<kmss:authShow roles="ROLE_LBPM_WORKFLOW_AUTHORITY_MANAGER;ROLE_LBPM_WORKFLOW_AUTHORIZE_ASSIGN">display:inline-block;</kmss:authShow>'>
								<input type="radio" name="authContents[${vstatus.index}].fdType" value="condition" onchange="switchSettingType(this)" <c:if test='${fdAuthContent.fdType == "condition" }'>checked</c:if>>使用条件选择
							</label>
							<label style='display:none;<kmss:authShow roles="ROLE_LBPM_WORKFLOW_AUTHORITY_MANAGER;ROLE_LBPM_WORKFLOW_AUTHORIZE_ASSIGN">display:inline-block;</kmss:authShow>'>
								<input type="radio" name="authContents[${vstatus.index}].fdType" value="formula" onchange="switchSettingType(this)" <c:if test='${fdAuthContent.fdType == "formula" }'>checked</c:if>>使用高级公式
							</label>
							<label class="add_condition_btn" style="float:right"><a style="color: #4285f4;" href="javascript:void(0)" onclick="addCondition(this);">新增条件</a></label>
						</div>
			...
			<div class="tb_normal" width=100% id="" style="text-align:center;margin-top: 20px;border:1px dashed #d2d2d2;cursor: pointer;" onclick="addSettingRow();">
				<span class="lui_icon_s lui_icon_s_icon_plusx" ></span>新增
			</div>
		</td>
	</tr>
	<!-- 授权设置end -->
