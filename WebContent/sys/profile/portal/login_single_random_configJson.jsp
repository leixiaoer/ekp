...
String[] normalKeys = new String[]{"login_logo_position","single_random_logo","loginBtn_bgColor","loginBtn_bgColor_hover","loginBtn_font_color","login_form_align","title_image","login_title_show"};
String[] langKeys = new String[]{"single_random_footerInfo","login_form_title","loginBtn_text","title"};
...
//动态KEY
Object linksObj = designConfig.get("single_random_head_links");
if(linksObj != null) {
	JSONArray links = (JSONArray)linksObj;
	out.print("config.single_random_head_links = [");
	for(int i = 0;i < links.size();i++) {
		JSONObject link = links.getJSONObject(i);
		String key = "single_random_head_link";
		out.print("{");
		for(String lang : langs.keySet()) {
			String configKey = key + "_"+lang;
		...
		out.print("single_random_head_link_href : '"+link.getString("single_random_head_link_href")+"'}");
		if(i != links.size() - 1)
			out.print(",");
	}
	out.println("];");
}else {
	out.println("config.single_random_head_links = [];");
}
%>
</script>