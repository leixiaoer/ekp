	...
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="7">
			...
			<kmss:authShow roles="SYSROLE_ADMIN;SYSROLE_SYSADMIN">
				<ui:button text="${ lfn:message('sys-profile:sys.profile.i18n.tools1') }" order="3" onclick="clearCache()"></ui:button>
				<ui:button text="${ lfn:message('sys-profile:sys.profile.i18n.tools3') }" order="4" onclick="resetI18n()"></ui:button>
				<ui:button text="${ lfn:message('sys-profile:sys.profile.i18n.tools2') }" order="5" onclick="clearFileKey()"></ui:button>
				<ui:button text="${ lfn:message('sys-profile:sys.profile.i18n.tools4.startLocaleQuery') }" title="${ lfn:message('sys-profile:sys.profile.i18n.tools4.startLocaleQuery') }" order="6" onclick="startLocaleQuery()"></ui:button>
				<ui:button text="${ lfn:message('sys-profile:sys.profile.i18n.tools5.closeLocaleQuery') }" title="${ lfn:message('sys-profile:sys.profile.i18n.tools5.closeLocaleQuery') }" order="7" onclick="closeLocaleQuery()"></ui:button>
			</kmss:authShow>
		</ui:toolbar>
	</template:replace>
	<template:replace name="content"> 
		<script>
			...
			seajs.use( [ 'lui/jquery', 'lui/dialog' ], function($, dialog) {
				<kmss:authShow roles="SYSROLE_ADMIN;SYSROLE_SYSADMIN">
				startLocaleQuery = function() {
					dialog.confirm("${ lfn:message('sys-profile:sys.profile.i18n.tools4.info.confirm') }", function(value) {
						if(value == true) {
							window.del_load = dialog.loading();
							$.ajax({
								url: '<c:url value="/sys/profile/i18n/sysProfileI18nConfig.do?method=startLocaleQuery"/>',
								type: 'POST',
								dataType: 'text',
				...
				closeLocaleQuery = function() {
					dialog.confirm("${ lfn:message('sys-profile:sys.profile.i18n.tools5.info.confirm') }", function(value) {
					if(value == true) {
						window.del_load = dialog.loading();
						$.ajax({
							url: '<c:url value="/sys/profile/i18n/sysProfileI18nConfig.do?method=closeLocaleQuery"/>',
							type: 'POST',
							dataType: 'text',
							success: function(data) {
								if(window.del_load != null){
