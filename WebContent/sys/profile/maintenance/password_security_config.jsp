			...
			<ui:tabpanel layout="sys.ui.tabpanel.security">
				...
				<ui:content id="tag1" title="${lfn:message('sys-profile:sys.profile.org.passwordSecurityConfig.tag1')}">
					<table class="tb_normal" width=95%>
						...
						<tr>
							<td class="td_normal_title"  width="35%">个人实名认证</td>
							</td>
							<td>
								<table class="tb_normal" width="95%" 
									...
									<tr>
										<td class="td_normal_title" width="150px">
											认证要素配置<span class="txtstrong">*</span>
										</td>
										<td>
											<xform:radio property="value(person.auth.Element)"
												alignment="V" required="true" subject="个人实名认证要素配置" showStatus="edit"
											...
											<c:if test="${isOtherPerson }">
												<table class="tb_normal" width="95%" style="margin: 0">
													<tr>
														<td class="td_normal_title" width="150px">其他配置</td>
														<td><xform:radio property="value(person.will.certMethod)"
																required="true"
																alignment="V" subject="其他配置" showStatus="edit">
																<xform:customizeDataSource
									...
									<tr>
						              	<td class="td_normal_title" width="150px">适用对象</td>
						                <td>
						                	<div id="_xform_fdCaManagerId" _xform_type="address">
												<xform:address propertyId="value(elec.fdAuthManagerIds)" 
													...
													showStatus="edit" textarea="true" style="width:95%;" subject="认证管理员" />
											</div>
										</td>
					              </tr>
								</table>
					    ...
					    <tr>
							<td class="td_normal_title"  width="35%">企业实名认证</td>
						<td>
							<table class="tb_normal" width="95%" id="priseId" style="margin:0">
							   ...
							   <tr class="tr_add_inner_prise">
							   		<td colspan="2">
							   			<ui:button style="float:left" text="快捷添加企业认证" onclick="openPriseDialog()"></ui:button>
											<br>
							   		</td>
							   </tr>
							   <tr>
									<td class="td_normal_title" width="150px">认证要素配置</td>
									<td>
										<xform:radio property="value(prise.auth.Element)"
											alignment="V" subject="认证要素配置" showStatus="edit" required="true"
											onValueChange="elementValueChange">
											<xform:simpleDataSource value="2">企业工商信息四要素信息（企业名称+企业社会统一信用代码+法人姓名+法人证件号）</xform:simpleDataSource>
										</xform:radio>
									</td>
								</tr>
								<tr>
									<td class="td_normal_title" width="150px">认证方式配置</td>
									<td>
										<div>
											<xform:radio subject="企业认证方式配置" required="true"
												property="value(enterprtise.authhenType)" showStatus="edit"
												 alignment="V">
												<c:if test="${isAgent==true}">
										   			<xform:simpleDataSource value="1">代理人认证（代理人实名认证信息+企业实名认证授权委托书信息+企业认证要素认证）<br>
														<div id="sendTypeId2" style="left:100px;display:inline-block;">
															<table>
																<tr>
																	<td style="min-width:140px; vertical-align: top;">
																		<span style="float:left">&nbsp;&nbsp;&nbsp;&nbsp;委托书提交方式：</span>
																	</td>
																	<td>
												 ...
												 <c:if test="${islegalPerson==true}">
												    <xform:simpleDataSource value="2">法人认证（法人个人实名认证+企业认证要素认证）<br>
													</xform:simpleDataSource>
												 </c:if>
											</xform:radio>
										</div>
										<div>说明：代理人或法人认证方式将遵循个人实名认证要素配置中选择的认证方式</div>
									</td>
								</tr>
								<tr>
									<td class="td_normal_title" width="150px">小额打款</td>
									<td>
										...
										<br>
										<div>说明：开启该功能后，则需要代理人在认证同时需要进行小额打款进行佐助认证，法人认证不适用</div>
										<div>用，该功能可在具体模块进行设置。</div>
										<div>小金额打款由第三方支持服务，向需认证机构打入0.01-1元的随机金额款项，用以验证验证企业-注册人关联</div>
									</td>
								</tr>
							</table>
							<div>说明：勾选对应选项则方可提供对应维度的企业实名认证能力，认证维度可在具体模块中进行设置</div>
						</td>
					  	</tr>
						</kmss:ifModuleExist>
					    <!-- 身份校验-->
						...
						<tr>
						<td class="td_normal_title"  width="35%">身份校验方式配置</td>
						</td>
						<td><xform:checkbox subject="身份校验方式配置"
						...
						<tr>
							<td class="td_normal_title" width="35%">校验异常配置</td>
									...
									<c:forEach items="${validateTypeMap}" var="mymap">
										<c:if test="${mymap.key ne 'faceAuthenVerify' }">
											<tr>
												...
												<td width="85%">单次校验在<xform:text showStatus="edit"
														property="value(Identity.Exception.num_${mymap.key})"
														style="width:30px" subject="${mymap.value}异常次数"  
														validators="required digits min(1) max(9)" />次，视为校验异常</td>
											</tr>
										</c:if>
									</c:forEach>
								</table></td>
						...
						<tr>
						<td class="td_normal_title" width="35%">账户冻结</td>
						<td width="85%"><ui:switch
							...
							<div id="dongJieId">
								...
								<xform:select property="value(Identity.dongJie.Unit)"
									subject="冻结单位" showStatus="edit" required="true">
									<xform:simpleDataSource value="1">小时</xform:simpleDataSource>
									<xform:simpleDataSource value="2">天</xform:simpleDataSource>
								</xform:select>
							</div></td>
					</tr>
					<tr>
						<td class="td_normal_title" width="15%">校验优先级序列号</td>
					...
					<c:if test="${!empty identityList}">
						...
						<tr>
							<td class="td_normal_title" colspan=2>
								<b><label>实名校验</label></b>
							</td>
						</tr>
						<tr>
						<td class="td_normal_title"  width="35%">实名校验方式</td>
						</td>
						<td>
						...
						<div id="indentifyAuthDiv3">
						   <xform:checkbox subject="身份校验方式配置"
					    ...
					    <div id="indentifyAuthDiv1">说明：开启实名校验方式前，请先开启并完成认证配置</div>
					    <div id="indentifyAuthDiv2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;实名校验方式仅限于个人实名认证的适用对象操作</div>					    
					  </td>
					    </tr>
		           </c:if>
						<!-- 其他 -->
