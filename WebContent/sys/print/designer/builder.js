...
//						debugger;
						...
						}else if(_fdType=='detailsTable'){
							control = new sysPrintDetailsTableControl(this,_fdType,{renderLazy:true});
	...
	function isFieldControl(fdType){
		var FIELD_CONTROLS = ['input','inputRadio','checkbox','select','address','relationSelect','textarea',
		                     ...
		                     'new_address','formula_calculation','attachment','rtf','relationRule','fSelect','prompt'];
				...
				if (node.nodeType != 3) {
					if (sysPrintUtil.isXFormDesignElement(node)) {
						...
						if(_fdType=='standardTable'){
							...
							node.setAttribute("printcontrol","true");
							node.setAttribute("fd_type","detailsTable");
							...
							$(node).children('a').css('text-decoration','none').css('cursor','default');
								...
								_fdType=='rtf' || _fdType=='datetime' || _fdType=='calculation' 
									|| _fdType=='chinaValue' || _fdType=='auditNote' || _fdType=='fSelect' || _fdType=='placeholder'){
							var values = eval('(' + node.getAttribute('fd_values').replace(/\r\n/ig,'\\r\\n').replace(/&#;/ig,'\\r\\n') + ')');
							//获取数据源
							...
							if(_fdType=='inputText' || _fdType=='calculation'){
							...
							if(_fdType=='inputText'  && values.dataType=='Double' ){
...
//							debugger;
							...
							$oldDom.remove();
							//属性
							$newDom.attr('_width',values.width).attr('_height',values.height).attr('fdMulti',values.fdMulti);
							node=null;
						}else if(_fdType=='map'){
							var values = eval('(' + node.getAttribute('fd_values').replace(/\r\n/ig,'\\r\\n').replace(/&#;/ig,'\\r\\n') + ')');
							//获取数据源
