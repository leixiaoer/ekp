(function(window, undefined){
	 ...
	 * 换行控件
	 */
	 var brControl=sysPrintDesignerControl.extend({
		 render:function(config){
		 	if(config && config.renderLazy) return;
		 	var id=sysPrintUtil.generateID();
		 	var html = [];
		 	html.push('<div printcontrol="true" fd_type="brcontrol" id="' +id);
		 	html.push('" style="">');
	 	...
	 	bind:function(){
	 		...
	 		this.addListener('mousedown',function(event){
				if(self instanceof sysPrintBrControl){
					//清空并重设选中控件
	 ...
	 window.sysPrintBrControl=brControl;
