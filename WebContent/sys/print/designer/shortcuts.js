(function(window,undefined){
	var shortCuts = function(){
			...
			//公用方法
			this.add = shortCutAdd;
			this.remove = shortCutRemove;
	};
	
	function shortCutAdd(shortCut, callback, options){
		shortCut = shortCut.toLowerCase();
		//参数配置
		var opt = {};
		sysPrintUtil.extend(true, opt, this.options);
		sysPrintUtil.extend(true, opt, options);
		//事件触发载体
		var element = sysPrintUtil.$(opt.target);
		//在keypress时可以调用的方法
		var _self = this;
		...
		//绑定事件
		sysPrintUtil.addEvent(element, opt['type'], func);
	}
	...
	function shortCutRemove(){
		...
		if(!binding) return;
		//删除相应绑定事件
		sysPrintUtil.removeEvent(binding['target'], binding['event'], binding['callback']);
	}
	
	window.sysPrintShortCuts = shortCuts;
})(window);