...
<form id="sysFtsearchReadLogForm" name="sysFtsearchReadLogForm"  action="<c:url value="/sys/ftsearch/expand/sys_ftsearch_read_log/sysFtsearchReadLog.do?method=save" />" method="post" target="_blank">
	...
	<div class="search_bar_tab_content">
		<div class="search_bar_tab">
			...
			</label> <label id="searchPeople" onclick="change(this)" class="current">
				<p>
					<span>${lfn:message('sys-ftsearch:PersonSearch')}</span>
				</p>
			</label> <label id="searchModel" onclick="change(this)">
				<p>
		...
		if(data.fdIsAvailable != false){
			...
			$}			
				if(data.fdIsAvailable != false && (data.isDepInfoPrivate != true)){
					{$
						<span>
							<c:out value="{%data.fdDeptName%}" escapeXml="true"></c:out>
						</span>
					$}
				}else{
					{$
			...
			$}			
				if(data.fdIsAvailable != false && (data.isContactPrivate != true)){
					{$
						<span>
							<c:out value="{%data.fdMobileNo%}" escapeXml="true"></c:out>
						</span>
					$}
				}else{
					{$
			...
			{$			
						<span>
							<c:out value="{%data.fdShortNo%}" escapeXml="true"></c:out>
						</span> 
			$}	
			}else{
					{$
			...
			$}			
				if(data.fdIsAvailable != false && (data.isContactPrivate != true)){
					{$
						<span>
							<c:out value="{%data.fdEmail%}" escapeXml="true"></c:out>
						</span>
					$}
				}else{
					{$
			...
			$}			
				if(data.fdIsAvailable != false && (data.isDepInfoPrivate != true)){
					{$
						<span>
							<c:out value="{%data.fdPostName%}" escapeXml="true"></c:out>
						</span>
					$}
				}else{
					{$
			   ...
			   $}	
				...
				{$
					<div id="contactBar_${sysOrgPerson.fdId}"  data-person-role="contact"
						class="contactBar"
						 data-person-param="&fdId=${sysOrgPerson.fdId}&fdName={%data.fdName%}&fdLoginName={%data.fdLoginName%}&fdEmail={%data.fdEmail%}&fdMobileNo={%data.fdMobileNo%}&dingUserid={%data.dingUserid%}&ldingUserid={%data.ldingUserid%}&dingCropid={%data.dingCropid%}">	
					</div>
				$}	
			}	
			{$
			...
			setTimeout(function(){
				...
				onRender(datas); 
                     ...
                     <!-- 人员名片 Ends --> 
                      ...
                      <% 
                      		String over = (String)request.getAttribute("isOver");
                      		if("true".equals(over)){
                      %>
                      <div align="center">${lfn:message('sys-ftsearch:sysZonePersonInfo.page.limited')}</div>
                      <% } %>
