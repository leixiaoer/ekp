...
<script type="text/javascript">
	...
	function paramsMappingUI(searchParam){
		...
		$("#sort_by_time").removeClass("onAsc");
	...
	function addGeneralResultRow(rowObject, index, allData){
		...
		if(docStatus!=null){
		 	if(docStatus.value =='10'){
		 		row_1_1 = '<span class="lui_icon_s lui_icon_s_icon_draft" title="${lfn:message('status.draft')}"></span>&nbsp;';
		 	}else if(docStatus.value =='00'){
		 		row_1_1 = '<span class="lui_icon_s lui_icon_s_icon_discard" title="${lfn:message('status.discard')}"></span>&nbsp;';
		 	}else if(docStatus.value =='11'){
		 		row_1_1 = '<span class="lui_icon_s lui_icon_s_icon_refuse" title="${lfn:message('status.refuse')}"></span>&nbsp;';
		 	}else if(docStatus.value =='40'){
		 		row_1_1 = '<span class="lui_icon_s lui_icon_s_icon_expire" title="${lfn:message('status.expire')}"></span>&nbsp;';
		 	}else if(docStatus.value =='20'){
		 		row_1_1 = '<span class="lui_icon_s lui_icon_s_icon_examine" title="${lfn:message('status.examine')}"></span>&nbsp;';
		 	}
		}
	/*	var row_1_1 = $('<span class="icon_collect"/>');
        ...
        if(null != fileName && docKey != null) {
            ...
            if("image.png" == fileIcon){
            	fdUrl = "/sys/attachment/sys_att_main/sysAttMain.do?method=download&fdId=" + tmp +"&open=1";
            }
        }
		var fdSystemName = (systemName != null ? systemName.value : "");
		...
		var row_1_2_2 = $('<span style="font-size: 16px;" id="record_'+index+'" />');
		...
		// 知识目录：
		row_3_1.html('${lfn:message("sys-ftsearch-db:search.ftsearch.knowledge.directory")}<a class="tag" href="' + modelUrl + '" target="_blank" >' + (modelName2 != null ? modelName2 : "") + '</a>');
		var row_3_2 = $('<span/>');
		//作者：
		row_3_2.html('${lfn:message("sys-ftsearch-db:sysFtsearch.simdoc.author")}<em><a style="text-decoration:none;color:#a3a3a3" >' + (creator != null ? creator.value : "") + '</a></em>');
		var row_3_3 = $('<span/>');
		//创建时间：
		...
		if($.trim(fdSummary) != "") {
			...
			var row_3_6 = $('<span/>');
			row_3_6.html("<a class='tag' href='" + snapshotUrl + "' target='_blank'>${lfn:message('sys-ftsearch-db:search.ftsearch.lookSnapshot')}</a>");
		}
...
function openByUrl(url){
	window.open("${LUI_ContextPath}/"+url);
}
