...
(function( $, app ) {
	...
	services.Cluster = ux.Class.extend({
		...
		convert: function( path ) {
			...
			//debugger;
			var mappings = [{
						"replaced": /\/head\/_cluster\/state$/,
						"replace": "/head/elasticsearchHead.do?method=_clusterState"
					},{
						"replaced": /\/head\/_status$/,
						"replace": "/head/elasticsearchHead.do?method=_status"	
					},{
						"replaced": /\/head\/_nodes\/stats\?/,
						"replace": "/head/elasticsearchHead.do?method=_nodesStats&"	
					},{
						"replaced": /\/head\/_cluster\/nodes\/stats$/,
						"replace": "/head/elasticsearchHead.do?method=_clusterNodesStats"	
					},{
						"replaced": /\/head\/_cluster\/nodes$/,
						"replace": "/head/elasticsearchHead.do?method=_clusterNodes"	
					},{
						"replaced": /\/head\/_nodes$/,
						"replace": "/head/elasticsearchHead.do?method=_nodes"	
					},{
						"replaced": /\/head\/_nodes\/plugin$/,
						"replace": "/head/elasticsearchHead.do?method=_nodesPlugin"	
					},{
						"replaced": /\/head\/_cluster\/health$/,
						"replace": "/head/elasticsearchHead.do?method=_clusterHealth"	
					},{
						"replaced": /\/head\/$/,
						"replace": "/head/elasticsearchHead.do?method=_base"	
					},{
						"replaced": /\/head\/_aliases$/,
						"replace": "/head/elasticsearchHead.do?method=_aliases"	
					},{
						"replaced": /\/head\/_template$/,
						"replace": "/head/elasticsearchHead.do?method=_template"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)\/_gateway\/snapshot$/,
						"replace": "/head/elasticsearchHead.do?method=_gatewaySnapshot&index=$1"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)$/,
						"replace": "/head/elasticsearchHead.do?method=index&index=$1"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)\/_refresh$/,
						"replace": "/head/elasticsearchHead.do?method=_refresh&index=$1"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)\/_flush$/,
						"replace": "/head/elasticsearchHead.do?method=_flush&index=$1"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)\/_optimize$/,
						"replace": "/head/elasticsearchHead.do?method=_optimize&index=$1"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)\/_analyze\?/,
						"replace": "/head/elasticsearchHead.do?method=_analyze&index=$1&"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)\/_close$/,
						"replace": "/head/elasticsearchHead.do?method=_close&index=$1"	
					},{
						"replaced": /\/head\/([a-zA-Z0-9]+)\/_open$/,
						"replace": "/head/elasticsearchHead.do?method=_open&index=$1"	
					},{
						"replaced": /\/head\/([\S]+)\/_search$/,
						"replace": "/head/elasticsearchHead.do?method=_search&path=$1"	
					},{
						"replaced": /\/head\/_cluster\/nodes\/([\S]+)\/_shutdown$/,
						"replace": "/head/elasticsearchHead.do?method=_shutdownCluster&nodeId=$1"	
					}
					]
			for(var i in mappings){
				var oldPath = path;
				path = path.replace(mappings[i].replaced,mappings[i].replace);
				if(path != oldPath){
					return path;
				}
			}
		...
		request: function( params ) {
			return $.ajax( $.extend({
				url: this.convert(this.base_uri + params.path),
				dataType: "json",
		...
		"put": function(path, data, success) {
			if(data == null) {data={};} 
			...
			return this.request( { type: "POST", path: path, data: data, success: success } ); 
			},
		"delete": function(path, data, success) {if(data == null){data={};} data.requestMethod = "DELETE"; return this.request( { type: "POST", path: path, data: data, success: success } ); }
	});
