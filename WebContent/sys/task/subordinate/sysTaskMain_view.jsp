...
<template:include ref="default.view" sidebar="no">
	...
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="1">
			<%--关闭--%> 
			<ui:button text="${lfn:message('button.close')}" order="1" onclick="Com_CloseWindow();" id="closeWindow">
			</ui:button>
		</ui:toolbar>
	</template:replace>
	<%--导航路径--%>
		...
		<html:form action="/sys/task/sys_task_main/sysTaskMain.do">
			...
			<div class="lui_form_content_frame" >
				...
				<table class="tb_normal" width=100%>	
					<tr>
						<td colspan="4" >
							<iframe width="100%" height="400px" frameborder="0" scrolling="auto"  src='<c:url value="/sys/task/subordinate/taskChart4Pc.jsp" />?fdId=${JsParam.fdId}&modelName=${JsParam.modelName}&orgId=${JsParam.orgId}'></iframe>
						</td>
					</tr>
					...
					<tr>
						...
						<td width=35% colspan="3"><c:choose> 
							<c:when test="${sysTaskMainForm.fdParentName != null}">
									...
									function toParent() {
										seajs.use(['lui/jquery','lui/dialog'], function($, dialog) {
											...
											$.ajax({
										        ...
										        dataType: "json",
												async : true,
										        data: { modelId: "${sysTaskMainForm.fdParentId}", modelName: "com.landray.kmss.sys.task.model.SysTaskMain" },
										        success: function (data) {
										        	if(data.check) {
														Com_OpenWindow("${LUI_ContextPath}/sys/subordinate/sysSubordinate.do?method=view&modelId=${sysTaskMainForm.fdParentId}&modelName=com.landray.kmss.sys.task.model.SysTaskMain&orgId=${JsParam.orgId}", "_self")
													} else {
								...
								<a class="com_btn_link" href="javascript:toParent();">
					...
					<tr>
						...
						<td colspan="3"><c:choose>
							<c:when test="${not empty sysTaskMainForm.fdSourceSubject && not empty sysTaskMainForm.fdSourceUrl}">
								<a class="com_btn_link" href='<c:url value="${sysTaskMainForm.fdSourceUrl}"/>' target="_blank" >
			...
			<ui:tabpage expand="false" >
				...
				<ui:content title="${lfn:message('sys-task:tag.feedback')}" expand="true">
					<ui:event event="show"> 
						document.getElementById("feedbackList").src= '<c:url value="/sys/task/subordinate/sysTaskFeedback_list.jsp" />?fdTaskId=${JsParam.fdId}&orgId=${JsParam.orgId}&feedbackcount=${feedbackcount}';
					</ui:event>
				...
				<ui:content title="${lfn:message('sys-task:sysTaskOverrule.record')}">
					<ui:event event="show" > 
						document.getElementById("overruleList").src= '<c:url value="/sys/task/subordinate/sysTaskOverrule_list.jsp" />?fdTaskId=${JsParam.fdId}&orgId=${JsParam.orgId}';
					</ui:event>
