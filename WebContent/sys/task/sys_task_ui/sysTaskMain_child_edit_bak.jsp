		...
		<script language="JavaScript">
			...
			seajs.use(['lui/jquery','lui/dialog'], function($,dialog) {
				...
				window.commitMethod=function(commitType, saveDraft){
					...
					if("${sysTaskMainForm.method_GET}" == 'addChildTask'){
						//if(divide.checked)
				...
				//快速选择时间
				window.changeDateTimeChild = function (obj){
					...
					document.getElementsByName("fdFirstLevelChildForms["+index+"].fdPlanCompleteDateTime")[0].value = date.format(Data_GetResourceString("date.format.date"))+" 00:00";
				};
				
				window.checkHasChild = function (){
					
				};
				
				window.checkChildren = function (fdId){ 
					var url="${KMSS_Parameter_ContextPath}sys/task/sys_task_main/sysTaskMain.do?method=checkChildren"; 
					var flag = false;
					 $.ajax({     
			   	     type:"post",   
			   	     url:url,     
				...
				window.deleteDocListRow = function (fdId){
					if(checkChildren(fdId)){
						alert("改任务下有子任务，不允许删除");
					}else{
				...
				function genHtml(){
			        ...
			        for(var i = 0 ;i<fdPerformIdArr.length;i++){
			        	...
			        	trHTML += '<td>';
			        	trHTML += '<input type="checkbox" name="DocList_Selected">';
						trHTML += '<input type="hidden" name="fdFirstLevelChildForms['+i+'].fdParentId" value="${sysTaskMainForm.fdId}"/>';
						trHTML += '</td>';
						trHTML += '<td>';
						trHTML += '<input name="fdFirstLevelChildForms['+i+'].docSubject" class="inputsgl" value="'+docSubject+'" type="text" validate="required" style="width:95%"><DIV class=txtstrong>*</DIV>';
						trHTML += '</td>';
						trHTML += '<td>';
						trHTML += '<input type="hidden" name="fdFirstLevelChildForms['+i+'].fdPerformId" value="'+fdPerformIdArr[i]+'"/>';
						trHTML += '<DIV onclick="SelectPerson(\''+i+'\')" class="inputselectsgl" style="WIDTH: 110px"><DIV class="input" style="WIDTH: 105px"><INPUT style="WIDTH: 100px"  name=fdFirstLevelChildForms['+i+'].fdPerformName value="'+fdPerformNameArr[i]+'" validate="required" subject="<%=ResourceUtil.getString(request,"sysTaskMainPerform.fdPerformId","sys-task")%>"></DIV><DIV class="orgelement" isChannel="true"></DIV><DIV class="txtstrong" style="HEIGHT: 100%; POSITION: absolute; LEFT: 105px; WIDTH: 20px">*</DIV></DIV>';
						trHTML += '</td>';
						trHTML += '<td>';
						trHTML += '<DIV onclick="selectDateTime(\'fdFirstLevelChildForms['+i+'].fdPlanCompleteDateTime\')" class="inputselectsgl" style="WIDTH: 160px"><DIV class="input" style="WIDTH: 160px"><INPUT style="WIDTH: 160px" name=fdFirstLevelChildForms['+i+'].fdPlanCompleteDateTime validate="required" value="'+d+'" subject="<%=ResourceUtil.getString(request,"sysTaskMain.fdPlanCompleteTime","sys-task")%>" ></DIV><DIV class="inputdatetime" isChannel="true"></DIV><DIV class="txtstrong" style="HEIGHT: 100%; POSITION: absolute; LEFT: 155px; WIDTH: 20px">*</DIV></DIV>';
						trHTML += '&nbsp;&nbsp;&nbsp;<select name="select_time" onChange="changeDateTimeChild(this.value)"  style="vertical-align: top;">';
						trHTML += '<option value="0"><bean:message bundle="sys-task" key="sysTaskMain.quick.select" /></option>';
						trHTML += '<option value="1"><bean:message bundle="sys-task" key="sysTaskMain.DateTime.today" /></option>';
						trHTML += '<option value="2"><bean:message bundle="sys-task" key="sysTaskMain.DateTime.tomorrow" /></option>';
						trHTML += '<option value="3"><bean:message bundle="sys-task" key="sysTaskMain.DateTime.after.tomorrow" /></option>';
						trHTML += '<option value="4"><bean:message bundle="sys-task" key="sysTaskMain.DateTime.next.week" /></option>';
						trHTML += '<option value="5"><bean:message bundle="sys-task" key="sysTaskMain.DateTime.next.month" /></option>';
						trHTML += '</select>';
						trHTML += '</td>';
						trHTML += '<td>';
						trHTML += '<input name="fdFirstLevelChildForms['+i+'].fdWeights" class="inputsgl" value="'+weightValue+'" type="text" style="width:95%">';
						trHTML += '</td>'; 
						trHTML += '<td class="td_normal_title">';
						trHTML +='<span style="cursor:pointer" class="optStyle opt_del_style" title="删除行" onmousedown="DocList_DeleteRow_ClearLast();">';
						trHTML +='</span>';
						trHTML += '</td>';
						trHTML += '</tr>';
						$("#TABLE_DocList tr:eq("+i+")").after(trHTML);
			        }
				}
				
				window.divideTask = function(){
					var fdPerformIds = document.getElementsByName("fdPerformId")[0].value ;
			        var ele = document.getElementsByName("fdFirstLevelChildForms[0].docSubject");
			        if(fdPerformIds != null && fdPerformIds != ""){
			        	 if(ele.length > 0){
			        		 dialog.confirm('自动分解将清除当前已添加子任务，是否继续？',function(value){
	     						if(value==true){
	     							$("#TABLE_DocList tr:not(:first):not(:last)").html("");
	     							genHtml();
	       					    }
	             	        });
					     }else{
				...
				<table class="tb_normal" width=100%>	
					...
					<tr>
						...
						<td width="85%" colspan="3">
							<xform:address isLoadDataDict="false" style="width:40%" required="true" subject="${lfn:message('sys-task:sysTaskMainPerform.fdPerformId') }"  propertyId="fdPerformId" propertyName="fdPerformName" orgType='ORG_TYPE_PERSON' mulSelect="true" showStatus="edit"></xform:address>
							<%--快速选择--%>
							...
							<label>
							  <span style="margin-left:8px">
							  	<input type="hidden" name="divide"/>
							  		<ui:button text="自动分解" order="2" onclick="divideTask();">
									</ui:button>
							  </span>
							</label>
						</td>	
						 	...
						 	<table border="0" width=100%  class="tb_normal" id="TABLE_DocList" align="center">
								...
								<tr KMSS_IsReferRow="1" style="display: none">
									...
									<td>
										...
										<xform:text property="fdFirstLevelChildForms[!{index}].docSubject" required="true" style="width:95%" showStatus="edit"/>
									</td>
									<td>
										<xform:address propertyName="fdFirstLevelChildForms[!{index}].fdPerformName" showStatus="edit" propertyId="fdFirstLevelChildForms[!{index}].fdPerformId"></xform:address>
									</td>
									<td>
										<xform:datetime property="fdFirstLevelChildForms[!{index}].fdPlanCompleteDateTime" required="true" showStatus="edit" dateTimeType="datetime"></xform:datetime>
									...
									<td>
										<xform:text property="fdFirstLevelChildForms[!{index}].fdWeights" showStatus="edit"/>
									</td>
									<!-- 删除按钮 -->
									<td align="center"  class="td_operate">
										<span style="cursor:pointer" class="optStyle opt_del_style" title="删除行" onmousedown="DocList_DeleteRow_ClearLast();">
										</span>
									</td>
								</tr>
								<c:forEach items="${sysTaskMainForm.fdFirstLevelChildForms}"  var="sysTaskFirstLevelChildForm" varStatus="vstatus">
									<tr KMSS_IsContentRow="1">
										...
										<td>
											<input type="hidden" name="fdFirstLevelChildForms[${vstatus.index}].fdId" value="${sysTaskFirstLevelChildForm.fdId}" /> 
											<input type="hidden" name="fdFirstLevelChildForms[${vstatus.index}].fdParentId" value="${sysTaskMainForm.fdId}" />
											<xform:text property="fdFirstLevelChildForms[${vstatus.index}].docSubject" required="true"  style="width:95%"  value="${sysTaskFirstLevelChildForm.docSubject}"/>
										</td>
										<td>
											<xform:address propertyName="fdFirstLevelChildForms[${vstatus.index}].fdPerformName" propertyId="fdFirstLevelChildForms[${vstatus.index}].fdPerformId" nameValue="${sysTaskFirstLevelChildForm.fdPerformName}" idValue="${sysTaskFirstLevelChildForm.fdPerformId}"></xform:address>
										</td>
										<td>
											<xform:datetime property="fdFirstLevelChildForms[${vstatus.index}].fdPlanCompleteDateTime" dateTimeType="datetime" value="${sysTaskFirstLevelChildForm.fdPlanCompleteDateTime}"></xform:datetime>
										</td>
										<td>
											<xform:text property="fdFirstLevelChildForms[${vstatus.index}].fdWeights" value="${sysTaskFirstLevelChildForm.fdWeights}"/>
										</td>
										<!-- 删除按钮 -->
										<td  align="center" class="td_operate">
											<span style="cursor:pointer" class="optStyle opt_del_style" title="删除行" onmousedown="deleteDocListRow('${sysTaskFirstLevelChildForm.fdId}');">
											</span>
										</td>
									</tr>	
								</c:forEach>
										...
										<div class="tr_normal_opt_content">
											<div class="tr_normal_opt_l">
												<label class="opt_ck_style">
													<input type="checkbox" name="DocList_SelectAll" onclick="DocList_SelectAllRow(this);">全选
												</label>
												<span class="optStyle opt_batchDel_style" onclick="DocList_BatchDeleteRow();" title="删除行">
												</span>
											</div>
												<div class="tr_normal_opt_c">
													<span class="optStyle opt_add_style" title="添加行" onclick="DocList_AddRow()"></span>
													<span class="optStyle opt_up_style" title="上移" onclick="DocList_MoveRowBySelect(-1);"></span>
													<span class="optStyle opt_down_style" title="下移" onclick="DocList_MoveRowBySelect(1);"></span>
												</div>
										</div>
									</td>
								</tr>
						...
						<td  width="100%" colspan=4 style="padding: 0px;">
							<table width="100%" class="tb_normal" height="100%">
								...
								<tr>
									...
									<td width=35%>
										<xform:text className="inputsgl" style="width:10%;" property="fdWeights" htmlElementProperties="onchange ='calculateParentWeight()'" showStatus="edit" required="true"></xform:text>
