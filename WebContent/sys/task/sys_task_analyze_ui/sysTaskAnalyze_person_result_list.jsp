...
function doClick(url){
	url = url +"&isincludechildtask=${lfn:escapeJs(isincludechildtask)}"
					+"&fdStartDate=${lfn:escapeJs(startDate)}"
					+"&fdEndDate=${lfn:escapeJs(endDate)}"
					+"&dateQueryType=${lfn:escapeJs(dateQueryType)}";
	window.open(url);
}
</script>
	...
	<c:if test="${rtnSize>0}">
		<c:forEach items="${rtnList}" var="taskAnalyzeContext" varStatus="vstatus">
			...
			<tr align="center" style="cursor: pointer" bgcolor="${bgcolor}"
				onclick="doClick('<c:url value="/sys/task/sys_task_ui/sysTaskMain_analyze_list.jsp" />?method=listByPerson&fdId=${sysTaskAnalyzeForm.fdId}&orgId=${taskAnalyzeContext.hbmOrgElement.fdId}&orgName=${taskAnalyzeContext.hbmOrgElement.fdName}')"
