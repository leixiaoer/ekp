(function($) {
	...
	var path = '${KMSS_Parameter_ContextPath}' + 'sys/task/resource/treeTable/';
	var maxDept = 1;
	$.fn.treeTable = function(opts) {
		...
		if ($('head').find('#tree_table_' + opts.theme).length == 0) {
			$('head').append('<link id="tree_table_'
									+ opts.theme
									+ '" href="'
									+ path
									+ opts.theme
		...
		$treeTable.click(function(event) {
			...
			if ($target.attr('controller')) {
				...
				// 判断是否是叶子节点
				if ($target.attr('class').indexOf(css['AN']) == -1
						&& $target.attr('class').indexOf(css['N']) == -1) {
					return;
				}
			...
			if ($target.attr('arrow')) {
				...
				if (className == css['AN'] + ' ' + css['HLO']
						|| className == css['AN'] + ' ' + css['HO']) {
					var id = $target.parents('tr[haschild]')[0].id;
					$target.attr('class', css['AN']
							+ " "
							+ (className.indexOf(css['HO']) != -1 ? css['HS']
									: css['HLS']));

					// 关闭所有孩子的tr
					shut(id);
					return;
				} else if (className == css['AN'] + ' ' + css['HLS']
						|| className == css['AN'] + ' ' + css['HS']) {
					var id = $target.parents('tr')[0].id;
					$target.attr('class', css['AN']
							+ " "
							+ (className.indexOf(css['HS']) != -1 ? css['HO']
									: css['HLO']));
		...
		$("span[arrow]", $treeTable)
				.click(
						function(event) {
							event.stopPropagation();
							var $target = $(event.target);
							if ($target.attr('controller')) {
								$target = $target.parents('tr[haschild]').find(
										'[arrow]');
								// 判断是否是叶子节点
								if ($target.attr('class').indexOf(css['AN']) == -1
										&& $target.attr('class').indexOf(
												css['N']) == -1) {
									return;
								}
								var id = $target.parents('tr[haschild]')[0].id;
								if (opts.onSelect
										&& opts.onSelect($treeTable, id) === false) {
									return;
								}
							}
									...
									$target
											.attr(
													...
													css['AN']
															+ " "
															+ (className
																	.indexOf(css['HO']) != -1 ? css['HS']
																	: css['HLS']));

									// 关闭所有孩子的tr
									shut(id);
									return false;
								} else if (className == css['AN'] + ' '
										+ css['HLS']
										|| className == css['AN'] + ' '
												+ css['HS']) {
									var id = $target.parents('tr')[0].id;
		...
		function initRelation($trs, hideLevel) {
			...
			$trs.each(function(i) {
						...
						if (hideLevel) {
							...
							if ($tr.attr('hasChild')
									&& $tr.attr('depth') < opts.expandLevel) {
								var className = $tr.attr('isLastOne') ? css['LO']
										: css['O'];
								$tr.find('.' + css['AN']).attr('class',
										css['AN'] + ' ' + className);
							}
							$($("thead tr:eq(0) th:eq(0)", $treeTable)).css("width",(getMaxDept()* opts.width) + "px");
						}
					});

			// 递归获取深度
			function getDepth(id) {
				if (cMap[id] == 0 || $("#"+cMap[id])[0]===undefined) {
					cMap[id] = 0;
					return 1;
				}
		...
		function shut(id) {
			...
			var dept = getMaxDept();
			var width = opts.width * dept;
			$($("thead tr:eq(0) th:eq(0)", $treeTable)).css("width",
					width + "px");
		}

		function getMaxDept() {
			var maxDept = 1;
			$("tr", $treeTable).each(function(index, el) {
				if ($(el).is(":visible")) {
					if (maxDept < $(el).attr("depth")) {
						maxDept = $(el).attr("depth");
					}
				}
			});
			return maxDept;
		...
		function open(id) {
			...
			var width = (opts.width * $('#' + pMap[id][0], $treeTable).attr("depth"));
			if (width >= getMaxDept() * opts.width) {
				$($("thead tr:eq(0) th:eq(0)", $treeTable)).css("width",
						width + "px");
			}
			...
			for ( var i = 0; i < pMap[id].length; i++) {
				...
				if (pMap[cId]) {
					...
					var className = $('#' + cId, $treeTable).find(
							'.' + css['AN']).attr('class');
					// $($('#' + cId,
					// $treeTable).find("td")[0]).css("max-width",(15*$('#' +
					// cId, $treeTable).attr("depth"))+"px");
					// 如果子节点是展开图表的，则需要展开此节点
					(className == css['AN'] + ' ' + css['O'] || className == css['AN']
							+ ' ' + css['LO'])
							&& open(cId);
				}
			}
		...
		function formatNode(tr) {
			...
			if (cMap[id] == 0) {
				...
				if (!$cur.attr('isFirstOne')) {
					var $preSpan = $('#' + $cur.attr('prevId'), $treeTable)
							.children("td").eq(opts.column).find('.prev_span')
							.clone();
				} else {
					var $parent = $('#' + cMap[id], $treeTable);
					// 没有上一个兄弟节点，则使用父节点的prev_span
					var $preSpan = $parent.children("td").eq(opts.column).find(
							'.prev_span').clone();

					// 如果父亲后面没有兄弟，则直接加空白，若有则加竖线
					if ($parent.attr('isLastOne') || cMap[cMap[id]] == 0) {
						$preSpan.append('<span class="' + css['N'] + ' '
								+ css['B'] + '"></span>');
					} else {
						$preSpan.append('<span class="' + css['N'] + ' '
								+ css['V'] + '"></span>');
