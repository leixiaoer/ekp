...
<template:replace name="content">
		...
		<list:criteria id="criteria">
		     <list:cri-ref key="fdName" ref="criterion.sys.docSubject" title="${lfn:message('sys-task:sysTaskCategory.fdName') }">
			</list:cri-ref>
		</list:criteria>
		<!-- 操作栏 -->
			...
			<div class="lui_list_operation_sort_btn">
				...
				<div class="lui_list_operation_sort_toolbar">
					<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" >
						<list:sortgroup>
							<list:sort property="fdOrder" text="${lfn:message('sys-task:sysTaskCategory.fdOrder') }" group="sort.list" value="up"></list:sort>
					...
					<ui:toolbar id="Btntoolbar">
						...
						<!-- 快速排序 -->					
							<c:import url="/sys/profile/common/change_order_num.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.sys.task.model.SysTaskCategory"></c:param>
		...
		<ui:source type="AjaxJson">
				{url:'/sys/task/sys_task_category/sysTaskCategory.do?method=list&fdId=${fdId}'}
		</ui:source>
		 	...
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
				...
				topic.subscribe('successReloadPage', function() {
		 		...
		 		window.edit = function(id) {
			 		if(id)
		 				Com_OpenWindow('<c:url value="/sys/task/sys_task_category/sysTaskCategory.do" />?method=edit&fdId=' + id);
		 		};
		 		// 删除
		 		window.deleteAll = function(id){
					...
					var url = '<c:url value="/sys/task/sys_task_category/sysTaskCategory.do?method=deleteall"/>';
				...
				window.enable = function(id){
					if(id){
						$.post('<c:url value="/sys/task/sys_task_category/sysTaskCategory.do?method=updateIsAvailable"/>',
