...
<template:include ref="default.simple">
	<template:replace name="title">${lfn:message('sys-task:sysTaskMain.portlet.category')}</template:replace>
	...
	<script>
		function selectCategory(){
			...
			if(selectNodes!=null){
				...
				alert("${lfn:message('sys-task:sysTaskMain.portlet.selectCategory')}");
			}
		}
	</script>
	<script>
	function generateTree(){
		LKSTree = new TreeView(
			"LKSTree",
			"${lfn:message('sys-task:sysTaskMain.portlet.selectCategory')}",
			document.getElementById("treeDiv")
		);
		LKSTree.isShowCheckBox = true;
		LKSTree.isMultSel = false;
		LKSTree.treeRoot.AppendBeanData('sysTaskCategoryTreeService&parentId=!{value}&portlet=true', null, null, null, null);
		LKSTree.Show();
	}
	</script>
