...
<tr>
	<td class="td_normal_title">
		<bean:message key="sysTaskAnalyze.fdDateType" bundle="sys-task"/>
	</td>
	<td colspan="3">
		...
		<xform:radio property="fdDateType" onValueChange="chgTimeSelect" value="${queryForm.fdDateType }" showStatus="edit">
		   <c:choose>
		       <c:when test="${JsParam.selfDefine==false}">
		            <xform:enumsDataSource enumsType="sys_task_stat_fd_date_type_withOutDay">
					</xform:enumsDataSource>
		        </c:when>
		       <c:otherwise>
		        	<xform:enumsDataSource enumsType="sys_task_stat_fd_date_type">
					</xform:enumsDataSource>
		       </c:otherwise>
		   </c:choose>
		</xform:radio>
...
<tr id="timeScope_5" name="timeScope">
	<td class="td_normal_title">
		<bean:message key="sysTaskAnalyze.fdDateType.scope" bundle="sys-task"/>
	</td>
	<td colspan="3">
		...
		<bean:message  bundle="sys-task" key="sysTaskAnalyze.fdDateType.to"/>
...
<tr style="display:none" id="timeScope_7" name="timeScope">
	...
	<td colspan="3">
		...
		<div class="inputselectsgl" onclick="selectDate('fdStartDate_7',null,setCurrTimeVal,setCurrTimeVal);" style="width:20%">
			<div class="input"><input type="text" name="fdStartDate_7" value="${queryForm.fdStartDate }" subject='<bean:message  bundle="sys-task" key="sysTaskAnalyze.fdStartDate"/>'></div>
			<div class="inputdatetime"></div>
		</div>
		<span style="position:relative;top:-7px;"><bean:message  bundle="sys-task" key="sysTaskAnalyze.fdDateType.to"/></span>
		<div class="inputselectsgl" onclick="selectDate('fdEndDate_7',null,setCurrTimeVal,setCurrTimeVal);" style="width:20%">
			<div class="input"><input type="text" name="fdEndDate_7" value="${queryForm.fdEndDate }" subject='<bean:message  bundle="sys-task" key="sysTaskAnalyze.fdEndDate"/>'></div>
			<div class="inputdatetime"></div>
		</div>
	</td>
</tr>
<script type="text/javascript">
	...
	var chgTimeSelect = function(val){
		...
		if(val=='7'){
			...
			if(oldVal!='' && oldVal.indexOf("-")==-1 && oldVal.indexOf("/")==-1){
