...
<c:set var="s_fdTaskId" value="${param.fdId}" scope="request" />
     <%
	  ...
	  %>
<form method="POST" name="sysTaskFeedbackForm" action="${KMSS_Parameter_ContextPath}sys/task/sys_task_feedback/sysTaskFeedback.do?method=save&flag=1&isAppflag=${HtmlParam.isAppflag}">
<div  style="width:100%" >
<table class="docView" width="100%">
		<input type="hidden" name="fdfeedbackId"/>
		<input type="hidden" name="fdTaskId" value="${HtmlParam.fdId}"/>	
	...
	<tr>
		<td class="td_title" width=25%>
			...
			<input type="text"  name="fdProgress" style="width:35px" value="${s_mainForm.fdProgress}"/>%
		</td>
	</tr>
	<tr>
	     <td class="td_title" width=25%>
