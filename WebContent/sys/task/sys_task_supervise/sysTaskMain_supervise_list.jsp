...
<list:data>
	<list:data-columns var="sysTaskMain" list="${queryPage.list }"  custom="false">
		...
		<%--任务名称--%>
		<list:data-column col="docSubject" title="${ lfn:message('sys-task:supervise_task_docSubject') }" style="text-align:left" escape="false">
		...
		<%-- 经办人 --%>
		<list:data-column headerStyle="width:60px;" col="fdAppoint.fdName" title="${ lfn:message('sys-task:supervise_task_responsiblet') }" escape="false" style="width:60px;">
			<ui:person personId="${sysTaskMain.fdAppoint.fdId}" personName="${sysTaskMain.fdAppoint.fdName}"></ui:person>
		</list:data-column>
		
		<%-- 协办单位责任人--%>
		<list:data-column headerStyle="width:100px;"  col="toSysOrgPerform" escape="false" title="${ lfn:message('sys-task:supervise_task_co_responsiblet') }" style="width:80px;">
			<%
		...
		<%-- 协办单位 --%>
		<list:data-column headerStyle="width:60px;" col="fdOrganizer.fdName" title="${ lfn:message('sys-task:supervise_task_co_organizer') }" escape="false" style="width:60px;">
			<%-- <ui:person personId="${sysTaskMain.fdOrganizer.fdId}" personName="${sysTaskMain.fdOrganizer.fdName}"></ui:person> --%>
			<c:out value="${sysTaskMain.fdOrganizer.fdName }" />
		</list:data-column>
		
		<%-- 计划开始 --%>
		<list:data-column headerStyle="width:80px" col="fdPlanStartTime" title="${ lfn:message('sys-task:supervise_task_plan_start_time') }" escape="false">
		    <kmss:showDate value="${sysTaskMain.fdPlanStartTime}" type="date" /> 
		</list:data-column>	
		
		<%-- 计划结束 --%>
		<%-- 计划完成时间:如果未过期显示"计划完成时间"，如果过期显示"超过X天" --%>
		<list:data-column headerStyle="width:140px;" col="fdPlanCompleteDateTime" escape="false" title="${ lfn:message('sys-task:supervise_task_plan_end_time') }" style="width:120px;">
			<%
				if(pageContext.getAttribute("sysTaskMain")!=null){
