		...
		<script type="text/javascript">
			...
			function _search() {
				seajs.use([ 'lui/topic' ], function(topic) {
					...
					topicEvent.criterions.push({
						"key" : "_keyword",
						"value" : [ keyword ]
					});
					topic.publish("criteria.changed", topicEvent);
					topic.subscribe("list.changed", function(data) {
						if (data.datas.length == 0) {
							$(".selection_dialog_btn_ok", parent.document)
									.hide();
						} else {
							$(".selection_dialog_btn_ok", parent.document)
									.show();
			...
			function _getSelectedData(emptyMsg) {
				var rowContents = [];
				...
				if (rowContents.length == 0) {
					seajs
							.use(
									...
									function(dialog) {
										dialog
												.alert(emptyMsg ? emptyMsg
														: "${lfn:message('sys-ui:ui.dialog.template.message')}");
									});
					return null;
				}
			...
			function _checkChg(id) {
				...
				if (selected[id]) {
					delete selected[id];
					rowContentObj.parents("tr").find("[name='List_Selected']")
							.prop("checked", false);
				} else {
			...
			<c:if test="${param.dialogType=='opener'}">
				<div style="text-align: right;">
					...
					<ui:button text="${ lfn:message('button.cancel')}"
						styleClass="lui_toolbar_btn_gray" onclick="window.close();"></ui:button>
				</div>
				<br />
			</c:if>
			<div style="display: table; width: 100%;">
				<div style="display: table-cell; width: 15%; font-size: 16px;">
					${ lfn:message('message.keyword')}:</div>
			...
			<div class="listContent">
				...
				<list:listview id="listview" cfg-criteriaInit="true">
					...
					<list:colTable isDefault="false" name="columntable"
						onRowClick="_checkChg('!{fdId}');">
						<c:if test="${param.mulSelect eq 'true'}">
							<list:col-html title="" headerStyle="width:10px;">
							{$
								<input type="checkbox" onclick="_checkChg('{%row.fdId%}');"
									name="List_Selected" value="{%row.fdId%}"
									{%selected[row.fdId]==null? '':'checked'%}/>
							$}
							</list:col-html>
						</c:if>
						<list:col-serial />
						<c:choose>
							<c:when test="${param.props!=null && param.props!='' }">
								<list:col-auto props="${param.props }" />
							</c:when>
							<c:when test="${param.props==null || param.props=='' }">
								<list:col-auto />
							</c:when>
						...
						<c:if test="${param.mulSelect==null || param.mulSelect==''}">
							<list:col-html
								title="${ lfn:message('sys-ui:ui.help.luiext.operation') }">
							{$
						...
						{$
							<input id="row_{%row.fdId%}" type="hidden"
								value='{%Com_HtmlEscape(domain.stringify(row))%}' />
						$}
						</list:col-html>
					</list:colTable>
				</list:listview>
