...
<table id="sys_ims_config" class="tb_normal" width=100%>

<% if(ImsPluginUtil.getConfigs().size()>0){ %>
	<tr>
		<td class="td_normal_title" colspan=7><xform:text
			property="value(kmss.ims.config)" showStatus="noShow"></xform:text> <input
			onclick="ims_tableToggle('sys_ims_config', this);" type="checkbox" /> <b><bean:message
			key="sys.ims.project" bundle="sys-ims" /></b></td>
	</tr>
	<%
	 for(Map<String,String> imsName:ImsPluginUtil.getConfigs()){
		 %>
	<c:import url="<%=imsName.get(ImsPluginUtil.CONFIG_JSP)%>">
	  <c:param name="imName" value="<%=imsName.get(ImsPluginUtil.IMSNAME)%>"/>
	</c:import>
	<%
	 }
	%>
	...
	<%
	for(Map<String,String> notifyCfgs:ImsPluginUtil.getNotifyConfigs()){
	%>
      <c:import url="<%=notifyCfgs.get(ImsPluginUtil.CFG_JSP)%>"></c:import>	
	<%
	 }
}
	%>
