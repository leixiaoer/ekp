...
div{ 
	...
	<tr class="table deptperson">
		...
		<td>
			...
			<input type="hidden" id="dbenableval" name="value(kmss.oms.in.db.enabled)"  value="" />
	 	...
	 	<script type="text/javascript">
	        ...
	        $.ajax({
	     	...
	     	function selectDatasource(value,str){
	     		...
	     		$.ajax({
	     			url : "${LUI_ContextPath}/sys/oms/sys_oms_temp_trx/sysOmsTempTrx.do?method=getDataSourceTableName&fdSourceName="+datasource,
					type : 'get',
					async : true,
					dataType : "json",
					success : function(data) {
	    	...
	    	function init(){
	     	...
	     	function config_chgEnabled() {
	    		var isChecked = "true" == $(
	    				"input[name='value\\\(kmss.oms.in.db.enabled\\\)']")
	    				.val();
	    		if (isChecked) {
	    			$("#dbenable").show();
	    			$("#dbenableval").val(true);
	    			init();
	    			
	    		} else {
	    			$("#dbenable").hide();
	    			$("#dbenableval").val(false);
