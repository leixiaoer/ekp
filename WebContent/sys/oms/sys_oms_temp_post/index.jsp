...
<template:include ref="default.list">
    <template:replace name="title">
        <c:out value="${ lfn:message('sys-oms:module.sys.oms') }-${ lfn:message('sys-oms:table.sysOmsTempPost') }" />
    </template:replace>
    <template:replace name="path">
        <ui:menu layout="sys.ui.menu.nav">
            <ui:menu-item text="${ lfn:message('home.home') }" icon="lui_icon_s_home" href="/" target="_top" />
            <ui:menu-item text="${ lfn:message('sys-oms:table.sysOmsTempPost') }" />
        </ui:menu>
    </template:replace>
    <template:replace name="nav">
        <ui:combin ref="menu.nav.create">
            <ui:varParam name="title" value="${ lfn:message('sys-oms:table.sysOmsTempPost') }" />
            <ui:varParam name="button">
                [ {"text": "","href": "javascript:void(0);","icon": "/sys/oms/sys_oms_temp_post/sysOmsTempPost.do"} ]
            </ui:varParam>
        </ui:combin>
    ...
    <template:replace name="content">
        <div style="margin:5px 10px;">
            ...
            <list:listview id="listview">
                <ui:source type="AjaxJson">
                    {url:appendQueryParameter('/sys/oms/sys_oms_temp_post/sysOmsTempPost.do?method=data')}
                </ui:source>
                <!-- 列表视图 -->
                <list:colTable isDefault="false" rowHref="/sys/oms/sys_oms_temp_post/sysOmsTempPost.do?method=view&fdId=!{fdId}" name="columntable">
                    <list:col-checkbox />
                    <list:col-serial/>
        ...
        <script>
            var listOption = {
                ...
                jPath: 'temp_post',
                modelName: 'com.landray.kmss.sys.oms.model.SysOmsTempPost',
                templateName: '',
                basePath: '/sys/oms/sys_oms_temp_post/sysOmsTempPost.do',
                canDelete: '${canDelete}',
                mode: '',
                templateService: '',
