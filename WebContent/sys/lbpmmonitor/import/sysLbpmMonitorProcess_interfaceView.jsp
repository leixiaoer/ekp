...
<c:if test="${sysWfBusinessForm.sysWfBusinessForm!=null}">
	...
	<!-- 接口执行记录 -->
	<ui:content id="tab_interfaceLog"
		title="${ lfn:message('sys-lbpmmonitor:sysLbpmMonitor.interface.retryMechanism') }" expand="true">
		<table width="100%" border=0 >
			<tr id="_interfaceLog">
				<td id="interfaceLogTableTD"
						${resize_prefix}onresize="lbpm.process_interface_Log_Frame();"><iframe
							width="100%" height="100%" scrolling="no" FRAMEBORDER=0></iframe>
					</td>
			</tr>
		</table>
	...
	<script>
		...
		lbpm.process_interface_Log_Frame=function(){
			lbpm.globals.load_Frame('interfaceLogTableTD', '<c:url value="/sys/lbpmmonitor/import/sysLbpmMonitorProcess_interfaceLog.jsp" />'
				+'?fdModelId=${param.processId}');
		};
