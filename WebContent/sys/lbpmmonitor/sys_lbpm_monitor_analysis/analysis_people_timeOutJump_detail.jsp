      ...
      <ul class="lui-flowMonitor-tabs">
        <li class="active">
        	<span class="txtlistpath">
				<a href="javascript:backUrl();"><bean:message key="sysLbpmMonitor.analysis.back" bundle="sys-lbpmmonitor" /> > <bean:message key="sysLbpmMonitor.analysis.peopleTimeoutJumpMaxMuch" bundle="sys-lbpmmonitor" /></a>
			</span>
        </li>
      </ul>
    </div>
    ...
    <div class="lui-flowMonitor-tab-content lui-flowMonitor-tab-content-detail">
      <div class="lui-flowMonitor-tab-pane fade active in">
        <ul class="lui-flowMonitor-statistics-board">
          ...
          <li>
            <div class="magnet">
              ...
              <p class="magnet-subhead" id="timeOutJump_people_max"></p>
            </div>
          </li>
        </ul>
        <ul class="lui-flowMonitor-chartList">
          <li>
            <div class="chart-item"  id="timeOutJumpPeople" style="display: block; height: 420px; text-align: center; font-size: 36px;">
            </div>
          </li>
        </ul>
      </div>
	   ...
	   seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($ ,dialog, topic) {
		   ...
		   var peopleTimeOutJumpItem = echarts.init(document.getElementById("timeOutJumpPeople"));
			  // 显示标题，图例和空的坐标轴
			 peopleTimeOutJumpItem.setOption({
			 	color: ['#4285F4'],
			 	title: {
			 		text: '<bean:message key="sysLbpmMonitor.analysis.peopleTimeoutJumpMaxRanking" bundle="sys-lbpmmonitor" />',
			 		x: 'left',
			 		textStyle:{
			 			color:'#333',
			 			fontSize:16
			 	...
			 	series: [{
			 		name:'<bean:message key="sysLbpmMonitor.analysis.peopleTimeoutJumpRankingMuch" bundle="sys-lbpmmonitor" />',
			         type:'bar',
			         barWidth: 20,
			         data:[]
			 	}]
			 ...
			 peopleTimeOutJumpItem.on('click', function (param) { 
				   
					// 指定图表的配置项和数据
			       var option = peopleTimeOutJumpItem.getOption();
			       ...
			       dialog.iframe('/sys/lbpmmonitor/sys_lbpm_monitor_analysis/analysis_list.jsp?fdPersonId=' + fdPersonId+"&excuteType=peopleTimeOutJump&fdModelName="+modelName,
				 			"流程清单", null, {
						width : 960,
						height : 400
					});
						    ...
						    $.post(url, param, function(data,status){
						 	...
						 	for(var i=0;i<datamap.length;i++){
						 		if(i==0){
							 		 ...
							 		 $("#timeOutJump_people_max").html(datamap[i].fdName);
						 		}
						 				...
						 				dataView: {readOnly: true,
						 					optionToContent: function(opt) {
						 						...
						 						var table = '<table class="lui_data_form_two" style="width:100%;text-align:center;background-color:#FFFFFF" border><tbody><tr>'
						 									 + '<td><bean:message key="sysLbpmMonitor.analysis.peopleTimeoutJumpMaxRanking" bundle="sys-lbpmmonitor" /></td>'
						 									 + '<td>' + series[0].name + '</td>'
						 									 + '</tr>';
