...
<template:include ref="default.simple">
      ...
      <ul class="lui-flowMonitor-tabs">
        <li class="active">
        		<span class="txtlistpath">
					<a href="javascript:backUrl();"><bean:message key="sysLbpmMonitor.analysis.back" bundle="sys-lbpmmonitor" /> > <bean:message key="sysLbpmMonitor.analysis.avgNodeConsumTime" bundle="sys-lbpmmonitor" /></a>
				</span>
        </li>
      </ul>
    </div>
    ...
    <div class="lui-flowMonitor-tab-content lui-flowMonitor-tab-content-detail">
      <div class="lui-flowMonitor-tab-pane fade active in">
        <ul class="lui-flowMonitor-statistics-board">
          ...
          <li>
            <div class="magnet">
              ...
              <h4 class="magnet-title"><bean:message key="sysLbpmMonitor.analysis.avgNodeConsumMaxTime" bundle="sys-lbpmmonitor" /></h4>
              <p class="magnet-subhead" id="avg_time_node_max"></p>
            </div>
          </li>
        </ul>
        <ul class="lui-flowMonitor-chartList">
          <li>
            <div class="chart-item"  id="nodeApprovalTime" style="display: block; height: 420px; text-align: center; font-size: 36px;">
            </div>
          </li>
        </ul>
      </div>
	   ...
	   setCookie("returnTab","tab-2");//保存选项卡值，返回的时候能跳到相关选项卡
	   console.log("节点"+getCookie("returnTab"));
	   seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($ ,dialog, topic) {
		    
		     var url = '<c:url value="/sys/lbpmmonitor/sys_lbpmmonitor_flow/SysLbpmMonitorFlow.do" />';
		 
		     //平均耗时
			 var nodeApprovalTimeItem = echarts.init(document.getElementById("nodeApprovalTime"));
			  // 显示标题，图例和空的坐标轴
			 nodeApprovalTimeItem.setOption({
			 	color: ['#FFD37D'],
			 	title: {
			 		text: '<bean:message key="sysLbpmMonitor.analysis.avgNodeConsumTime" bundle="sys-lbpmmonitor" />',
			 		x: 'left',
			 		textStyle:{
			 			color:'#333',
			 			fontSize:16
			 	...
			 	series: [{
			 		name:'<bean:message key="sysLbpmMonitor.analysis.avgNodeConsumTimeMuch" bundle="sys-lbpmmonitor" />',
			         type:'bar',
			         barWidth: 20,
			         data:[]
			 	}]
			 });
			 nodeApprovalTimeItem.showLoading(); 
			 
			 
			
			 nodeApprovalTimeItem.on('click', function (param) { 
				   
					// 指定图表的配置项和数据
			       var option = nodeApprovalTimeItem.getOption();
					
					//获取自定义参数Id值
					var fdNodeId=option.series[param.seriesIndex].rawNodedata[param.dataIndex];
					var fdTempletModelId=option.series[param.seriesIndex].rawModelData[param.dataIndex];
					
			       console.log("fdNodeId:"+fdNodeId);
			       console.log("fdTempletModelId:"+fdTempletModelId);
			       console.log(param);
			       
			       dialog.iframe('/sys/lbpmmonitor/sys_lbpm_monitor_analysis/analysis_list.jsp?fdNodeId=' + fdNodeId+"&fdTempletModelId="+fdTempletModelId,
				 			"流程清单", null, {
						width : 960,
						height : 400
					});
			...
			topic.channel("channel_common_approve").subscribe('criteria.changed',
			 ...
			 function postAction(url,modelName){
				 ...
				 var param = {
						method : "getNodeApprovalTimeList",
						fdModelName:modelName
						 	...
						 	var nodeIdData=[];//存放节点ID
						 	var templateModelIdData=[];//存放模版ID
						    ...
						    $.post(url, param, function(data,status){
						 	...
						 	for(var i=0;i<datamap.length;i++){
						 		if(i==0){
							 		...
							 		$("#avg_time_node_max").html(datamap[i].fdName);
							 			
						 		}
						 		
						 		nodeIdData.unshift(datamap[i].fdFactNodeId);
						 		templateModelIdData.unshift(datamap[i].fdTemplateModelId);
						 		yData.unshift(datamap[i].fdName);
						 		xData.unshift(parseInt(datamap[i].processCount));
						 	}
						 	
						 	nodeApprovalTimeItem.hideLoading(); 
						 	
						 	nodeApprovalTimeItem.setOption({        //加载数据图表
						 		yAxis: {
						             data: yData
						         },
						         toolbox: {
						 				...
						 				dataView: {readOnly: true,
						 					optionToContent: function(opt) {
						 					    ...
						 					    if(safariVersion <= DEFAULT_VERSION ){  
						 					    	var div$One=$("#nodeApprovalTime").children("div").get(0);
						 							 $(div$One).hide();
						 					    }; 
						 						...
						 						var table = '<table class="lui_data_form_two" style="width:100%;text-align:center;background-color:#FFFFFF" border><tbody><tr>'
						 									 + '<td><bean:message key="sysLbpmMonitor.analysis.avgNodeConsumTime" bundle="sys-lbpmmonitor" /></td>'
						 									 + '<td>' + series[0].name + '</td>'
						 									 + '</tr>';
						 						for (var i = 0, l = axisData.length; i < l; i++) {
						 							table += '<tr>'
						 									 + '<td>' + axisData[i] + '</td>'
						 									 + '<td>' +  (series[0].data[i]==null ? "":series[0].data[i]) + 'h</td>'
						 									 + '</tr>';
						 						}
						 						table += '</tbody></table>';
						         ...
						         series: [{
						             // 根据名字对应到相应的系列
						             data: xData,
						             rawNodedata:nodeIdData,
						             rawModelData:templateModelIdData
						         }]
						     });
