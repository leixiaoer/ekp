...
<template:include ref="default.edit" width="95%" sidebar="no">
	...
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" var-navwidth="95%">
			<c:choose>
				<c:when test="${ sysPortalMainForm.method_GET == 'addAnonymous' }">
					<ui:button text="${lfn:message('button.save') }" order="2" onclick=" Com_Submit(document.sysPortalMainForm, 'save');">
					</ui:button>
				</c:when>
				<c:when test="${ sysPortalMainForm.method_GET == 'editAnonymous' }">					
	...
	<template:replace name="content"> 
		...
		<p class="txttitle">${ lfn:message('sys-portal:sysPortalMain.anonymous.setting') }</p>
		...
		<script>
			...
			function selectPage(){
				seajs.use(['lui/dialog'],function(dialog){
					dialog.iframe('/sys/portal/sys_portal_page/sysPortalPage_select.jsp?fdAnonymous=1',"${lfn:message('sys-portal:sysPortalPage.msg.selectpage.anonymous')}",function(val){
						//debugger;
		...
		<center><table class="tb_normal" width=95%>
			<tr>
				<td class="td_normal_title" width=15%>
					<bean:message bundle="sys-portal" key="sysPortalMain.fdName"/>
				</td>
				<td width="35%">
					<xform:text required="true" subject="${ lfn:message('sys-portal:sysPortalMain.fdName') }" property="fdName" style="width:85%" showStatus="true" />
				</td>
			...
			<tr>
				...
				<td width="35%">
					<xform:dialog propertyId="fdParentId" propertyName="fdParentName" style="width:90%" showStatus="true">
					Dialog_Tree(false,'fdParentId','fdParentName',';','sysPortalMainTreeService&fdAnonymous=1','${ lfn:message('sys-portal:sysPortalPage.msg.selectParent') }',null,null,'${ sysPortalMainForm.fdId }');
					</xform:dialog>
				</td>
			</tr>
			<tr>				
				...
				<td width="35%">
					<xform:radio property="fdEnabled" showStatus="true">
						<xform:enumsDataSource enumsType="common_yesno"></xform:enumsDataSource>		
					</xform:radio>
				</td>
				<td class="td_normal_title" width=15%>		
					<bean:message bundle="sys-portal" key="sysPortalLinkDetail.fdTarget"/>					
				</td>
				<td width="35%">
					<xform:radio property="fdTarget" className="pageTarget" showStatus="true">
			...
			<tr>
				<td class="td_normal_title" width=15%>							
					<bean:message bundle="sys-portal" key="sysPortalMain.fdIsQuick"/>
				</td>
			...
			<c:if test="${ empty sysPortalMainForm.fdParentId }">
				<tr>
					<td class="td_normal_title" width=15%>
						<bean:message bundle="sys-portal" key="sysPortalLinkDetail.fdOrder"/>
					</td>
					<td colspan="3">
						<xform:text property="fdOrder" style="width:85%" showStatus="true" />
					</td>
				</tr>
				<%-- 所属场所 --%>
			...
			<tr>
				...
				<td width="35%" colspan="3">
					...
					function selectTheme(){
				 	...
				 	<xform:dialog subject="${ lfn:message('sys-portal:sysPortalPage.fdTheme') }" style="width:90%" propertyId="fdTheme" propertyName="fdThemeName" showStatus="true">
					selectTheme()
					</xform:dialog>
				</td>
			</tr>
				...
				<td colspan="3">
					...
					<select id="portal_page_header_select" onchange="fdHeaderSelectChange()">
						...
						for(int i=0;i < hs.size(); i++){
							...
							if ("anonymous".equals(h.getFdFor())) {
				...
				<td colspan="3">
					...
					function fdFooterSelectChange(val){
						...
						if( sindex > 0){
							...
							if(footerId == "footer.anonymous.default") {
								customFooter();
							}
						}else{
							LUI.$("#fdFooterId").val("");
			...
			<tr>
				...
				<td colspan="3">
					<xform:address textarea="true" mulSelect="true" propertyId="authEditorIds" propertyName="authEditorNames" style="width:96%;height:90px;" showStatus="true" />
			...
			<c:if test="${sysPortalMainForm.method_GET!='addAnonymous'}">
			<tr>
				<td class="td_normal_title" width=15%>
		...
		LUI.ready(function(){
			...
			if(headerId){
				...
				if(headerId===""&&"${ sysPortalMainForm.method }"==="editAnonymous"){
					//编辑话,为空选择无页眉
					LUI.$("#portal_page_header_select").val(headerId);
				}else{
					//新建的话,为空选择默认页眉
			...
			if($.trim(footerId) == ""){
				if("${ sysPortalMainForm.method }"==="addAnonymous"){
				...
				//页脚编号非空则带出对应页脚配置页面 add by zhouwen 20220111
				LUI.$("#portal_page_footer_select").val(footerId);
			}
