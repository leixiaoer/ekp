...
function createPortalTable(dataList){
	...
	html+="<td nowrap width='120'>"+titleInfo.fdOpts+"</td>";
	html+="</tr>";
...
function createPortalRow( dataList, childRow ){
	for (var i=0; i<dataList.length; i++) {
	    ...
	    // 当前行有子数据的情况下，显示“展开”图标
		if(hasChildren){
			childRow.html+="<div id='t_"+rowData.fdId+"' class='collapse' onclick='toggle(this)' style='float:left;height:20px;width:20px;' ></div>";
		}else{
		...
		// 5、《操作》列   
		childRow.html+="<td style='width:120px'>";
		if(rowData.isCanEdit){ // 判断是否允许“编辑”
			childRow.html+="<div style='float:left;width: 40px;'>";
		...
		if(rowData.isCanDel){  // 判断是否允许“删除”
			...
			childRow.html+="<div style='float:left;width: 40px;'><span>&nbsp;</span></div>";
		}
		...
		if(hasChildren){
			childRow.html+="<tr id='d_"+rowData.fdId+"' >";
