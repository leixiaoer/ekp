...
<c:if test="${empty param['j_content'] }">
	...
	<html>
		...
		<body class="lui_portal_body lui_portal_t_banner_content">
		<c:if test="${empty param.j_rIframe && empty param.j_iframe}">
			<!-- Tbanner 模板 -->
		    <!-- 头部页眉  -->
			<portal:header scene="portal" var-width="'100%'" />
		</c:if>
			...
			<div class="lui_portal_main_background_normal" style="background-image:${empty param['backgroundimagepath'] ? 'none;' : 'url('.concat(LUI_ContextPath).concat(param['backgroundimagepath']).concat(');')}" >
				...
				<div class="lui_portal_t_banner_banner">
					<template:block name="body1"></template:block>
				</div>
				<div class="lui_portal_t_banner_container" style="margin: 0px auto;${empty param['pagewidth'] ? 'width:980px' : lfn:concat('width:',param['pagewidth']) };min-width:980px; max-width:${fdPageMaxWidth}">
...
<c:if test="${not empty param['j_content'] && param['j_content'] == true }">
	...
	<div class="lui_portal_main_background_quick lui_portal_t_banner_content" style="background-image:${empty param['backgroundimagepath'] ? 'none;' : 'url('.concat(LUI_ContextPath).concat(param['backgroundimagepath']).concat(');')}" >
