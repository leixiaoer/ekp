    ...
    <template:include ref="default.edit">
        <template:replace name="head">
            ...
            <script type="text/javascript">
                ...
                Com_IncludeFile("form_option.js", "${LUI_ContextPath}/sys/portal/sys_portal_material_main/", 'js', true);
        ...
        <template:replace name="title">
            <c:choose>
                <c:when test="${sysPortalMaterialMainForm.method_GET == 'add' }">
                    <c:out value="${ lfn:message('operation.create') } - ${ lfn:message('sys-portal:table.sysPortalMaterialMain') }" />
                </c:when>
                <c:otherwise>
        ...
        <template:replace name="toolbar">
            <ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="3">
                <c:choose>
                    <c:when test="${ sysPortalMaterialMainForm.method_GET == 'edit' }">
                        <ui:button text="${ lfn:message('button.update') }" onclick="if(validateDetail()){Com_Submit(document.sysPortalMaterialMainForm, 'update');}" />
                    </c:when>
                    <c:when test="${ sysPortalMaterialMainForm.method_GET == 'add' }">
                        <ui:button text="${ lfn:message('button.save') }" onclick="if(validateDetail()){Com_Submit(document.sysPortalMaterialMainForm, 'save');}" />
                    </c:when>
                </c:choose>
        ...
        <template:replace name="path">
            <ui:menu layout="sys.ui.menu.nav">
                ...
                <ui:menu-item text="${ lfn:message('sys-portal:table.sysPortalMaterialMain') }" />
            </ui:menu>
        </template:replace>
        <template:replace name="content">
            <html:form action="/sys/portal/sys_portal_material_main/sysPortalMaterialMain.do">
                ...
                <ui:tabpage expand="false" var-navwidth="90%">
                    <ui:content title="${ lfn:message('sys-portal:py.JiBenXinXi') }" expand="true">
                        ...
                        <table class="tb_normal" width="100%">
                            ...
                            <tr>
                                <td class="td_normal_title" width="15%">
                                    ${lfn:message('sys-portal:sysPortalMaterialMain.fdLength')}
                                </td>
                                <td width="35%">
                                    <%-- 长--%>
                                ...
                                <td class="td_normal_title" width="15%">
                                    ${lfn:message('sys-portal:sysPortalMaterialMain.attMaterial')}
                                </td>
                                <td width="35%">
                                    <%-- 附件--%>
                                    <c:import url="/sys/attachment/sys_att_main/sysAttMain_edit.jsp" charEncoding="UTF-8">
                                        <c:param name="fdKey" value="attMaterial" />
                                        <c:param name="formBeanName" value="sysPortalMaterialMainForm" />
                            ...
                            <tr>
                                <%-- 标签--%>
                                <td class="td_normal_title" width="15%">
                                    ${ lfn:message('sys-portal:py.BiaoQian') }
                                </td>
                                <td width="35%">
                                    <c:import url="/sys/tag/import/sysTagMain_import_edit.jsp" charEncoding="UTF-8">
