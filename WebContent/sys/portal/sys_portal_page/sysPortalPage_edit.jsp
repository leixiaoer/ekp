	...
	<template:replace name="content">
		    ...
		    function submenuMaintenance(){
				...
				$.ajax({
					...
					success: function (data) {
					  var url = "";
                      if(data && data.fdId){
                    	 url = '<c:url value="/sys/portal/sys_portal_nav/sysPortalNav.do?method=edit&s_css=default"/>&fdId='+data.fdId+'&switchAnonymous=0';
                      }else{
          		    	 url = '<c:url value="/sys/portal/sys_portal_nav/sysPortalNav.do?method=add&s_css=default"/>&fdPageId=${sysPortalPageForm.fdId}&switchAnonymous=0';
                      }
		...
		<ui:tabpanel id="pageEdit">
			<ui:content title="${ lfn:message('sys-portal:sysPortalPage.msg.baseInfo')}">
			 	<table class="tb_normal" width=100%>
					...
					<tr>						
						...
						<td width="35%">			 
							<xform:text property="fdTitle" style="width:90%" />
						</td>
					</tr>
					<tr>
						<td class="td_normal_title" width=15%>
					...
					<c:if test="${sysPortalPageForm.method_GET!='add'}">
					<tr>
						<td class="td_normal_title" width=15%>
			...
			<ui:content title="${ lfn:message('sys-portal:sysPortalPage.msg.pageEdit')}" id="designerPanel">
				<portal:designer scene="portal" style="width:100%;" property="pageDetails[0].docContent"/>
			</ui:content>
		</ui:tabpanel> 
