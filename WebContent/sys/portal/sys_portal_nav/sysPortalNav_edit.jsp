...
<template:include ref="default.edit" width="95%" sidebar="no">
	...
	<template:replace name="path">
		<ui:menu layout="sys.ui.menu.nav" style="height:40px;line-height:40px;">
			...
			<ui:menu-item text="${ lfn:message('sys-portal:table.sysPortalNav') }">
			</ui:menu-item>
		</ui:menu>
	</template:replace>
	<template:replace name="toolbar">
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" var-navwidth="95%">
				<c:choose>
					<c:when test="${ sysPortalNavForm.method_GET == 'add' }">
						<ui:button text="${lfn:message('button.save') }" order="2" onclick="submitForm('save',true);">
						</ui:button>
					</c:when>
					<c:when test="${ sysPortalNavForm.method_GET == 'edit' }">					
						<ui:button text="${lfn:message('button.update') }" order="2" onclick="submitForm('update',true);">
						</ui:button>	
						<kmss:auth requestURL="/sys/portal/sys_portal_nav/sysPortalNav.do?method=delete&fdId=${param.fdId}" requestMethod="GET">
	...
	<link href="<%=request.getContextPath()%>/sys/portal/sys_portal_nav/resource/css/helpTip.css" rel="stylesheet" type="text/css">
	<script>
	...
	function deleteTree(){
		seajs.use(['lui/dialog'],function(dialog){
			dialog.confirm("${ lfn:message('sys-portal:sysPortalPage.msg.delete') }",function(val){
				if(val==true){
					location.href = "${LUI_ContextPath}/sys/portal/sys_portal_nav/sysPortalNav.do?method=delete&fdId=${sysPortalNavForm.fdId}";
				}
			})
		});
	}
		...
		seajs.use(['lui/dialog','lui/jquery'], function(dialog, $){
			dialog.build({
				...
				callback : function(data) {
					...
					var iconName = getIconName(data[0].url);
					if(iconName){
						iconName = "iconfont_nav lui_iconfont_nav" + iconName;
						$('#tmpIconPreview').removeClass().addClass("lui_iconfont_selectIcon " + iconName);
						$("input[name='tmpIcon']").val(iconName);
					}
				}
			}).show(); 
		});
	...
	function selectPortalPage(){
		seajs.use(['lui/dialog'],function(dialog){
			dialog.iframe('/sys/portal/sys_portal_page/sysPortalPage_select.jsp?single=true',
					"${lfn:message('sys-portal:sysPortalPage.msg.selectpage')}",function(val){
					if(val){
						document.getElementsByName("tmpURL")[0].value = "/sys/portal/page.jsp?mainPageId=" + val.fdId;
						document.getElementsByName("tmpNodeName")[0].value = val.fdName;
					}
	...
	function modifyIcon(){
		seajs.use(['lui/dialog','lui/util/str'],function(dialog,str){
			dialog.iframe('/sys/ui/jsp/iconfont.jsp',"${ lfn:message('sys-portal:sysPortalNav.icon.title') }",function(value){
...
<html:form action="/sys/portal/sys_portal_nav/sysPortalNav.do">
 
<p class="txttitle"><bean:message bundle="sys-portal" key="table.sysPortalNav"/></p>

<center>
<table class="tb_normal" width=95%>
	<tr>
		<td class="td_normal_title" width=15%>
			<bean:message bundle="sys-portal" key="sysPortalNav.fdName"/>
		</td>
		<td colspan="3">
			<xform:text property="fdName" required="true" subject="${ lfn:message('sys-portal:sysPortalNav.fdName') }" style="width:85%" />
		</td>
	</tr> 
	<tr>
		<td class="td_normal_title" width=15%>
		...
		<td colspan="3">
		 	<c:import
				...
				charEncoding="UTF-8">
				<c:param name="formName" value="sysPortalNavForm" />
	...
	<tr>
		<td class="td_normal_title" width=15%>
			<bean:message bundle="sys-portal" key="sysPortalNav.fdContent"/>
		</td>
					...
					<td colspan="2">
						...
						<div id=DIV_AllControl style="display:none">
							...
							<span lks_info="NewEditNode">
								...
								<a onclick="selectPortalPage();return false;" href=""><bean:message  bundle="sys-portal" key="sysHomeNav.button.selectPagePortal"/></a>
								<div lks_info_item="tmpURL" class="item">
									<bean:message  bundle="sys-portal" key="sysHomeNav.msg.url"/><input name="tmpURL" value="" style="width:450px" class="inputsgl">
								...
								%>
								<div lks_info_item="tmpTarget" class="item" style="padding-top: 5px;">
								    <div style="display: inline-block;*display: inline;*zoom: 1;position: relative;">
								        <bean:message  bundle="sys-portal" key="sysPortalNav.msg.target"/>
								    </div>
									<div style="display: inline-block;*display: inline;*zoom: 1;position: relative;">
									    <input type=radio name="tmpTarget" value="_top"><bean:message  bundle="sys-portal" key="sysPortalNav.msg.target.top"/>
									</div>
									<div style="display: inline-block;*display: inline;*zoom: 1;position: relative;margin-left: 16px;">
									    <input type=radio name="tmpTarget" value="_blank" ><bean:message  bundle="sys-portal" key="sysPortalNav.msg.target.blank"/>
									</div>
									<div style="display: inline-block;*display: inline;*zoom: 1;position: relative;margin-left: 16px;">
										<input type=radio name="tmpTarget" value="_content"><bean:message  bundle="sys-portal" key="sysPortalNav.msg.target.content"/>
									    <div class="lui_prompt_tooltip">
									      <label class="lui_prompt_tooltip_drop">
									         <img src="<%=request.getContextPath()%>/sys/portal/sys_portal_nav/resource/image/helpTip.png" />
									      </label>
									      <div class="lui_dropdown_tooltip_menu">${lfn:message('sys-portal:sysPortalNav.msg.target.content.tips')}</div>
									    </div> 
									</div>
								</div>
								<!-- 展开方式 -->
								<div lks_info_item="tmpexpand" class="item" style="padding-top: 5px;">
								    <div style="display: inline-block;*display: inline;*zoom: 1;position: relative;">
								        <bean:message  bundle="sys-portal" key="sysPortalNav.msg.extend"/>
								    </div>
									<div style="display: inline-block;*display: inline;*zoom: 1;position: relative;">
									    <input type=radio name="tmpexpand" value=true checked ><bean:message  bundle="sys-portal" key="sysPortalNav.msg.extend.open"/>
									</div>
									<div style="display: inline-block;*display: inline;*zoom: 1;position: relative;margin-left: 16px;">
									    <input type=radio name="tmpexpand" value=false ><bean:message  bundle="sys-portal" key="sysPortalNav.msg.extend.close"/>
									</div>
								</div>
									
								<div style="margin-top: 10px;">
									<bean:message  bundle="sys-portal" key="sysPortalNav.msg.icon"/>
									<div id='tmpIconPreview' class="lui_iconfont_selectIcon iconfont_nav" style="display: inline-block;">
										</div>
									<a href="javascript:void(0)" onclick="modifyIcon()">${ lfn:message('sys-portal:sysPortalPage.msg.select') }</a>
									<input type="hidden" name="tmpIcon">
								</div>
							</span>
	...
	<c:import url="/sys/authorization/sys_auth_area/sysAuthArea_field.jsp" charEncoding="UTF-8">
        <c:param name="id" value="${sysPortalNavForm.authAreaId}"/>
    </c:import> 
	<tr>
	...
	<tr>
		<td class="td_normal_title" width=15%>
			<bean:message bundle="sys-portal" key="sysPortalNav.docCreator"/>
		</td><td width="35%">
			<xform:address propertyId="docCreatorId" propertyName="docCreatorName" orgType="ORG_TYPE_PERSON" showStatus="view" />
		</td>
		<td class="td_normal_title" width=15%>
			<bean:message bundle="sys-portal" key="sysPortalNav.docCreateTime"/>
		</td><td width="35%">
			<xform:datetime property="docCreateTime" showStatus="view" />
		</td>
	</tr>
	<c:if test="${ sysPortalNavForm.method_GET != 'add' }">
	<tr>
		<td class="td_normal_title" width=15%>
			<bean:message bundle="sys-portal" key="sysPortalNav.docAlteror"/>
		</td><td width="35%">
			<xform:address propertyId="docAlterorId" propertyName="docAlterorName" orgType="ORG_TYPE_PERSON" showStatus="view" />
		</td>
		<td class="td_normal_title" width=15%>
			<bean:message bundle="sys-portal" key="sysPortalNav.docAlterTime"/>
		</td><td width="35%">
