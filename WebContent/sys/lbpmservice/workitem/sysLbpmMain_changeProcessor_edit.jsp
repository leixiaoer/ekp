...
function _switchOp(switchFlag){
	...
	if(switchFlag=="all"){
		top._switchLoadFrame(topFrame,"all");
	}else{
		top._switchLoadFrame(topFrame);
	}
}
...
<script>
	var topFrame = top.document.getElementById("topFrame");
	top.WorkFlow_InitialWindow(topFrame,"all");
</script>
</c:if>
<c:if test="${param.switchFlag ne 'all'}">
<script>
	var topFrame = top.document.getElementById("topFrame");
	top.WorkFlow_InitialWindow(topFrame);
</script>
</c:if>