...
lbpm.globals.generateNextNodeInfo=function(fromCalcBranch){
	...
	if(operatorInfo.currNodeNextHandlersId && currentNodeObj.processType == lbpm.constant.PROCESSTYPE_SERIAL){
		...
		langNodeName=langNodeName.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
		var identifier =  _lbpmIsRemoveNodeIdentifier() ? "" : (currentNodeObj.id + ".");
		html = '<label id="nextNodeName"><b>' + identifier + langNodeName + '</b></label>';
			...
			nextShowHandlerName)
{
	...
	langNodeName=langNodeName.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
	var identifier =  _lbpmIsRemoveNodeIdentifier() ? "" : (nodeObj.id + ".");
	...
	$.each(routeLines, function(i, lineObj) {
		...
		if(lineObj.startNode.nodeDescType=='splitNodeDesc'){
			if(lineObj.startNode.splitType&&lineObj.startNode.splitType=="custom"){
				html += "<label class='lui-lbpm-checkbox' style='line-height:26px;' id='nextNodeName[" + i + "]'><input "+(onlyOneSelect==true?"style='display:none' checked=true ":((nodeObj.id == futureNodeId || lineObj.id == lineObj.startNode.defaultBranch) ? "checked=true " :""))+"type='checkbox' validate='required' subject='即将流向'  manualBranchNodeId='"+lineObj.startNode.id+"'name='futureNode' key='futureNodeId' index='" + i + "' value='" + nodeObj.id 
