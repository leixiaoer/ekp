...
lbpm.globals.setModifyEmbeddedSubFlowDivInfo = function(embeddedNodeIds,modifyEmbeddedSubFlowDIV){
	...
	for(var i=0;i<embeddedNodeIds.length;i++){
		$(modifyEmbeddedSubFlowDIV).append("<a class='com_btn_link' onclick='lbpm.globals.modifyEmbeddedSubFlow(\""+embeddedNodeIds[i]+"\")'>"+Data_GetResourceString("sys-lbpmservice:select.modify.org")+embeddedNodeIds[i]+"."+lbpm.nodes[embeddedNodeIds[i]].name+"</a>&nbsp;&nbsp;&nbsp;&nbsp;")
	}
};
				...
				if(nowNode.isInit == "true"){
					...
					for(var j=0;j<nowFlow.lines.length;j++){
						if(Com_ArrayGetIndex(nodeIds, nowFlow.lines[j].startNodeId) > -1 && nowFlow.lines[j].endNodeId!=groupEndNodeId){
							processData.lines.push($.extend(true, {}, nowFlow.lines[j]));
	...
	if(nodeObj.useOptHandlerOnly == "true"){
		...
		if(handlerSelectType=="formula" || handlerSelectType=="rule" || handlerSelectType=="matrix"){
			...
			if(nodeObj.nodeDescType=="shareReviewNodeDesc"){
				// 微审批节点处理人选择控制
				hrefObj += " onclick=\"{Dialog_AddressList(false, lbpm.nodeParseHandlerIdNode || null,lbpm.nodeParseHandlerNameNode || null, ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
			} else {
				hrefObj += " onclick=\"{Dialog_AddressList(true, lbpm.nodeParseHandlerIdNode || null,lbpm.nodeParseHandlerNameNode || null, ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
			}
		}
		else{
			if(nodeObj.nodeDescType=="shareReviewNodeDesc"){
				// 微审批节点处理人选择控制
				hrefObj += " onclick=\"{Dialog_AddressList(false, '"+handlerIdObj+"','"+handlerNameObj+"', ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
			}
			else{
				hrefObj += " onclick=\"{Dialog_AddressList(true, '"+handlerIdObj+"','"+handlerNameObj+"', ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
			}
		...
		if(handlerSelectType=="formula" || handlerSelectType=="rule" || handlerSelectType=="matrix"){
			...
			if(nodeObj.nodeDescType=="shareReviewNodeDesc"){
				...
				hrefObj += " onclick=\"{"+dialogAddsFunc+"(false, lbpm.nodeParseHandlerIdNode || null,lbpm.nodeParseHandlerNameNode || null, ';', "+selectType+", function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, null, null, null, null, null, null, '" + nodeObj.id + "','"+defaultOptionBean+"');}\"";
				optHrefObj += " onclick=\"{Dialog_AddressList(false, null,null, ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
				hrefObj_formula += " onclick=\"{"+formulaDialogFunc+"('"+handlerIdObj+"','"+handlerNameObj+"', '"+modelName+"', '"+nodeObj.id+"',true)}\"";
			} else{
				...
				hrefObj += " onclick=\"{"+dialogAddsFunc+"(true, lbpm.nodeParseHandlerIdNode || null,lbpm.nodeParseHandlerNameNode || null, ';', "+selectType+", function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, null, null, null, null, null, null, '" + nodeObj.id + "','"+defaultOptionBean+"');}\"";
				optHrefObj += " onclick=\"{Dialog_AddressList(true, null,null, ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
			...
			if(nodeObj.nodeDescType=="shareReviewNodeDesc"){
				// 微审批节点处理人选择控制
				var selectType = 'ORG_TYPE_PERSON';
				hrefObj += " onclick=\"{"+dialogAddsFunc+"(false, '"+handlerIdObj+"','"+handlerNameObj+"', ';', "+selectType+", function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, null, null, null, null, null, null, '" + nodeObj.id + "','"+defaultOptionBean+"');}\"";
				optHrefObj += " onclick=\"{Dialog_AddressList(false, '"+handlerIdObj+"','"+handlerNameObj+"', ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
				hrefObj_formula += " onclick=\"{"+formulaDialogFunc+"(null, null, '"+modelName+"', '"+nodeObj.id+"',true)}\"";
			} else {
				var selectType = (lbpm.globals.checkNodeType(lbpm.constant.NODETYPE_SEND,nodeObj)) ? 'ORG_TYPE_ALL | ORG_TYPE_ROLE' : 'ORG_TYPE_POSTORPERSON | ORG_TYPE_ROLE';
				...
				optHrefObj += " onclick=\"{Dialog_AddressList(true, '"+handlerIdObj+"','"+handlerNameObj+"', ';', '" + defaultOptionBean + "',function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, '"+searchBean+"', null, null, lbpm.workitem.constant.COMMONSELECTALTERNATIVE);}\"";
...
function resetContent(nodeObj, afterChangeFunc, handlerIdObj, handlerNameObj){
	...
	if(optHandlerSelectType == 'dept' || optHandlerSelectType == 'mechanism'){
		...
		if(nodeObj.nodeDescType=="shareReviewNodeDesc"){
			// 微审批节点处理人选择控制
			return " onclick=\"{Dialog_Address(false,'"+handlerIdObj+"','"+handlerNameObj+"', ';', "+selectType+",function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, null, null, true, null, null, ["+exceptValue+"], '"+deptLimit+"');}\"";
		} else {
			return " onclick=\"{Dialog_Address(true,'"+handlerIdObj+"','"+handlerNameObj+"', ';', "+selectType+",function(rtv){"+afterChangeFunc+"(rtv,'"+lbpm.constant.ADDRESS_SELECT_ORG+"','"+nodeObj.id+"')}, null, null, true, null, null, ["+exceptValue+"], '"+deptLimit+"');}\"";
		}
	}
	return null;
}
...
/*  <div id="divselect">
	...
	var html = "<span id='choseFromSysOrgForHandler' style='"+(defaultHide?"display:none":"")+"' index='"+hrefIndex+"' class='divselect'><span class='cite' title='" + displayText + "'><a>"+displayText+"</a></span><ul>";
