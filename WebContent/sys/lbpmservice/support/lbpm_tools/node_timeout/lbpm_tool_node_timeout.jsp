...
<table class="tb_normal lbpmToolsTable" width="90%">
	...
	<tr>
		...
		<td width="80%">
			...
			<a href="javascript:void(0);" onclick="lbpmToolsSelectSubFlow('nodeTimeoutTemplateId','nodeTimeoutTemplateName');" style="color:#666"><kmss:message key="FlowChartObject.Lang.Node.select" bundle="sys-lbpmservice" /></a>
			</br>
		</td>
	</tr>
	<tr>
...
seajs.use(['lui/jquery', 'lui/dialog'], function($, dialog) {
	window.searchNodeTimeout = function() {
		...
		if(nodeTimeoutType=="8"||nodeTimeoutType=="9"){
			myIframe.src='${LUI_ContextPath}/sys/lbpmservice/support/lbpm_tools/node_timeout/lbpm_tool_node_timeout2_list.jsp?range='+$("input[name='nodeTimeoutTemplateId']").val()+'&nodeName='+encodeURIComponent($("input[name='nodeTimeoutNodeName']").val())+'&nodeTimeoutType='+nodeTimeoutType+'&dayOfUpdate='+dayOfUpdate+'&hourOfUpdate='+hourOfUpdate+'&minuteOfUpdate='+minuteOfUpdate+'&more=2';
		}else{
			var url='${LUI_ContextPath}/sys/lbpmservice/support/lbpm_tools/node_timeout/lbpm_tool_node_timeout_list.jsp?range='+$("input[name='nodeTimeoutTemplateId']").val()+'&nodeName='+encodeURIComponent($("input[name='nodeTimeoutNodeName']").val())+'&nodeTimeoutType='+nodeTimeoutType+'&dayOfUpdate='+dayOfUpdate+'&hourOfUpdate='+hourOfUpdate+'&minuteOfUpdate='+minuteOfUpdate;
