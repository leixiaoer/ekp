...
<list:data>
	...
	<list:data-columns var="lbpmNodeDefinition" list="${queryPage.list }">
		...
		<!--指纹审批-->
		<list:data-column col="isCanFingerPrint" title="${ lfn:message('sys-lbpmservice-support:lbpmTools.fingerPrint')}" escape="false">
			<div class="conf_show_more_w">
				<div class="conf_btn_edit">
					<c:choose>
						<c:when test="${isCanFingerPrintMap[lbpmNodeDefinition.fdId]=='true'}">
							<a class="btn_txt" href="javascript:update('${lbpmNodeDefinition.fdId}')">禁用</a>
						</c:when>
						<c:when test="${isCanFingerPrintMap[lbpmNodeDefinition.fdId]=='false'||isCanFingerPrintMap[lbpmNodeDefinition.fdId]==null}">	
							<a class="btn_txt" href="javascript:update('${lbpmNodeDefinition.fdId}')">启用</a>
						</c:when>
					</c:choose>
				</div>
			</div>
