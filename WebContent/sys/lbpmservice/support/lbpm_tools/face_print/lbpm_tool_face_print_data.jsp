...
<list:data>
	...
	<list:data-columns var="lbpmNodeDefinition" list="${queryPage.list }">
		...
		<!--人脸审批-->
		<list:data-column col="isCanFacePrint" title="${ lfn:message('sys-lbpmservice-support:lbpmTools.facePrint')}" escape="false">
			<div class="conf_show_more_w">
				<div class="conf_btn_edit">
					<c:choose>
						<c:when test="${isCanFacePrintMap[lbpmNodeDefinition.fdId]=='true'}">
							<a class="btn_txt" href="javascript:update('${lbpmNodeDefinition.fdId}')">禁用</a>
						</c:when>
						<c:when test="${isCanFacePrintMap[lbpmNodeDefinition.fdId]=='false'||isCanFacePrintMap[lbpmNodeDefinition.fdId]==null}">	
