...
define([
	...
	return declare("sys.lbpmservice.mobile.workitem.FutureNodesChekBoxGroup", [_CheckBoxGroup], {
		...
		tmpl : '<input type="checkbox" id="futureNodeId[!{index}]" data-dojo-type="sys/lbpmservice/mobile/workitem/FutureNodesChekBox"'
		...
		startup: function() {
			...
			this.store = array.map(routeLines, function(lineObj, i) {
				...
				return {
					...
					checked: _self.value == nodeObj.id || (!_self.value && _self.checkedNodeIds && _self.checkedNodeIds.indexOf(nodeObj.id) >= 0), 
					value: nodeObj.id,
