...
define([
  ...
  "dojo/query",
  "dojo/dom-construct"
], function(util, request, registry, msg1, msg2,query,domConstruct) {
  function jsLoad(params, load) {
    ...
    function loadAssignInfo() {
    	/*query("#operationsRow_Scope")
        .closest("div.actionCenter")
        .show()*/
      if (query("#assignOprRow").length == 0) {
        // 构建分发操作信息行
        var oprRowHtml = '<tr><td colspan="2" id="assignOprRow">'
        oprRowHtml +=
          '<div class="titleNode operation_title" id="assignOprTitle" lbpmdetail="operation">' +
          msg2['lbpmAssign.fdOperType.assign'] +
          "</div>"
        oprRowHtml +=
          '<div class="detailNode" id="assignOprContent" lbpmdetail="operation">'
        oprRowHtml += "</div></td></tr>";
        domConstruct.place(oprRowHtml,query("#operationsRow_ScopeTr")[0],'after');
        //query("#operationsRow_Scope").parents("tr").eq(0).after(oprRowHtml)
