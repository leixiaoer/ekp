...
define(function(){
	...
	**********************************************************/
	...
	workFlow.WorkFlow_BuildXMLString = window.WorkFlow_BuildXMLString = function(data, nodeName, includeNullStr){
		if(nodeName==null) {
			if(data["XMLNODENAME"]) {
		...
		for(var p in data){
			...
			switch(typeof(value)){
				...
				break;
				case "object":
					childStr += "\r\n" + window.WorkFlow_BuildXMLString(value, p, includeNullStr);
					continue;
			}
			rtnStr += " "+ p + "=\"" + value + "\"";
		}
	...
	workFlow.WorkFlow_LoadXMLData = window.WorkFlow_LoadXMLData = function(xml, isArray,nodeNames){
		var xmlObj = xml;
		...
		for(var node=xmlObj.firstChild; node!=null; node=node.nextSibling){
			if(node.nodeType==1){
				if(isArray)
					rtnVal[rtnVal.length] = window.WorkFlow_LoadXMLData(node, false,nodeNames);
				else
					rtnVal[node.nodeName] = window.WorkFlow_LoadXMLData(node, true,nodeNames);
			}
		}
		return rtnVal;
	};

	workFlow.WorkFlow_SetInnerText = window.WorkFlow_SetInnerText = function(obj, text){
	...
	workFlow.WorkFlow_PutDataToField = window.WorkFlow_PutDataToField = function(data, fieldFilter){
		for(var o in data){
			var fieldName = o;
			if(fieldFilter!=null)
			...
			switch(fields[0].tagName){
				...
				break;
				case "SPAN":
					window.WorkFlow_SetInnerText(fields[0],value);
				break;
			}
		}
	}
	...
	workFlow.WorkFlow_GetDataFromField = window.WorkFlow_GetDataFromField = function(data, propertyFilter, obj){
		if(obj==null)
			obj = document;
		var fields = obj.getElementsByTagName("INPUT");
		var getFieldProperty = function(field){
	...
	//		tdId：通知方式字段所在的TD的id
	//		value：通知方式的值
	workFlow.WorkFlow_RefreshNotifyType = window.WorkFlow_RefreshNotifyType = function(tdId, value){
	...
	//参数：
	//		xml：状态信息XML
	workFlow.WorkFlow_GetStatusObjectByXML = window.WorkFlow_GetStatusObjectByXML = function(xml){
		var unknownNode = [];
		var statusData = window.WorkFlow_LoadXMLData(xml);
		...
		for(i=0; i<statusData.historyNodes.length; i++){
			...
			switch(nodeInfo.routeType){
				case "BACK":
					routePath = window.WorkFlow_GetStatusInfoByModelId(statusData, nodeInfo.targetId).routePath;
				break;
				case "RESUME":
					for(var j=i-1; j>=0; j--){
			...
			for(var j=0; j<targetIds.length; j++){
				var statusInfo = window.WorkFlow_GetStatusInfoById(statusData, targetIds[j], i);
				if(statusInfo!=null) {
					//statusInfo.routePath = routePath;
					window.WorkFlow_MergeStatusRoutePath(statusInfo, routePath);
				} else {
					...
					for (j = 0; j < unknownNode.length; j++) {
						if (hNode.id == unknownNode[j].nodeId) {
							...
							for(var k=0; k<targetIds.length; k++){
								var statusInfo = window.WorkFlow_GetStatusInfoById(statusData, targetIds[k], statusData.historyNodes.length);
								if (statusInfo != null) {
	...
	workFlow.WorkFlow_MergeStatusRoutePath = window.WorkFlow_MergeStatusRoutePath = function(statusInfo, routePath) {
	...
	workFlow.WorkFlow_GetStatusInfoByModelId = window.WorkFlow_GetStatusInfoByModelId = function(statusData, modelId){
	...
	workFlow.WorkFlow_GetStatusInfoById = window.WorkFlow_GetStatusInfoById = function(statusData, id, index){
	...
	workFlow.WorkFlow_GetCurrUserLang = window.WorkFlow_GetCurrUserLang = function(){
	...
	workFlow.WorkFlow_getLangLabel = window.WorkFlow_getLangLabel = function(defLabel,langs,key,lang){
		if(typeof langs =="undefined" || (typeof _isLangSuportEnabled !="undefined" && !_isLangSuportEnabled)){
			return defLabel;
		}
		if(typeof lang =="undefined"){
			lang = window.WorkFlow_GetCurrUserLang();
		}
		var langArr=[];
