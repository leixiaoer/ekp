				...
				$(".lui-lbpm-foldOrUnfold-box div").click(function(e){
					 ...
					 if(!e.isPropagationStopped()){//确定stopPropagation是否被调用过
						 if($('.lui-lbpm-foldOrUnfold-box i').hasClass('lui-lbpm-outerIconStyle')){
						       $('.lui-lbpm-foldOrUnfold-box span').html(Data_GetResourceString('sys-lbpmservice:lbpmRight.fold'));
						       $('.lui-lbpm-foldOrUnfold-box i').removeClass('lui-lbpm-outerIconStyle');
						    }else{
					    	   $('.lui-lbpm-foldOrUnfold-box span').html(Data_GetResourceString('sys-lbpmservice:lbpmRight.Unfold'));
...
lbpm.globals.setHIddenByRoletype = function(){
	...
	if(lbpm.constant.ROLETYPE!='' && lbpm.constant.ROLETYPE!=lbpm.constant.PROCESSORROLETYPE){
		...
		if ((isNotifyCurrentHandler === "false" && lbpm.constant.ROLETYPE === "authority") || lbpm.constant.ROLETYPE === lbpm.constant.HISTORYHANDLERROLETYPE){
			$("#notifyLevelRow").hide();
			$("#notifyTypeRow").hide();
		}
...
lbpm.globals.loadRightInfo = function(){
	...
	if(handlerTypeRow.length > 0){
		var options = [];
		if(lbpm.drafterInfoObj && lbpm.drafterInfoObj.length>0){
			options.push({'text':Data_GetResourceString("sys-lbpmservice:lbpmProcess.handlerType.drafter"),value:lbpm.constant.DRAFTERROLETYPE});
		}
		if(lbpm.authorityInfoObj && lbpm.authorityInfoObj.length>0 && lbpm.constant.DOCSTATUS!='10'){
			options.push({'text':Data_GetResourceString("sys-lbpmservice:lbpmProcess.handlerType.authority"),value:lbpm.constant.AUTHORITYROLETYPE});
		}
		if(lbpm.historyhandlerInfoObj && lbpm.historyhandlerInfoObj.length>0 && lbpm.constant.DOCSTATUS!='10'){
			options.push({'text':Data_GetResourceString("sys-lbpmservice:lbpmProcess.handlerType.hisHandler"),value:lbpm.constant.HISTORYHANDLERROLETYPE});
		}
		if(lbpm.allMyProcessorInfoObj && lbpm.allMyProcessorInfoObj.length>0 && lbpm.constant.DOCSTATUS!='10'){
			options.push({'text':Data_GetResourceString("sys-lbpmservice:lbpmProcess.handlerType.curHandler"),value:lbpm.constant.PROCESSORROLETYPE});
		}
		if(options.length>0){
		...
		$('.lui_lbpm_handlerType>span').click(function() {
			...
			if(value == 'authority' || value == 'historyhandler' || value == 'drafter'){
				//起草、特权和已处理人操作不进行@
...
lbpm.globals.validateControlItem=function(){
	...
	if (lbpm.isFreeFlow) {
		...
		if(lbpm.constant.ISINIT){
			...
			$(".lbpm_freeflow_modifyFlowContent").addClass("lbpm_freeflow_defaultTemp");
		}
	}
	...
	//流程说明行-开关控制显示
 	if(Lbpm_SettingInfo.isShowFlowDescription == "false"){
 		var fdFlowDescription =  $("#fdFlowDescription");
 	...
 	if (Lbpm_SettingInfo.isShowNotifyType == "false"){
 		var notifyDrafter = $("#dayOfNotifyDrafter").closest("tr")[0];
 		lbpm.globals.hiddenObject(notifyDrafter, true);
			...
			if (!tab.isShow) {
				...
				$.each(panel.contents, function(i) {
					if (this == tab) {
						...
						} else if (panel.setSelectedIndex){
