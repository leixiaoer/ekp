...
lbpm.globals.newAuditSimpleTagFlowSubmitEvent = function(src){
	//检验是否选择了操作项
						...
						if ("handler_pass" == operation.id||'handler_sign'== operation.id) {
							...
							if(isSuperRefuse){
								if($(obj).attr('rejectType')=='1'){
									...
									$("#refuse_"+tagId).html(Com_HtmlEscape(superRefuseName)+ Data_GetResourceString("sys-lbpmservice:lbpm.nodeType.draftNode"));
								}
							}else{
								if($(obj).attr('rejectType')=='1'){
									...
									$("#refuse_"+tagId).html(Com_HtmlEscape(operation.name)+Data_GetResourceString("sys-lbpmservice:lbpm.nodeType.draftNode"));
								}
							}
