 ...
 * 根据 节点数组 获取节点id 串
 */
lbpm.globals.getIdsByNodes=function(nodes){
	var throughNodeIds = [];
...
lbpm.globals.objContain=function(o, arr){
	if (arr.length == 0) {
		return false;
	}
 ...
 */
 lbpm.globals.getThroughNodes=function(callbackFun,beforeRequest,completeRequest,async,startNodeId){
	...
	//alert("变了");
	lbpm.globals.getFilterInfo(changeFlag,bundleFilesInfo,callbackFun,beforeRequest,completeRequest,async,startNodeId);
};
//检查值自动决策缓存值是否发送变化 TODO:暂未使用
lbpm.globals.CheckBindFieldValueChange=function(){
	//获取自动决策节点关联的域信息
	var bundleFilesInfo = lbpm.globals.getBundleFilesInfo();
...
function _getPassedNodeIds(targetNode, passedNodeIds) {
	if(targetNode && targetNode.targetId) {
		...
		if(targetNode.routeType == "RESUME") {
			// 驳回返回本节点
			var nextNodes = lbpm.globals.getNextNodeObjs(targetNode.id);
			for (var i = 0; i < nextNodes.length; i++) {
				if(lbpm.globals.objContain(nextNodes[i].id, passedNodeIds)){
					continue;
				}
				passedNodeIds.push(nextNodes[i].id);
				_getPassedNodeIds(nextNodes[i], passedNodeIds);
			}
		} 
	}
};
...
/**
 * 根据当前节点计算已经流经的历史节点（排除曾经经过）
 */
function _getPassedHistoryNodes(runningNodes) {
	...
	$.each(lbpm.nodes, function(index, nodeData) {
		if(nodeData.Status==lbpm.constant.STATUS_PASSED && !lbpm.globals.objContain(nodeData.id, passedNodeIds)){
 ...
 * 节点过滤，节点计算函数
 */
lbpm.globals.getFilterInfo=function(changeFlag,bundleFilesInfo,callbackFun,beforeRequest,completeRequest,async,startNodeId){
	 async = (async != null) ? async : true;
	...
	var targetNodesMapJSON = _getPassedTargetNodesMapJSON(availableHistoryNodes, runningNodes);
	...
	if(changeFlag!=1){//add by wubing 如果为条件分支时，且条件值有变化时应该需要重新计算 date 2016-10-12
	...
	paramArray.push("processId="+encodeURIComponent(lbpm.globals.getWfBusinessFormModelId()));
	paramArray.push("modelName="+encodeURIComponent(lbpm.globals.getWfBusinessFormModelName()));
	...
	//流程集成组件参数传递
	//var _thirdSysFormList = [{"name":"project_amount","label":"项目金额","type":"Number"}];
	if(typeof _thirdSysFormList !="undefined"){
		paramArray.push("_thirdSysFormList="+encodeURIComponent(JSON.stringify(_thirdSysFormList)));
	}
	// 节点过滤的标识
	...
	$.ajax({
		...
		data:params,
		dataType: "json",
		url:Com_Parameter.ContextPath+"sys/lbpm/engine/jsonp.jsp?s_bean=lbpmRouteDecisionRuleDataBean",
 ...
 * 同后台获取域的逻辑一直
 */
lbpm.globals.getFiledsIdByLines=function(allLines){
	var filedsId=[];
 ...
 */
lbpm.globals.getBundleFilesInfo=function(){
	//所有自动决策节点
	...
	for(var i=0;i<autoFiledIds.length;i++){
		var canApplyAutoFieldId = autoFiledIds[i];
        if (autoFiledIds[i].indexOf(".") > 0) {
            var autoFieldIdSplit = autoFiledIds[i].split(".");
            if (autoFieldIdSplit.length > 1 && $("#TABLE_DL_" + autoFieldIdSplit[0]).length > 0) {
                canApplyAutoFieldId = autoFieldIdSplit[1];
            }
        }
        var filedsInfo = GetXFormFieldById(canApplyAutoFieldId, true);
		...
		if(filedsInfo&&filedsInfo.length>1)
		{
			...
			for(var j=0;j<filedsInfo.length;j++)
			{
				...
				if(typeof $form !="undefined"
					&& typeof $form(canApplyAutoFieldId) !="undefined"
					&& $form(canApplyAutoFieldId).target
					&& $form(canApplyAutoFieldId).target.type
					&& $form(canApplyAutoFieldId).target.type ==="fSelect"){
					var matchFieldName = filedsInfo[j].name;
					if (/\.(\d+)\./ig.test(matchFieldName)) {
						matchFieldName = matchFieldName.replace(/\.(\d+)\./ig, ".");
					}
					if(matchFieldName == autoFiledIds[i] || matchFieldName == ("extendDataFormInfo.value("+autoFiledIds[i]+")")) {
						// 排除标签的特殊处理
						var temp=filedsInfo[j];
						filedsInfo=[];
						filedsInfo.push(temp);
		...
		for(var j=0;j<filedsInfo.length;j++){
			var matchFieldName = filedsInfo[j].name;
			if (filedsInfo[j].type == 'radio' || filedsInfo[j].type == 'checkbox' || filedsInfo[j].type == 'select' || filedsInfo[j].type == 'select-one' || filedsInfo[j].type == 'hidden') {
				if (/\.(\d+)\./ig.test(matchFieldName)) {
                    matchFieldName = matchFieldName.replace(/\.(\d+)\./ig, ".");
                }
				if(matchFieldName == "_"+autoFiledIds[i] || matchFieldName == ("_extendDataFormInfo.value("+autoFiledIds[i]+")")) {
					// 排除标签的特殊处理
					continue;
				}
			}
			// 动态表单控件取实际值时要过滤掉无关值
			if (matchFieldName && matchFieldName.indexOf("extendDataFormInfo.value("+autoFiledIds[i]+"_") == 0) {
				continue;
			}
			...
			//处理合同管理模块合同金额无法过滤节点
			var validateAttr=filedsInfo[j].getAttribute("validate");
			if(validateAttr!=undefined && validateAttr!="" && validateAttr.search(/currency-dollar/)>-1){
				var applyFdAccount=filedsInfo[j].value;
				valueInfo.push(applyFdAccount.replace(/,/g, ''));
			}
			else{
...
lbpm.globals.getAllThroughNodes=function(id,autoBranchTargets,parentNode, _throughNodesTemp){
	...
	if(lbpm.globals.isDecisionNode(currentNode)){
		...
		if(his) {
			//获取并行分支自定义启动的流向
			if(IsSplitNodeAndCustom(currentNode)&&lbpm.globals.isNodeRunning()){
...
/**
 * 验证节点是否是并行分直结点并设置了自定义启动属性
 */
function IsSplitNodeAndCustom(node){
	var result=false;
	if(node.nodeDescType=="splitNodeDesc"&&node.splitType&&node.splitType=="custom"){
		result=true;
	}
	return result;
}
 ...
 * 获取目标节点流向的节点id值
 */
lbpm.globals.getFutureNodeId = function() {
	var arr = new Array();
	//如果有分支设置分支
...
lbpm.globals.isSameEndLines = function(lines1, lines2) {
	if(lines1 && lines2) {
...
lbpm.globals.isNodeRunning = function() {
	var running = false;
	$.each(lbpm.nodes, function(index, nodeData) {
...
lbpm.globals.clearIndicatorDiv=function(obj,win){
	win = win|| window;
	win.document.body.removeChild(obj.newMaskDiv);
	win.document.body.removeChild(obj.infoDiv);
};

lbpm.globals.openIndicatorDiv=function(win,_id) {
	  win = win|| this;
	  var m = "mask_div";  
...
lbpm.globals.throughNodesSorter = function(node1, node2) {
