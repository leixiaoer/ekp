...
// 打开起草人或特权人处理流程的小窗口, roleType的值为drafter、authority或者historyhandler，operationType:默认选中的操作  
...
lbpm.globals.openExtendRoleOptWindow = function(roleType, operationType, selectedTaskId, isShowHisHandlerOpt) {
	...
	var url = Com_Parameter.ContextPath
			+ "sys/lbpmservice/include/sysLbpmProcess_panel_frame.jsp?modelId="
			+ lbpm.globals.getWfBusinessFormModelId() + "&roleType=" + roleType
...
lbpm.globals.controlExtendRoleOptRow = function() {
	...
	if (window.LUI) {
		...
		$(document).ready(initExtendRoleOptRow);
	}
	/*
	 * if(extendRoleOptRow != null){ if(index == 0){
	 * lbpm.globals.hiddenObject(extendRoleOptRow, true); }else{
