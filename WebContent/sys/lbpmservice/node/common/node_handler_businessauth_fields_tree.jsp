...
<script type="text/javascript">
	...
	function generateTree() {
		LKSTree = new TreeView("LKSTree", "<bean:message bundle='sys-lbpmservice-support' key='lbpmext.businessAuth.fieldType'/>", document.getElementById("treeDiv"));
	...
	function getVars(){
		...
		var fieldType = top.dialogObject.Parameters.fieldType;
		var valueData = dialogObject.valueData.GetHashMapArray()[0];
		for(var i=0; i < varInfo.length; i++){
			var dataType = varInfo[i].type;
			if(dataType=="BigDecimal_Money"){
				dataType = "BigDecimal";
			}
			if(dataType=="BigDecimal"||dataType=="Double"){
