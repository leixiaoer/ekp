...
(function(FlowChartObject) {
	...
	FlowChartObject.Nodes.InitializeTypes.push(function() {
		var nodeType = new FlowChartObject.Nodes.NodeType("freeSubFlowNode");
		nodeType.ImgUrl = '../images/buttonbar_freeSubFlowNode.png';
		nodeType.isGroupNode = true;
		nodeType.ButtonIndex = 15;
		nodeType.CreateNode = function() {
			var gNode = new FlowChart_Node("freeSubFlowNode");
			gNode.MoveTo(EVENT_X, EVENT_Y, true);
			FlowChart_NodeType_AutoLink(gNode);
			// 创建freeSubFlowNode的同时，把groupStartNode和groupEndNode以及连线一并创建（组内的子都挪到可见区域外）
		...
		nodeType.Initialize = function() { // 节点初始化代码
			...
			this.Check = function() {
				...
				if (this.Data.initSubNodeHandlerIds == null || this.Data.initSubNodeHandlerIds == "") {
					...
					if ((!desc.isHandler && !desc.isDraftNode) || desc.isSendNode || preNode.Type == "voteNode") {
						alert(FlowChartObject.Lang.GetMessage(FlowChartObject.Lang.checkFreeSubFlowNodeLineIn, this.Data.id + "." +this.Data.name));
						return false;
					}
				}
				return true;
