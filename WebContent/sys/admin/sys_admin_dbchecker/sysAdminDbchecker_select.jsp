			...
			<c:if test="${not empty sqlserver}">
				<tr>
					<td class="rd_title primary-title">
						<label>
							<html:radio property="fdCheckType" value="3" onclick="chooesType(this.value);" /><bean:message key="sysAdminDbchecker.fdCheckType.3" bundle="sys-admin" />
						</label>
					</td>
				</tr>
			</c:if>
		...
		<script type="text/javascript">
			...
			seajs.use([ 'lui/jquery', 'lui/dialog' ], function($, dialog) {
				window.check = function() {
					...
					if(flag) {
						dialog.confirm('<bean:message bundle="sys-admin" key="sysAdminDbchecker.startCheck.comfirm"/>', function(value){
							if(value == true) {
								Com_Submit(document.sysAdminDbcheckerForm, 'check');
								// 开启进度条
						...
						dialog.alert('<bean:message bundle="sys-admin" key="sysAdminDbchecker.noSelect"/>');
						return false;
					}
				}
				...
				window._progress = function () {
					...
					data.UseCache = false;
					data.AddBeanData("sysAdminDbCheckerXMLDataBean");
