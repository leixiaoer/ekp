...
<table class="tb_normal" width=98% id="table_result">
	...
	<c:set var="isNormalCheck" value="${empty dbChecker_checkType}"/>
	...
	<c:choose>
		...
		<td align="left">
		<c:if test="${empty dbChecker_checkType and !(checkResultDetailMap.key.type eq 'INFO') and !(checkResultDetailMap.key.type eq 'SUGGEST')}">
			<ui:toolbar>
		...
		<td align="left">
		<c:if test="${empty dbChecker_checkType and !(checkResultDetailMap.key.type eq 'INFO') }">
	...
	<tr module="tr_${checkResultDetailMap.key.type}" item="tr_${checkResultDetailMap.key.type}_${checkResultModuleMap.key}">
		...
		<br>
			<c:choose>
				<c:when test="${not empty checkResult.resultRepair.fdSuggest.fdName}">
				<c:choose>
					<c:when test="${not empty dbChecker_checkType and dbChecker_checkType eq '3' and not empty checkResult.resultRepair.fdSuggest.fdDescription}">
