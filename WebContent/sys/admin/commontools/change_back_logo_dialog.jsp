...
<template:include ref="default.simple">
	...
	<template:replace name="head">
		...
		<script>
			...
			seajs.use(['lui/jquery', 'lui/dialog','lui/util/str'], function($,dialog,str) {
				...
				window.commitMethod = function(title){
					$.ajax({
						url: '${ LUI_ContextPath }/sys/ui/sys_ui_logo/sysUiLogo.do?method=saveProfileLogo',
						type: 'POST',
						data:{logoTitle:title},
						async : false,
						success: function(data){
							var rtn = str.toJSON(data);
							if(rtn.result == 'success'){
								dialog.success('<bean:message key="return.optSuccess" />');
							}else{
								dialog.failure('<bean:message key="ui.portlet.select.error" bundle="sys-ui" />');
							}
						}
				   });
			 	}
		...
		<div style="height: 100%;width:100%; overflow: hidden;">
			...
			<html:form enctype="multipart/form-data" style="padding:5px;" action="/sys/ui/sys_ui_logo/sysUiLogo.do" method="post">  
				<input type="file" style="width: 350px;" name="file" />
		 		<input type="submit" value="<bean:message key='common.fileUpLoad.upload'/>" style="width: 115px;" />
		 		<input type="hidden" name="method" value="uploadProfileLogo" />
			</html:form>
		</div>
	</template:replace>
</template:include>