...
define(function(require, exports, module) {
	...
	var ResAbstractCalenderMode = Calendar.AbstractCalenderMode.extend({
		...
		_render:function(start ,end ,datas,renderType){
			...
			var main = datas.main;
			if(main != null){
				this.leftRightContent =  $('<table />').addClass('res_calendar_rescontent_content').appendTo(this.resContent); 
				for(var id in main){
					//绘制资源内容行
					this.leftRightContent.append(this.renderResRow(main[id]));
					//绘制日程行
		...
		resetContent : function(){
			var resrow = this.resContent.children('.res_calendar_rescontent_content'); 
			if(resrow){
				resrow.remove();
			}
		...
		renderCate : function(categorys, categoryName){
			var self = this;
			var node = $('<table />')
						.addClass('res_calendar_rescontent_header');
			
			var nodeTr= $('<tr />').addClass('res_calendar_rescontent_header_row');
			var nodeTd = $('<td />').addClass('res_calendar_rescontent_header_row_column').append($('<div/>').addClass('res_calendar_cate_title').append(categoryName));
			nodeTd.appendTo(nodeTr); 
			nodeTr.appendTo(node); 
			//分类选择框	
			var	dialog = $('<div/>')
		...
		renderResRow : function(data){
			...
			//每一列的值
			var column  = $('<td/>').addClass('res_calendar_rescontent_row_column');
			column.appendTo(row); 
			var divObj = $('<div/>').addClass('res_calendar_rescontent_row_column_text').html(data.name);
			divObj.appendTo(column);
		...
		afterRender : function(){
			...
			rows.each(function(){
				...
				// 设置行高
				t.__setRowHeight(this, cateRows, Maxtop);
		...
		// 设置行高
		__setRowHeight: function(row, cateRows, Maxtop) {
			var __rowHeight = $(row).height();
			if(__rowHeight > 0) {
				//最大设定高度重新
				var rowIndex = $(row).index(),
					rowHeight = ($(row).height() > Maxtop ? $(row).height() : Maxtop) + 15,
					cate = cateRows.eq(rowIndex);
			
				var divHeight = $(cate).children().children("div").height(); 
				rowHeight = Math.floor(rowHeight > divHeight ? rowHeight : divHeight);
				
				//标题表格的高度
				$(cate).children().height(rowHeight); 
				//日历表格的高度
				$(row).children().height(rowHeight);
			} else {
				var self = this;
				setTimeout(function() {
					self.__setRowHeight(row, cateRows, Maxtop);
				}, 100);
			}
		}
	});
	...
	var ResDayCalendarMode = ResWeekCalendarMode.extend({
		...
		renderTableRow : function(start, end, data){
			...
			for(var i = 0; i <= 22;i=i+2){
				var cellStartDate = $.fullCalendar.cloneDate(start);
				cellStartDate.setHours(i);
				cellStartDate.setMinutes(0);
				var cellEndDate = $.fullCalendar.cloneDate(cellStartDate);
				cellEndDate.setHours(i+2);
			...
			if(!renderType){
				...
				var dayLeft = $('<div/>').addClass('res_calendar_day_left').appendTo(rightContent).click(function(){
					...
					if(left < 0){
						...
						$(".lui_calendar_fixed_box").find('.res_calendar_allRightContent').css("left", left + gridwith + "px");
		...
		getDataLength : function(startDate, endDate, isAllDay){
			...
			if(endDate.getDate() == startDate.getDate()){
				...
				if(tstart.getDate() == startDate.getDate()){
					...
					days =  Math.ceil(interval / 2);
					if(endDate.getHours() % 2 == 0 && endDate.getMinutes() > 0){//偶数的情况
						days = days + 1;
					}
