...
define(function(require, exports, module) {
	...
	var treeviewComponent = require('lui/address/component/treeviewComponent');
		...
		afterLayout:function(){
			...
			if(this.centerDom.length > 0){
				...
				this.clearRecentDom.on('click',function(){
					var delUrl = Com_Parameter.ContextPath + 'sys/organization/sys_organization_recent_contact/sysOrganizationRecentContact.do?method=delContacts';
				...
				kmssData.SendToBean('orgDialogRecentContactList', function(rtnData){
					topic.channel(self.refName).publish( constant.event['ADDRESS_DATASOURCE_CHANGED'],{
						data : rtnData,
						sourceComponent : self //发出数据改变事件的对象
					});
			...
			if(this.navDom.length > 0){
				...
				this.navComponent = new navComponent({
					...
					searchfunc : function(value,context){
						...
						kmssdata.Format("id:name:info", null, null, function(result) {
							self.getDataByIds(result.data,function(rtnData){
								topic.channel(self.refName).publish( constant.event['ADDRESS_DATASOURCE_CHANGED'],{
									data : rtnData,
									sourceComponent : self //发出数据改变事件的对象
								});
