				...
				if(typeof(tempUrl)=="string"){
					...
					if(tempUrl.init.length>0){
						...
						$.ajax({
							url : ajaxUrl,
							type : 'post',
							data : {fdId:tempUrl.init,rowsize:init.length},
							async : true,           
							error : function(){
							} ,   
			...
			function _getSelectedData(){
				...
				if(rowContents.length == 0){
					seajs.use(['lui/dialog'],function(dialog){
						var _key = '${JsParam._key}';
						if(_key=='dialog_listManagerIds'){
							dialog.alert("${lfn:message('sys-ui:ui.dialog.template.message.dialog_listManagerIds')}");
						}else if(_key=='dialog_fdServerId'){
							dialog.alert("${lfn:message('sys-ui:ui.dialog.template.message.dialog_fdServerId')}");
						}else{
				...
				if(rowContents.length > 200) {
					seajs.use(['lui/dialog'],function(dialog){
						dialog.alert("已选数量不能大于200，请重新选择");
					});
					return null;
				}
