 function initUpComingProcess(url){
	 //待办最多的流程
	 ...
	 upcomingProcessItem.setOption({
	 	color: ['#3398DB'],
	 	title: {
	 		text: Data_GetResourceString("sys-lbpmmonitor:sysLbpmMonitor.analysis.UpcomingMaxProcess"),
	 		x: 'left',
	 		textStyle:{
	 			color:'#333',
	 			fontSize:16
	    ...
	    grid: {
	 	...
	 	series: [{
	 		 name:Data_GetResourceString("sys-lbpmmonitor:sysLbpmMonitor.analysis.UpcomingMaxProcessCount"),
	         type:'bar',
	         barWidth : 20,
	         data:[]
	 	}]
	 ...
	 var param = {
	    ...
	    $.post(url, param, function(data,status){
	 	...
	 	for(var i=0;i<datamap.length;i++){
	 		if(i==0){
	 			$("#upcoming_max_process").html(datamap[i].fdName);
	 		}
	 				...
	 				dataView: {readOnly: true,
	 					optionToContent: function(opt) {
	 						...
	 						var table = '<table class="lui_data_form_two" style="width:100%;text-align:center;background-color:#FFFFFF" border><tbody><tr>'
	 									 + '<td>'+Data_GetResourceString("sys-lbpmmonitor:sysLbpmMonitor.analysis.UpcomingMaxTemplate")+'</td>'
	 									 + '<td>' + series[0].name + '</td>'
	 									 + '</tr>';
