 function initPeopleTimeOutTurnDoProcess(url){
	//待办最多的流程
	 ...
	 peopleTimeOutTurnDoItem.setOption({
	 	color: ['#FFD37D'],
	 	title: {
	 		text: Data_GetResourceString("sys-lbpmmonitor:sysLbpmMonitor.analysis.peopleTimeoutTodoMuchRanking"),
	 		x: 'left',
	 		textStyle:{
	 			color:'#333',
	 			fontSize:16
	 	...
	 	series: [{
	 		name:Data_GetResourceString("sys-lbpmmonitor:sysLbpmMonitor.analysis.peopleTimeoutTodoRankingMuch"),
	         type:'bar',
	         barWidth: 20,
	         data:[]
	 	}]
	 ...
	 var param = {
	 				...
	 				dataView: {readOnly: true,
	 					optionToContent: function(opt) {
	 						...
	 						var table = '<table class="lui_data_form_two" style="width:100%;text-align:center;background-color:#FFFFFF" border><tbody><tr>'
	 									 + '<td>'+Data_GetResourceString("sys-lbpmmonitor:sysLbpmMonitor.analysis.peopleTimeoutTodoMuchRanking")+'</td>'
	 									 + '<td>' + series[0].name + '</td>'
	 									 + '</tr>';
	    ...
	    return peopleTimeOutTurnDoItem;
 }