...
function addLiRow(oneData, no){
	if(no == 0){
		var rowDiv = $('<li/>').attr('class','first_order').appendTo(contentDiv);
	}else if(no==1){
	...
	//序号
	var noSpan = $('<span/>').attr('class','order').appendTo(rowDiv);
	noSpan.text(no+1);
	
	//文本区域
	var textArea = $("<p/>").attr('class','title textEllipsis')
			.appendTo(rowDiv);
	var linkBox, text;
	//标题
	...
	//最新的时候显示发布日期
	if(oneData['type'] == 'docPublishTime'){
		//文本区域
		var viewcount = $("<p/>").attr('class','view-count')
				.appendTo(rowDiv);
		viewcount.text(oneData['created']);
	}else{
		//文本区域
		var viewcount = $("<p/>").attr('class','view-count')
				.appendTo(rowDiv);
		viewcount.text("浏览量："+oneData['docReadCount']);
	}
