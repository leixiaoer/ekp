...
var contentPanel = $("<div/>").appendTo(dataView.element);
if(stretch){
	contentPanel.attr('class','lui_portlet_imgTxt_silder_wrap lui_portlet_imgTxt_silder_highlight_wrap lui_portlet_imgTxt_silder_imgCover_wrap');
}else {
	contentPanel.attr('class','lui_portlet_imgTxt_silder_wrap lui_portlet_imgTxt_silder_highlight_wrap');
}
if(position == '1'){
...
function SlideTuwen(config) {
	...
	this.bindEnter = function() {
		...
		LUI.$("#" + self.id + " .lui_portlet_switch_next").on("click", function() {
			self.titleScroll(self.firstIndex + 6);
		});
	...
	this.titleScroll = function(index) {
		...
		if(index <= 0)
			this.firstIndex = 0;
		else if(this.slideNum - index < 6)
			this.firstIndex = this.slideNum - 6 ;
...
~~function() {
	setTimeout(function(){
		var _height = element.parent().height() , _th = element.find(".lui_portlet_txt_item_wrap").height();
		if(_height < 270){
			_height = _th > 120 ? _th < 270 ? _th : 270 : 120;
		}
