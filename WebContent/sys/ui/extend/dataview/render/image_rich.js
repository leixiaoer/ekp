...
function buildTab(data) {
	...
	for(var k = 1; k < rowNum + 1 ; k++){
		...
		for (var i = 0; i < columnNum ; i++) {
			...
			var first_a = $('<a class="image_rich_portlet_img_url" data-href="' + env.fn.formatUrl(data[tempNum].href) + '" target="view_window" onclick="Com_OpenNewWindow(this);"></a>');
			var first_img = $('<img src="' + env.fn.formatUrl(data[tempNum].image) +'">');
			
			var second = $('<div class="second">');
			var second_a = $('<a class="image_rich_portlet_subject_url" data-href="' + env.fn.formatUrl(data[tempNum].href) + '" target="view_window" onclick="Com_OpenNewWindow(this);" title="' + data[tempNum].text +'">' + data[tempNum].text +'</a>');
			...
			if(data[tempNum].personId){
				third_left_span2_a = $('<a class="com_author" target="_blank" data-href="' + env.fn.formatUrl(personZoneUrl+data[tempNum].personId) + '" onclick="Com_OpenNewWindow(this);">'+ data[tempNum].personName + '</a>');
			}else{
