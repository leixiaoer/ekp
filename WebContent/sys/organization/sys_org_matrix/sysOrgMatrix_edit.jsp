...
<template:include ref="default.edit" sidebar="no">
		...
		<ui:toolbar id="toolbar" layout="sys.ui.toolbar.float" count="5">
			...
			<c:choose>
				<c:when test="${ sysOrgMatrixForm.method_GET == 'add' }">
					<ui:button text="${lfn:message('button.save')}" order="2" onclick="saveMatrix();" />
				</c:when>
	...
	<template:replace name="head">
		...
		<script language="JavaScript">
			...
			seajs.use(['lui/jquery','lui/dialog'], function($, dialog) {
				...
				$(function() {
					enabledCate("${sysOrgMatrixForm.fdIsEnabledCate eq 'true' ? '1' : '0'}");
				});
			});
		</script>
	</template:replace>
	<template:replace name="content">
		...
		<ui:step id="__step" onSubmit="commitMethod('save','false');" layout="sys.ui.step.fixed">
			<ui:content title="${lfn:message('sys-organization:sysOrgMatrix.edit.base')}" toggle="true">
				<table class="tb_normal" width=100%>
					...
					<tr>
						<td width=15% class="td_normal_title">
							...
							<c:if test="${sysOrgMatrixForm.matrixType!='1'}">
								<sunbor:enums property="fdIsAvailable" value="${sysOrgMatrixForm.fdIsAvailable}" enumsType="sys_org_available" elementType="radio"/>
							</c:if>
					...
					<tr>
						<td width=15% class="td_normal_title">
							...
							<label><input type="radio" name="fdIsEnabledCate" value="1" onchange="enabledCate(this.value);" ${sysOrgMatrixForm.fdIsEnabledCate eq 'true' ? 'checked' : ''}><bean:message bundle="sys-organization" key="sysOrgMatrix.dataCate.enabled"/></label>
							<label><input type="radio" name="fdIsEnabledCate" value="0" onchange="enabledCate(this.value);" ${sysOrgMatrixForm.fdIsEnabledCate eq 'false' ? 'checked' : ''}><bean:message bundle="sys-organization" key="sysOrgMatrix.dataCate.disabled"/></label>
							<table class="tb_normal" width=100% id="TABLE_DocList_Cates" align="center">
								<tr>
									...
									<td align="center" class="td_normal_title" style="width:20%">
										<img src="${KMSS_Parameter_StylePath}icons/add.gif" class="cate_add" style="cursor:pointer" onclick="_DocList_AddRow('TABLE_DocList_Cates')">
									</td>
								</tr>
								<!-- 基准行 -->
								<tr KMSS_IsReferRow="1" style="display: none">
									...
									<td>
										<xform:address style="width:94%" propertyId="fdDataCates[!{index}].fdElementId" propertyName="fdDataCates[!{index}].fdElementName" orgType='ORG_TYPE_POSTORPERSON' className="input"></xform:address>
									</td>
									<td align="center">
				...
				<ui:content title="${lfn:message('sys-organization:sysOrgMatrix.edit.data')}" toggle="true">
		...
		<script language="JavaScript">
			...
			seajs.use(['lui/jquery','lui/dialog', 'lui/topic'], function($, dialog, topic) {
				...
				$(function() {
					toStep('${JsParam.step}');
					//当直接进入”矩阵数据“页签中，自动触发点击刷新获取数据类别
					if('${JsParam.step}' == '2') {
						setTimeout(function() {$(".lui_step_tabs_fixed_titleM").find("ul").find("li:last").click();}, 300);
					}
				});
				
				// 下一步/上一步
				window.clickSysUiStep = function(stepFlag) {
					var idx = LUI("__step").__index;
				...
				window.saveMatrix = function() {
					if(window.STEP && window.STEP.cur == 2) {
						// 保存矩阵数据
						saveMatrixData();
					} else {
						// 保存矩阵基本信息
				...
				window.getFormData = function() {
					var datas = $(document.sysOrgMatrixForm).serializeArray();
					// select在禁用状态下无法更新，需要处理
					$("select:disabled").each(function(i, n) {
						var data = {"name": $(n).attr("name"), "value": $(n).val()};
						datas.push(data);
					});
					var newDatas = [];
					for(var i in datas) {
				...
				window.saveMatrixField = function(){
					...
					$.ajax({
						url : '${LUI_ContextPath}/sys/organization/sys_org_matrix/sysOrgMatrix.do?method=saveMatrixField',
						type: 'POST',
						dataType: 'json',
						data: getFormData(),
						success: function(res) {
							if(res.success) {
								// 保存成功
								window.MatrixResult = res.data;
								// 保存过字段信息，设置一个标识
								window.reBuildVersion = [];
								if(!res.data.fdIsEnabledCate) {
									$(".matrix_data_cate").empty();
								}
								// 发布一个事件
								topic.publish("showMatrixData", res.data);
								location.href = "${LUI_ContextPath}/resource/jsp/success.jsp";
							} else {
								dialog.failure(res.msg);
							}
						},
						error: function() {
							dialog.failure("<bean:message key="errors.unknown"/>");
						}
					});
				...
				window.saveMatrixData = function() {
					...
					downloadForm.find("[name=cateId]").val(cateId);
				...
				topic.subscribe('JUMP.STEP', function(evt) {
					...
					if(evt && evt.cur == 2) {
						...
						// 暂时移除禁用标识
						var disableds = [];
						$("#matrix_field_type select[disabled]").each(function(i, n) {
							disableds.push($(n));
