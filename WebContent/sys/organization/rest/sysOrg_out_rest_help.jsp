...
<div style="width: 95%;text-align: left;">
	<p>&nbsp;&nbsp;EKP系统提供了组织架构接出RestFul接口服务，接口包含两部分，获取所有组织架构基本信息（getElementsBaseInfo）以及获取需要更新的组织架构信息（getUpdatedElements）等2个接口，
...
<table border="0" width="95%">
	...
	<tr>
		<td style="cursor: pointer;font-size: 15px;" onclick="expandMethod(this);">&nbsp;&nbsp;1.1&nbsp;&nbsp;获取所有组织架构基本信息（/api/sys-organization/sysSynchroGetOrg/getElementsBaseInfo）
		...
		<td>
			<table class="tb_normal" cellpadding="0" cellspacing="0"  style="width: 85%;margin-left: 40px;">
				<tr>
					...
					<td style="padding: 0px;"><div style="margin: 8px;">&nbsp;&nbsp;组织架构基本信息接出body数据（JSON对象），详细说明如下:</div>
						<table class="tb_normal" cellpadding="0" cellspacing="0" width=100%>
							...
							<tr>
								...
								<td>为空,返回的基本信息仅包含:id(唯一标识),lunid(唯一标示，可作为数据主健),name(名称),type(组织架构类型)信息。<br/>
								          不为空,则表示返回的基本信息中处理id,name,type外,还包含设置的字段列表内容。
								    <font style="font-weight: bolder;">格式为:[{\"type\":\"no\"},..]</font>。<br/>
								    <font style="font-weight: bolder;"> 可供设置的字段名称有:no(编号),order(排序号),keyword(关键字)。</font>
								</td>
							</tr>
					</table>
					</td>
						...
						<table class="tb_normal" width=100%>
							...
							<tr>
								...
								<td>
									...
									1、returnType为空时,返回信息格式为：<br/>[{\"id\":\"13425\",\"name\":\"测试部\",\"type\":\"dept\"},...]。<br/><br/>
									2、returnType不为空时，返回的信息格式和returnType的设置对应，<br/>
									如returnType设置为 [{\"type\":\"no\"},{\"type\":\"keyword\"},{\"type\":\"order\"}]时,<br/>
									则对应返回信息格式为：<br/>[{\"id\":\"13425\",\"name\":\"测试部\",\"type\":\"dept\",\"no\":\"CSDL01\",\"keyword\":\"dept_01\",\"order\":\"1001\"},...]。
								</td>
							</tr>
							<tr>
								<td>count</td>
	...
	<tr>
		<td style="cursor: pointer;font-size: 15px;" onclick="expandMethod(this);"><br/>&nbsp;&nbsp;1.2&nbsp;&nbsp;获取需要更新的组织架构信息（/api/sys-organization/sysSynchroGetOrg/getUpdatedElements）
			<img src="${KMSS_Parameter_StylePath}icons/collapse.gif" border="0" align="bottom"/>
			</td>
	</tr>
	<tr>
		<td>
			<table class="tb_normal" cellpadding="0" cellspacing="0"  style="width: 85%;margin-left: 40px;">
				<tr>
					<td class="td_normal_title" width="20%">参数信息</td>
					<td style="padding: 0px;"><div style="margin: 8px;">&nbsp;&nbsp;组织架构信息接出body数据（JSON对象），详细说明如下:</div>
						<table class="tb_normal" cellpadding="0" cellspacing="0" width=100%>
							<tr class="tr_normal_title">
								<td align="center" width="10%"><b>程序名</b></td>
								<td align="center" width="10%"><b>描述</b></td>
				...
				<tr>
					<td class="td_normal_title" width="20%">返回信息</td>
					<td style="padding: 0px;"><div style="margin: 8px;">&nbsp;&nbsp;返回的body数据（JSON对象），详细说明如下:</div>
						<table class="tb_normal" width=100%>
							<tr class="tr_normal_title">
								<td align="center" width="10%"><b>程序名</b></td>
								<td align="center" width="10%"><b>描述</b></td>
	...
	<tr>
		<td style="cursor: pointer;font-size: 15px;" onclick="expandMethod(this);"><br/>&nbsp;&nbsp;1.3&nbsp;&nbsp;分页获取需要更新的组织架构信息（/api/sys-organization/sysSynchroGetOrg/getUpdatedElementsByToken），该接口和1.2的getUpdatedElements接口在功能上是一样的，只是getUpdatedElementsByToken加了按数目分页的功能，防止当ekp的组织架构批量更新导致更新时间一样时，用getUpdatedElements接口一次返回太多数据。
			<img src="${KMSS_Parameter_StylePath}icons/collapse.gif" border="0" align="bottom"/>
			</td>
	</tr>
	<tr>
