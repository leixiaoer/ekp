...
<template:include ref="config.profile.list">
	<template:replace name="title">${ lfn:message('sys-organization:sysOrgElement.dept') }</template:replace>
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			...
			<list:cri-auto modelName="com.landray.kmss.sys.organization.model.SysOrgDept" property="fdIsBusiness" />
		</list:criteria>
		<!-- 操作栏 -->
			...
			<div style="float:left">
				<div style="display: inline-block;vertical-align: middle;">
					...
					<list:sortgroup>
						...
						<list:sort property="fdName" text="${lfn:message('sys-organization:sysOrgDept.fdName') }" group="sort.list"></list:sort>
						<list:sort property="fdNo" text="${lfn:message('sys-organization:sysOrgDept.fdNo') }" group="sort.list"></list:sort>
					</list:sortgroup>
					</ui:toolbar>
				</div>
			</div>
				...
				<div style="display: inline-block;vertical-align: middle;">
					<ui:toolbar id="Btntoolbar" count="5">
						...
						<kmss:auth requestURL="${actionUrl}method=invalidatedAll" requestMethod="POST">
							...
							<!-- 快捷调换部门 -->
							<ui:button text="${lfn:message('sys-organization:sysOrgDept.quickChangeDept')}" onclick="changeParent()" order="4" ></ui:button>
						</kmss:auth>
						</c:if>
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/sys/organization/sys_org_dept/sysOrgDept.do?method=list&available=${JsParam.available}&all=${JsParam.all}&sys_page=${JsParam.sys_page}&parent=${JsParam.parent}&fdFlagDeleted=${JsParam.fdFlagDeleted}&fdImportInfo=${JsParam.fdImportInfo}'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" 
				rowHref="/sys/organization/sys_org_dept/sysOrgDept.do?method=view&fdId=!{fdId}">
				<list:col-checkbox></list:col-checkbox>
				<list:col-auto props="fdOrder,fdParent,fdName,fdNo,fdThisLeader,fdSuperLeader,fdPersonCount,operations"></list:col-auto>
			</list:colTable>
	 	...
	 	<script type="text/javascript">
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
				...
				topic.subscribe('successReloadPage', function() {
			 	...
			 	// 增加
		 		window.add = function() {
		 			Com_OpenWindow('<c:url value="/sys/organization/sys_org_dept/sysOrgDept.do" />?method=add&parent=${JsParam.parent}');
		 		};
		 		// 编辑
		 		window.edit = function(id) {
			 		if(id)
		 				Com_OpenWindow('<c:url value="/sys/organization/sys_org_dept/sysOrgDept.do" />?method=edit&fdId=' + id);
		 		};
		 		// 新建人员
		 		window.addPerson = function(id) {
			 		if(id)
		 				Com_OpenWindow('<c:url value="/sys/organization/sys_org_person/sysOrgPerson.do" />?method=add&parent=' + id);
		 		};
		 		// 转机构
		 		window.toOrg = function(id) {
			 		if(id) {
						$.ajax({
							url : '<c:url value="/sys/organization/sys_org_org/sysOrgOrg.do" />?method=parentIsOrg',
							type : 'GET',
							data : $.param({"deptId" : id}, true),
							dataType : 'json',
							error : function(data) {
								if(window.del_load != null) {
									window.del_load.hide(); 
								}
								if(data.responseJSON.message && data.responseJSON.message.length > 1)
									dialog.alert(data.responseJSON.message[0].msg);
							},
							success: function(data) {
								if(data.isOrg==false){
									dialog.alert("${lfn:message('sys-organization:sysOrgOrg.updateDeptToOrg.ajax.error')}");
								}else{
						 			dialog.confirm("${lfn:message('sys-organization:sysOrgOrg.updateDeptToOrg.warn')}", function(value) {
										if(value == true) {
					 						window.del_load = dialog.loading();
											$.ajax({
												url : '<c:url value="/sys/organization/sys_org_org/sysOrgOrg.do" />?method=updateDeptToOrg',
												type : 'POST',
												data : $.param({"deptId" : id}, true),
												dataType : 'json',
												error : function(data) {
		 		...
		 		window.invalidated = function(id) {
					...
					var url  = '<c:url value="/sys/organization/sys_org_dept/sysOrgDept.do?method=invalidatedAll&parent=${parent}"/>';
				...
				window.changeParent = function() {
					...
					dialog.iframe("/sys/organization/sys_org_dept/sysOrgDept_chgDept.jsp?orgType=ORG_TYPE_DEPT", 
							...
							buttons:[{
									...
									fn : function(value, _dialog) {
										...
										$.ajax({
											url : '<c:url value="/sys/organization/sys_org_dept/sysOrgDept.do"/>?method=changeDept',
											type : 'POST',
											data : $.param({"deptIds":_dialog.fdIds,"parentId":_dialog.parentId}, true),
											dataType : 'json',
											error : function(data) {
												if(data.responseJSON.message && data.responseJSON.message.length > 0)
													dialog.alert(data.responseJSON.message[0].msg);
