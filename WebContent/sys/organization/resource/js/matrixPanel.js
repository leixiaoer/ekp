define(function(require, exports, module) {
	...
	var MatrixPanel = base.Container.extend({
				...
				dialog.iframe("/sys/organization/sys_org_matrix/sysOrgMatrix_edit_data_replace.jsp?fdId=" + window.MatrixResult.fdId + "&curVersion=" + window.curVersion, 
					...
					buttons:[{
						...
						fn : function(value, _dialog) {
							...
							// 获取原数据和目标数
							var oriId_person, oriId_post, targetId_person, targetName_person, targetId_post, targetName_post, oriId, targetId, targetName;
							...
							for(var i=0; i<ids.length; i++) {
								...
								if(_field.type == "person_post") {
									...
									if(td.find("input[type=hidden]").val() == oriId) {
										td.find("input[type=hidden]").val(targetId);
										td.find("input[type=text]").val(targetName);
									}
								}
							}
							_dialog.hide();
		...
		initDataTab: function(addLine) {
			...
			// 生成分组
			if(window.generateCateData)
				window.generateCateData(window.MatrixResult.fdDataCates);
			...
			for(var idx=0; idx<10; idx++) {
				...
				if(idx == 0) {
					var dataTr = [];
					dataTr.push("<tr style='height: 50px;'>");
					for(var i=0; i<con.length; i++) {
						dataTr.push("<th class='lui_matrix_td_normal_title lui_maxtrix_condition_th' data-field='" + con[i].fdFieldName + "'><pre>" + con[i].fdName + "</pre></th>");
					}
					for(var i=0; i<res.length; i++) {
						dataTr.push("<th class='lui_matrix_td_normal_title lui_maxtrix_result_th' data-field='" + res[i].fdFieldName + "'><pre>" + res[i].fdName + "</pre></th>");
					}
					dataTr.push("</tr>");
		...
		resizeTr: function() {
			...
			setTimeout(function() {self.resizeTr();}, 300);
		},
		/* 编辑页填充数据 */
		initData: function(page) {
			var self = this;
			if(self.version != window.curVersion) {
				return;
			}
			page = page || 1;
			// 如果没有结果数据，不处理
			...
			$.ajax({
				url : Com_Parameter.ContextPath + 'sys/organization/sys_org_matrix/sysOrgMatrix.do?method=findMatrixPage&fdId=' + window.MatrixResult.fdId + '&fdVersion=' + window.curVersion + '&fdDataCateId=' + window.fdDataCateId + '&method_GET=edit',
				type: 'POST',
				dataType: 'json',
				data: {'pageno': page, 'rowsize': self.rowsize, 'pagingtype': 'default'},
				success: function(res) {
					// 设置数据加载成功标识
						...
						for(var i=0; i<res.datas.length; i++) {
							...
							for(var j=0; j<datas.length; j++) {
								...
								if(data.col == "fdId") {
									...
									// 处理内容
									var td = self.dataTab.find("tr:eq(" + (i + 1) + ")").find("td:eq(" + (j - 1) + ")");
									if(temp.length < 3) {
		...
		showPage: function(page, pageSize, totalSize) {
			...
			if(window.dataTablePage) {
				...
				seajs.use(['lui/jquery','lui/parser'],function($,parser){
					parser.parse($("#matrix_data_table_"+ self.version+"_page").parent()[0],function(e){
						setTimeout(function() {self.showPage(page, pageSize, totalSize);}, 100);
					})
				});
		...
		saveData: function() {
			...
			seajs.use(['lui/jquery','lui/parser','lui/pinwheel'],function($,parser,p){
				...
				parser.parse($("#matrix_data_table_V2_page")[0],function(e){
					console.log(e);
				})
			});
			var __data = {};
		...
		addLine: function(first) {
			...
			var con = window.MatrixResult.fdRelationConditionals, res = window.MatrixResult.fdRelationResults, idx = self.seq++;
			var hasBr = false;
			...
			for(var i=0; i<con.length; i++) {
				...
				if(con[i].fdType == 'sys') {
					...
					dataTr.push("	<a onclick=\"Dialog_MainData('" + con[i].fdId + "[" + idx + "]', '" + con[i].fdFieldName + "[" + idx + "]', '" + con[i].fdMainDataText + "');\" class=\"selectitem\"/>");
					dataTr.push("</div>");
				} else if(con[i].fdType == 'cust') {
					dataTr.push("<div class=\"inputselectsgl\" style=\"border-bottom:none; width: 100%;\">");
					dataTr.push("	<input type=\"hidden\" data-type=\"fieldId\" name=\"" + con[i].fdId + "[" + idx + "]\" value=\"" + con[i].fdConditionalId + "\">");
					dataTr.push("	<input type=\"text\" name=\"" + con[i].fdFieldName + "[" + idx + "]\" value=\"" + con[i].fdConditionalValue + "\" style=\"width:90%; min-width:100px;\" readonly=\"true\" class=\"inputsgl\">");
					dataTr.push("	<a onclick=\"Dialog_CustData(false, '" + con[i].fdId + "[" + idx + "]', '" + con[i].fdFieldName + "[" + idx + "]', null, 'sysOrgMatrixMainDataService&id=" + con[i].fdMainDataType + "', '" + con[i].fdMainDataText + "');\" class=\"selectitem\"/>");
					dataTr.push("</div>");
					...
					dataTr.push("<div class=\"inputselectsgl\" style=\"border-bottom:none; width: 100%;\">");
					dataTr.push("	<input type=\"hidden\" data-type=\"fieldId\" name=\"" + con[i].fdId + "[" + idx + "]\" value=\"" + con[i].fdConditionalId + "\">");
					dataTr.push("	<div class=\"luiTmplField\" data-name=\"" + con[i].fdFieldName + "[" + idx + "]\">"+con[i].fdConditionalValue+"</div><input type=\"hidden\" name=\"" + con[i].fdFieldName + "[" + idx + "]\" value=\"" + con[i].fdConditionalValue + "\" style=\"width:90%; min-width:100px;\" readonly=\"true\" class=\"inputsgl\">");
					dataTr.push("	<a onclick=\"Dialog_Address_Cust(false, '" + con[i].fdId + "[" + idx + "]', '" + con[i].fdFieldName + "[" + idx + "]', null, " + orgType + ");\" class=\"orgelement\"/>");
					dataTr.push("</div>");
				}
				dataTr.push("</td>");
			}
			for(var i=0; i<res.length; i++) {
				...
				if(res[i].fdType == 'person_post') {
					...
					dataTr.push("		<a onclick=\"Dialog_Address_Cust(false, '" + res[i].fdId + "[" + idx + "]_person', '" + res[i].fdFieldName + "[" + idx + "]_person', null, ORG_TYPE_PERSON, resultCheck2);\" class=\"orgelement\"/>");
					dataTr.push("	</div>");
					dataTr.push("	<br>");
					...
					dataTr.push("		<a onclick=\"Dialog_Address_Cust(false, '" + res[i].fdId + "[" + idx + "]_post', '" + res[i].fdFieldName + "[" + idx + "]_post', null, ORG_TYPE_POST, resultCheck2);\" class=\"orgelement\"/>");
					dataTr.push("	</div>");
				} else {
					...
					dataTr.push("		<a onclick=\"Dialog_Address_Cust(true, '" + res[i].fdId + "[" + idx + "]', '" + res[i].fdFieldName + "[" + idx + "]', null, " + orgType + ",resultCheck);\" class=\"orgelement\"/>");
					dataTr.push("	</div>");
				}
				dataTr.push("</td>");
			}
