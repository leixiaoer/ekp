	...
	Dialog_Address(true, null, null, null, ORG_TYPE_ALLORG, function(rtnData){
		...
		runAction("quickAdd", function(result){
			...
			for(var i=1; i<result.length; i++){
				var temp = node.AppendChild(result[i].text, null, null, result[i].value, null, result[i].nodeType);
				if(!newNode) {
					newNode = temp;
				}
			}
...
function opt_add(){
	...
	openDialog(url,function(rtnVal){
		if(rtnVal==null)
			return;
		var newNode = node.AppendChild(rtnVal.text, null, null, rtnVal.value, null, rtnVal.nodeType);
		node.isExpanded = true;
		LKSTree.Show();
		if(newNode) {
			LKSTree.ClickNode(newNode);
