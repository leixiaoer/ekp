seajs.use(['lui/jquery','lui/dialog', 'lui/topic','lang!sys-organization'], function($, dialog, topic,lang) {
		...
		window.addRes = function(elem) {
			...
			var idx = $(elem.parentNode).parent().prevAll().length + 1;
			var conLen = allFields.con.length > 3 ? allFields.con.length : 3;
			allFields.res.splice(idx - conLen, 0, {"fdType": "", "fdName": "", "fdValueCount": 0});
			// 增加一列
		...
		window.resetTable = function() {
			$("#matrix_table").find("[name^='fdRelationConditionals'][name$='fdType']").each(function(i, elem) {
			...
			$("#matrix_table").find("[name^='fdRelationResults'][name$='fdType']").each(function(i, elem) {
		...
		window.saveField = function() {
			...
			$("#matrix_table").find("[name^='fdRelationConditionals'][name$='fdType']").each(function(i, elem) {
				allFields.con.push({"fdId": "", "fdType": "", "fdName": "", "fdMainDataType": "", "fdMainDataText": "","fdIncludeSubDept":"","fdIsUnique":"","fdValueCount": tempCon[i].fdValueCount});	
			...
			$("#matrix_table").find("[name^='fdRelationResults'][name$='fdType']").each(function(i, elem) {
				allFields.res.push({"fdId": "", "fdType": "", "fdName": "", "fdValueCount": tempRes[i].fdValueCount});	
		...
		window.initTable = function() {
			...
			for(var i=0; i<conLen; i++) {
				if(con.length > i) {
					...
					typeField.append("<td class=\"lui_maxtrix_condition_th\" data-readonly=\"" + isDel + "\">" + conTypeTpl + "</td>");
