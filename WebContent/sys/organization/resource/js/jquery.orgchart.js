...
(function ($) {
    ...
    function getNodeContent($node, level, index, opts){
        ...
        $nodeDiv.click(function (e) {
            if (e.target.id.indexOf("arrowImg") == -1) { return; }
            var w = worldMap;
            var $this = $(this);
            opts.nodeClicked($this.data("orgchart-node"), $this);
            if (opts.interactive) {
                var $row = $this.closest("tr");
                if ($row.next("tr").is(":visible")) { // 收缩节点
                    if (e.target.src.indexOf("arrowClose.png") > -1) {
                        e.target.src = e.target.src.replace("arrowClose.png", "arrowOpen.png");
                    }
                    if (opts.fade) {
                        $row.nextAll("tr").fadeOut(opts.speed,function(){setOrgMapStyle();});
                    }
                    else {
                        $row.nextAll("tr").hide();
                    }
                    ...
                    if (e.target.src.indexOf("arrowOpen.png") > -1) {
                        e.target.src = e.target.src.replace("arrowOpen.png", "arrowClose.png");
                    }
                    ...
                    var divId = $this[0].firstChild.firstChild.id;
                    var fdId = divId.replace("div_","");
                    var load_child_callback = function($newNode){
                    	var $newTable = getNodeContent($newNode, 0, index, opts);
                    	$table.replaceWith($newTable);
                    	setOrgMapStyle();
                    };
                    loadChildNode( fdId, load_child_callback ); // 加载子级节点内容
        ...
        if ($childNodes.length > 0) {
            if (opts.depth == -1 || level + 1 < opts.depth) {
                ...
                if ($childNodes.length > 0) {
                    $nodeDiv.addClass("hasChildren");		//鼠标指针
                    if (opts.showLevels == -1 || level < opts.showLevels - 1) {
                        $nodeDiv.addClass("shownChildren");
                        $nodeDiv.find("img").each(function(i){
                        	if (this.src && this.src.indexOf("arrowOpen.png") > -1) {
                        		this.src = this.src.replace("arrowOpen.png", "arrowClose.png");
                            }	
                        });
                    }
                    else {
                        ...
                        $nodeDiv.find("img").each(function(i){
                        	if (this.src && this.src.indexOf("arrowClose.png") > -1) {
                        		this.src = this.src.replace("arrowClose.png", "arrowOpen.png");
                        	}
                        });
                    }
                    if (opts.interactive) {
    ...
    function buildNode($node, $appendTo, level, index, opts) {
        var $table = getNodeContent($node, level, index, opts);
