...
<%
	...
	if (StringUtil.isNotNull(request.getHeader("If-Modified-Since"))) {
		...
		if(StringUtil.isNotNull(orgId)&&!orgId.equals("formula")&&!orgId.equals("matrix")&&!orgId.equals("rule")){
			...
			if(elem.getFdOrgType() == 8){
			...
			}else if(elem.getFdOrgType() == 16){
				String src = "/sys/organization/resource/image/group.png";
				...
				request.getRequestDispatcher(src).forward(
						request, response);
			}else if(elem.getFdOrgType() == 2){
				String src = "/sys/organization/resource/image/dept.png";
				request.getRequestDispatcher(src).forward(
						request, response);
			}else if(elem.getFdOrgType() == 1){
				String src = "/sys/organization/resource/image/org.png";
				request.getRequestDispatcher(src).forward(
						request, response);
			}
		}
