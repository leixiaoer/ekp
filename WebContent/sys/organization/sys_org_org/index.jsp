...
<template:include ref="config.profile.list">
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			...
			<list:cri-auto modelName="com.landray.kmss.sys.organization.model.SysOrgOrg" property="fdIsBusiness" />
		</list:criteria>
		...
		<div class="lui_list_operation">
			...
			<div style="float:left">
				<div style="display: inline-block;vertical-align: middle;">
					...
					<list:sortgroup>
						<list:sort property="fdOrder" text="${lfn:message('sys-organization:sysOrgOrg.fdOrder') }" group="sort.list" value="up"></list:sort>
						<list:sort property="fdName" text="${lfn:message('sys-organization:sysOrgOrg.fdName') }" group="sort.list"></list:sort>
						<list:sort property="fdNo" text="${lfn:message('sys-organization:sysOrgOrg.fdNo') }" group="sort.list"></list:sort>
					</list:sortgroup>
					</ui:toolbar>
				</div>
			</div>
		...
		<list:listview>
			<ui:source type="AjaxJson">
				{url:'/sys/organization/sys_org_org/sysOrgOrg.do?method=list&available=${JsParam.available}&all=${JsParam.all}&parent=${JsParam.parent}&fdFlagDeleted=${JsParam.fdFlagDeleted}&fdImportInfo=${JsParam.fdImportInfo}'}
			</ui:source>
			<list:colTable isDefault="true" layout="sys.ui.listview.columntable" 
				rowHref="/sys/organization/sys_org_org/sysOrgOrg.do?method=view&fdId=!{fdId}">
	 	...
	 	<script type="text/javascript">
		 	seajs.use(['lui/jquery', 'lui/dialog', 'lui/topic'], function($, dialog, topic) {
				...
				topic.subscribe('successReloadPage', function() {
			 	...
			 	// 增加
		 		window.add = function() {
		 			Com_OpenWindow('<c:url value="/sys/organization/sys_org_org/sysOrgOrg.do" />?method=add&parent=${JsParam.parent}');
		 		};
		 		// 编辑
		 		window.edit = function(id) {
			 		if(id)
		 				Com_OpenWindow('<c:url value="/sys/organization/sys_org_org/sysOrgOrg.do" />?method=edit&fdId=' + id);
		 		};
		 		// 查看日志
		 		...
		 		window.invalidated = function(id) {
					...
					var url  = '<c:url value="/sys/organization/sys_org_org/sysOrgOrg.do?method=invalidatedAll"/>';
				...
				window.changeParent = function() {
					...
					dialog.iframe("/sys/organization/sys_org_org/sysOrgOrg_chgDept.jsp?orgType=ORG_TYPE_ORG", 
