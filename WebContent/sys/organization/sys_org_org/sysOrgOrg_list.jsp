			...
			<div class="conf_show_more_w">
				<div class="conf_btn_edit">
					<kmss:auth requestURL="/sys/organization/sys_org_org/sysOrgOrg.do?method=edit&fdId=${sysOrgOrg.fdId}" requestMethod="GET">
						<!-- 编辑 -->
						<a class="btn_txt" href="javascript:edit('${sysOrgOrg.fdId}')">${lfn:message('button.edit')}</a>
					</kmss:auth>
					...
					<kmss:auth requestURL="/sys/organization/sys_org_org/sysOrgOrg.do?method=invalidatedAll" requestMethod="POST">
						<!-- 禁用 -->
						<a class="btn_txt" href="javascript:invalidated('${sysOrgOrg.fdId}')">${ lfn:message('sys-organization:sys.org.available.false') }</a>
					</kmss:auth>
					</c:if>
					<% if (!TripartiteAdminUtil.IS_ENABLED_TRIPARTITE_ADMIN) { %>
					<!-- 日志 -->
					<a class="btn_txt" href="javascript:viewLog('${sysOrgOrg.fdId}', '${sysOrgOrg.fdName}')">${ lfn:message('sys-organization:sys.org.operations.log') }</a>
					<% } %>
				</div>
			</div>
			<!--操作按钮 结束-->
