...
function EvaluationOpt(modelName , modelId , key , langCfg, fdIsCommented, attModelId){
	...
	this.openBqBox = function(evt){
		...
		var biaoqing = $("#eval_ViewMain").parent().find("div[class='eval_biaoqing']");//关闭所有表情框
			...
			success : function(data) {
					...
					if(replyInfo!=null && replyInfo.length>0){
						...
						for(var i=0;i<replyInfo.length;i++){
							...
							replyCont = replyCont.replace(/\[face(\d*)\]/g, '<img src="' + 
									Com_Parameter.ContextPath + 'sys/evaluation/import/resource/images/bq/$1.gif" type="face" /></img>');
