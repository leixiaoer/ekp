...
<script type="text/javascript">
    function SharePersonOpt(){
        this.onload = function(){
            //提交按钮事件初始化
            $("#share_to_person_button").click(function(){
                ...
                // 分享内容
                var shareTxt = parent.window.shareTxt;
                ...
                var contentVal = $.trim(contentObj);
               
                // 是否通知创建者
                var fdIsNotify = $("#fdPersonIsNotify").is(":checked");
                
                // 通知类型
                var fdNotifyType = $("input[name='fdPersonNotifyType']")[0].value;
                
                var paramDara = {
                        fdShareMode: "1", // 1个人  3圈子
                    	docSubject: shareTxt,
                    	goalPersonids: introducTo,
                    	fdShareReason: contentVal,
                ...
                seajs.use(['lui/dialog'],
                    function(dialog) {
                        ...
                        LUI.$.ajax({
                            type : "POST",
                            url :  "<c:url value='/sys/evaluation/sys_evaluation_share/sysEvaluationShare.do?method=saveEvalShare&type=person'/>",
                            data: paramDara,
                            dataType : 'json',
                            async: false,
                            success : function(data) {
                                if (data && data['flag'] === true) {
                                    ...
                                    dialog.success("${ lfn:message('sys-evaluation:sysEvaluationShare.shareSuccess') }");
                                }else{
                ...
                for(var i=0;i<4;i++){
                    ...
                    for(var j=0;j<20;j++){
                        bqHTML += '<td class="lui_share_biaoqing" style="padding:0px 0px !important;">' +
                            '<a onclick="share_to_person_opt.selectIcon(this)"><img src="' + Com_Parameter.ContextPath
                            +'sys/evaluation/resource/evaluation/share/img/bq/' + num
    ...
    function checkWordsNum(){
        ...
        if(l<=280){
            promptVar.html("${lfn:message('sys-evaluation:sysEvaluationShare.moreWords')}" +
                '<font style="font-family: Constantia, Georgia; font-size: 20px;color:#F19703">' +
                Math.abs(parseInt((280-l) / 2))+"</font>"+"${lfn:message('sys-evaluation:sysEvaluationShare.word')}");
            promptVar.css({'color':''});
            enabledBtn(true);
        }else{
            promptVar.html("${lfn:message('sys-evaluation:sysEvaluationShare.exceed.words')}"+'<font style="font-family: Constantia, Georgia; font-size: 20px;">'
                + Math.abs(parseInt((l-280) / 2)+1)+"</font>"+"${lfn:message('sys-evaluation:sysEvaluationShare.word')}");
    ...
    function enabledBtn(enable){
        var buttonVar = $("#share_to_person_button");
        if(enable==true){
        ...
        $(evt).find("img").each(function(){
            if(this.src.indexOf(".gifface")>-1 || $(this).attr("type") == "face"){
                var cutedSrc = Com_Parameter.ContextPath +
                    this.src.substring(this.src.indexOf("sys/evaluation"));
   ...
   function bindPersonIframeEvent() {
