...
<tr LKS_LabelName="${lfn:message('sys-remind:module.sys.remind')}">
	<td>
		...
		<script type="text/javascript">
			...
			window.showTask = function(fdId) {
				Dialog_PopupWindow(Com_Parameter.ContextPath + "sys/remind/sys_remind_main_task/index.jsp?remindId=" + fdId, 900, 500);
			}
			
			// 查看日志
			window.showLog = function(fdId) {
				Dialog_PopupWindow(Com_Parameter.ContextPath + "sys/remind/sys_remind_main_task_log/index.jsp?remindId=" + fdId, 900, 500);
			}
			
			// 查看提醒设置
			window.showRemind = function(fdId) {
				Dialog_PopupWindow(Com_Parameter.ContextPath + "sys/remind/sys_remind_main/sysRemindMain.do?method=view&fdId=" + fdId, 900, 500);
			}
