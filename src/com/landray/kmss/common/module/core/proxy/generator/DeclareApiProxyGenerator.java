...
public class DeclareApiProxyGenerator<T> implements ICachedProxyGenerator<T> {
	...
	private static class ServiceInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) {
			...
			serviceProxy.setPrepareCallBack(new AnalyseApiImplements(apiInterface));
			return serviceProxy.invoke(method.getName(), args);
		}

	}

	private static class AnalyseApiImplements implements IProxyCallBack{

		private final Class<?> apiInterface;
		
		public AnalyseApiImplements(Class<?> apiInterface) {
			this.apiInterface = apiInterface;
		}
		
		@Override
		public void afterPrepare(MethodInvoker invoker) {
			analyseApiImplements(invoker);
		}
