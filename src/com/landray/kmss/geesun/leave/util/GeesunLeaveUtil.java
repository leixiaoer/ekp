...
public class GeesunLeaveUtil {

	public static void initModelFromRequest(IBaseModel model, RequestContext request) throws Exception {
		if (request.getParameterMap() == null) {
			return;
		}
		String modelName = ModelUtil.getModelClassName(model);
		SysDataDict dataDict = SysDataDict.getInstance();
		Map<String, SysDictCommonProperty> propMap = dataDict.getModel(modelName).getPropertyMap();
		for (Entry<String, String[]> entry : request.getParameterMap().entrySet()) {
			...
			if (values == null || values.length == 0 || StringUtil.isNull(values[0])) {
				continue;
			}
			// 属性可写
	...
	public static String buildCriteria(String serviceBean, String selectBlock, String whereBlock, String orderBy)
			throws Exception {
		// 查询
	...
	@SuppressWarnings("rawtypes")
	public static final void convert(IBaseService baseService, IExtendForm $baseForm, IBaseModel $baseModel,
			String[] props, RequestContext requestContext) throws Exception {
	 ...
	 * 
	 * @param value
	 *            (#now or #dayslater;1 or #hourslater or #minuteslater)
	 * @return
	 */
	 ...
	 * @return
	 */
	public static final String getDateStrByValue(String value, String dateType) {
	 ...
	 * @param hqlInfo
	 * @param paramList
	 *            ("key":"eq.docCreateTime","value":"#dayslater;1","dateType":
	 *            "dateTime")
	 * @param clazz
	 */
	public static void buildHqlInfoDateType(HQLInfo hqlInfo, List<Map<String, String>> paramList, Class<?> clazz) {
		if (paramList != null) {
			...
			for (Map<String, String> paramMap : paramList) {
				...
				if (opt.equals("isnull")) {
					hqlInfo.setWhereBlock(StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", shortName + "."
							+ fieldName + "is null"));
				} else if (opt.equals("nnull")) {
					hqlInfo.setWhereBlock(StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", shortName + "."
							+ fieldName + "is not null"));
				} else if (opt.equals("eq")) {
					hqlInfo.setWhereBlock(StringUtil.linkString(hqlInfo.getWhereBlock(), " and ", shortName + "."
							+ fieldName + "=:" + fieldName + String.valueOf(i)));
					hqlInfo.setParameter(fieldName + String.valueOf(i), getDateByValue(valueInfo, dateType));
	...
	public static void buildHqlInfoDate(HQLInfo hqlInfo, HttpServletRequest request, Class<?> clazz) {
		...
		if (dateInfo != null) {
			...
			buildHqlInfoDateType(hqlInfo, paramList, clazz);
		}
	}

	public static void buildHqlInfoModel(HQLInfo hqlInfo, HttpServletRequest request) throws Exception {
		...
		if (modelInfo != null) {
			for (String info : modelInfo) {
				...
				if (infos != null) {
					...
					buildHqlInfoModelType(property, property, hqlInfo, opt, values);
				}
			}
		}
	...
	public static void buildHqlInfoModelType(String shortName, String key, HQLInfo hqlInfo, String opt, String value)
			throws Exception {
		...
		String propKey = shortName.contains(".fdId") ? shortName : (shortName + ".fdId");
		...
		if ("is null".equals(optKey) || "is not null".equals(optKey)) {
			...
			if (value.contains(";")) {
				...
				if ("in".equals(optKey)) {
					hqlW = HQLUtil.buildPreparedLogicIN(propKey, null, valueList);
				} else {
					hqlW = _buildPreparedLogicNotIN(propKey, null, valueList);
				}
	...
	private static HQLWrapper _buildPreparedLogicNotIN(String item, String alias, List<?> valueList) {
		String param = alias;
	...
	public static org.json.simple.JSONArray importData(FormFile file, org.json.simple.JSONArray jsArray)
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			job = new JSONObject();
			job.element("value", values.substring(1, values.length()));
			jsArray.add(job);
		}
		return jsArray;
	}

	public static HSSFWorkbook buildWorkBook(String[] Headers) throws Exception {
	...
	public static HSSFWorkbook exportWorkBook(String[] Headers, List list) throws Exception {
		HSSFRow row = null;
		int rowCount = 1;
	...
	private static HSSFCellStyle getCellStyle(final HSSFWorkbook wb, final boolean isCenter) {
	...
	private static void setBackgroundColor(final HSSFCellStyle style, final boolean isMain) {
	...
	private static HSSFFont getTitleFont(final HSSFWorkbook wb, final boolean isRed) {
	...
	private static void buildTemplate(final HSSFWorkbook wb, final HSSFSheet sheet, String[] Headers) {
		HSSFRow row = null;
		HSSFCell cell = null;
	...
	private static void buileOneNcCustomer(HSSFRow row, List values) throws Exception {
		HSSFCell cell = null;
		int cellIndex = 0;
