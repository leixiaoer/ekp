...
public final class IcalParseUtil {
	...
	//private static final Pattern DATE_VALUE2 = Pattern.compile("(\\d{8})(?:T(\\d{6})(Z)?)?");

	private static final Pattern DATE_VALUE = Pattern
			.compile("(\\d{4})(\\d\\d)(\\d\\d)"
					+ "(?:T(\\d\\d)(\\d\\d)(\\d\\d)(Z)?)");
	...
	public static DateValue parseDateValue(String s, TimeZone tzid)
		...
		int year = Integer.parseInt(m.group(1)), month = Integer.parseInt(m
				.group(2)), day = Integer.parseInt(m.group(3));
		if (null != m.group(4)) {
			int hour = Integer.parseInt(m.group(4)), minute = Integer
					.parseInt(m.group(5)), second = Integer
					.parseInt(m.group(6));
			boolean utc = null != m.group(7);

			DateValue dv = new DTBuilder(year, month, day, hour, minute, second)
					.toDateTime();
			if (!utc && null != tzid) {
	 ...
	 * from the given timezone to UTC. This does not yet recognize the
	 * &lt;start&gt;/&lt;duration&gt; form.
	 */
	public static PeriodValue parsePeriodValue(String s, TimeZone tzid)
		...
		DateValue start = parseDateValue(s.substring(0, sep), tzid), end = parseDateValue(
				s.substring(sep + 1), tzid);
	 ...
	 * content lines. Content lines are delimited by a line break, which is a
	 * CRLF sequence (US-ASCII decimal 13, followed by US-ASCII decimal 10).
	 * 
	 * <p>
	...
	public static String unfoldIcal(String foldedContentLines) {
		return IGNORABLE_ICAL_WHITESPACE.matcher(foldedContentLines)
				.replaceAll("");
	}

	private static final Pattern IGNORABLE_ICAL_WHITESPACE = Pattern
			.compile("(?:\\r\\n?|\\n)[ \t]");
