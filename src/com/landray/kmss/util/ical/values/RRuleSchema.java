...
class RRuleSchema extends IcalSchema {
	...
	private static final Pattern X_NAME_RE = Pattern.compile("^X-",
			Pattern.CASE_INSENSITIVE);
	private static final Pattern RRULE_PARTS = Pattern
			.compile(
	...
	static {
		...
		objectRules.put("RRULE", new ObjectRule() {
			public void apply(IcalSchema schema, Map<String, String> params,
					String content, IcalObject target) throws ParseException {
		...
		ParamRule xparamsOnly = new ParamRule() {
			public void apply(IcalSchema schema, String name, String value,
					IcalObject out) throws ParseException {
		 ...
		 * recur = "FREQ"=freq *(
		 * 
		 * ; either UNTIL or COUNT may appear in a 'recur', ; but UNTIL and
		 * COUNT MUST NOT occur in the same 'recur'
		 * 
		 * ( ";" "UNTIL" "=" enddate ) / ( ";" "COUNT" "=" 1*DIGIT ) /
		 * 
		 * ; the rest of these keywords are optional, ; but MUST NOT occur more
		 * than once
		 * 
		 * ( ";" "INTERVAL" "=" 1*DIGIT ) / ( ";" "BYSECOND" "=" byseclist ) / (
		 * ";" "BYMINUTE" "=" byminlist ) / ( ";" "BYHOUR" "=" byhrlist ) / (
		 * ";" "BYDAY" "=" bywdaylist ) / ( ";" "BYMONTHDAY" "=" bymodaylist ) /
		 * ( ";" "BYYEARDAY" "=" byyrdaylist ) / ( ";" "BYWEEKNO" "=" bywknolist
		 * ) / ( ";" "BYMONTH" "=" bymolist ) / ( ";" "BYSETPOS" "=" bysplist )
		 * / ( ";" "WKST" "=" weekday ) /
		 * 
		 * ( ";" x-name "=" text ) )
		 */
		contentRules.put("recur", new ContentRule() {
			public void apply(IcalSchema schema, String content,
					IcalObject target) throws ParseException {
				...
				for (int i = 0; i < parts.length; ++i) {
					...
					String k = m.group(1).toUpperCase(), v = m.group(2);
				...
				if (partMap.containsKey("UNTIL")
						&& partMap.containsKey("COUNT")) {
				...
				for (Map.Entry<String, String> part : partMap.entrySet()) {
					...
					schema.applyContentSchema(part.getKey(), part.getValue(),
							target);
				}
			}
		});
		...
		contentRules.put("FREQ", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
				...
				((RRule) target).setFreq((Frequency) schema.applyXformSchema(
						"freq", value));
			}
		});
		...
		contentRules.put("UNTIL", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setUntil((DateValue) schema.applyXformSchema(
						"enddate", value));
			}
		});
		...
		contentRules.put("BYSECOND", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setBySecond((int[]) schema.applyXformSchema(
						"byseclist", value));
			}
		});
		...
		contentRules.put("BYMINUTE", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setByMinute((int[]) schema.applyXformSchema(
						"byminlist", value));
			}
		});
		...
		contentRules.put("BYHOUR", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setByHour((int[]) schema.applyXformSchema(
						"byhrlist", value));
			}
		});
		...
		contentRules.put("BYDAY", new ContentRule() {
			...
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setByDay((List<WeekdayNum>) schema
						.applyXformSchema("bywdaylist", value));
			}
		});
		...
		contentRules.put("BYMONTHDAY", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setByMonthDay((int[]) schema.applyXformSchema(
						"bymodaylist", value));
			}
		});
		...
		contentRules.put("BYYEARDAY", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setByYearDay((int[]) schema.applyXformSchema(
						"byyrdaylist", value));
			}
		});
		...
		contentRules.put("BYWEEKNO", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setByWeekNo((int[]) schema.applyXformSchema(
						"bywknolist", value));
			}
		});
		...
		contentRules.put("BYMONTH", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setByMonth((int[]) schema.applyXformSchema(
						"bymolist", value));
			}
		});
		...
		contentRules.put("BYSETPOS", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setBySetPos((int[]) schema.applyXformSchema(
						"bysplist", value));
			}
		});
		...
		contentRules.put("WKST", new ContentRule() {
			public void apply(IcalSchema schema, String value, IcalObject target)
					throws ParseException {
				((RRule) target).setWkSt((Weekday) schema.applyXformSchema(
						"weekday", value));
			}
		});
		...
		// tzidprefix = "/"
		paramRules.put("rdtparam", new ParamRule() {
			public void apply(IcalSchema schema, String name, String value,
					IcalObject out) throws ParseException {
				if ("value".equalsIgnoreCase(name)) {
					if ("date-time".equalsIgnoreCase(value)
						...
						((RDateList) out).setValueType(IcalValueType
								.fromIcal(value));
					} else {
						schema.badParam(name, value);
					}
					...
					if (value.startsWith("/")) {
						// is globally defined name. We treat all as globally
						// defined.
						value = value.substring(1).trim();
					}
					// TODO(msamuel): proper timezone lookup, and warn on
					// failure
					TimeZone tz = TimeUtils.timeZoneForName(value.replaceAll(
							" ", "_"));
					if (null == tz) {
						schema.badParam(name, value);
					}
		...
		contentRules.put("rdtval", new ContentRule() {
			public void apply(IcalSchema schema, String content,
				...
				for (int i = 0; i < parts.length; ++i) {
					...
					datesUtc[i] = IcalParseUtil.parseDateValue(part, rdates
							.getTzid());
				}
	...
	private static int[] parseIntList(String commaSeparatedString, int absmin,
			int absmax, IcalSchema schema) throws ParseException {
	...
	private static int[] parseUnsignedIntList(String commaSeparatedString,
			int min, int max, IcalSchema schema) throws ParseException {
