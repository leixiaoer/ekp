...
package com.landray.kmss.util.ical.util;
...
import com.landray.kmss.util.ical.values.DateTimeValue;
...
public class TimeUtils {
	...
	private static long timetMillisFromEpochSecs(long epochSecs, TimeZone zone) {
		...
		cal.set(date.year(), date.month() - 1, date.day(), date.hour(), date
				.minute(), date.second());
		return cal.getTimeInMillis();
	}

	private static DateTimeValue convert(DateTimeValue time, TimeZone zone,
			int sense) {
	...
	public static DateValue fromUtc(DateValue date, TimeZone zone) {
		return (date instanceof DateTimeValue) ? fromUtc((DateTimeValue) date,
				zone) : date;
	}
	...
	public static DateValue toUtc(DateValue date, TimeZone zone) {
		return (date instanceof TimeValue) ? convert((DateTimeValue) date,
				zone, -1) : date;
	}

	private static DateTimeValue addSeconds(DateTimeValue dtime, int seconds) {
		return new DTBuilder(dtime.year(), dtime.month(), dtime.day(), dtime
				.hour(), dtime.minute(), dtime.second() + seconds).toDateTime();
	}
	...
	public static int fixedFromGregorian(int year, int month, int day) {
		int yearM1 = year - 1;
		return 365 * yearM1 + yearM1 / 4 - yearM1 / 100 + yearM1 / 400
				+ (367 * month - 362) / 12
	...
	static {
		...
		for (int m = 1; m < 12; ++m) {
			MONTH_START_TO_DOY[m] = MONTH_START_TO_DOY[m - 1]
					+ monthLength(1970, m);
		}
	...
	public static DateTimeValue timeFromSecsSinceEpoch(long secsSinceEpoch) {
		...
		int year = (daysSinceEpoch >= fixedFromGregorian(approx + 1, 1, 1)) ? approx + 1
				: approx;
		int jan1 = fixedFromGregorian(year, 1, 1);
		int priorDays = daysSinceEpoch - jan1;
		int march1 = fixedFromGregorian(year, 3, 1);
		int correction = (daysSinceEpoch < march1) ? 0 : isLeapYear(year) ? 1
				: 2;
		...
		if (!(hour >= 0 && hour < 24))
			throw new AssertionError("Input was: " + secsSinceEpoch
					+ "to make hour: " + hour);
		DateTimeValue result = new DateTimeValueImpl(year, month, day, hour,
				minute, second);
	...
	public static DateValue toDateValue(DateValue dv) {
		return (!(dv instanceof TimeValue) ? dv : new DateValueImpl(dv.year(),
				dv.month(), dv.day()));
	}

	private static final TimeZone BOGUS_TIMEZONE = TimeZone
			.getTimeZone("noSuchTimeZone");

	private static final Pattern UTC_TZID = Pattern.compile(
