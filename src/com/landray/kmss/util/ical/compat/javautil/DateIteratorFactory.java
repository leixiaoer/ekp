...
package com.landray.kmss.util.ical.compat.javautil;
...
public class DateIteratorFactory {
	 ...
	 *            and ignored.
	 */
	public static DateIterator createDateIterator(String rdata, Date start,
			TimeZone tzid, boolean strict) throws ParseException {
		return new RecurrenceIteratorWrapper(RecurrenceIteratorFactory
				.createRecurrenceIterator(rdata, dateToDateValue(start, true),
						tzid, strict));
	}
	 ...
	 *            and ignored.
	 */
	public static DateIterable createDateIterable(String rdata, Date start,
			TimeZone tzid, boolean strict) throws ParseException {
		return new RecurrenceIterableWrapper(RecurrenceIteratorFactory
				.createRecurrenceIterable(rdata, dateToDateValue(start, true),
						tzid, strict));
	}
	...
	static Date dateValueToDate(DateValue dvUtc) {
		...
		if (dvUtc instanceof TimeValue) {
			...
			c.set(dvUtc.year(), dvUtc.month() - 1, // java.util's dates are
													// zero-indexed
	...
	static DateValue dateToDateValue(Date date, boolean midnightAsDate) {
		GregorianCalendar c = new GregorianCalendar(TimeUtils.utcTimezone());
		c.setTime(date);
		int h = c.get(Calendar.HOUR_OF_DAY), m = c.get(Calendar.MINUTE), s = c
				.get(Calendar.SECOND);
		if (midnightAsDate && 0 == (h | m | s)) {
			return new DateValueImpl(c.get(Calendar.YEAR), c
					.get(Calendar.MONTH) + 1, c.get(Calendar.DAY_OF_MONTH));
		} else {
			return new DateTimeValueImpl(c.get(Calendar.YEAR), c
					.get(Calendar.MONTH) + 1, c.get(Calendar.DAY_OF_MONTH), h,
					m, s);
		}
	}
