 ...
 * 
 * <p>
 * See <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4328058" >Sun
 * bug 4328058</a>.
 * 
...
final class TimeZoneConverter {
	...
	private static final Pattern HOUR_MINUTE = Pattern
			.compile("^[+-]?[0-9]{1,2}:[0-9]{2}(:[0-9]{2})?$");
	...
	/**
	 * return a <code>java.util.Timezone</code> object that delegates to the
	 * given Joda <code>DateTimeZone</code>.
	 */
	public static TimeZone toTimeZone(final DateTimeZone dtz) {
		...
		TimeZone tz = new TimeZone() {
			...
			public int getOffset(long instant) {
				// This method is not abstract, but it normally calls through to
				// the
				// method below.
			...
			public int getOffset(int era, int year, int month, int day,
				...
				int millis = milliseconds; // milliseconds is day in standard
											// time
				...
				// get the time in UTC in case a timezone has changed it's
				// standard
				...
				try {
					...
					// Java does not complain if you try to convert a Date that
					// does not
					// exist due to the offset shifting forward, but Joda time
					// does.
					// Since we're trying to preserve the semantics of TimeZone,
					// shift
					// forward over the gap so that we're on a time that exists.
					// This assumes that the DST correction is one hour long or
					// less.
					if (hour < 23) {
						dt = new DateTime(year, month + 1, day, hour + 1,
						...
						int year2 = c.get(Calendar.YEAR), month2 = c
								.get(Calendar.MONTH), day2 = c
								.get(Calendar.DAY_OF_MONTH), hour2 = c
								.get(Calendar.HOUR_OF_DAY);
						dt = new DateTime(year2, month2 + 1, day2, hour2,
								minute, second, millis, dtz);
					}
				}
				...
				DateTime stdDt = new DateTime(year, month + 1, day, hour,
						minute, second, millis, DateTimeZone
	...
	static String cleanUpTzid(String tzid) {
		...
		"GMT" + (tzid.startsWith("-") || tzid.startsWith("+") ? "" : "+")
				+ tzid
				: tzid;
	}
