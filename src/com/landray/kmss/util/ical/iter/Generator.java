 ...
 * 
 * <p>
 * Each field generator takes as input the larger fields, and modifies its
 * field, leaving the other fields unchanged. A year generator will update
 * bldr.year, leaving the smaller fields unchanged, a month generator will
 * update bldr.month, taking its cue from bldr.year, also leaving the smaller
 * fields unchanged.
 * </p>
 * 
...
abstract class Generator {
	...
	/**
	 * @param bldr
	 *            both input and output. non null. modified in place.
	 * @return true iff there are more instances of the generator's field to
	 *         generate. If a generator is exhausted, generating a new value of
	 *         a larger field may allow it to continue, so a month generator
	 *         that runs out of months at 12, may start over at 1 if called with
	 *         a bldr with a different year.
	 * @throws IteratorShortCircuitingException
	 *             when an iterator reaches a threshold past which it cannot
	 *             generate any more dates. This indicates that the entire
	 *             iteration process should end.
	 */
	 ...
	 * RecurrenceIterator's inner loop.
	 * </p>
	 */
