...
import com.landray.kmss.util.ical.util.Predicate;
import com.landray.kmss.util.ical.util.TimeUtils;
import com.landray.kmss.util.ical.values.DateValue;
import com.landray.kmss.util.ical.values.Frequency;
...
class InstanceGenerators {
	...
	static Generator serialInstanceGenerator(
		...
		if (skipSubDayGenerators(hourGenerator, minuteGenerator,
				secondGenerator)) {
			...
			return new Generator() {
				...
				public boolean generate(DTBuilder builder)
						...
						while (!secondGenerator.generate(builder)) {
							...
							while (!minuteGenerator.generate(builder)) {
								...
								while (!hourGenerator.generate(builder)) {
									// until we run out of days in the current
									// month
									while (!dayGenerator.generate(builder)) {
										// until we run out of months in the
										// current year
										while (!monthGenerator
												.generate(builder)) {
											// if there are more years available
											// fetch one
											if (!yearGenerator
													.generate(builder)) {
												// otherwise the recurrence is
												// exhausted
												return false;
											}
										}
					...
					// TODO: maybe group the filters into different kinds so we
					// don't
	...
	static Generator bySetPosInstanceGenerator(int[] setPos,
			final Frequency freq, final Weekday wkst,
			final Predicate<? super DateValue> filter,
			final Generator yearGenerator, final Generator monthGenerator,
			final Generator dayGenerator, final Generator hourGenerator,
			final Generator minuteGenerator, final Generator secondGenerator) {
				...
				while (null == candidates || i >= candidates.size()) {
					...
					if (null != pushback) {
						...
						case MONTHLY:
							...
							break;
						case WEEKLY:
							// consume because just incrementing date doesn't do
							// anything
							DateValue nextWeek = Util.nextWeekStart(builder
									.toDateTime(), wkst);
							do {
					...
					// absolute
					// positions, so we don't need to generate all of the
					// instances for
					// the period.
					...
					while (limit > dates.size()) {
						if (!serialInstanceGenerator.generate(builder)) {
							...
							// return false
							// once the instances we have generated are
							// exhausted.
							// If this is returning false due to some artificial
							// limit, such
							// as the 100 year limit in serialYearGenerator,
							// then we exit
							// via an exception because otherwise we would pick
							// the wrong
							// elements for some uSetPoses that contain negative
							// elements.
							done = true;
							break;
						}
						...
						if (null == d0) {
							...
							case WEEKLY:
								...
								// if there isn't a whole week in between them
								// and the
								// later day is later in the week than the
								// earlier day.
								contained = nb < 7
										&& ((7 + Weekday.valueOf(d).javaDayNum - wkst.javaDayNum) % 7) > ((7 + Weekday
												.valueOf(d0).javaDayNum - wkst.javaDayNum) % 7);
								break;
							case MONTHLY:
								contained = d0.month() == d.month()
										&& d0.year() == d.year();
								break;
							case YEARLY:
	...
	static boolean skipSubDayGenerators(Generator hourGenerator,
			Generator minuteGenerator, Generator secondGenerator) {
