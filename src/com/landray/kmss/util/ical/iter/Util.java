...
class Util {
	...
	static void rollToNextWeekStart(DTBuilder builder, Weekday wkst) {
		...
		builder.day += (7 - ((7 + (Weekday.valueOf(bd).javaDayNum - wkst.javaDayNum)) % 7)) % 7;
		builder.normalize();
	}
	...
	static DateValue nextWeekStart(DateValue d, Weekday wkst) {
		...
		builder.day += (7 - ((7 + (Weekday.valueOf(d).javaDayNum - wkst.javaDayNum)) % 7)) % 7;
	...
	/**
	 * given a weekday number, such as -1SU, returns the day of the month that
	 * it falls on. The weekday number may be refer to a week in the current
	 * month in some contexts or a week in the current year in other contexts.
	 * 
	 * @param dow0
	 *            the day of week of the first day in the current year/month.
	 * @param nDays
	 *            the number of days in the current year/month. In
	 *            [28,29,30,31,365,366].
	 * @param weekNum
	 *            -1SU in the example above.
	 * @param d0
	 *            the number of days between the 1st day of the current
	 *            year/month and the current month.
	 * @param nDaysInMonth
	 *            the number of days in the current month.
	 * @return 0 indicates no such day
	 */
	static int dayNumToDate(Weekday dow0, int nDays, int weekNum, Weekday dow,
			int d0, int nDaysInMonth) {
	 ...
	 * starts on dow0 with nDays, then -1SU is 5SU. Depending on where its used
	 * it may refer to the last Sunday of the year or of the month.
	 * 
	 * @param weekdayNum
	 *            -1SU in the example above.
	 * @param dow0
	 *            the day of the week of the first day of the week or month. One
	 *            of the RRULE_WDAY_* constants.
	 * @param nDays
	 *            the number of days in the month or year.
	...
	/**
	 * the number of occurences of dow in a period nDays long where the first
	 * day of the period has day of week dow0.
	 */
