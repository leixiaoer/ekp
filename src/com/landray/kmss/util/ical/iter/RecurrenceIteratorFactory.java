...
import com.landray.kmss.util.ical.values.IcalObject;
import com.landray.kmss.util.ical.values.RDateList;
import com.landray.kmss.util.ical.values.RRule;
 ...
 * 
 * <h4>Glossary</h4> Period - year|month|day|...<br>
 ...
 * 
 * <h4>Abstractions</h4> Generator - a function corresponding to an RRULE part
 * that takes a date and returns a later (year or month or day depending on its
 * period) within the next larger period. A generator ignores all periods in its
 * input smaller than its period.
 * <p>
...
public class RecurrenceIteratorFactory {
	...
	private static final Logger LOGGER = Logger
			.getLogger(RecurrenceIteratorFactory.class.getName());
	 ...
	 *            ical text.
	 * @param dtStart
	 *            the date of the first occurrence in timezone tzid, which is
	 *            used to fill in optional fields in the RRULE, such as the day
	 *            of the month for a monthly repetition when no ther day
	 ...
	 *            and ignored.
	 */
	public static RecurrenceIterator createRecurrenceIterator(String rdata,
			DateValue dtStart, TimeZone tzid, boolean strict)
			throws ParseException {
		return createRecurrenceIterable(rdata, dtStart, tzid, strict)
				.iterator();
	}

	public static RecurrenceIterable createRecurrenceIterable(String rdata,
			final DateValue dtStart, final TimeZone tzid, final boolean strict)
			throws ParseException {
		...
		return new RecurrenceIterable() {
			public RecurrenceIterator iterator() {
				...
				/*
				 * inclusions.add(new RDateIteratorImpl( new DateValue[] {
				 * TimeUtils.toUtc(dtStart, tzid) }));
				 */
				for (IcalObject contentLine : contentLines) {
					try {
						...
						if ("rrule".equalsIgnoreCase(name)) {
							...
							inclusions
									.add(createRecurrenceIterator((RDateList) contentLine));
						...
						LOGGER.log(Level.SEVERE,
								"Dropping bad recurrence rule line: "
										+ contentLine.toIcal(), ex);
					}
				}
	 ...
	 */
	public static RecurrenceIterator createRecurrenceIterator(String rdata,
			DateValue dtStart, TimeZone tzid) throws ParseException {
	 ...
	 * 
	 * @param rrule
	 *            the recurrence rule to iterate.
	 * @param dtStart
	 *            the start of the series, in tzid.
	 * @param tzid
	 *            the timezone to iterate in.
	 */
	public static RecurrenceIterator createRecurrenceIterator(RRule rrule,
			DateValue dtStart, TimeZone tzid) {
		assert null != tzid;
		assert null != dtStart;
		...
		if (bySetPos.length != 0) {
			...
			switch (freq) {
			case YEARLY:
				start = dtStart instanceof TimeValue ? new DateTimeValueImpl(
						start.year(), 1, 1, 0, 0, 0) : new DateValueImpl(start
						.year(), 1, 1);
				break;
			case MONTHLY:
				start = dtStart instanceof TimeValue ? new DateTimeValueImpl(
						start.year(), start.month(), 1, 0, 0, 0)
						: new DateValueImpl(start.year(), start.month(), 1);
				break;
			case WEEKLY:
				int d = (7 + wkst.ordinal() - Weekday.valueOf(dtStart)
						.ordinal()) % 7;
		...
		case SECONDLY:
			if (bySecond.length == 0 || interval != 1) {
				secondGenerator = Generators.serialSecondGenerator(interval,
						dtStart);
				if (bySecond.length != 0) {
		...
		case MINUTELY:
			if (byMinute.length == 0 || interval != 1) {
				minuteGenerator = Generators.serialMinuteGenerator(interval,
						dtStart);
		...
		case HOURLY:
			if (byHour.length == 0 || interval != 1) {
				hourGenerator = Generators.serialHourGenerator(interval,
						dtStart);
				if (byHour.length != 0) {
		...
		case WEEKLY:
			...
			if (0 != byDay.length) {
				...
				byDay = NO_DAYS;
				if (interval > 1) {
					filters.add(Filters.weekIntervalFilter(interval, wkst,
							dtStart));
				}
			} else {
				dayGenerator = Generators.serialDayGenerator(interval * 7,
						dtStart);
			}
			break;
		case YEARLY:
		...
		case MONTHLY:
			if (0 != byMonthDay.length) {
				...
				dayGenerator = Generators
						.byMonthDayGenerator(byMonthDay, start);
				byMonthDay = NO_INTS;
				...
				// for
				// YEARLY rules.
				dayGenerator = Generators.byWeekNoGenerator(byWeekNo, wkst,
						start);
				byWeekNo = NO_INTS;
				...
				dayGenerator = Generators.byDayGenerator(byDay,
						Frequency.YEARLY == freq && 0 == byMonth.length, start);
				byDay = NO_DAYS;
			} else {
		...
		if (0 != byDay.length) {
			filters.add(Filters.byDayFilter(byDay, Frequency.YEARLY == freq,
					wkst));
			byDay = NO_DAYS;
		}
		...
		if (0 != bySetPos.length) {
			instanceGenerator = InstanceGenerators.bySetPosInstanceGenerator(
					bySetPos, freq, wkst, filter, yearGenerator,
					monthGenerator, dayGenerator, hourGenerator,
		...
		return new RRuleIteratorImpl(dtStart, tzid, condition,
	 ...
	 * iterators.
	 */
	public static RecurrenceIterator join(RecurrenceIterator a,
			RecurrenceIterator... b) {
		List<RecurrenceIterator> incl = new ArrayList<RecurrenceIterator>();
		incl.add(a);
		incl.addAll(Arrays.asList(b));
		return new CompoundIteratorImpl(incl, Collections
				.<RecurrenceIterator> emptyList());
	}
	 ...
	 * @return non null.
	 */
	public static RecurrenceIterator except(RecurrenceIterator included,
			RecurrenceIterator excluded) {
		return new CompoundIteratorImpl(Collections
				.<RecurrenceIterator> singleton(included), Collections
				.<RecurrenceIterator> singleton(excluded));
	}

	private static final Pattern FOLD = Pattern.compile("(?:\\r\\n?|\\n)[ \t]");
	private static final Pattern NEWLINE = Pattern.compile("[\\r\\n]+");
	private static final Pattern RULE = Pattern.compile("^(?:R|EX)RULE[:;]",
			Pattern.CASE_INSENSITIVE);
	private static final Pattern DATE = Pattern.compile("^(?:R|EX)DATE[:;]",
			Pattern.CASE_INSENSITIVE);

	private static IcalObject[] parseContentLines(String rdata, TimeZone tzid,
		...
		for (int i = 0; i < lines.length; ++i) {
			...
			try {
				...
				LOGGER.log(Level.SEVERE, "Dropping bad recurrence rule line: "
						+ line, ex);
				++nbad;
			} catch (IllegalArgumentException ex) {
				if (strict) {
