...
import com.landray.kmss.util.ical.util.DTBuilder;
import com.landray.kmss.util.ical.util.TimeUtils;
import com.landray.kmss.util.ical.values.DateValue;
import com.landray.kmss.util.ical.values.DateValueImpl;
import com.landray.kmss.util.ical.values.TimeValue;
import com.landray.kmss.util.ical.values.Weekday;
import com.landray.kmss.util.ical.values.WeekdayNum;
...
final class Generators {
	 ...
	 * a span of 400 years before giving up and concluding that the rule
	 ...
	 * 
	 * @param interval
	 *            number of years to advance each step.
	 * @param dtStart
	 *            non null
	 ...
	 * 
	 * @param interval
	 *            number of months to advance each step.
	 * @param dtStart
	 *            non null.
	 * @return the year in dtStart the first time called and interval + last
	 *         return value on subsequent calls.
	 */
	static Generator serialMonthGenerator(final int interval,
			final DateValue dtStart) {
		return new Generator() {
			...
			boolean generate(DTBuilder builder) {
				...
				if (year != builder.year) {
					int monthsBetween = (builder.year - year) * 12
							- (month - 1);
	 ...
	 */
	static Generator serialDayGenerator(final int interval,
			final DateValue dtStart) {
		return new Generator() {
			int year, month, date;
			...
			boolean generate(DTBuilder builder) {
				...
				if (year == builder.year && month == builder.month) {
					...
					if (interval != 1) {
						...
						// new
						// month andthe old date and extend it to make it an
						// integer
						// multiple of interval
						int daysBetween = TimeUtils.daysBetween(
								new DateValueImpl(builder.year, builder.month,
										1),
								new DateValueImpl(year, month, date));
						ndate = ((interval - (daysBetween % interval)) % interval) + 1;
						if (ndate > nDays) {
							// need to early out without updating year or month
							// so that the
							// next time we enter, with a different month, the
							// daysBetween
	 ...
	 * 
	 * @param interval
	 *            number of hours to advance each step.
	 * @param dtStart
	 *            non null.
	 * @return the day in dtStart the first time called and interval + last
	 *         return value on subsequent calls.
	 */
	static Generator serialHourGenerator(final int interval,
			final DateValue dtStart) {

		return new Generator() {
			int hour = ((dtStart instanceof TimeValue) ? ((TimeValue) dtStart)
					.hour() : 0)
					- interval;
			int day = dtStart.day();
			...
			boolean generate(DTBuilder builder) {
				...
				if (day != builder.day || month != builder.month
						|| year != builder.year) {
					int hoursBetween = daysBetween(builder, year, month, day)
							* 24 - hour;
	 ...
	 * 
	 * @param interval
	 *            number of minutes to advance each step.
	 * @param dtStart
	 *            non null.
	 * @return the day in dtStart the first time called and interval + last
	 *         return value on subsequent calls.
	...
	static Generator serialMinuteGenerator(final int interval,
		...
		return new Generator() {
			int minute = (dtStart instanceof TimeValue ? ((TimeValue) dtStart)
					.minute() : 0)
					- interval;
			int hour = dtStart instanceof TimeValue ? ((TimeValue) dtStart)
					.hour() : 0;
			...
			boolean generate(DTBuilder builder) {
				int nminute;
				if (hour != builder.hour || day != builder.day
						|| month != builder.month || year != builder.year) {
					int minutesBetween = (daysBetween(builder, year, month, day)
							* 24 + builder.hour - hour)
							* 60 - minute;
	...
	/**
	 * constructs a generator that generates seconds in the given builder's
	 * minute successively counting from the first second passed in.
	 * 
	 * @param interval
	 *            number of seconds to advance each step.
	 * @param dtStart
	 *            non null.
	 * @return the day in dtStart the first time called and interval + last
	 *         return value on subsequent calls.
	 */
	static Generator serialSecondGenerator(final int interval,
			final DateValue dtStart) {

		return new Generator() {
			int second = (dtStart instanceof TimeValue ? ((TimeValue) dtStart)
					.second() : 0)
					- interval;
			int minute = dtStart instanceof TimeValue ? ((TimeValue) dtStart)
					.minute() : 0;
			...
			boolean generate(DTBuilder builder) {
				...
				if (minute != builder.minute || hour != builder.hour
						...
						|| year != builder.year) {
					int secondsBetween = ((daysBetween(builder, year, month,
							day)
							* 24 + builder.hour - hour)
							* 60 + builder.minute - minute)
							* 60 - second;
	...
	static Generator byHourGenerator(int[] hours, final DateValue dtStart) {
		int startHour = dtStart instanceof TimeValue ? ((TimeValue) dtStart)
				.hour() : 0;
	...
	static Generator byMinuteGenerator(int[] minutes, final DateValue dtStart) {
		...
		if (minutes.length == 0) {
			minutes = new int[] { dtStart instanceof TimeValue ? ((TimeValue) dtStart)
					.minute()
					: 0 };
		}
	...
	static Generator bySecondGenerator(int[] seconds, final DateValue dtStart) {
		...
		if (seconds.length == 0) {
			seconds = new int[] { dtStart instanceof TimeValue ? ((TimeValue) dtStart)
					.second()
					: 0 };
		}
	...
	/**
	 * constructs a function that yields the specified dates (possibly relative
	 ...
	 * 
	 * @param days
	 *            day of week, number pairs, e.g. SU,3MO means every sunday and
	 *            the 3rd monday.
	 * @param weeksInYear
	 *            are the week numbers meant to be weeks in the current year, or
	 *            weeks in the current month.
	 * @param dtStart
	 *            non null
	 */
	static Generator byDayGenerator(WeekdayNum[] days,
			final boolean weeksInYear, final DateValue dtStart) {
		...
		return new Generator() {
			...
			void generateDates() {
				...
				for (int j = 0; j < udays.length; ++j) {
					...
					if (0 != day.num) {
						int date = Util.dayNumToDate(dow0, nDays, day.num,
								day.wday, d0, nDaysInMonth);
						if (0 != date) {
							udates.add(date);
						}
					} else {
						int wn = w0 + 6;
						for (int w = w0; w <= wn; ++w) {
							int date = Util.dayNumToDate(dow0, nDays, w,
									day.wday, d0, nDaysInMonth);
							if (0 != date) {
								udates.add(date);
							}
			...
			@Override
			public String toString() {
				return "byDayGenerator:" + Arrays.toString(udays) + " by "
						+ (weeksInYear ? "year" : "week");
			}
		};
	}
	 ...
	 * 
	 * @param weekNos
	 *            (elements in [-53,53] != 0) week numbers
	 * @param wkst
	 *            (in RRULE_WDAY_*) day of the week that the week starts on.
	 * @param dtStart
	 *            non null
	 */
	static Generator byWeekNoGenerator(int[] weekNos, final Weekday wkst,
			final DateValue dtStart) {
		...
		return new Generator() {
			...
			void checkMonth() {
				...
				// the week of the year of the 1st day of the month.
				// approximate.
				...
				for (int j = 0; j < uWeekNos.length; j++) {
					...
					if (weekNo >= weekOfMonth - 1 && weekNo <= weekOfMonth + 6) {
						for (int d = 0; d < 7; ++d) {
							int date = ((weekNo - 1) * 7 + d
									+ doyOfStartOfWeek1 - doyOfMonth1) + 1;
							if (date >= 1 && date <= nDays) {
								udates.add(date);
							}
						}
	...
	private static int daysBetween(DTBuilder builder, int year, int month,
			int day) {
		if (year == builder.year && month == builder.month) {
			return builder.day - day;
		} else {
			return TimeUtils.daysBetween(builder.year, builder.month,
					builder.day, year, month, day);
		}
	}
