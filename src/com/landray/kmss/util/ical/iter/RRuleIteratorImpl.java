...
final class RRuleIteratorImpl implements RecurrenceIterator {
	...
	/**
	 * a function that applies the various period generators to generate an
	 * entire date. This may involve generating a set of dates and discarding
	 * all but those that match the BYSETPOS rule.
	 */
	private final Generator instanceGenerator_;
	/**
	 * a function that takes a builder and populates the year field. Returns
	 * false if there aren't more years available.
	 */
	private final ThrottledGenerator yearGenerator_;
	/**
	 * a function that takes a builder and populates the month field. Returns
	 * false if there aren't more months available in the builder's year.
	 */
	...
	RRuleIteratorImpl(DateValue dtStart, TimeZone tzid,
			Predicate<? super DateValue> condition,
			...
			Generator secondGenerator, boolean canShortcutAdvance) {
		...
		try {
			...
			if (InstanceGenerators.skipSubDayGenerators(hourGenerator,
					minuteGenerator, secondGenerator)) {
				toInitialize = new Generator[] { yearGenerator, monthGenerator };
				builder.hour = ((SingleValueGenerator) hourGenerator)
						.getValue();
				builder.minute = ((SingleValueGenerator) minuteGenerator)
						.getValue();
				builder.second = ((SingleValueGenerator) secondGenerator)
						.getValue();
			} else {
				toInitialize = new Generator[] { yearGenerator, monthGenerator,
						dayGenerator, hourGenerator, minuteGenerator, };
			}
		...
		while (!this.done_) {
			...
			if (null == this.pendingUtc_) {
				this.done_ = true;
				break;
			} else if (this.pendingUtc_.compareTo(TimeUtils
					.toUtc(dtStart, tzid)) >= 0) {
	...
	public void advanceTo(DateValue dateUtc) {
		...
		if (this.pendingUtc_ != null
				&& dateUtc.compareTo(this.pendingUtc_) <= 0) {
			return;
		}
	...
	private static final DateValue MIN_DATE = new DateValueImpl(
	...
	private DateValue generateInstance() {
		try {
			do {
				...
				DateValue dUtc = this.dtStart_ instanceof TimeValue ? TimeUtils
						.toUtc(this.builder_.toDateTime(), this.tzid_)
