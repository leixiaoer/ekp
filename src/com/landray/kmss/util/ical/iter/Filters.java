...
import com.landray.kmss.util.ical.util.Predicates;
...
class Filters {
	 ...
	 */
	static Predicate<DateValue> byDayFilter(final WeekdayNum[] days,
			final boolean weeksInYear, final Weekday wkst) {
		return new Predicate<DateValue>() {
			public boolean apply(DateValue date) {
				...
				if (weeksInYear) {
					...
					instance = TimeUtils.dayOfYear(date.year(), date.month(),
							date.day());
				} else {
					nDays = TimeUtils.monthLength(date.year(), date.month());
					dow0 = Weekday.firstDayOfWeekInMonth(date.year(), date
							.month());
					instance = date.day() - 1;
				}
	...
	/**
	 * constructs a filter that accepts only every interval-th week from the
	 * week containing dtStart.
	 * 
	 * @param interval
	 *            > 0 number of weeks
	 * @param wkst
	 *            day of the week that the week starts on.
	 * @param dtStart
	 *            non null
	 */
	static Predicate<DateValue> weekIntervalFilter(final int interval,
			final Weekday wkst, final DateValue dtStart) {
		return new Predicate<DateValue>() {
			DateValue wkStart;
			{
				// the latest day with day of week wkst on or before dtStart
				DTBuilder wkStartB = new DTBuilder(dtStart);
				wkStartB.day -= (7 + Weekday.valueOf(dtStart).javaDayNum - wkst.javaDayNum) % 7;
			...
			public boolean apply(DateValue date) {
				...
				if (daysBetween < 0) {
					...
					daysBetween += (interval * 7 * (1 + daysBetween
							/ (-7 * interval)));
				}
