...
public class HQLUtil {
	private static final Log logger = LogFactory.getLog(HQLUtil.class);
		...
		for (int i = 0; i <= n; i++) {
			...
			if (isString) {
				...
				for (int j = i * 1000; j < size; j++) {
					objStr = valueList.get (j);
					if (objStr != null) {
						tmpStr = objStr.toString ().replaceAll("'", "''");
					}
    ...
    public static String escapeHql(String hql, List<HQLParameter> parameterList) {
        ...
        if (hql.contains("select ") && hql.contains(" where ") && hql.contains(" like ") && CollectionUtils.isNotEmpty(parameterList)) {
            ...
            if (logger.isDebugEnabled() && count > 0) {
				...
				logger.debug("特殊字符转义处理（耗时：" + (end - start) + "毫秒）：\r\n \t原HQL：" + hql + "\r\n \t转义参数：" + count
						+ "个，参数内容：" + _value + "\r\n \t现HQL：" + _hql);
            }
		...
		if (CollectionUtils.isNotEmpty(patterns)) {
			if (!isEscaped) {
                ...
                if (!value.equals(newValue)) {
					...
					escaped = true;
                    if (logger.isDebugEnabled()) {
						logger.debug("参数值替换，参数名：[" + parameter.getName() + "]，原参数值：[" + value + "]，现参数值：[" + newValue + "]");
                    }
				} else {
					isInsert = false;
                }
