...
public class ResourceUtil {
	...
	public static void addLangCookie(ServletRequest request,
		...
		try {
			if (request != null && response != null
				...
				Locale locale = (Locale) httpServletRequest.getSession()
						.getAttribute(Globals.LOCALE_KEY);
				if (locale != null) {
					httpServletResponse.addHeader(
							"Set-Cookie",
							"j_lang" + "=" + locale.getLanguage() + "-"
									+ locale.getCountry()
									+ "; Version=0"
									+ "; Path=/");
				}
	...
	private static Locale getLocaleByUser(HttpServletRequest request) {
		...
		if (locale == null) {
			...
			if (request != null) {
				// 获取URL里的语言信息
				locale = getLocale(request.getParameter("j_lang"));
				if (locale == null) {
					// 获取user-agent里的语言信息
					...
					if (locale == null) {
						...
						if (locale == null) {
							...
							if (locale != null) {
								...
								if (StringUtil.isNotNull(languageTag)
										&& languageTag.split("-").length > 1) {
	 ...
	 * @return
	 */
	private static String getStringValueOri(String key, String bundle,
			Locale locale) {
		...
		try {
			...
			try {
				if (locale == null) {
					...
					if (isDebug()) {
						return "{"
								+ java.util.ResourceBundle.getBundle(resource).getString(
										key) + "}";
					} else {
	...
	public static String getStringValue(String key, String bundle, Locale locale) {
		...
		if (isDebug()) {
			...
			//系统较多的地方采用了null的判断，如果msg本身是null不应该返回[null]
			if(StringUtil.isNull(msg)) {
				return msg;
			}
			return "[" + msg + "]";
	...
	public static Locale getLocaleByUA(String userAgent) {
		...
		if (userAgent.indexOf("language") > -1) {
			String language = userAgent
					.substring(userAgent.lastIndexOf("language") + 9);
